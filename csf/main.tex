...
\begin{lstlisting}[language=c,basicstyle=\scriptsize\ttfamily,morekeywords={size_t}]
...
\begin{definition}[Property Satisfaction]\label{def:propsat}
  \bul{$\sat{\progvar}{\pi}$}
  $\isdef$
  if \iul{$\progstepto{\progvar}{\runtimetermvar}{\trace}$},
  then \oul{$\trace\in\pi$}.
\end{definition}
...
\begin{definition}[Robust Compilation]\label{def:rtp}
  ...
  %, iff 
  $\isdef$
  \rul{$\forall \left(\pi\in\class\right) \left(\src{p}\in\src{L}\right),$} if \iul{$\rsat{\src{\progvar}}{\pi}$}, then \oul{$\rsat{\cc{\src{L}}{\trg{L}}\left(\src{p}\right)}{\pi}$}.
\end{definition}
...
\begin{figure}[!ht]
  \judgbox{\traceagree{\pi}{\trace}{\abstrace}}{,,Specification-level trace $\trace$ agrees with monitor-level\\trace $\abstrace$ with respect to property $\pi$.''}
  %
...
int hello() {
  char* greet = new char[11] { "Hello CSF!" };
...
\begin{exampleenv}[Data-independent timing mode]
...
\begin{gather*}
  ...
  \\
  \begin{aligned}
  \mi{(Types)}~\type\bnfdef\natt \mid \type[_{1}] \times \type[_{2}]\hspace{0.5cm}
...
\begin{gather*}
  \begin{aligned}
  \mi{(Pre\text{-}Events)}~\event[_{b}]\bnfdef&\ \ev{Alloc}\ \loc\ \valueexpr \mid \ev{Dealloc}\ \loc\\
                                       &\mid \ev{Get}\ \loc\ \valueexpr \mid \ev{Set}\ \loc\ \valueexpr\ \valueexpr['] \mid \cdots \\
...
\begin{center}
  ...
  \typerule{$e-\text{get}-\in$}{
    ...
    \loc+n\in\operatorname{dom}\statevar.\heap[^{\sandboxtag}]
  }{
    \pstepto{\rtt{\statevar}{\lget{x}{n}}}{\rtt{\statevar}{\statevar.\heap[^{\sandboxtag}](\loc+n)}}{\tup{\ev{Get}\ \loc\ n}{\sandboxtag}}
  }{e-get-in}
  %
  \typerule{$e-\text{set}-\notin$}{
    \sandboxtag=\statevar.\sandboxtag &
  ...
  \typerule{$e-\text{new}$}{
    ...
    \statevar['] = \statevar\subst{\heap[^{\sandboxtag}]}{\heap[^{\sandboxtag}_1]}\subst{\store}{\store[_1]}
  }{
    \pstepto{\rtt{\statevar}{\llet{x}{\lnew{\valueexpr}{n}}{e}}}{\rtt{\statevar[']}{\expr\subst{z}{x}}}{\tup{\ev{Alloc}\ \loc\ n}{\sandboxtag}}
  }{e-new}
  %
...
Contrary, $\ev{Call\ \comptoctx\ foo\ \valueexpr}$ and $\ev{Return\ \comptoctx\ \valueexpr}$ signal that program execution transitions from component- to context-level.
For calls without this context switch, the contextual semantics attaches the $\nocomm$ tag.
In the following, $\neg\ctx=\comp$ and $\neg\comp=\ctx$.
...
The contextual semantics is mostly straightforward.
In \Cref{tr:e-ret-notsame,tr:e-call-notsame}, the judgement $\Omega.\commlib\vdash\text{foo}:\Omega.\sandboxtag\dashv \comm$ checks whether $\text{foo}$ is a component-level name by looking it up in the list of component-level names $\commlib$ and emits the appropriate transfer tag, i.e., either $\comptoctx$ or $\ctxtocomp$.
...
\begin{exampleenv}[Instrumented \texttt{strncpy}]
  Consider again \texttt{strncpy}, but instrumented for \gls*{sms}:
    \begin{lstlisting}[language=c,basicstyle=\ttfamily\scriptsize, morekeywords={size_t}]
