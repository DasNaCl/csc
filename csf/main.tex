...
It is worth noting that in this setting, the theorem may not be applicable.
...
$\cc{\trg{L}}{\obj{L}}$ modifies the behavior and because of this, the theorem is inapplicable.
\MPin{
	this is still applicable though
	many use it, and we showcase it too.
}

This section re-introduces secure compiler composition without this limitation. 
To this end, it utilises existing work on secure compilation~\cite{abate2021extacc} that defines Galois connections for several real-world compiler scenarios, such as a compilation pass that exploits undefined behavior.
This section is concerned about source-level properties, and henceforth 
\MPin{
	different source and target trace models
	.
	traces are related
	.
	there is a function tau that lefts the relation from individual traces to props/hprops
}
\MPin{
	drop next 3 defs
}

uses the existential image of a source-level property $\src{\pi}$ to map $\src{\pi}$ to a target-level property:
...
\bul{A cross-language relation $\sim$ is well-formed with respect to $\src{\Pi}$} iff \rul{the existential images preserve membership}.
\begin{definition}[Well-formedness of Cross-Language Trace Relations for a Concrete Property $\src{\Pi}$]\label{def:wf:tracerel}
  $$
  \text{\bul{$\wf{\sim}{\src{\Pi}}$}} \isdef \text{\rul{$\forall \src{\pi}\in\src{\Pi}, \tau_\sim(\src{\pi})\in\tilde{\tau}_\sim(\src{\Pi})$}}
  $$
\end{definition}
\begin{definition}[Well-formedness of Cross-Language Trace Relations for a Class $\src{\class}$]\label{def:wfc:tracerel}
  $$
  \wfc{\sim}{\src{\class}} \isdef \forall \src{\Pi}\subseteq\src{\class}, \tau_\sim(\src{\Pi})\subseteq\tilde{\tau}_\sim(\src{\class})
  $$
\end{definition}
\MPin{
	explain with text and underline
}
...
\Thmref{def:rtp} needs to be adapted to account for different target-level trace-models:
...
\begin{definition}[Composing Cross-Language Trace Relations]
  Given $\sim_1 : \src{\trace}\to\irl{\trace}\to\text{Prop},\sim_2 : \irl{\trace}\to\trg{\trace}\to\text{Prop}$, then 
  $$  
  \sim_1\bullet\sim_2\ \isdef \lambda \src{\trace}\ \trg{\trace}. \exists \irl{\trace}. \src{\trace}\sim_1\irl{\trace} \text{ and } \irl{\trace}\sim_2\trg{\trace}
  $$
\end{definition}
...
\begin{theorem}[Sequential Composition of Secure Compilers]\label{thm:rtpsim}
  If $\rtpsim{\cc{\src{L}}{\trg{L}}}{\class[_{1}]}{\sim_1}$, $\rtpsim{\cc{\trg{L}}{\obj{L}}}{\tilde{\tau}_{\sim_1}\left(\class[_{2}]\right)}{\sim_2}$, and $\wf{\sim_1}{\class[_2]}$, then $\rtpsim{\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}}{\class[_{1}]\cap\class[_{2}]}{\sim_1\bullet\sim_2}$. \Coqed
\end{theorem}

Reconsidering the example from the beginning of the section, suppose $\sim_{{\smssafe}}$ is a suitable trace relation such that $\trg{\trace}\sim_{{\smssafe}}\irl{\trace}$ establishes the fact that $\irl{\trace}$ contains an allocation of a table containing bounds information which is never deallocated.
...
With this, the previous claim follows as a simple corollary:
\begin{corollary}
  $\tilde{\tau}_{{{\sim_{\tmssafe}}\bullet{\sim_{\smssafe}}}}\left(\tmssafe\cap\smssafe\right)=\emptyset$.
\end{corollary}
\begin{proof}
  Note that $\neg\left(\wf{\sim_{{\smssafe}}}{{\tmssafe}}\right)$, since the bounds-table is never deallocated and therefore, for any $\trg{\trace}\in\tmssafe$, $\tau_{\sim_{\tmssafe}}\left(\trg{\trace}\right)\notin\tilde{\tau}_{\sim_{\tmssafe}}\left({\tmssafe}\right)$.
  However, by assumption, $\wf{\sim_{{\tmssafe}}}{{\tmssafe}}$.
  Using \Thmref{lem:useless} it follows that $\tilde{\tau}_{{{\sim_{\tmssafe}}\bullet{\sim_{\smssafe}}}}\left(\tmssafe\right)=\emptyset$.
\end{proof}

\MPin{
	what about sim2 and C1?
	-> no relation 
}
\MPin{
	For the sake of simplicity we go back to the singel trace model and go back to the framework of single trace model
}
