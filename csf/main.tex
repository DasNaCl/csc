...
So far, this paper assumes a unified trace-model and secure compilers that do not change the behavior.
It is worth noting that, as demonstrated in our case study, quite a few compilation passes do not change the behavior in such a way that the projections to the specification trace model become unequal to each other, i.e., source and target language of the compiler essentially have a different trace model. 
...
To this end, the section utilises existing work on secure compilation~\cite{abate2021extacc} that defines Galois connections for several real-world compiler scenarios, such as a compilation pass that exploits undefined behavior.
This section is concerned about a compilation setting with different source and target trace models. 
To this end, there needs to be a way to say that two traces $\src{\trace}$ and $\trg{\trace}$ are related ($\sim$) and a way to translate a source-level property $\src{\pi}$ into a target-level property ($\tau_\sim\left( \src{\pi} \right)$~\cite{abate2021extacc}) using this trace relation.
Moreover, the function to map source-level properties to target-level properties can be lifted to translate source-level classes of properties $\src{\class}$ to target-level classes ($\tilde{\tau}_\sim\left(\src{\class}\right)$).
...
\Thmref{def:rtp} needs to be adapted to account for different target-level trace-models~\cite{abate2021extacc}:
...
\begin{theorem}[Sequential Composition of Secure Compilers]\label{thm:rtpsim}
  If \bul{$\rtpsim{\cc{\src{L}}{\trg{L}}}{\src{\class[_{1}]}}{\sim_1}$}, \rul{$\rtpsim{\cc{\trg{L}}{\obj{L}}}{\tilde{\tau}_{\sim_1}\left(\src{\class[_{2}]}\right)}{\sim_2}$}, and \iul{$\wf{\sim_1}{\src{\class[_2]}}$}, then \oul{$\rtpsim{\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}}{\src{\class[_{1}]}\cap\src{\class[_{2}]}}{\sim_1\bullet\sim_2}$}. \Coqed
\end{theorem}
...
Note that it is not necessary to require $\sim_2$ to be well-formed with respect to $\src{\class[_1]}$.

However, reconsidering the example from the beginning of the section, suppose $\sim_{{\smssafe}}$ is a suitable trace relation such that $\trg{\trace}\sim_{{\smssafe}}\irl{\trace}$ establishes the fact that $\irl{\trace}$ contains an allocation of a table containing bounds information which is never deallocated.
...
However, the resulting class $\tilde{\tau}_{{{\sim_{\tmssafe}}\bullet{\sim_{\smssafe}}}}\left(\tmssafe\cap\smssafe\right)=\emptyset$, which the following lemma shows to hold in general.

\begin{lemma}[Composition is Useless]\label{lem:useless}
  If $\wf{\sim_1}{\class}$, $\neg\left(\wf{\sim_2}{\tilde{\tau}_{\sim_1}\class}\right)$, then $\tilde{\tau}_{\sim_1\bullet\sim_2}\left(\class\right)=\emptyset$. \Coqed
\end{lemma}
...
The case-study demonstrates that, while the compositionality framework without the consideration for different trace-models may seem overly simplistic, it is useful and simpler in practice, if applicable.
