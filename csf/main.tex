...
% \author{
% \IEEEauthorblockN{1\textsuperscript{st} Matthis Kruse\orcidlink{0000-0003-4062-9666}}
...
\begin{itemize}
  ...
  \item %
        ...
        This paper takes the secure compilation framework of Abate et al.~\cite{abate2019jour} and extends it to reason about the security of all different known forms of compiler composition (\Cref{sec:compcomp}).
...
% \MP{
% 	check that we say classes are hp
% }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\MPin{
  moved to get a feeling of what's what
}
...
% \MP{check, we ened more than this}
...
Moreover secure compilers (in the sense of \Cref{def:rtp}) cannot change the behavior of compiled programs.
It is worth noting that, as our case study in \Cref{sec:casestud:defs} demonstrates, several compilation passes do not change the behavior of compiled programs.
% in such a way that the projections to the specification trace model become unequal to each other, 
% i.e., source and target language of the compiler essentially have a different trace model. 
...
By \Thmref{def:trace:tmsdef}, this violates $\tmssafe$.
...
$\cc{\trg{L}}{\obj{L}}$ modifies the behavior and this section re-introduces the compositionality framework with support for these situations. 
...
% These compilers require a Galois connections for several real-world compiler scenarios, such as a compilation pass that exploits undefined behavior.
% This section is concerned about a compilation setting with different source and target trace models. 
...
With this, \bul{a cross-language relation $\sim$ is well-formed with respect to a class of source-level properties $\src{\class}$} iff \rul{the existential images preserve set inclusion}.
\begin{definition}[Well-formedness of Cross-Language Trace Relations for a Class $\src{\class}$]\label{def:wfc:tracerel}
  $$
  \text{\bul{$\wfc{\sim}{\src{\class}}$}} \isdef \text{\rul{$\forall \src{\Pi}\subseteq\src{\class}, \tau_\sim(\src{\Pi})\subseteq\tilde{\tau}_\sim(\src{\class})$}}
  $$
\end{definition}

\Cref{def:rtpsim} shows how to adapt \Thmref{def:rtp} to account for different target-level trace-models~\cite{abate2021extacc}:

\begin{definition}[Robust Compilation with $\sim$]\label{def:rtpsim}
...
To state a similar theorem as \Thmref{thm:rtp} but for \Cref{def:rtpsim}, it is necessary to define composition of cross-language trace relations.
...
For the adapted sequential composition theorem, let \bul{$\cc{\src{L}}{\trg{L}}$ robustly preserve the class $\src{\class[_1]}$ under $\sim_1$} and let \rul{$\cc{\trg{L}}{\obj{L}}$ robustly preserve the class $\tilde{\tau}_{\sim_1}\left(\src{\class[_2]}\right)$ under $\sim_2$}.
Then, \iul{when the cross-language relation $\sim_1$ is well-formed with respect to class $\src{\class[_2]}$}, and \pul{relation $\sim_2$ is well-formed with respect to $\src{\class[_1]}$}, it follows that \oul{the composed compiler $\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}$ robustly preserves the intersection of classes $\src{\class[_1]}\cap\src{\class[_2]}$ under $\sim_1\bullet\sim_2$}.
% 
...
% \MPin{why}
\begin{theorem}[Sequential Composition of Secure Compilers]\label{thm:rtpsim}
  If \bul{$\rtpsim{\cc{\src{L}}{\trg{L}}}{\src{\class[_{1}]}}{\sim_1}$}, \rul{$\rtpsim{\cc{\trg{L}}{\obj{L}}}{\tilde{\tau}_{\sim_1}\left(\src{\class[_{2}]}\right)}{\sim_2}$}, and \iul{$\wf{\sim_1}{\src{\class[_2]}}$}, and \pul{$\wf{\sim_2}{\src{\class[_1]}}$}, then \oul{$\rtpsim{\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}}{\src{\class[_{1}]}\cap\src{\class[_{2}]}}{\sim_1\bullet\sim_2}$}. \Coqed
\end{theorem}

The reason we need assumptions about both relations is motivated by \Cref{lem:useless}.
% 
In fact, if \bul{that a relation $\sim_1$ is well-formed for a property $\class$} but \rul{another relation $\sim_2$ is not well-formed for the property mapped with the first relation $\tilde{\tau}_{\sim_1}\class$} then \iul{their composition is empty}.
% 
...
\begin{lemma}[Composition is Useless]\label{lem:useless}
  If \bul{$\wf{\sim_1}{\class}$}, \rul{$\neg\left(\wf{\sim_2}{\tilde{\tau}_{\sim_1}\class}\right)$}, then \iul{$\tilde{\tau}_{\sim_1\bullet\sim_2}\left(\class\right)=\emptyset$}. \Coqed
\end{lemma}
...
and $\wf{{\sim_{\tmssafe}}}{{\smssafe}}$ allows to apply \Thmref{thm:rtpsim}.
...
% may seem overly simplistic, it is useful and simpler in practice, if applicable.
...
\begin{gather*}\small
  \begin{aligned}
  \mi{(Security\ Tag)}~\sigma\bnfdef&\ \lock \mid \unlock
  \qquad
...
\MPin{
  if we cut all of this, we gain 2 pages.
  we still need this subsection where we say that:
  - proofs using the traces definitions above are tricky
  - for simplicity, we use an equivalent encoding as monitors, which is doable for safety properties, and we use safety properties
  - for each property we have a monitor (do we need to say how we indicate that monitor? or will later code not use it)
  - for each property, we show its monitor correctly captures it
  - we need to add another trace model for teh monitor and a relation between the property and the monitor's traces but that is standard
  - details are in the appendices
}
\MPin{
  likely, we can get away with the aforementioned stuff in 1 column
}
\MPin{
  with the extra space:
  - define SS as taken from exorcising (and probably some other work, Xaver should know), and its trace model extension
  - reintroduce the language definition using a align* environment in V.A
  - add V.F, defining L-spect, a speculatively-unsafe language, with an example showing the canonical Spectre V1 gadget (it normally uses heap-allocated arrays, which we don't have in our language, but the memory is N-indexed, so we should be able to do this ?)
  - add VI.F defining the fence-inserting compiler, defining its trace relation and the extra lemmas that make it interesting
}
\MPin{
  extra things to note:
  - start talking about SS in the intro, 
  - check the paper (like, conclusions) for ms+scct and replace with the new acronym (ms+scct+ss == ? we should find a single name, like SMC: speculative memory safety and CT)
  - drop the last paragraph of III and replace with 'another example is SS, which we do in ... ...'
  - change the title of III.C ? if we do a pass with the cross-trace relation, then 
  - how is III.A.1 affected by III.C ? do we get another/a different corollary for TAU optimising passes? is it worthwile moving III.A.1 at the end, as III.D, with a more general discussion?
}
...
Since reasoning on monitors is easier than on just traces, this section presents a monitor for each of the previously shown safety properties (\Cref{subsec:propdefs}).
%\MPpost{
...
% }{tms-abort}
  ...
  \typerule{tms-dealloc}{
    ...
    (\loc;\sandboxtag)\notin\tmsmonitor.\text{F} 
    \\
    \tmsmonitor[']=\left\{\text{A}: \tmsmonitor.\text{A}\setminus\left\{(\loc;\sandboxtag)\right\} \text{F}: \tmsmonitor.\text{F}\cup\left\{(\loc;\sandboxtag)\right\}\right\}
  }{
...
\begin{center}\small
  \judgbox{\tmstraceagree[]{\event}{\absevent}}{,,Abstract event $\absevent$ is equivalent to $\event$ w.r.t. \gls*{tms}.''}$\;$\\
  %
...
The trace agreement maps actions to their respective (e.g., \Cref{tr:tms-alloc-authentic}), so we elide most cases.
% 
Since branches are not relevant for \gls*{tms}, they are mapped to $\absev{\emptyevent}$ (\Cref{tr:tms-branch-authentic}).
% \MP{
...
Relating this trace to abstract monitor events yields trace:
  ...
  However, having returned from \texttt{strncpy}, the \texttt{greet} function continues and deallocates $\loc[_{y}]$ whose subsequent use in \texttt{printf} is a use-after-free bug.
...
\subsubsection{Combining \glsfirst*{tms} and \glsfirst*{sms} Monitors into \glsfirst*{ms}}\label{subsubsec:ms}
...
\begin{center}
  \judgbox{\sccttraceagree[]{\event}{\absevent}}{,,Abstract event $\absevent$ is equivalent to $\event$ w.r.t. \gls*{cct}.''}$\;$\\
  %
...
Programs ($\prog{\library_{\ctx}}{\library_{\comp}}$) consist of libraries ($\library$), which are lists of functions ($\function$).
...
Projection $\msfilterLtms{\src{\event}}$ translates an action into its correspondent (e.g., \Cref{tr:ltms-filter-alloc}); since these rules are straightforward they are mostly omitted.
Crucially, the projection filters irrelevant actions such as context-made ones (\Cref{tr:ltms-filter-context}) and interaction events (\Cref{tr:ltms-filter-start}).
...
The following rules demonstrate how this is handled semantically. \MP{which ones ? refs?}
...
\begin{exampleenv}[$\Lscct$ with and without data independent timing]\label{ex:lscct}
  ...
  The left half of \Cref{fig:ex-cct} ($\Lscct$, the two columns on the left), describes the execution trace of the program, while the right side of the table ($Spec$, the two columns on the right), describes the respective projections $\scctfilterLscct{\obj{\trace}}$ to the specification trace model.
...
\cca(\src{\lfunction{g}{x:\natt\to\type[_{e}]}{\expr}})  = &\ \lfunction[\trg]{\trg{g}}{\trg{x}}{\lifzt[\trg]{\trg{\lhast{x}{\natt}}}{\\%
...
% \begin{align*}
  ...
  \partialeval{\irl{\llet{x}{n}{\expr}}}{\irl{\substlist}} & = \partialeval{\irl{\expr}}{\irl{\subst{x}{n}\cdot\substlist}} 
  % \\
