...
\MPin{
	we should add briefly which real-world compilers (taken from the elided examples) are upper and lower compos
}
\MP{no coq here ?}
...
This section starts by extending the memory safety trace model (\Cref{subsec:basic:memsafety:tracemodel}) with a {\em security tag} ($\securitytag$) that indicates whether the involved event contains sensitive information ($\lock$) or not ($\unlock$).
...
To remedy this issue, let $\sim_{\ctsafe}$ be a cross-language trace relation whose definition can be sketched as follows:\MP{why just sketched?}
\[
...
So far, the considered trace models do not let us express speculative execution attacks such as Spectre~\cite{}\MP{cits}. 
...
Even though the considered Spectre variants are just SPECTRE-PHT~\cite{kocher2019spectre}, NONE just describes secret data as in \gls*{scct} (see \Cref{subsec:scct:tracemodel}), the trace model is general enough to allow for potential future extension with different variants.\MP{cits}
...
\MPin{recap its meaning for security}
...
\begin{definition}[\glsfirst*{ss}]\label{def:trace:ss}
  ...
  \end{nscenter}
\end{definition}
\MPin{why isn't it ok to have locks with NONE in the trace?}
...
To this end, let $\sim_{\mathghost}$ be a cross-language trace relation whose definition can be sketched as follows:\MP{sketch?}
...
We conclude this section by defining the ultimate property of interest for secure compilers: \gls*{specms}.
...
\MPin{this section does not define anything, not even the sandbox tag metavariable}
\MPin{
	move b/t/a/e/F/CHI/H/Delta/Phi/Psi to here.
	their explanation too.
	make black.
}
\MPin{
	what are pointer maps and Control states?
}
...
Operational state ($\src{\Omega}$) is a triple consisting of control-flow state ($\src{\cfstate}$), a control tag ($\src{\sandboxtag}$), and a memory state ($\src{\memstate}$), which carries information about the memory.
Because pointers are kept ``alive'' in pointer maps ($\src{\Delta}$), $\Ltms$ dynamic semantics does not get stuck if it encounters a memory-safety violation such as a double--free.
...
Even though this could be used for blame preservation~\cite{patrignani2023blame}, this beyond the focus of this paper.
...
Functions may receive arguments that are not $\trg{\natt}$ and values are extended with tuples\MP{add this?}\MP{no values in previous grammar. add?}.
...
Security tags $\obj{\securitytag}$ are arranged in the usual secrecy lattice\MP{NI cits}, where $\obj{\lock}\le\obj{\securitytag}$ and $\obj{\securitytag}\le\obj{\unlock}$.
...
\MPin{what's the i for? explain. intuition missing}
...
$\Lspec$ extends $\Lscct$ (\Cref{subsec:lscct}) with speculative dynamic semantics that is inspired by existing work~\cite{guarnieri2018spectector}\MP{cit xaver's}.
After branching or accessing memory\MP{?}, speculation starts by pushing the current configuration on the speculation register ($\ird{S}$), and run subsequent code in a predetermined window until a rollback of operational state is performed. 
The window is set inside the control-flow state ($\ird{\cfstate}$), whose details are in the technical report.
...
\[
  \begin{array}{rrcl}
    ...
    \text{(Speculation Register)} & \ird{S} &:=& \ird{\hole{\cdot}} \mid \left(\ird{\Omega};\ird{\expr}\right)
  \end{array}
\]
...
\newcommand{\exprevald}[5]{\expreval{\ird{#1}}{\ird{#2}}{\ird{#3}}{\ird{#4}}{\ird{#5}}}
  ...
  \typerule{$e-\ird{ifz}-\ird{true}-\text{spec}$}{
    ...
    \ird{S} = ({\ird{\cfstate;\sandboxtag;D;\memstate}};{\ird{\expr[_2]}}),\ird{\hole{\cdot}}
  }{
    \pstepto{\rtt{(\ird{\cfstate;\sandboxtag;D;\memstate});\ird{\hole{\cdot}}}{\ird{\lifz{0^{\securitytag}}{\expr[_1]}{\expr[_2]}}}}{\rtt{(\ird{\cfstate['];\sandboxtag;D;\memstate});\ird{S}}{\ird{\expr[_1]}}}{\ird{\trace}}
  }{e-ifz-true-spec}
  %
  \typerule{$e-\ird{spec}-\text{eat}$}{
    \ird{\cfstate}.\ird{\operatorname{win}} > 0 &
    \pstepto{\rtt{(\ird{\cfstate;\sandboxtag;D;\memstate};\ird{\hole{\cdot}})}{\ird{\expr}}}{\rtt{(\ird{\cfstate['];\sandboxtag;D;\memstate};\ird{\hole{\cdot}})}{\ird{\expr[']}}}{\ird{\event}}
    \\
    \ird{\cfstate['']}.\ird{\operatorname{win}} = \ird{\cfstate[']}.\ird{\operatorname{win}}-1 &
    \ird{S} \not=\ird{\hole{\cdot}}
  }{
    \ghoststepto{\rtt{(\ird{\cfstate;\sandboxtag;D;\memstate};\ird{S})}{\ird{\expr}}}{\rtt{(\ird{\cfstate[''];\sandboxtag;D;\memstate};\ird{S})}{\ird{\expr[']}}}{\ird{\event}}
  }{e-spec-eat}
  % 
  \typerule{$e-\ird{spec}-\text{eaten}$}{
    \ird{\cfstate}.\ird{\operatorname{win}} = 0 &
    \ird{S} = \left(\ird{\Omega};\ird{\expr[']}\right)
  }{
    \ghoststepto{\rtt{(\ird{\cfstate;\sandboxtag;D;\memstate};\ird{S})}{\ird{\expr}}}{\rtt{(\ird{\Omega};\ird{\hole{\cdot}})}{\ird{\expr[']}}}{\ird{\event}}
  }{e-spec-eaten}
  %
  \typerule{$e-\ird{spec}-\text{barrier}$}{
    \ird{\cfstate'} = \ird{\cfstate}[.\ird{\operatorname{win}} = 0] &
    \ird{S} \not=\ird{\hole{\cdot}}
  }{
    \ghoststepto{\rtt{(\ird{\cfstate;\sandboxtag;D;\memstate};\ird{S})}{\ird{\lbarrier}}}{\rtt{(\ird{\cfstate';\sandboxtag;D;\memstate};\ird{\hole{\cdot}})}{\ird{\lbarrier}}}{\ird{\event}}
  }{e-spec-barrier}
\end{center}
...
A $\ird{\lbarrier}$ prevents any execution whatsoever besides $\ird{\ev{Rlb}}$ when run in speculative execution mode and does nothing when run in normal mode, e.g., \Cref{tr:e-spec-barrier}.%
...
\MPin{what's the point of this relation? where is it used? why define it here?}
...
Contrary to the previous compiler, $\ccb$ may change the trace of the original $\Ltrg$ program: \MP{make a uniformity check: always uppercase or always lowercase after :}if there is a memory access, it needs to be protected with a bounds check.
The corresponding relation $\sim^{\Ltrg}_{\Lms}$ that describes this semantic effect of the compiler may be sketched as follows.
\MPin{no sketching. rephrase / say that omitted things are omitted for a reason}
...
Action $\trg{Set}$ is treated analogously; any other event is just a trivial re-colouring.\MP{no explicit mentioning of colours. identity functions}
Note that neither $\Ltrg$ nor $\Lms$ expose branches on the trace.\MP{so what?}
The bounds check is left abstract above, since the precise definition needs some technicalities that only inhibit readability without any technical insights.\MP{?? unclear}
...
\Cref{corr:ccab:rtp:ms} states that the composition of $\cca$ and $\ccb$ is secure w.r.t. \gls*{ms} and \MP{it. many missing such subjects}follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtpsim:sig}.
...
\footnote{For context actions, the relation is arbitrary.
\MPin{is it really?}
}
