...
This paper uses a more general notion~\cite{abate2021extacc} that applies to compilers translating from one trace model into another.
...
These projections map source-level (resp. target-level) properties to target-level (resp. source-level) properties and can be lifted to work with classes, too, by taking the powerset.
...
\begin{definition}[Robust Compilation with $\tau_\sim$]\label{def:rtp:tau}
  % $\;$\\
  % \vspace{-1em}
  % \begin{nscenter}
...
\begin{definition}[Robust Compilation with $\sigma_\sim$]\label{def:rtp:sigma}
  % $\;$\\
  % \vspace{-1em}
  % \begin{nscenter}
  % Compiler $\cc{\src{L}}{\trg{L}}$ robustly preserves $\class$, 
...
\bul{A cross-language relation $\sim$ is well-formed with respect to a class of source-level properties $\src{\class}$} iff \rul{the existential image preserves set membership}.
\begin{definition}[Well-formedness of Trace Relations for a Class $\src{\class}$]\label{def:wfc:tau:tracerel}
  % $$

  \noindent
  \text{\bul{$\wfctau{\sim}{\src{\class}}$}} \isdef \text{\rul{$\forall \src{\Pi}\in\src{\class}, \tau_\sim(\src{\Pi})\in\tilde{\tau}_\sim(\src{\class})$}}
  % $$
\end{definition}
...
Let \bul{$\cc{\src{L}}{\trg{L}}$ robustly preserve the class $\src{\class[_1]}$ under $\sim_1$} and let \rul{$\cc{\trg{L}}{\obj{L}}$ robustly preserve the class $\tilde{\tau}_{\sim_1}\left(\src{\class[_2]}\right)$ under $\sim_2$}.
Then, \iul{when the cross-language relation $\sim_1$ is well-formed with respect to class $\src{\class[_2]}$}, it follows that \oul{the composed compiler $\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}$ robustly preserves the intersection of classes $\src{\class[_1]}\cap\src{\class[_2]}$ under $\sim_1\bullet\sim_2$}.
% 
...
% \MPin{why}
\begin{theorem}[Sequential Composition of Secure Compilers with respect to $\tau$]\label{thm:rtpsim:tau}
  If \bul{$\rtptau{\cc{\src{L}}{\trg{L}}}{\src{\class[_{1}]}}{\sim_1}$}, \rul{$\rtptau{\cc{\trg{L}}{\obj{L}}}{\tilde{\tau}_{\sim_1}\left(\src{\class[_{2}]}\right)}{\sim_2}$}, and \iul{$\wfctau{\sim_1}{\src{\class[_2]}}$}, then \oul{$\rtptau{\cc{\src{L}}{\trg{L}}\circ\cc{\trg{L}}{\obj{L}}}{\src{\class[_{1}]}\cap\src{\class[_{2}]}}{\sim_1\bullet\sim_2}$}. \Coqed
\end{theorem}
...
The degenerate case where the cross-language trace relation is equality has a simpler compositionality result:
...
\begin{definition}[Well-formedness of Trace Relations for a Class $\src{\class}$]\label{def:wfc:tracerel}
  % $$
...
% 
In fact, if \bul{a relation $\sim_1$ is well-formed for a property $\class$} but \rul{another relation $\sim_2$ is not well-formed for the property mapped with the first relation $\tilde{\tau}_{\sim_1}\class$} then \iul{their composition is empty}.
% 
...
\begin{definition}[\glsfirst*{tms}]\label{def:trace:tmsdef}
  ...
  \tmssafe:=\left\{\trace \left| \begin{array}{rcl}
                              \ev{Alloc\ \loc\ n;\sandboxtag}&\le_{\trace}&\ev{Dealloc\ \loc;\sandboxtag} \\
                              \ev{Use\ \loc\ n;\sandboxtag}&\le_{\trace}&\ev{Dealloc\ \loc;\sandboxtag} \\
                              \text{if }\ev{Alloc\ \loc\ n;\sandboxtag}\text{ in }\trace&\text{then}&\ev{Dealloc\ \loc;\sandboxtag}\text{ in }\trace \\
                              \text{at most one }\ev{Dealloc\ \loc;\sandboxtag}&\text{in}&\trace \\
                              \text{at most one }\ev{Alloc\ \loc\ n;\sandboxtag}&\text{in}&\trace \\
...
% \small
  ...
  \smssafe:=\left\{\trace \left|\begin{array}{rcl}
                                  \text{If }\ev{Alloc\ \loc\ n;\sandboxtag}\le_{\trace}\ev{Use\ \loc\ m;\sandboxtag}, \text{ then }m<n
                                \end{array}\right.\right\}
                                $$
  % \end{nscenter}
\end{definition}
...
\MKin{fill in the blank based off of spectetor}
...
% \begin{lemma}[Monitor Satisfaction is $\tmssafe$]\label{lem:mon:tmssafe}
...
% \begin{lemma}[Monitor Satisfaction is $\smssafe$]\label{lem:mon:smsafe}
%   $\;\;$
...
% \begin{lemma}[Monitor Satisfaction is $\mssafe$]\label{lem:mon:msafe}
...
% \begin{lemma}[Monitor Satisfaction is $\scctsafe$]\label{lem:mon:scctsafe}
%   If $\monscctsafe{\trace}$, then $\trace\in\scctsafe$. %\Coqed
% \end{lemma}

% \begin{exampleenv}[Data-independent timing mode]\label{ex:ditm}
%   Consider the function call \texttt{strncpy(1,x,y)} to the \texttt{strncpy} function (\Cref{ex:strncpy}) with low ($\unlock$) security for \texttt{x} and high security ($\lock$) for \texttt{y}.
...
% \begin{lemma}[Monitor Satisfaction is $\msscctsafe$]\label{lem:mon:msscctsafe}
%   % $\;$\\\vspace{-1em}
%   % \begin{nscenter}
%   % \noindent
