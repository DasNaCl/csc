...
To express Constant Time, we extend the memory safety trace model (\Cref{subsec:basic:memsafety:tracemodel}) with a {\em security tag} ($\securitytag{}$) that indicates whether the involved event contains sensitive information ($\lock$) or not ($\unlock$).
...
Thus, traces of \gls*{ms} trivially satisfy \gls*{scct}.
...
Security tags $\obj{\securitytag{}}$ are arranged in the usual secrecy lattice~\cite{zdancewicphd}, where $\obj{\lock}\sqsubseteq\obj{\securitytag{}}$ and $\obj{\securitytag{}}\sqsubseteq\obj{\unlock}$.
...
The next step in the chain ensures that code stays \gls*{scct} (\Cref{thm:ccscct:rtp:scct}) when compiled from $\Lms$ to $\Lscct$, which is done by switching on a data-independent timing mode for the computation.
...
Each of the presented compilers use a cross-language trace relation, which is also used to translate the property from one language to the other one (via the universal image).
% 
Whenever such translations occur, it is important to check whether there has been a change of meaning.
% 
That is, we could be using a trace relation that translates a property in a language to a different one in another language.
% 
To ensure such a bad translation of properties does not occur, \Cref{thm:prop-rel-corr} states (in a general fashion) that that each security property is properly translated using the universal image according to the cross-language trace relation used by the compiler.
...
% 
What happens in practice is that programmers are given the relations from each language to \ev{L} and they need to build a compiler that induces a relation $\sim^{\src{L}}_{\trg{L}}$ that respects this theorem.
% 
% We now informally argue why this theorem holds in all of its specialised cases.
...
% 
{\em All} traces $\trg{\trace}$ with $\trg{\trace}\sim^{\Ltrg}_{\Lms}\irl{\trace}$ are identical to $\irl{\trace}$ except for get and set actions, which require for in-bound accesses: this clearly respect $\mssafe$.
% 
...
Since $\ev{Use\ \loc\ n}\sim_{\irl{ms}}\irl{Branch\ n}\cdot\irl{Spec}\cdots$, where $\cdots$ does not contain a $\ird{\ev{Barrier}}$ event, the resulting class is \emph{not} the original $\sssafe$ that is intended and it would break the corresponding \Cref{thm:prop-rel-corr}.
...
Our secure compilation proofs rely on established proof techniques called backtranslation~\cite{abate2019jour,patrignani2021rsc}, which let one construct a source context starting from either target traces (aka trace-based backtranslations) or target contexts (aka context-based backtranslations).
...
All backtranslations in the case-study are trace based except for those required by $\ccdce$ and $\cccf$, which are context-based (and they are essentially an identity function). 
