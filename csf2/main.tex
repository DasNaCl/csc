...
To remedy this issue, let $\sim_{\ctsafe}$ be a cross-language trace relation whose definition is partially follows:
\[
...
To this end, let $\sim_{\mathghost}$ be a cross-language trace relation whose definition is partially as follows:
...
The memory state carries two heaps to model sandboxing between a context ($\src{H^\ctx}$) and a component ($\src{H^\comp}$).
...
Memory accesses to secret data need to be present to reason about memory safety, even when execution is in data-independent timing mode, e.g, \Cref{tr:to-e-get-in-noleak}.
...
Within such windows, data may leak, marked as high $\ird{\lock}$ and with an annotation that indicates the respective speculative execution variant, in this case just SPECTRE-PHT~\cite{kocher2019spectre}, whose starting-point is \Cref{tr:e-ifz-true-spec}. % where $\omega$ is the size of the speculation window, i.e., a number of reduction steps to be taken under speculative semantics.
...
\newcommand{\exprevald}[5]{\expreval{\ird{#1}}{\ird{#2}}{\ird{#3}}{\ird{#4}}{\ird{#5}}}
  ...
  \typerule{$e-\ird{ifz}-\ird{true}-\text{spec}$}{
    ...
    \pstepto{\rtt{\ird{\cfstate;\sandboxtag;D;\memstate}}{\ird{\lifz{0^{\securitytag}}{\expr[_1]}{\expr[_2]}}}}{\ird{S}}{\ird{\ev{Branch}\ 0;\sandboxtag;\securitytag}\cdot\ird{\ev{Spec};\sandboxtag;\securitytag}}
  }{e-ifz-true-spec}
  %
...
Lastly, $\ird{Rlb}$ is emitted when the speculation window is zero and the operationl state is rolled back (see \Cref{tr:e-spec-eaten}). 
...
\Cref{corr:ccab:rtp:ms} states that the composition of $\cca$ and $\ccb$ is secure w.r.t. \gls*{ms} and it follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtpsim:sig}.
