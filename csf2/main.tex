...
From an engineering perspective, this is the desirable approach to building secure compilers.
  ...
  % Ideally, compiler engineers develop two different compilation passes, one that ensures memory safety and another one that is security preserving w.r.t. cryptographic constant time. %, because this style of engineering is well-established for its simplifying divide-and-conquer approach as well as enhanced reusability.
  % \MKin{object+findings}
  % This paper answers that question positively: The composition of compilers that are secure w.r.t. some security-properties yields the intersection of these properties.
  % \MKin{object}
  ...
  % \MKin{conclusion}
  % The compositionality results enable modular proofs for the presented, secure compiler. % that is secure w.r.t. both memory safety and cryptographic constant time.
...
For sake of self-containment, the definition of the universal image is reiterated~\cite{abate2021extacc}:
...
For each of the presented properties, the technical appendix accompanying the paper defines corresponding monitors, which refine a given property and have a major role in the proofs of this paper. 
...
%AAAAAAAAAAAAAWith these control-tags it is possible to ignore effects performed by a spurious context that, e.g., immediately deallocates a memory location twice, thus violating \gls*{tms}~\cite{nagarakatte2010cets}.
...
Both operations are also modelled in the trace model by extending the set of base-events with branches on some value $n$ ($\ev{Branch\ n}$) and binary operations with result $n$ ($\ev{Binop\ n}$).
...
Moreover, programs that operate on secrets and have a ``data independent timing mode'' enabled do not leak these secrets, despite performing computation on them (see \Cref{ex:lscct}). 
...
The relation is extended from just events to traces in a standard way, skipping the empty event $\emptyevent$ on either side, and it is now possible to define \gls*{msscct} using the universal image:
...
\subsection{Memory-Safety, Constant-Time Trace Model with Speculation}\label{subsec:msctss:tracemodel}
...
Moreover, add pre-events signaling the beginning of a speculative execution ($\ev{Spec}$), a barrier ($\ev{Barrier}$) that signals that any speculative execution may not go past it, as well as a rollback event ($\ev{Rlb}$), which signals that execution resumes to where speculation started.
...
The intuition is that speculation is ignored, which amounts to dropping events $\ev{Spec}$, $\ev{Rlb}$, or $\ev{Barrier}$, as well as all base events tagged with $\lock_{\text{PHT}}$. 
Now, define \gls*{specms} as follows:
...
\[
  \begin{array}{rrcl}
    ...
    \text{(Pre-Event)} & \obj{\preevent} &:=& \dots \mid \obj{iGet\ \loc\ v} \mid \obj{iSet\ \loc\ v} \\
                       &&& \mid\ \obj{Binop\ n} \mid \obj{Branch\ n} \\
    \text{(Event)} & \obj{\event} &:=& \obj{\event[^b];\sandboxtag;\securitytag} \mid \obj{\emptyevent} \mid \obj{\lightning} \\
...
\[
  \begin{array}{rrcl}
    \text{(Pre-Events)} & \ird{\preevent} &:=& \dots \mid \ird{Spec} \mid \ird{Rlb} \mid \ird{Barrier}\\
...
\newcommand{\exprevald}[5]{\expreval{\ird{#1}}{\ird{#2}}{\ird{#3}}{\ird{#4}}{\ird{#5}}}
  ...
  \typerule{$e-\ird{ifz}-\ird{true}-\text{spec}$}{
    ...
    \pstepto{\rtt{\ird{\cfstate;\sandboxtag;0;\memstate}}{\ird{\lifz{0^{\securitytag}}{\expr[_1]}{\expr[_2]}}}}{\begin{array}{c}\rtt{\ird{\cfstate['];\sandboxtag;0;\memstate}}{\ird{\expr[_1]}}\\\rtt{\ird{\cfstate;\sandboxtag;0;\memstate}}{\ird{\expr[_2]}}\end{array}}{\ird{\ev{Branch}\ 0;\sandboxtag;\securitytag}\cdot\ird{\ev{Spec};\sandboxtag;\securitytag}}
  }{e-ifz-true-spec}
  %
...
\begin{figure*}[!h]
  ...
  \begin{tikzpicture}
    ...
    \draw[thick,dotted,->] (S) to[bend right=33,sloped] node[align=center] {\gls*{ms}\\ \Cref{thm:ccab:rtp:ms}} (M);
...
\Cref{thm:ccab:rtp:ms} states that the composition of $\cca$ and $\ccb$ is secure w.r.t. \gls*{ms} and follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtp}.
...
Because of this, the compiler is secure w.r.t. \gls*{scct}, similarly proven as in \Cref{subsec:cs:tms}.
