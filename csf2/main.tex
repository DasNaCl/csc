...
However, these trace models are not the same trace model we used to specify the properties of \Cref{sec:compprop} (which we denote with \ev{L}), and which serves as the ``ground truth'' of the meaning of a certain property.
% 
To bridge this gap, the formal development requires specifying additional trace relations, from each of the language trace models to the \ev{L} trace model, that, for example, relate $\src{Get\ \loc\ n}$ and $\src{Set\ \loc\ n}$ to $\ev{Use\ \loc\ n}$.
% 
...
\begin{theorem}[Properties Relation Correctness]\label{thm:prop-rel-corr}
  \begin{align*}
    ...
    \\
    \text{ if }& \sigma_{\sim^{\trg{L}}_{\ev{L}}}(\pi) \sim^{\trg{L}}_{\ev{L}} \pi
    \text{ and } \sigma_{\sim^{\src{L}}_{\trg{L}}}(\trg{\pi}) \sim^{\src{L}}_{\trg{L}} \sigma_{\sim^{\trg{L}}_{\ev{L}}}(\pi)
    \\
    \text{ then }& \sigma_{\sim^{\src{L}}_{\trg{L}}}(\trg{\pi}) \sim^{\src{L}}_{\ev{L}} \pi
  \end{align*}
\end{theorem}
...
Recall that $\sim^{\Ltms}_{\Ltrg}$ is an equality, so if $\src{\trace}\sim^{\Ltms}_{\Ltrg}\trg{\trace}$, we know that $\src{\trace}$ and $\trg{\trace}$ are identical.
