...
Consider an examples of compilers fulfilling \Cref{def:rtp}.
The first example is a mechanized compiler that preserves all safety properties for a language with pointers compiled to an assembly-like language~\cite{korashy2022secureptrs}.
The mechanization has well over 20k lines in total, which demonstrates that proving a compiler secure with respect to \Cref{def:rtp} is a significant effort.

% i don't see how adding a def of cct is beneficial
The second example is a compiler ensuring the \gls*{cct} property. %, whose precise definition can be found in \Cref{sec:cct}.
While we were not able to find compilers in literature that provably fulfill \Cref{def:rtp}, we anticipate that FaCT~\cite{cauligi2019fact} would satisfy this hyperproperty.
As common, FaCT proves constant-timeness by means of overapproximation, i.e., instead of directly proving the hyperproperty, they fall back to a property that overapproximates the corresponding hyperproperty.
...
% 
Note that the previous results also hold for the existential image $\tau_\sim\left(\src{\pi}\right)$ as well as for classes of hyperproperties, but these results have been moved to the technical report in the interest of space.
...
Plugging the examples given in \Cref{subsec:bg:rtp} into \Cref{thm:rtpsim:sig}, we get a compiler that robustly preserves safety properties and the \gls*{cct} hyperproperty. 
...
This mode is inspired by a combination of the effect of the FaCT compiler~\cite{cauligi2019fact} as well as a data-independent timing mode, a feature that is present in both Arm~\cite[p.~543]{arm-refman} and Intel~\cite[p.~80]{intel-refman} processors.
