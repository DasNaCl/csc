...
\institute{}
% CISPA Helmholtz Center for Information Security and Saarland University
% \and
...
% \email{matthis.kruse@cispa.de \and director@cispa.de \and marco.patrignani@unitn.it}
...
\end{center}
\keywords{memory-safety, secure compilation, formal methods}
\end{abstract}
...
$\Ltrg$ extends the syntax presented earlier (\Cref{subsec:cs:defs}) with dynamic typechecks ($\trg{\lhast{\expr}{\type}}$).
The pointers carry poison tags, which signify a pointer being allocated ($\trg{\poisoned}$) or freed ($\trg{\poisonless}$). 
The syntax of $\Ltrg$ is extended further with a way to inspect whether a pointer is invalid ($\trg{\lispoisoned{x}}$).
