...
\myfig{
  \begin{center}
    ...
    \judgbox{\xlangstateeq{\Omega}{\Omega}}{,,The $\mmlAs$ state $\configs$ agrees with $\mmlAt$ state $\configt$.''} \\
    %
    \typerule{state-eq}{
    ...
    %
    \judgbox{\xlangstateeq{\memstate}{\memstate}}{,,The $\mmlAs$ memory-state $\src{\memstate}$ agrees with $\mmlAt$ one $\trg{\memstate}$.''} \\
    %
    \typerule{memstate-eq}{
      \src{\memstate}=\src{\Delta;H} &
      \trg{\memstate}=\trg{\Delta;H} \\
...
\myfig{
  \begin{center}
    %
    \judgbox{\xlangstateeq{H}{H}}{,,The $\mmlAs$ heap $\src{H}$ agrees with $\mmlAt$ one $\trg{H}$.''} \\
    %
    \typerule{empty-heap-eq}{
    }{
      \xlangstateeq{\hole{\cdot}}{\hole{\cdot}}
    }{compile-empty-heap-eq}
    %
    ...
    }{compile-cons-heap-eq}
    %
    \judgbox{\xlangstateeq{\Delta}{\Delta}}{,,The $\mmlAs$ location-map $\src{\Delta}$ agrees with the $\mmlAt$ one $\trg{\Delta}$.''} \\
    %
    \typerule{empty-locmap-eq}{
    }{
      \xlangstateeq{\hole{\cdot}}{\hole{\cdot}}
    }{compile-empty-locmap-eq}
    %
  ...
  \end{center}
}{tms-pres-memstate-relations}{Memory-State Relations from $\mmlAs$ to $\mmlAt$ and vice versa.}
\myfig{
  \begin{center}
    %
...
$\;$\MK{fix kontstack relation. intuitively, one $\src{\kontstack}$ may be related to multiple $\trg{\kontstack}$, but each $trg{K}$ should have $\trg{\nocomm}$}
$\;$\MK{do more or less the same for $\multimap$}
    %
