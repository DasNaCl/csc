...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-top-level}{
      ...
      \trg{\trace[_0]}=\trg{Call\ \comptoctx\ main\ 0\cdot\trace[_0']\cdot Call\ \ctxtocomp\ foo\ \interfacevalue}\\
...
\begin{lemma}[Backtranslation Correctness]\label{lem:backtranslation:correct}
  ...
  \item $\xlangstateeq[\emptyset]{\Omega}{\Omega}$
...
\end{lemma}
\begin{incompleteproof}
  Inverting \Cref{asm:backtrans:correct:backtrans} gives\\$\trg{\trace} = \trg{Call\ \comptoctx\ main\ 0\cdot\trace[_{0}]\cdot Call\ \ctxtocomp\ foo\ \interfacevalue_{0}\cdot\trace[_{comp}]\cdot Ret\ \comptoctx\ \interfacevalue_{1}\cdot\trace[_{1}]\cdot Ret\ \ctxtocomp\ \interfacevalue_{2}}$, $\vdash\trg{\trace[^{comp}]}\operatorname{non-int-\trg{\trace}}$, $\mmlAmmlAtbackctxtocomp[\Game,\trg{\hole{\cdot}}]{\trace[_0]} = \Game',\trg{\overline{\loc}},\src{e_0},\src{e_0'},\src{let\ z\ =\ new\ n_0\ in\ e_0''}$, and $\mmlAmmlAtbackcomptoctx[\Game',\trg{\overline{\loc}}]{\trace[_1]} = \Game'',\trg{\hole{\cdot}},\src{e_1},\src{e_1'},\src{e_1''}$, such that $\src{e}=\src{e_0;e_0';let\ z\ =\ new\ n_0\ in\ \left(e_0'';e_1\right);e_1';e_1''}$ and $\src{\library_{ctx}}=\src{let\ main\ x:\nat\to\nat:=e,\hole{\cdot}}$.

  By transitivity and inversion, $\exect{\Omega}{call\ main\ 0}{\Omega}{e_0}{Call\ \comptoctx\ main\ 0}$ and $\exect{\Omega}{e_0}{\Omega'}{\finalexprnoerr}{\trace[_a]}$, where $\trg{\trace}=\trg{Call\ \comptoctx\ main\ 0\cdot\trace[_a]}$.

  Now use \Thmref{lem:exprbacktranslation:vbeg:wrapper:correct} to get
  \begin{passumptions}{H}
    \item $\emptyset\subseteq\delta_{0}$
    \item $\src{\hole{\cdot}}\subseteq\src{L_0}$
    \item $\src{\hole{\cdot}}\subseteq\src{X_0}$
    \item $\execs{\Omega}{call\ main\ 0}{\commlib;\library;\hole{\cdot}^{main}_{\ctxtocomp},\hole{\cdot};\memstate}{e_{main}}{\trace}$
    \item $\xlangbackstateeq[\delta_0;\src{L_0}]{\Omega}{\commlib;\library;\hole{\cdot}^{main}_{\ctxtocomp},\hole{\cdot};\memstate}$
    \item $\src{\trace}\thexlangtraceeq[\delta_0;\src{X_0}]\trg{\trace}$
  \end{passumptions}

  %\Cref{lem:exprbacktranslation:beg:wrapper:correct}
\end{incompleteproof}
