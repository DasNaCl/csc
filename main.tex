...
\begin{proof}
  ...
  \begin{assumptions}
    \item[$F_0$:] $\emptyset\subseteq\delta_{\text{TMS}}^{(p)}$
    \item[$F_1$:]\label[ass]{asm:mmlatmsviamonitor:traceagreeasm} $\tmstraceagree[\delta^{(p)}]{\overline{a_p}}{\src{\overline{a_p}}}$
    \item[$F_2$:]\label[ass]{asm:mmlatmsviamonitor:monitorcheckasm} $\monitorcheck{\emptyset}{\tmsmonitor}{\overline{a_p}}$
    \item[$F_3$:] $\storeagreetms[\delta^{(p)}]{\tmsmonitor}{\src{\Omega_p}.\src{\Delta}}$
  \end{assumptions}

  Now instantiate with $\delta_{\text{TMS}}^{(p)}$, $\overline{a_p}$, and $\tmsmonitor$, so the remaining goals are:
