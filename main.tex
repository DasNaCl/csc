...
\myfig{
  \begin{center}
    ...
    $\;$
    %
    \judgbox{\delta(\trg{\loc})=\src{\loc}}{,,The $\mmlAt$ memory location $\trg{\loc}$ corresponds to\\the $\mmlAs$ memory location $\src{\loc}$.''} \\
...
\begin{lemma}[Backtranslation Correctness]\label{lem:backtranslation:correct}
  ...
  \item $\xlangbackstateeq{\Omega}{\Omega}$
  \end{assumptions}
  then $\exists \delta'\ \src{\Omega'}\ \src{\trace}\ \src{f}$,
  \begin{goals}
  \item $\delta\subseteq\delta'$
  \item $\exec{\configs}{\src{e}}{\src{\configs'}}{\src{f}}{\src{\trace}}$
...
\end{lemma}
\begin{incompleteproof}
  First consider $\trg{\finalexpr}=\trg{stuck}$. In this case, we use \Cref{lem:backtranslation:stuck:correct,tr:back-state-fail-eq} and are done.
\end{incompleteproof}
