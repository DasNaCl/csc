...
\myfig{
  ...
  \typerule{splitEmptyL}{
    ...
    \src{\Gamma} = \src{\hole{\cdot}}\circ\src{\Gamma}
  }{t-empty-splitL}
  %
  \typerule{splitEmptyR}{
  }{
    \src{\Gamma} = \src{\Gamma}\circ\src{\hole{\cdot}}
  }{t-empty-splitR}
  %
  \typerule{$\nat$split}{
...
This is inspired by linear logic. To deal with program contexts, we have two additional rules \Cref{tr:t-let,tr:t-hole} that carry additional type informationn.
Note that $\src{hole : \type}$ has no runtime action, which is why we have an additional judgement as presented in \Cref{fig:runtime-term} that ensures that a given well-typed program can run.
...
\myfig{
  \begin{center}
    \judgbox{\hasholes{x:\type_1}{e}{\type_2}}{,,$\src{e}$ contains exacty one $\src{hole:(x:\type_1)\to\type_2}$ that\\may be filled with an expression $\src{e':\type_2}$ that\\uses $\src{x:\type_1}$.''}
...
\myfig{
  ...
  \typechecks{}{H;\Delta\triangleright e}{\type}
	}{w-t-prog}
  \end{center}
...
For \Cref{tr:w-t-ctx}, note that $\src{e_{1}}$ encodes the input to the component bound to variable $\src{x}$.
\myfig{
  \begin{center}
    \judgbox{\left[\typechecks{}{\Omega\triangleright\src{e}}{\type}\right]}{,,$\src{e}$ is a valid runtime term.''}
    \begin{center}
      \typerule{runtime-term}{
        \neg\left(\hasholes{x:\type_1}{e}{\type_2}\right) &
        \typechecks{}{\Omega\triangleright e}{\type}
      }{
        \left[\typechecks{}{\Omega\triangleright\src{e}}{\type}\right]
      }{runtime-term-rule}
    \end{center}
  \end{center}
...
\myfig{
  \begin{center}
  \judgbox{\plugs{e_0}{e_1}=\src{e_2}}{,,Replace any $\src{hole}$ in $\src{e_1}$ by $\src{e_0}$, resulting in $\src{e_2}$.\\$Q$ is a placeholder for either $Q$ or $N$ and may be used to switch\\hole filling modes.''}
    \typerule{plug-hole}{
      \typechecks{x:\type_1}{e}{\type_2}
    }{
      \plugs{e}{hole:(x:\type_1)\to\type_2}=\src{calling\ x; returning\ e}
    }{s-plug-hole}
    %
    \typerule{plug-hole'}{
      \fresh{{\color{red}TODO}}{\src{z}} &
    }{
      \plugs[N]{e}{hole:(x:\type_1)\to\type_2}=\src{let\ z = hole:(x:\type_1)\to\type_2\ in\ e;z}
    }{s-plug-holep}
    %
...
\Cref{tr:s-plug-holep} is needed for backtranslation. We insert wrapper code that allocates memory, which, as enforced by the typesystem, wants to be freed.
...
\myfig{
  ...
  \typerule{$e-\text{wprog}$}{
    ...
    \execs{\configs_0}{e_{po}}{\configs_{po}}{e_0''}{\trace[_p]\cdot Ret\ !\finalexprnoerr_p}\\
  ...
  \typerule{$e-\text{wprog}-$failp}{
    ...
    \typechecks{}{prog\ e_0\ e_p\ e_1}{\nat} \\
    \execs{\hole{\cdot};\hole{\cdot}}{e_0'}{\configs_0}{e_{po}}{\trace[_0]\cdot Call\ ?\finalexprnoerr_0}&
    \execs{\configs_0}{e_{po}}{\configs_{po}}{stuck}{\trace[_p]}\\
  }{
...
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\msevent \bnfdef&\ \specificev{\emptyevent}\mid \specificev{Alloc\ \loc\ n} \mid \specificev{Dealloc\ \loc} \mid \specificev{Use\ \loc\ n} \\
...
The empty action $\specificev{\emptyevent}$ is necessary. When proving lemmas by structural induction on the primitive steps (\Cref{fig:w-expr-eval}), we get a concrete event $\src{\emptyevent}$ that needs to be related to some $\msevent$. The natural candidate is $\specificev{\emptyevent}$.
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      \mi{Abstract\ References}~\bm{L} \subseteq& \nat \hspace{0.5cm}
      \mi{Abstract\ Store}~\tmsmonitor =\{A : L, F : L\} \\
      \mi{Abstract\ Events}~\absevent \bnfdef&\ \bm{\varepsilon} \mid \textbf{Alloc}\ \loc \mid \textbf{Dealloc}\ \loc \mid \textbf{Use}\ \loc \mid \abstermination\\
      \mi{References\ Agreement}~\delta_{\text{tms}} :& \specificev{L}\to \bm{L} \\[0.6cm]
      ...
      \{\bm{\loc}\}\cup \tmsmonitor &= \{A : \{\bm{\loc}\}\cup \tmsmonitor[.A], F : \tmsmonitor[.F]\}\\
      \tmsmonitor\setminus\{\bm{\loc}\} &= \{A : \tmsmonitor[.A]\setminus\{\bm{\loc}\}, F : \tmsmonitor[.F]\cup\left\{\bm{\loc}\right\}\}\\
    \end{aligned}
  \end{gather*}
}{tms-monitor}{\gls{tms} Monitor.}

We have $\bm{\varepsilon}$ in order to accompany unimportant events, such as $\specificev{Call\ ?\finalexprnoerr}$ or $\specificev{Ret\ !\finalexprnoerr}$.
Moreover, when doing structural induction over primitive steps (\Cref{fig:w-expr-eval}) we may encounter the $\src{\emptyevent}$, for which $\tospecificevs{\emptyevent}=\specificev{\emptyevent}$, which needs a ,,partner'' in the abstract events as defined in \Cref{fig:tms-monitor}: $\bm{\varepsilon}$.
...
\myfig{
  \begin{center}
    ...
    %
    \typerule{TMS-Use}{
      \bm{\loc}\in \tmsmonitor[.A] &
      \bm{\loc}\not\in \tmsmonitor[.F] &
    }{
      \monitorcheck[]{\tmsmonitor}{\tmsmonitor}{\textbf{Use}\ \bm{\loc}}
    }{tms-use}
    %
    \typerule{TMS-Alloc}{
      \tmsmonitor[']=\left\{\bm{\loc}\right\}\cup\tmsmonitor&
      \bm{\loc}\not\in \tmsmonitor&
    }{
      \monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\textbf{Alloc}\ \bm{\loc}}
    }{tms-alloc}
    %
    \typerule{TMS-Dealloc}{
      \bm{\loc}\in \tmsmonitor[.A] &
      \bm{\loc}\not\in \tmsmonitor[.F] &
      \tmsmonitor[']=\tmsmonitor\setminus\left\{\bm{\loc}\right\}
    }{
      \monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\textbf{Dealloc}\ \bm{\loc}}
    }{tms-dealloc}
    %
    \typerule{TMS-Refl}{
    }{
...
\myfig{
  \begin{center}
    \judgbox{\delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}}{,,A map from $\mmlAs$ memory locations $\specificev{\loc}$ to abstract memory locations $\bm{\loc}$.''}
    ...
    \typerule{TMS-Alloc-Authentic}{
      \delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}
    }{
      \tmstraceagree{\specificev{Alloc\ \loc\ n}}{\textbf{Alloc}\ \bm{\loc}}
    }{tms-alloc-auth}
    %
    \typerule{TMS-Dealloc-Authentic}{
      \delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}
    }{
      \tmstraceagree{\specificev{Dealloc\ \loc}}{\textbf{Dealloc}\ \bm{\loc}}
    }{tms-dealloc-auth}
    %
    \typerule{TMS-Use-Authentic}{
      \delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}
    }{
      \tmstraceagree{\specificev{Use\ \loc\ n}}{\textbf{Use}\ \bm{\loc}}
    }{tms-use-auth}
    %
    ...
    \typerule{TMS-Call-Authentic}{
      ...
      \tmstraceagree{\specificev{Call\ ?n}\cdot\mstrace}{\bm{\varepsilon}\cdot\abstrace}
    }{tms-call-auth}
    %
    \typerule{TMS-Ret-Authentic}{
      \tmstraceagree{\mstrace}{\abstrace}
    }{
      \tmstraceagree{\specificev{Ret\ !n}\cdot\mstrace}{\bm{\varepsilon}\cdot\abstrace}
    }{tms-ret-auth}
  \end{center}
...
\myfig{
  \begin{center}
    \judgbox{\storeagreetms{\tmsmonitor}{\src{\Delta}}}{,,The abstract memory state $\tmsmonitor$ describes the concrete state $\src{\Delta}$.''}
    \typerule{Empty-Agree}{
    }{
    ...
    %
    \typerule{Cons-Agree}{
      \tospecificevs{\loc}=\specificev{\loc}&
      \delta_{\text{tms}}\left(\specificev{\loc}\right)=\bm{\loc}&
      \bm{\loc}\not\in\tmsmonitor[.F]&
      \storeagreetms{\tmsmonitor}{\src{\Delta}}
    }{
      \storeagreetms{\left\{\bm{\loc}\right\}\cup \tmsmonitor}{\src{x\mapsto\loc_\poisonless,\Delta}}
    }{store-cons-agree}
    %
...
\begin{scontents}[store-env=buffer]
  If
  \begin{assumptions}
  \item\label[ass]{asm:mmlatmsviamonitor:run} $\wexecs{prog\ e_0\ e_p\ e_1}{\configs}{v}{\trace}$
  \end{assumptions}
  Then
  \begin{goals}
...
\begin{proof}
  ...
  First note that $\src{\nat}\ \operatorname{int}$ by \Cref{tr:src-int-type-nat}.
  ...
  \bigskip
  \noindent\makebox[\textwidth]{%
    \typerulederiv{$\text{\Cref{tr:w-t-ctx}}$}{
      \hasholes{x:\nat}{e_1}{\nat}&
      \typechecks{}{let\ y=e_1\ in\ y}{\nat}&
      \src{\nat}\ \operatorname{int}&
      \src{\nat}\ \operatorname{int}
    }{
      \typecheck{}{\src{context\ }(\src{e_1})\ (\src{y})}{\src{(\nat\to\nat)\to\nat}}
    }}
  \bigskip

  We omit subsequent derivations, but note that they can be found using \Cref{tr:hashole-Llet,tr:hashole-hole} and \Cref{tr:t-let,tr:t-nat,tr:t-var,tr:t-hole}.
  So, both context and component typecheck.

  Therefore, we can plug using \Cref{tr:src-plugging} which yields \\$\src{prog}\ (\src{let\ x=42\ in\ hole:(x:\nat)\to\nat})\ (\src{e_p})\ (\src{y})$.\\[0.3cm]
  Running the whole program via \Cref{tr:ws-e-prog} yields trace prefix\\$\src{\trace}=\src{Ret\ !0}\cdot\src{Call\ ?42}\cdot\src{Alloc\ \loc\ 42}\cdot\src{Get\ \loc\ 1337}\cdot\src{Dealloc\ \loc}\cdot\src{Ret\ !1729}\cdot\src{Call\ ?1729}$.
  We omit the precise derivation for brevity.\\[0.3cm]
  By \Cref{fig:w-events-project-to-specific}, $\tospecificevs{\trace}=\specificev{Ret\ !0}\cdot\specificev{Call\ ?42}\cdot\cdot\specificev{Alloc\ \loc\ 42}\cdot\specificev{Use\ \loc\ 1337}\cdot\specificev{Dealloc\ \loc}\cdot\specificev{Ret\ !1729}\cdot\specificev{Call\ ?1729}$.\\[0.3cm]
...
\end{lemma}
\begin{proof}
  Simple unfolding of \Cref{tr:w-t-wprog}.
\end{proof}
...
\end{lemma}
\begin{proof}
  By induction on \Cref{asm:plugsnohole}.
\end{proof}
  ...
  \begin{description}
    \item[$\src{e}=\src{delete\ x}$:]
      ...
      \begin{passumptions}{H}
        \item $\typechecks{}{\src{H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}\triangleright \src{delete\ x}}{\type}$
      ...
      Choose $\absevent=\textbf{Dealloc\ }\bm{\loc}$ and $\delta_{\text{tms}}'=\delta_{\text{tms}}$.

      \Cref{goal:basetmsmonitor:subseteq}, $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}$, follows trivially.

      For $\tmstraceagree{\specificev{Dealloc\ \loc}}{\textbf{Dealloc}\ \bm{\loc}}$ (\Cref{goal:basetmsmonitor:tmstraceagree}), apply \Cref{tr:tms-dealloc-auth}, where $\tospecificevs{\loc}=\specificev{\loc}$ and $\delta_{\text{tms}}(\specificev{\loc})=\loc$ follow by inverting \Cref{asm:basetmsmonitor:storeagree}.
      ...
      \item[\Cref{tr:store-cons-agree}:] \ \\
        ...
        \item $\left[\typechecks{}{\src{H;\Delta_1},\src{x\mapsto\loc_\poisonless},\src{\Delta_2}\triangleright \src{delete\ x}}{\type}\right]$
        \item $\storeagreetms{\tmsmonitor[_1]}{\src{\Delta_1}}$
        \item $\tospecificevs{\loc}=\specificev{\loc}$
        \item $\delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}$
        \item $\storeagreetms{\tmsmonitor[_2]}{\src{\Delta_2}}$
        \item $\tmsmonitor=\tmsmonitor[_1]\cup\left\{\bm{\loc}\right\}\cup \tmsmonitor[_2]$
        \end{passumptions}
        then $\exists \tmsmonitor[']$,
        \begin{goals}
        \item\label[goal]{goal:basetmsmonitor:monitorcheck1} $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\textbf{Dealloc}\ \bm{\loc}}$
        ...
        \Cref{goal:basetmsmonitor:monitorcheck1}, $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\textbf{Dealloc}\ \bm{\loc}}$, follows immediately by \Cref{tr:tms-dealloc}.
      ...
      \item[\Cref{tr:store-poison-agree}:] \ \\
        If
        \begin{passumptions}{F}
        \item\label[ass]{asm:basetmsmonitor:typing} $\typechecks{}{\src{H;\Delta_1},\src{x\mapsto\loc_\poisoned},\src{\Delta_2}\triangleright \src{delete\ x}}{\type}$
        \item $\storeagreetms{\tmsmonitor[_1]}{\src{\Delta_1}}$
        \item $\tospecificevs{\loc}=\specificev{\loc}$
        \item $\delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}$
        \item $\storeagreetms{\tmsmonitor[_2]}{\src{\Delta_2}}$
        ...
        then $\exists \tmsmonitor[']$,
        \begin{goals}
        \item\label[goal]{goal:basetmsmonitor:monitorcheck2} $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\textbf{Dealloc}\ \bm{\loc}}$
      ...
      \end{description}
    \item[$\src{e}=\src{returning\ \finalexprnoerr}$:]
      If
      \begin{passumptions}{H}
        \item $\left[\typechecks{}{\src{\Omega}\triangleright \src{returning\ \finalexprnoerr}}{\type}\right]$
        \item\label[ass]{asm:basetmsmonitor:case1:storeagree} $\storeagreetms{\tmsmonitor}{\src{\Omega}}$
      \end{passumptions}
      Then $\exists \absevent\ \delta_{\text{tms}}'\ \tmsmonitor['],$
      \begin{goals}
        \item\label[goal]{goal:basetmsmonitor:case1:subseteq} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
        \item\label[goal]{goal:basetmsmonitor:case1:tmstraceagree} $\traceagree[\delta_{\text{tms}}']{\specificev{Ret\ !\finalexprnoerr}}{\absevent}$
        \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\absevent}$
    ...
    \item[$\src{e}=\src{calling\ \finalexprnoerr}$:]
      If
      \begin{passumptions}{H}
        \item $\left[\typechecks{}{\src{\Omega}\triangleright \src{calling\ \finalexprnoerr}}{\type}\right]$
        \item\label[ass]{asm:basetmsmonitor:case2:storeagree} $\storeagreetms{\tmsmonitor}{\src{\Omega}}$
      \end{passumptions}
      Then $\exists \absevent\ \delta_{\text{tms}}'\ \tmsmonitor['],$
      \begin{goals}
        \item\label[goal]{goal:basetmsmonitor:case2:subseteq} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
        \item\label[goal]{goal:basetmsmonitor:case2:tmstraceagree} $\traceagree[\delta_{\text{tms}}']{\specificev{Call\ ?\finalexprnoerr}}{\absevent}$
        \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta']{\tmsmonitor[']}{\src{\Omega}}$
      \end{goals}
      Choose $\absevent=\bm{\emptyevent}$, $\delta'_{\text{tms}}=\delta_{\text{tms}}$, and $\tmsmonitor[']=\tmsmonitor$.
    ...
    \item[$\src{e}=\src{let\ x=new\ n\ in\ e'}$:]
      If
      \begin{passumptions}{H}
        \item $\left[\typechecks{}{\src{H};\src{\Delta}\triangleright\src{let\ x=new\ n\ in\ e'}}{\type}\right]$
        \item\label[ass]{asm:basetmsmonitor:case4:storeagree} $\storeagreetms{\tmsmonitor}{\src{H};\src{\Delta}}$
      ...
      The goals become:
      \begin{goals}
        \item\label[goal]{goal:basetmsmonitor:case4:subseteq} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}\cup\left\{\specificev{\loc}\mapsto\bm{\loc}\right\}$
        \item\label[goal]{goal:basetmsmonitor:case4:tmstraceagree} $\traceagree[\delta_{\text{tms}}\cup\left\{\specificev{\loc}\mapsto\bm{\loc}\right\}]{Alloc\ \loc\ n}{\bm{Alloc\ \loc}}$
        \item\label[goal]{goal:basetmsmonitor:case4:monitorstep} $\monitorcheck[]{\tmsmonitor}{\left\{A:\tmsmonitor.A\cup\left\{\loc\right\},F:\tmsmonitor.F\right\}}{\bm{Alloc\ \loc}}$
        \item\label[goal]{goal:basetmsmonitor:case4:storeagree} $\storeagreetms[\left\{\specificev{\loc}\mapsto\bm{\loc}\right\}\cup\delta]{\left\{\bm{\loc}\right\}\cup\tmsmonitor}{\src{z\mapsto\loc_{\poisonless},\Delta}}$
      \end{goals}
    ...
    \item[$\src{e}=\src{x[n]\leftarrow\valueexpr}$:]
      ...
      \begin{passumptions}{H}
        \item $\left[\typechecks{}{\src{H};\src{\Delta_1,x\mapsto\loc_\poison,\Delta_2}\triangleright\src{x[n]}}{\type}\right]$
        \item $\storeagreetms{\tmsmonitor}{\src{\Delta_1,x\mapsto\loc_\poison,\Delta_2}}$
        \item $\src{H'}=\src{H}\left(\src{l+n}\mapsto\src{\valueexpr}\right)$
      \end{passumptions}
      Then $\exists \absevent\ \delta_{\text{tms}}'\ \tmsmonitor['],$
      \begin{goals}
        \item $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
...
\begin{lemma}[Ctx \gls{tms} via Monitor]\label{lem:ctxsteptmsmonitor}
  If
  \begin{assumptions}
    \item\label[ass]{asm:ctxtmsmonitor:typechecks} $\left[\typechecks{}{\src{\Omega}\triangleright e}{\src{\type}}\right]$
...
\begin{lemma}[Steps \gls{tms} via Monitor]\label{lem:generaltms:monitor}
  If
  \begin{assumptions}
    \item\label[ass]{asm:stepstmsmonitor:typing} $\left[\typechecks{}{\src{\Omega}\triangleright e}{\src{\type}}\right]$
  ...
  \begin{goals}
    \item\label[goal]{goal:stepstmsmonitor:delta} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
    \item\label[goal]{goal:stepstmsmonitor:traceagree} $\tmstraceagree[\delta']{\tospecificevs{\trace}}{\abstrace}$
    \item\label[goal]{goal:stepstmsmonitor:monitorstep} $\monitorcheck{\tmsmonitor}{\tmsmonitor[']}{\abstrace}$
    \item\label[goal]{goal:stepstmsmonitor:storeagree} $\storeagreetms[\delta']{\tmsmonitor[']}{\src{\Omega'}.\src{\Delta}}$
  \end{goals}.
\end{lemma}
\begin{proof}
  ...
  \begin{description}
    ...
    \item[Induction step:]
          ...
          \begin{passumptions}{A}
            \item $\left[\typechecks{}{\src{\Omega_{\IH}}\triangleright e_{\IH}}{\src{\type}}\right]$
            \item $\storeagreetms[\delta^{0}]{\tmsmonitor[^{\IH}]}{\src{\Omega_{\IH}}.\src{\Delta}}$
          \end{passumptions}
          then $\exists \abstrace_{\IH}\ \delta_{\text{tms}}^{\IH}\ \tmsmonitor[_{\IH}'],$
          \begin{passumptions}{\IH}
            \item $\delta_{\text{tms}}^{0}\subseteq\delta_{\text{tms}}^{\IH}$
            \item $\tmstraceagree[\delta^{\IH}]{\tospecificevs{\trace}}{\abstrace_{\IH}}$
            \item $\monitorcheck{\tmsmonitor^{\IH}}{\tmsmonitor[_{\IH}']}{\abstrace_{\IH}}$
            \item $\storeagreetms[\delta^{\IH}]{\tmsmonitor[_{\IH}']}{\src{\Omega'}.\src{\Delta}}$
          \end{passumptions}
          ...
          \begin{passumptions}{H}
            ...
            \item\label[ass]{asm:stepstmsmonitor:case0:checks} $\left[\typechecks{}{\src{\Omega}\triangleright e}{\src{\type}}\right]$
            \item\label[ass]{asm:stepstmsmonitor:case0:storeagree} $\storeagreetms{\tmsmonitor}{\src{\Omega}.\src{\Delta}}$
          \end{passumptions}
          Then $\exists \abstrace\ \delta_{\text{tms}}'\ \tmsmonitor['],$
          \begin{goals}
            \item\label[goal]{goal:stepstmsmonitor:final:delta} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
            \item\label[goal]{goal:stepstmsmonitor:final:traceagree} $\tmstraceagree[\delta']{\tospecificevs{\event[_{0}]\cdot\trace}}{\abstrace}$
            \item\label[goal]{goal:stepstmsmonitor:final:monitorstep} $\monitorcheck{\tmsmonitor}{\tmsmonitor[']}{\abstrace}$
            \item\label[goal]{goal:stepstmsmonitor:final:storeagree} $\storeagreetms[\delta']{\tmsmonitor[']}{\src{\Omega'}.\src{\Delta}}$
          \end{goals}.
          ...
          \begin{passumptions}{F}
            \item\label[ass]{asm:stepstmsmonitor:deltasubset0} $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}^{0}$
            \item\label[ass]{asm:stepstmsmonitor:traceagree0} $\tmstraceagree[\delta^{0}]{\tospecificevs{\event[_{0}]}}{\absevent_{0}}$
            \item\label[ass]{asm:stepstmsmonitor:monitorcheck0} $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[_{0}]}{\absevent_{0}}$
            \item\label[ass]{asm:stepstmsmonitor:case0:storeagree1} $\storeagreetms[\delta^{0}]{\tmsmonitor[_{0}]}{\src{\Omega_{0}}.\src{\Delta}}$
          \end{passumptions}
          ...
          \begin{passumptions}{G}
            \item\label[ass]{asm:stepstmsmonitor:deltasubset1} $\delta_{\text{tms}}^{0}\subseteq\delta_{\text{tms}}^{\IH}$
            \item\label[ass]{asm:stepstmsmonitor:traceagree1} $\tmstraceagree[\delta^{\IH}]{\tospecificevs{\trace}}{\abstrace_{\IH}}$
          ...
          We solve our goals by instantiating $\abstrace=\absevent\cdot\abstrace_{\IH}$, $\delta_{\text{tms}}'=\delta_{\text{tms}}^{\IH}$, and $\tmsmonitor[']=\tmsmonitor[^{\IH}]$.
          ...
          Similarily for \Cref{goal:stepstmsmonitor:final:traceagree} using \Cref{asm:stepstmsmonitor:traceagree0,asm:stepstmsmonitor:traceagree1}, making use of \Thmref{lem:trace-agree-weaken}.
...
\myfig{
  ...
  \mi{Expressions}~\trg{e} \bnfdef&\ \trg{x} \mid \trg{v} \mid \trg{e_{1} \oplus e_{2}} \mid \trg{x[e]} \mid \trg{let\ x = e_{1}\ in\ e_{2}}\mid \trg{x[e_{1}]\leftarrow e_{2}} \\
    & \mid \trg{let\ x=new\ e_{1}\ in\ e_{2}} \mid \trg{delete\ x} \mid \trg{calling\ e} \mid \trg{returning\ e} \\
    & \mid \trg{ifz\ e_1\ then\ e_2\ else\ e_3} \mid \trg{abort()} \mid \trg{hole:x} \\
  ...
  %
  \mi{Values}~\trg{v} \bnfdef\ \trg{\langle n, n\rangle} \mid \trg{n}\in\trg{\nat} \hspace{0.5cm}
  ...
  \mi{Final\ Result}\ \trg{\finalexprnoerr} \bnfdef\ \trg{\valueexpr} \mid \trg{x}\\
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      \trg{let\ x=K\ in\ e}\\
                     ...
                     &\mid \trg{ifz\ K\ then\ e_1\ else\ e_2}\mid\trg{K\ has\ \type}\mid\trg{calling\ K}\mid\trg{returning\ K}
    \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    \judgbox{\hasholet{x}{e}}{,,$\trg{e}$ contains exacty one $\trg{hole:x}$ that\\may be filled with an expression $\trg{e'}$ that\\uses $\trg{x}$.''}
  \typerule{t-has-hole-$\trg{hole}$}{
  }{
    \hasholet{x}{hole:x}
  }{t-hashole-hole}
  %
  \typerule{t-has-hole-L$\trg{\oplus}$}{
    \hasholet{x}{e_1}&
    \neg\left(\hasholet{x}{e_2}\right)
  }{
    \hasholet{x}{e_1\oplus e_2}
  }{t-hashole-Lbinop}
  %
  \typerule{t-has-hole-R$\trg{\oplus}$}{
    \neg\left(\hasholet{x}{e_2}\right)&
    \hasholet{x}{e_2}
  }{
    \hasholet{x:\type_1}{e_1\oplus e_2}
  }{t-hashole-Rbinop}
  %
  \typerule{t-has-hole-$\trg{y[e]}$}{
    \hasholet{x}{e}
  }{
    \hasholet{x}{y[e]}
  }{t-hashole-access}
  %
  \typerule{t-has-hole-L$\trg{let}$}{
    \hasholet{x:\type_1}{e_1}&
    \neg\left(\hasholet{x}{e_2}\right)
  }{
    \hasholet{x}{let\ y=e_1\ in\ e_2}
  }{t-hashole-Llet}
  %
  \typerule{t-has-hole-R$\trg{let}$}{
    \neg\left(\hasholet{x}{e_1}\right)&
    \hasholet{x}{e_2}
  }{
    \hasholet{x}{let\ y=e_1\ in\ e_2}
  }{t-hashole-Rlet}
  %
  \typerule{t-has-hole-Lput}{
    \hasholet{x}{e_1} &
    \neg\left(\hasholet{x}{e_2}\right)
  }{
    \hasholet{x}{y[e_1]\leftarrow e_2}
  }{t-hashole-Lput}
  %
  \typerule{t-has-hole-Rput}{
    \neg\left(\hasholet{x}{e_1}\right)&
    \hasholet{x}{e_2}
  }{
    \hasholet{x}{y[e_1]\leftarrow e_2}
  ...
  %
  \typerule{t-has-hole-L$\trg{new}$}{
    \hasholet{x}{e_1}&
    \neg\left(\hasholet{x}{e_2}\right)
  }{
    \hasholet{x}{let\ y=new\ e_1\ in\ e_2}
  }{t-hashole-Lnew}
  %
  \typerule{t-has-hole-R$\trg{new}$}{
    \neg\left(\hasholet{x}{e_1}\right)&
    \hasholet{x}{e_2}
  }{
    \hasholet{x}{let\ y=new\ e_1\ in\ e_2}
  ...
  %
  \typerule{t-has-hole-C$\trg{ifz}$}{
    \hasholet{x}{e_1}\\
    \neg\left(\hasholet{x}{e_2}\right)&
    \neg\left(\hasholet{x}{e_3}\right)
  }{
    \hasholet{x}{ifz\ e_1\ then\ e_2\ else\ e_3}
  }{t-hashole-Cif}
  %
  \typerule{t-has-hole-T$\trg{ifz}$}{
    \neg\left(\hasholet{x}{e_1}\right)\\
    \hasholet{x}{e_2}&
    \neg\left(\hasholet{x:\type_1}{e_3}\right)
  }{
    \hasholet{x}{ifz\ e_1\ then\ e_2\ else\ e_3}
  }{t-hashole-Tif}
  %
  \typerule{t-has-hole-F$\trg{ifz}$}{
    \neg\left(\hasholet{x}{e_1}\right)\\
    \neg\left(\hasholet{x}{e_2}\right)&
    \hasholet{x}{e_3}&
  }{
  ...
  %
  \typerule{t-has-hole-Lpair}{
    \hasholet{x}{e_1}\\
    \neg\left(\hasholet{x}{e_2}\right)
  }{
    \hasholet{x}{\langle e_1,e_2 \rangle}
  }{t-hashole-Lpair}
  %
  \typerule{t-has-hole-Rpair}{
    \neg\left(\hasholet{x}{e_1}\right)\\
    \hasholet{x}{e_2}
  }{
    \hasholet{x}{\langle e_1,e_2 \rangle}
  ...
  %
  \typerule{t-has-hole-$\trg{\pi_{1}}$}{
    \hasholet{x}{e}
  }{
    \hasholet{x}{\pi_1 e}
  }{t-hashole-pi1}
  %
  \typerule{t-has-hole-$\trg{\pi_{2}}$}{
    \hasholet{x}{e}
  }{
    \hasholet{x}{\pi_2 e}
  }{t-hashole-pi2}
  %
  \typerule{t-has-hole-$\trg{has\ \type}$}{
    \hasholet{x}{e}
  }{
    \hasholet{x}{e\ has\ \type}
  }{t-hashole-hastype}
  \end{center}
}{t-hashole}{A check for $\mmlAt$ expressions to see whether they contain exactly one $\trg{hole:x}$.}
...
\myfig{
  ...
  %
  \typerule{$e-\trg{returning}$}{
  }{
    \exprevalt{\configt}{returning\ \finalexprnoerr}{\configt}{\finalexprnoerr}{Ret\ !\finalexprnoerr}
  }{tw-e-returning}
  %
  \typerule{$e-\trg{calling}$}{
  }{
    \exprevalt{\configt}{calling\ \finalexprnoerr}{\configt}{\finalexprnoerr}{Call\ ?\finalexprnoerr}
  }{tw-e-calling}
  %
  ...
  \typerule{$e-\trg{let}-\trg{\finalexprnoerr}$}{
    ...
    \exprevalt{\tconfig{H}{\Delta}}{let\ x=\finalexprnoerr\ in\ e}{\tconfig{H}{\Delta}}{e[\finalexprnoerr/x]}{\emptyevent}
  }{tw-e-let-x}
  %
  \typerule{$e-\trg{delete}$}{
  }{
...
\myfig{
  \begin{center}
  \judgbox{\plugt{e_0}{e_1}=\trg{e_2}}{,,Replace any $\trg{hole:x}$ in $\trg{e_1}$ by $\trg{e_0}$, resulting in $\trg{e_2}$.\\$Q$ is a placeholder for either $Q$ or $N$ and may be used to switch\\hole filling modes.''}
    \typerule{t-plug-hole}{
    }{
      \plugt{e}{hole:x}=\trg{calling\ x; returning\ e}
    }{t-plug-hole}
    %
    \typerule{t-plug-hole'}{
      \fresh{{\color{red}TODO}}{\trg{z}} &
    }{
      \plugt[N]{e}{hole:x}=\trg{let\ z = hole:x\ in\ e;z}
    }{t-plug-holep}
    %
    \typerule{t-plug-finalexprnoerr}{
    }{
      \plugt{e}{\finalexprnoerr}=\trg{\finalexprnoerr}
    }{t-plug-finalexprnoerr}
    %
    \typerule{t-plug-$\trg{delete}$}{
    }{
      \plugt{e}{delete\ x}=\trg{delete\ x}
    }{t-plug-delete}
    %
    \typerule{t-plug-$\trg{abort}$}{
    }{
      \plugt{e}{abort}=\trg{abort}
    }{t-plug-abort}
    %
    \typerule{t-plug-$\trg{\oplus}$}{
      \plugt{e}{e_0}=\trg{e_0'} &
      \plugt{e}{e_1}=\trg{e_1'}
    }{
      \plugt{e}{e_0\oplus e_1}=\trg{e_0'\oplus e_1'}
    }{t-plug-binop}
    %
    \typerule{t-plug-$\trg{get}$}{
      \plugt{e}{e_0}=\trg{e_0'}
    }{
      \plugt{e}{x[e_0]}=\trg{x[e_0']}
    }{t-plug-get}
    %
    \typerule{t-plug-$\trg{put}$}{
      \plugt{e}{e_0}=\trg{e_0'}&
      \plugt{e}{e_1}=\trg{e_1'}
    }{
      \plugt{e}{x[e_0]\leftarrow e_1}=\trg{x[e_0']\leftarrow e_1'}
    }{t-plug-put}
    %
    \typerule{t-plug-$\trg{let}$}{
      \plugt{e}{e_0}=\trg{e_0'}&
      \plugt{e}{e_1}=\trg{e_1'}
    }{
      \plugt{e}{let\ x = e_0\ in\ e_1}=\trg{let\ x = e_0'\ in\ e_1'}
    }{t-plug-let}
    %
    \typerule{t-plug-$\trg{new}$}{
      \plugt{e}{e_0}=\trg{e_0'}&
      \plugt{e}{e_1}=\trg{e_1'}
    }{
      \plugt{e}{let\ x = new\ e_0\ in\ e_1}=\trg{let\ x = new\ e_0'\ in\ e_1'}
    }{t-plug-new}
    %
    \typerule{t-plug-$\trg{ifz}$}{
      \plugt{e}{e_0}=\trg{e_0'}&
      \plugt{e}{e_1}=\trg{e_1'}&
      \plugt{e}{e_2}=\trg{e_2'}
    }{
      \plugt{e}{ifz\ e_0\ then\ e_1\ else\ e_2}=\trg{ifz\ e_0'\ then\ e_1'\ else\ e_2'}
    }{t-plug-ifz}
    %
    \typerule{t-plug-pair}{
      \plugt{e}{e_0}=\trg{e_0'}&
      ...
      \plugt{e}{\langle e_0, e_1 \rangle}=\trg{\langle e_0',e_1' \rangle}
    }{t-plug-pair}
    %
    \typerule{t-plug-$\trg{\pi_{1}}$}{
      \plugt{e}{e_0}=\trg{e_0'}
    }{
      \plugt{e}{\pi_1 e_0}=\trg{\pi_1 e_0'}
    }{t-plug-pi1}
    %
    \typerule{t-plug-$\trg{\pi_{2}}$}{
      \plugt{e}{e_0}=\trg{e_0'}
    }{
      \plugt{e}{\pi_2 e_0}=\trg{\pi_2 e_0'}
    }{t-plug-pi2}
    %
    \typerule{t-plug-$\trg{has\ \type}$}{
      \plugt{e}{e_0}=\trg{e_0'}
    }{
      \plugt{e}{e_0\ has\ \type}=\trg{e_0'\ has\ \type}
    }{t-plug-has}
  \end{center}
}{w-plug-s}{Replace any $\trg{hole:x}$ in an expression.}
\myfig{
  \begin{center}
  ...
  \typerule{$te-\text{wprog}$}{
    \plugt{e_p}{e_0}=\trg{e_0'}\\
    \exect{\hole{\cdot};\hole{\cdot}}{e_0'}{\configt_0}{e_{po}}{\trace[_0]\cdot Call\ ?\finalexprnoerr_0}&
    \exect{\configt_0}{e_{po}}{\configt_{po}}{e_0''}{\trace[_p]\cdot Ret\ !\finalexprnoerr_p}\\
    \exect{\configt_{po}}{e_0''}{\configt_0'}{\finalexprnoerr_p'}{\trace[_0']}&
    \exect{\configt_0'}{e_1[\finalexprnoerr_p'/y]}{\configt_1}{\finalexprnoerr_1}{\trace[_1]}
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\configt_1}{\finalexprnoerr_1}{Ret\ !0\cdot\trace{_0}\cdot Call\ ?\finalexprnoerr_0\cdot \trace[_p]\cdot Ret\ !\finalexprnoerr_p\cdot\trace[_0']\cdot \trace[_1]\cdot Call\ ?\finalexprnoerr_1}
  }{wt-e-prog}
  \typerule{$te-\text{wprog}-$fail0}{
    \plugt{e_p}{e_0}=\trg{e_0'}\\
    \exect{\hole{\cdot};\hole{\cdot}}{e_0'}{\lightning}{stuck}{\trace[_0]}&
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{Ret\ !0\cdot\trace{_0}}
  }{wt-e-prog-fail0}
  \typerule{$te-\text{wprog}-$failp}{
    \plugt{e_p}{e_0}=\trg{e_0'}\\
    \exect{\hole{\cdot};\hole{\cdot}}{e_0'}{\configt_0}{e_{po}}{\trace[_0]\cdot Call\ ?\finalexprnoerr_0}&
    \exect{\configt_0}{e_{po}}{\configt_{po}}{stuck}{\trace[_p]}\\
  }{
  ...
  \typerule{$te-\text{wprog}-$fail0'}{
    ...
    \exect{\hole{\cdot};\hole{\cdot}}{e_0'}{\configt_0}{e_{po}}{\trace[_0]\cdot Call\ ?\finalexprnoerr_0}&
    \exect{\configt_0}{e_{po}}{\configt_{po}}{e_0''}{\trace[_p]\cdot Ret\ !\finalexprnoerr_p}\\
    \exect{\configt_{po}}{e_0''}{\lightning}{stuck}{\trace[_0']}&
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{Ret\ !0\cdot\trace{_0}\cdot Call\ ?\finalexprnoerr_0\cdot \trace{_p}\cdot Ret\ !\finalexprnoerr_p\cdot \trace{_0'}}
  }{wt-e-prog-fail0p}
  \typerule{$te-\text{wprog}-$fail1}{
    \plugt{e_p}{e_0}=\trg{e_0'}\\
    \exect{\hole{\cdot};\hole{\cdot}}{e_0'}{\configt_0}{e_{po}}{\trace[_0]\cdot Call\ ?\finalexprnoerr_0}&
    \exect{\configt_0}{e_{po}}{\configt_{po}}{e_0''}{\trace[_p]\cdot Ret\ !\finalexprnoerr_p}\\
    \exect{\configt_{po}}{e_0''}{\configt_0'}{\finalexprnoerr_p'}{\trace[_0']}&
    \exect{\configt_0'}{e_1[\finalexprnoerr_p'/y]}{\lightning}{stuck}{\trace[_1]}
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{Ret\ !0\cdot\trace[_0]\cdot Call\ ?\finalexprnoerr_0\cdot \trace[_p]\cdot Ret\ !\finalexprnoerr_p\cdot \trace[_0']\cdot \trace[_1]}
  }{wt-e-prog-fail1}
  \end{center}
...
\myfig{
  \begin{center}
    ...
    \mmlAmmlAtcomp{hole:(x:\type_{1})\to\type_{2}} &\ = & \trg{hole:}\mmlAmmlAtcomp{x} \\
    \mmlAmmlAtcomp{calling\ e} &\ = & \trg{calling\ }\mmlAmmlAtcomp{e}\\
    \mmlAmmlAtcomp{returning\ e} &\ = & \trg{returning\ }\mmlAmmlAtcomp{e}\\
    \mmlAmmlAtcomp{abort()} &\ = & \trg{abort()}\\
...
\myfig{
  \begin{center}
    ...
    \begin{array}{rll}
      ...
      \mmlAmmlAtcomp{calling\ K} &\ = & \trg{calling\ }\mmlAmmlAtcomp{K}\\
      \mmlAmmlAtcomp{returning\ K} &\ = & \trg{returning\ }\mmlAmmlAtcomp{K}\\
    \end{array}
    $$
  \end{center}
...
\myfig{
  \begin{center}
    ...
    $$
    \begin{array}{rcl}
    \mmlAmmlAtcomp{component\ e_p} =& \trg{component\ }&\trg{ifz\ x\ has\ \nat\ then\ }\\
                                    &                  &~~\mmlAmmlAtcomp{e_p}\\
                                    &                  &\trg{else\ ifz\ x\ has\ \nat\times\nat\ then\ }\\
                                    &                  &~~\trg{abort()}\\
                                    &                  &\trg{else}\\
...
For example, by adding the dynamic type check we prevent contexts from binding $\trg{\langle 42,1729\rangle}$ to $\trg{x}$.
\myfig{
  \begin{center}
...
\begin{lemma}[Step Forward Simulation]\label{lem:step:forwardsim}
  ...
  \begin{assumptions}
    ...
    \item\label[ass]{asm:step:forwardsim:srccheck} $\left[\typechecks{}{\Omega\triangleright e}{\nat}\right]$
  \end{assumptions}
...
\end{lemma}
\begin{proof}
  We proceed by induction on \Cref{asm:step:forwardsim:srcstep}. Note that by \Cref{asm:step:forwardsim:srccheck} we know that the $\src{e}$ does not contain a $\src{hole:(x:\type_{1})\to\type_{2}}$.
  \begin{description}
...
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtbackdetail[\Game,B]{\mstrace} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and\\an $\mmlAs$ expression from memory-specific $\mmlAt$ trace $\trg{\mstrace}$.\\$B$ may be $()$ or $\src{x:\type_1\to\type_2}$.''}$\;$\\
    \judgbox{\mmlAmmlAtbackdetail[\Game,B]{\specificev{Call\ ?\finalexprnoerr}} = \Game',\partial\ell,\src{e}}{,,Given a \varmapbacktransds $\Game$ and $B$ either $()$ or $\src{x:\type_1\to\type_2}$,\\construct a \varmapbacktransds $\Game'$, $\partial\ell$ as either $()$ or a location\\$\trg{\ell}$, and an $\mmlAs$ expression from memory-specific $\mmlAt$\\event $\trg{\specificev{Call\ !\finalexprnoerr}}$.''}$\;$\\
    \judgbox{\mmlAmmlAtbackdetail[\Game,\partial\ell,\src{C}]{\specificev{Ret\ !\finalexprnoerr}} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, $\partial\ell$ as either $()$ or $\trg{\ell}$, and $\src{C}$ as either\\$()$ or $\src{e}$, construct a \varmapbacktransds $\Game'$ and an $\mmlAs$ expression from\\memory-specific $\mmlAt$ event $\trg{\specificev{Ret\ !\finalexprnoerr}}$.''}$\;$\\
    %
    \typerule{backtrans-alloc}{
      \Game'=\Game[\trg{\loc}] &
      \Game'\left(\trg{\loc}\right) = \src{z} &
      \mmlAmmlAtbackdetail[\Game',B]{\mstrace} = \Game'',\src{e} &
    }{
      \mmlAmmlAtbackdetail[\Game',B]{\specificev{Alloc\ \loc\ n}\cdot\mstrace} = \Game'', \src{let\ z = new\ }\mmlAmmlAtbackv{n}\src{\ in\ e}
    }{backtrans-alloc}
    %
    \typerule{backtrans-dealloc}{
      ...
      \mmlAmmlAtbackdetail[\Game',B]{\specificev{Dealloc\ \loc}\cdot\mstrace} = \Game'', \src{delete\ }\Game'(\trg{\loc})\src{; e}
    }{backtrans-dealloc}
    %
    \typerule{backtrans-use}{
      ...
      \mmlAmmlAtbackdetail[\Game',B]{\specificev{Use\ \loc\ n}\cdot\mstrace} = \Game'',\src{z[}\mmlAmmlAtbackv{n}\src{];e}
    }{backtrans-use}
    \begin{flushleft}
    \noindent{\tiny without wrapper}\hfill%
    \vspace{-0.2cm}
    \end{flushleft}
    \hrule
    %
    \typerule{backtrans-empty-unit}{
    }{
      \mmlAmmlAtbackdetail[\Game,()]{\hole{\cdot}} = \Game, \src{42}
    }{backtrans-empty-unit}
    %
    %
    \typerule{backtrans-call-nohole}{
    }{
      \mmlAmmlAtbackdetail[\Game,()]{\specificev{Call\ ?\finalexprnoerr}} = \Game, (), \mmlAmmlAtbackv{\finalexprnoerr}
    }{backtrans-call-nohole}
    %
    \typerule{backtrans-ret-nowrap}{
    }{
      \mmlAmmlAtbackdetail[\Game,(),()]{\specificev{Ret\ !\finalexprnoerr}} = \Game, \mmlAmmlAtbackv{\finalexprnoerr}
    }{backtrans-ret-nowrap}
    %
    \begin{flushleft}
    \noindent{\tiny wrapper}\hfill%
    \vspace{-0.2cm}
    \end{flushleft}
    \hrule
    \typerule{backtrans-empty-hole}{
    }{
      \mmlAmmlAtbackdetail[\Game,\src{x:\type_1\to\type_2}]{\hole{\cdot}} = \Game, \src{hole : (x:\type_1)\to\type_2}
    }{backtrans-empty-hole}
    %
    \typerule{backtrans-call-hole}{
      \fresh{\Game}{\trg{\loc}} &
      \Game'=\Game[\trg{\loc}] &
      \mmlAmmlAtbackdetail[\Game',\src{x:\type_0\to\type_p}]{\specificev{Alloc\ \loc\ 42}} = \Game'', \src{e}
    }{
      \mmlAmmlAtbackdetail[\Game,\src{x:\type_0\to\type_p}]{\specificev{Call\ ?\finalexprnoerr}} = \Game'', \trg{\loc}, \src{let\ x = }\mmlAmmlAtbackv{\finalexprnoerr}\src{\ in\ e}
    }{backtrans-call-hole}
    %
    \typerule{backtrans-ret-wrap}{
      \src{z} = \Game(\trg{\loc}) &
      \src{e'} = \plugs[N]{delete\ z}{e}
    }{
      \mmlAmmlAtbackdetail[\Game,\trg{\loc},\src{e}]{\specificev{Ret\ !\finalexprnoerr}} = \Game', \src{e';}\mmlAmmlAtbackv{\finalexprnoerr}
    }{backtrans-ret-wrap}
  \end{center}
}{old-tms-pres-backtrans}{{\bfseries OLD} Trace-Based Backtranslation from $\mmlAt$ events to $\mmlAs$ terms.}
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtbackdetail[\Game,B]{\mstrace} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and\\an $\mmlAs$ expression from memory-specific $\mmlAt$ trace $\trg{\mstrace}$.\\$B$ may be $()$ or $\src{x:\type_1\to\type_2}$.''}$\;$\\
    \judgbox{\mmlAmmlAtbackdetail[\Game,B]{\specificev{Call\ ?\finalexprnoerr}} = \Game',\partial\ell,\src{e}}{,,Given a \varmapbacktransds $\Game$ and $B$ either $()$ or $\src{x:\type_1\to\type_2}$,\\construct a \varmapbacktransds $\Game'$, $\partial\ell$ as either $()$ or a location\\$\trg{\ell}$, and an $\mmlAs$ expression from memory-specific $\mmlAt$\\event $\trg{\specificev{Call\ !\finalexprnoerr}}$.''}$\;$\\
...
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtback[\Game,B,\partial\ell,C]{\mstrace} = \Game',\src{e_{wrap}},\src{e},\partial\ell'}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and\\two $\mmlAs$ expressions from memory-specific $\mmlAt$\\,,interfacing''-trace $\trg{\mstrace}$.''}$\;$\\
    \judgbox{\mmlAmmlAtback[\Game,\src{x:\type_{1}\to\type_{2}}]{\mstrace} = \Game',\src{context\ e_{0}\ e_{1}}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$\\and an $\mmlAs$ expression from memory-specific\\$\mmlAt$ ,,top-level'' trace $\trg{\mstrace}$.''}$\;$\\
    \typerule{backtrans-subtop-level}{\vspace{0.33cm}
      \trg{\mstrace}=\trg{\specificev{Ret\ !\finalexprnoerr_0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_1}}\\\vspace{0.33cm}
      \mmlAmmlAtbackdetail[\Game,\partial\ell,C]{\specificev{Ret\ !\finalexprnoerr_{0}}} = \Game',\src{e_{wrapend}} &
      \mmlAmmlAtbackdetail[\Game',()]{\mstrace[_0]} = \Game'', \src{e_0^{(1)}} \\
      \mmlAmmlAtbackdetail[\Game'',B]{\specificev{Call\ ?\finalexprnoerr_1}} = \Game''',\partial\loc,\src{e_0^{(2)}} &
      \src{e} = \src{e_0^{(1)};e_0^{(2)}}
    }{
      \mmlAmmlAtback[\Game,B,\partial\ell,C]{\mstrace} = \Game''', \partial\ell, \src{e_{wrapend}}, \src{e}
    }{backtrans-subtop-level}
    $\;$\vspace{1cm}$\;$
    %
    \typerule{backtrans-top-level}{\vspace{0.33cm}
      \trg{\mstrace}=\trg{\specificev{Ret\ !0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_0}\cdot\mstrace[_p]\cdot\specificev{Ret\ !\finalexprnoerr_p}\cdot\mstrace[_1]\cdot\specificev{Call\ ?\finalexprnoerr_1}}\\\vspace{0.33cm}
      \mmlAmmlAtbackdetail[\Game,\src{x:\type_{1}\to\type_{2}},(),()]{\specificev{Ret\ !0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_0}} = \Game',\src{e_{wrapend}^{(0)}},\src{e_{0}'},\trg{\ell}\\\vspace{0.33cm}
      \mmlAmmlAtbackdetail[\Game',(),\src{e_{0}'},\trg{\ell}]{\specificev{Ret\ !\finalexprnoerr_{p}}\cdot\mstrace[_1]\cdot\specificev{Call\ ?\finalexprnoerr_1}} = \Game'',\src{e_{wrapend}^{(1)}},\src{e_{1}},()\\
      \src{e_{0}} = \src{e_{wrapend}^{(0)};e_{wrapend}^{(1)}} &
    }{
      \mmlAmmlAtback[\Game,\src{x:\type_{1}\to\type_{2}}]{\mstrace} = \Game'', \src{context\ e_{0}\ e_{1}}
    }{backtrans-top-level}
  \end{center}
...
\myfig{
  \begin{center}
    \typerule{backtrans-top-level}{
      \judgbox{\mmlAmmlAtback[\Game]{\mstrace} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and an $\mmlAs$\\ expression from memory-specific $\mmlAt$ ,,top-level'' trace $\trg{\mstrace}$.''}$\;$\\
      \trg{\mstrace}=\trg{\specificev{Ret\ !0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_0}\cdot\mstrace[_p]\cdot\specificev{Ret\ !\finalexprnoerr_p}\cdot\mstrace[_1]\cdot\specificev{Call\ ?\finalexprnoerr_1}}\\
      \typecheck{\src{x:\wptr}}{\mmlAmmlAtbackv{\finalexprnoerr_0}}{\src{\type_0}}\\
      \mmlAmmlAtbackv{\finalexprnoerr_p}=\src{y}\implies\typecheck{\src{y:\wptr}}{\mmlAmmlAtbackv{\finalexprnoerr_p}}{\src{\type_p}}\\
      \mmlAmmlAtbackv{\finalexprnoerr_p}\not=\src{y}\implies\typecheck{}{\mmlAmmlAtbackv{\finalexprnoerr_p}}{\src{\type_p}}\\
      \mmlAmmlAtbackdetail[\Game,(),()]{\specificev{Ret\ !0}} = \Game,\src{e_0^{(0)}} &
      \mmlAmmlAtbackdetail[\Game,()]{\mstrace[_0]} = \Game', \src{e_0^{(1)}} \\
      \mmlAmmlAtbackdetail[\Game',\src{x:\type_0\to\type_p}]{\specificev{Call\ ?\finalexprnoerr_0}} = \Game'',\trg{\loc},\src{e_0^{(2)}} &
      \mmlAmmlAtbackdetail[\Game'',\trg{\loc},\src{e_0^{(2)}}]{\specificev{Ret\ !\finalexprnoerr_p}} = \Game''',\src{e_0^{(3)}} \\
      \src{e_0}=\src{e_0^{(0)};e_0^{(1)};e_0^{(3)}}&
      \mmlAmmlAtbackdetail[\Game''',()]{\mstrace[_1]}=\Game'''',\src{e_1^{(0)}} \\
      \mmlAmmlAtbackdetail[\Game'''',()]{\specificev{Call\ ?\finalexprnoerr_1}} = \Game'''',(),\src{e_1^{(1)}} &
      \src{e_1} = \src{e_1^{(0)};e_1^{(1)}}
    }{
      \mmlAmmlAtback[\Game]{\mstrace} = \Game'''', \src{context\ e_0\ e_1}
    }{backtrans-top-level}
    %
    \typerule{backtrans-top-level-crash}{
      \mmlAmmlAtbackdetail{\mstrace} = \Game', \src{e}
    }{
      \mmlAmmlAtback[\Game]{\mstrace\cdot\specificev{\lightning}} = \Game', \src{e;abort()}
    }{backtrans-top-level-crash}
  \end{center}
}{old-tms-pres-top-level-backtrans}{{\bfseries OLD} ,,Top-Level'' trace-based Backtranslation from $\mmlAt$ events to $\mmlAs$ terms.}
We use the syntactic sugar $\src{e_1;e_2}$ for $\src{let\ \_ = e_1\ in\ e_2}$.
Note that \Cref{tr:backtrans-top-level} introduces a {\em wrapper} that performs a memory effect, in case the trace is not stuck.
...
\begin{lemma}[Expression-based Backtranslation Correctness without Wrapper]\label{lem:exprbacktranslation:wowrapper:correct}
  ...
  \item $\mmlAmmlAtback[\Game,()]{\mstrace}=\Game',\src{e}$
  \item $\xlangbackstateeq{\Omega}{\Omega}$
  \end{assumptions}
...
\begin{incompleteproof}
  ...
  \item $\mmlAmmlAtback[\Game,()]{\hole{\cdot}}=\Game,\src{e}$
  ...
  Instantiate our goal with $\delta' = \delta, \src{\Omega'}=\src{\Omega}, \src{\trace}=\src{\hole{\cdot}}, \src{\valueexpr}=\src{42}$, the first two goals and the fourth follow immediately by reflexivity, the third goal follows by \Cref{asm:backtranscorr:case0:stateeq} after inverting \Cref{asm:backtranscorr:case0:exect}.
  ...
  \begin{description}
    ...
    \item\label[ass]{asm:backtranscorr:indu:backtrans} $\mmlAmmlAtback[\Game_{x},()]{\mstrace}=\Game',\src{e_a}$
      ...
      Inversion on \Cref{asm:backtranscorr:indu:spectrace} to get $\specificevt{Alloc\ \loc\ n}=\trg{Alloc\ \loc\ n}$, subsequently inverting \Cref{asm:backtranscorr:indu:step}\todo{not inversion,technical lemma} yields:
      \begin{passumptions}{H}
      \setcounter{enumi}{4}
    ...
    \item\label[ass]{asm:backtranscorr:indu:steps} $\exect{H';z\mapsto\loc_{\poisonless},\Delta}{e'}{\Omega''}{v}{\trace}$
      ...
      \begin{goals}
      \item $\delta\subseteq\delta'$
      \item $\exec{\configs}{\src{let\ x=new\ }\mmlAmmlAtbackv{n}\src{\ in\ e'}}{\src{\configs'}}{\src{\valueexpr}}{\src{\trace}}$
      \item $\xlangbackstateeq[\delta']{\Omega''}{\Omega'}$
      ...
      \item $\exec{\configs}{\src{let\ x=new\ }\mmlAmmlAtbackv{n}\src{\ in\ e'}}{\src{\configs''}}{\src{\valueexpr}}{\src{Alloc\ \loc\ n\cdot\trace}}$
...
\begin{incompleteproof}
  ...
  Using \Thmref{lem:compvals:insertion}, we rewrite \Cref{rtmsp:runt:ep}:
  \begin{passumptions}{H'}
    \setcounter{enumi}{1}
\item\label[ass]{rtmsp:runt:ep:compv0} $\exec{\trg{\Omega_1^\diamond}}{\mmlAmmlAtcomp{e_p}\trg{[}\mmlAmmlAtcomp{\valueexpr_1}\trg{/x]}}{\trg{\Omega_p}}{\mmlAmmlAtcomp{\valueexpr_p}}{\trg{\trace[_p]}}$
  ...
  \end{passumptions}
  Now use \Thmref{lem:substitution} and note that $\src{\hole{\cdot}}=\src{\hole{\cdot}}\circ\src{\hole{\cdot}}$ holds by \Cref{tr:t-empty-split}:
  \begin{passumptions}{H'}
    \setcounter{enumi}{3}
  ...
  \end{passumptions}
  Finally, use \Thmref{lem:stateinsertion} on \Cref{rtmsp:runt:statebackeq}:
  \begin{passumptions}{H'}
    \setcounter{enumi}{5}
  ...
  Using \Thmref{lem:stateinsertion} on \Cref{rtmsp:runt:stateeq2}, we get:
  \begin{passumptions}{F}
    \setcounter{enumi}{4}
  \item\label[ass]{rtmsp:runt:statebackeq2} $\xlangbackstateeq[\delta_p]{\Omega_p}{\Omega_p}$
  \end{passumptions}

  Using these facts, we apply \Thmref{lem:backtranslation:correct}\MK{backtranslation needs to be generalized over substitutions} on \Cref{rtmsp:runt:e2}:
