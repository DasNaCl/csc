...
\begin{incompleteproof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtrans:correct:libeq} $\xlangstateeq[]{\library}{\library}$\MK{use $\commlib$, so that only $\library_{comp}$ must be compiled; we agreed to use traces though, so somehow extract that info from the trace \textbf{now}}
