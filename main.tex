...
\myfig{
  ...
  & \mid \src{let\ x=new\ e_{1}\ in\ e_{2}} \mid \src{delete\ x} \mid \src{e^\diamond}
  ...
  %
  \mi{Importance}~\ \src{q_i}~\bnfdef&\  \mid \;\src{\diamond}\hspace{0.5cm}
  \mi{Typing. Env.}~\src{\Gamma} \bnfdef\ \src{\hole{\cdot}} \mid \Gammas,\src{x:\tau} \hspace{0.5cm}
  \mi{Store}~\src{\Delta} \bnfdef\ \src{\hole{\cdot}} \mid \src{x\mapsto\loc^\poison_{q_i}},\Deltas\\
  %
...
\myfig{
  \begin{center}
    ...
    %
    \typerule{$T\text{ignore}\Delta$}{
    \typechecks{}{\Delta}{\Gamma}
    }{
    \typechecks{}{x\mapsto\loc^\rho_\diamond,\Delta}{\Gamma}
    }{t-ignore-store}
    %
    \typerule{$T\text{ref}\nat$}{
    \typechecks{}{\Delta}{\Gamma}
...
\myfig{
  ...
  \typerule{store-qualify}{
    \src{\Delta'}=\src{\Delta^\diamond}
  }{
    \src{H;\Delta'}=\src{(H;\Delta)^\diamond}
  }{t-store-q}
  %
  \typerule{store-mark-nil}{
  }{
    \src{\hole{\cdot}}=\src{\hole{\cdot}^\diamond}
  }{t-store-mark-nil}
  %
  \typerule{store-mark-cons}{
    \src{\Delta'}=\src{\Delta^\diamond}
  }{
  \src{x\mapsto\loc^\rho_\diamond,\Delta'}=\src{(x\mapsto\loc^\rho,\Delta)^\diamond}
  }{t-store-mark-cons}
  \end{center}
...
\myfig{
  ...
  %
  \typerule{$e-\text{ctx}-\text{unimportant}$}{
    \exprevals{\configs}{e}{\configs'}{e'}{a}
  }{
    \execs[]{\configs}{(K[e])^\diamond}{\configs'}{(K[e'])^\diamond}{a^\diamond}
  }{w-e-ectx-unimportant}
  %
  \typerule{$e-\src{\oplus}$}{
...
%    \execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\overline{a_1}} &
    ...
    \wexecs{prog\ e_0\ e_p\ e_1}{\configs}{v}{\overline{a}}%a_0^\diamond\cdot a_p\cdot a_1^\diamond}}
  }{w-e-prog}
  \end{center}
}{wprog-run}{Running a whole $\mmlA$ program.}
\MK{Figure 16: Maybe mark unimportant stuff in $\src{\Omega_1}$}
...
\myfig{
  \begin{center}
    ...
    \typerule{Cons-Agree}{
      ...
      \storeagree{\tmsmonitor}{\src{\Delta}}
    }{
      \storeagree{\left\{\loc\right\}\cup \tmsmonitor}{\src{x\mapsto\loc^\poisonless,\Delta}}
    }{store-cons-agree}
    %
    \typerule{Ignore-Agree}{
      ...
      \storeagree{\tmsmonitor}{\src{x\mapsto\loc^\poisoned,\Delta}}
    }{store-ignore-agree}
  \end{center}
...
\end{lemma}
\begin{proof}
  Easy induction on $\monitorcheck{\tmsmonitor}{\tmsmonitor[']}{a}$ using \Cref{lem:monitor-step-subset}.
\end{proof}
...
\begin{lemma}[Base \gls{tms} via Monitor]
  ...
  \begin{assumptions}
    \item $\typechecks{}{\src{\Omega}\triangleright e}{\src{\nat}}$
    \item $\exprevals{\configs}{e}{\configs'}{e'}{a}$
    \item $\storeagree{\tmsmonitor}{\src{\Omega}.\src{\Delta}}$
  \end{assumptions}
  ...
  \begin{goals}
    ...
    \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{a}$
    \item $\storeagree[\delta_{\text{tms}}']{\tmsmonitor[']}{\src{\Omega'}.\src{\Delta}}$
  \end{goals}.
\end{lemma}
\begin{incompleteproof}
...
\item[$\src{e}=\src{free\ x}$:]
  ...
  \begin{assumptions}
    ...
    \item\label[ass]{asm:basetmsmonitor:storeagree} $\storeagree{\tmsmonitor}{\src{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}$
  \end{assumptions}
  Then $\exists a\ \delta_{\text{tms}}'\ \tmsmonitor['],$
  \begin{goals}
    ...
    \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{a}$
    \item $\storeagree[\delta_{\text{tms}}']{\tmsmonitor[']}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.
...
\item[\Cref{tr:store-cons-agree}:]
  ...
  \item $\storeagree{\tmsmonitor[_1]}{\src{\Delta_1}}$
  \item $\delta_{\text{tms}}(\src{\loc})=\loc$
  \item $\storeagree{\tmsmonitor[_2]}{\src{\Delta_2}}$
  ...
  \item\label[goal]{goal:basetmsmonitor:storeagree1} $\storeagree[\delta_{\text{tms}}']{\tmsmonitor[']}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.
  ...
  \Cref{goal:basetmsmonitor:storeagree1}, $\storeagree[\delta_{\text{tms}}']{\tmsmonitor[']}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$, follows immediately by \Cref{tr:store-ignore-agree}.
...
\item[\Cref{tr:store-ignore-agree}:]
  ...
  \item\label[goal]{goal:basetmsmonitor:storeagree2} $\storeagree[\delta_{\text{tms}}']{\tmsmonitor[']}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.
...
\begin{proof}
  ...
  \begin{equation}
    \storeagree{\emptyset}{\src{\emptyset}}\label{asm:mmlatmsviamonitor:agree}
  \end{equation}

  Inverting \Cref{asm:mmlatmsviamonitor:run}, we get 
  \begin{itemize}
    \item[$H_0$:] $\typechecks{}{prog\ e_0\ e_p\ e_1}{\nat}$
    \item[$H_1$:]\label{asm:mmlatmsviamonitor:run2} $\execs{\hole{\cdot};\hole{\cdot}}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\configs}{v}{\overline{a}}$
  \end{itemize}

  Note that $\typechecks{}{prog\ e_0\ e_p\ e_1}{\nat}$ is equivalent to:
  \begin{equation}
    \typechecks{}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\nat}\label{asm:mmlatmsviamonitor:checks}
  \end{equation}

  Now obtain witnesses $\overline{a}, \delta_{\text{TMS}}'$, and $\tmsmonitor'$ from supplying \Cref{lem:generaltms:monitor} with \Cref{asm:mmlatmsviamonitor:agree}, \Cref{asm:mmlatmsviamonitor:run2}, and \Cref{asm:mmlatmsviamonitor:checks} and the respective assumptions:
  ...
  $\texttt{TMS}(\src{\overline{a}})$ follows trivially by choosing as witnesses the ones obtained from applying the lemma.
\end{proof}
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      \trg{let\ x=K\ in\ e}\mid \trg{x[K]\leftarrow e}\mid \trg{x[v]\leftarrow K}\mid \trg{let\ x=new\ K\ in\ e} \\
                     ...
                     \mid \trg{\langle K; e\rangle} \mid \trg{\langle n; K\rangle}
    \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\trg{a} \bnfdef& \trg{\emptyevent} \mid \trg{Alloc\ \loc\ n} \mid \trg{Dealloc\ \loc} \mid \trg{Read\ \loc\ n} \mid \trg{Write\ \loc\ n} \mid \trg{\lightning}
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    $$
    \begin{array}{rll} % there is a bug! the = disappears without \ in front of it 
...
\begin{lemma}[Robust \gls{tms} Preservation]
  If
  \begin{assumptions}
  \item $\wexecs{prog\ e_0\ e_p\ e_1}{\configs_1}{v_1}{\overline{a}}$
  \item $\texttt{TMS}(\src{\bar{a}})$
  \item $\wexec{\trg{prog\ e_0}\ \mmlAmmlAtcomp{e_p}\ \trg{e_1}}{\trg{\configt_1}}{\trg{v_1}}{\trg{\overline{a}}}$
  \end{assumptions}
  then
  \begin{goals}
