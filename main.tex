...
\myfig{
  \begin{center}
    ...
    %
    \judgbox{\src{\Gamma}\vdash\src{\commlib}\ \operatorname{ok}}{,,$\mmlAs$ library typechecks.''}$\;$\\
    %
    \typerule{t-$\src{\commlib}$-empty}{
    }{
      \src{\Gamma}\vdash\src{\hole{\cdot}}\ \operatorname{ok}
    }{w-t-relevant-empty}
    %
    \typerule{t-$\src{\commlib}$-cons}{
      \typecheck{\src{\Gamma}}{\src{foo}}{\src{\type}_\lambda}
    }{
      \src{\Gamma}\vdash\src{foo,\commlib}\ \operatorname{ok}
    }{w-t-relevant-cons}\\[0.33cm]
    %
    ...
    \typerule{$t-\text{prog}$}{
      ...
      \src{\Gamma_0}\vdash\src{\commlib}\ \operatorname{ok}
    }{
      \vdash\src{prog\ \commlib\ \library}\operatorname{ok}
    }{w-t-wprog}
    %
...
\myfig{
  ...
  \mi{Events}~\src{\event} \bnfdef&\ \src{\emptyevent} \mid \src{Alloc\ \loc\ n} \mid \src{Dealloc\ \loc} \mid \src{Get\ \loc\ n} \mid \src{Set\ \loc\ n} \mid \src{\lightning} \\
                                  & \mid \src{Call\ \comm\ foo\ n} \mid \src{Ret\ \comm\ n} \\
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    ...
    \typerule{$e-\text{ctx}-\src{call}$}{
      ...
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{\kontstack}) = \src{\comm} &
      \src{\Omega'}=\src{\commlib;\library;K_{\comm}^{foo},\kontstack;\memstate} \\
    }{
      \execs[]{\Omega}{K[call\ foo\ \finalexprnoerr]}{\Omega'}{return\ e[x/\finalexprnoerr]}{Call\ \comm\ foo\ f}
    }{w-e-ectx-call}
    %
    \typerule{$e-\text{ctx}-\src{return}$}{
      \rho_{\tiny ret}\left(\src{\comm}\right)=\src{\comm_0}\\
    }{
      \execs[]{\commlib;foo;\library;K_{\comm},\kontstack;\memstate}{K'[return\ \finalexprnoerr]}{\commlib;\library;\kontstack;\memstate}{K[f]}{Ret\ \comm_0\ \finalexprnoerr}
    }{w-e-ectx-ret}
    %
    \judgbox{\rho_{\tiny ret}(\src{\comm})=\src{\comm_0}}{,,Given the last comm. and we are returning, yield the current.''}
    %
    \typerule{comm-ret-ctxtocomp}{
    }{
      \rho_{\tiny ret}(\src{\ctxtocomp})=\src{\comptoctx}
    ...
    \typerule{comm-ret-comptoctx}{
      ...
      \rho_{\tiny ret}(\src{\comptoctx})=\src{\ctxtocomp}
    }{w-comm-ret-comptoctx}\\[0.33cm]
    %
    \judgbox{\rho_{\tiny call}(\src{\commlib},\src{foo},\src{\kontstack})=\src{\comm_0}}{,,Given the relevance and current ctx. info, yield comm.''}
    %
    \typerule{comm-call-empty-stack-comp}{
      \src{foo}\notin\src{\commlib}
    }{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{\hole{\cdot}})=\src{\ctxtocomp}
    }{w-comm-call-empty-stack-comp}
    %
    \typerule{comm-call-empty-stack-ctx}{
      \src{foo}\notin\src{\commlib}
    }{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{\hole{\cdot}})=\src{\comptoctx}
    }{w-comm-call-empty-stack-ctx}
    %
    \typerule{comm-call-nonempty-stack}{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{bar})=\src{\comm_0}
    }{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{K^{bar}_{\comm},\kontstack})=\src{\comm_0}
    }{w-comm-call-nonempty-stack}
    %
    \judgbox{\rho_{\tiny call}(\src{\commlib},\src{foo},\src{bar_{cur}})=\src{\comm_0}}{,,Given the relevance and current ctx. info, yield comm.''}
    %
    \typerule{comm-call-ctxtocomp}{
      \src{foo}\notin\src{\commlib} &
      \src{\commlib}=\src{\commlib_1},\src{bar_{cur}},\src{\commlib_2}
    }{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{bar_{cur}})=\src{\comptoctx}
    }{w-comm-call-ctxtocomp}
    %
    \typerule{comm-call-internal-both-irrrelevant}{
      \src{foo}\notin\src{\commlib} &
      \src{bar_{cur}}\notin\src{\commlib}
    }{
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{bar_{cur}})=\src{\nocomm}
    }{w-comm-call-internal-both-irrrelevant}
    %
    ...
    \typerule{comm-call-comptoctx}{
      ...
      \rho_{\tiny call}(\src{\commlib},\src{foo},\src{bar_{cur}})=\src{\ctxtocomp}
...
\myfig{
  \begin{center}
    \judgbox{\wexecs{prog\ \commlib\ \library}{\configs}{\finalexpr}{\trace}}{,,Run $\mmlAs$ program $\src{prog\ \library}$, giving dynamic state $\configs$\\ and emitting trace $\src{\trace}$.''}
  \typerule{$e-\text{wprog}$}{
    \vdash\src{prog\ \commlib\ \library}\operatorname{ok} &
    \execs{\commlib;\library;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\Omega}{\finalexpr}{\trace}
  }{
    \wexecs{prog\ \commlib\ \library}{\configs}{\finalexpr}{\trace}
  }{ws-e-prog}
  \end{center}
...
%    \judgbox{\tospecificevctxs{\trace} = \src{\mstrace}}{,,Project an $\mmlAs$ trace $\src{\trace}$ to $\src{\mstrace}$, ignoring component.''}
    ...
    \begin{array}{rcll}
      ...
      \tospecificevs[_{\delta_{MS}}]{Call\ \comm\ foo\ n} &\ = & \specificev{\varepsilon} &\\
      \tospecificevs[_{\delta_{MS}}]{Ret\ \comm\ n} &\ = & \specificev{\varepsilon} &\\
...
\begin{scontents}[store-env=buffer]
  If
  \begin{assumptions}
  \item\label[ass]{asm:mmlatmsviamonitor:run} $\wexecs{prog\ \commlib\ \library}{\configs}{v}{\trace}$
  \end{assumptions}
  Then
  \begin{goals}
...
\myfig{
  ...
  \trg{K}\bnfdef &\ \trg{\hole{\cdot}}\mid \trg{K\oplus e}\mid \trg{v\oplus E}\mid \trg{x[K]}\mid \trg{let\ x=K\ in\ e} \\
...
\myfig{
  \begin{center}
    ...
    \typerule{syntactic-plugging}{
      \operatorname{dom}\trg{\library_1}\downarrow\cap\operatorname{dom}\trg{\library_2}\downarrow = \emptyset &
      \trg{\library}\equiv\trg{\library_1}\cup\trg{\library_2}
    }{
      \trg{\library_1\linker\library_2} = \trg{\library}
    }{trg-plugging}
  \end{center}
}{w-plugging-t}{$\mmlAt$ plugging of libraries.}
...
\myfig{
  ...
  \mi{Events}~\trg{\event} \bnfdef& \trg{\emptyevent} \mid \trg{Alloc\ \loc\ n} \mid \trg{Dealloc\ \loc} \mid \trg{Get\ \loc\ n} \mid \trg{Set\ \loc\ n}\\
                             & \mid \trg{\lightning} \mid \trg{Call\ ?foo\ \finalexprnoerr} \mid \trg{Ret\ !\finalexprnoerr}
  \end{aligned}
  \end{gather*}
...
\myfig{
  ...
  \typerule{$e-\trg{get}-\in$}{
    ...
    \exprevalt{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{x[n]}{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{H(\loc+n)}{Get\ \loc\ n}
  }{tw-e-get-in}
  %
  \typerule{$e-\trg{get}-\notin$}{
    \trg{\loc}+\trg{n}\notin\text{dom }\trg{H}
  }{
    \exprevalt{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{x[n]}{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{1729}{Get\ \loc\ n}
  }{tw-e-get-notin}
  %
  \typerule{$e-\trg{set}$}{
    \trg{H'} = \trg{H}(\trg{\loc+n}\mapsto \trg{v})
  }{
    \exprevalt{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{x[n]\leftarrow \valueexpr}{\library;\kontstack;H';\Delta_1,x\mapsto\loc_\poison,\Delta_2}{v}{Set\ \loc\ n\ \valueexpr}
  }{tw-e-set}
  %
  ...
  \typerule{$e-\trg{delete}$}{
    ...
    \exprevalt{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{delete\ x}{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poisoned,\Delta_2}{0}{Dealloc\ \loc}
  }{tw-e-delete}
  %
  \typerule{$e-\trg{new}$}{
  ...
  \exprevalt{\library;\kontstack;H;\Delta}{let\ x=new\ n\ in\ e}{\library;\kontstack;H';z\mapsto \loc_\poisonless,\Delta}{e[z/x]}{Alloc\ \loc\ n}
  }{tw-e-let-new}
  %
  ...
  \typerule{$e-\trg{x\ is\ \poisoned}-$yes}{
    \configt=\trg{\library;\kontstack;H;\Delta} &
    \trg{\Delta}=\trg{\Delta_1,x\mapsto\loc_\poisoned,\Delta_2}
  }{
    \exprevalt{\configt}{x\ is\ \poisoned}{\configt}{0}{\emptyevent}
  ...
  \typerule{$e-\trg{x\ is\ \poisoned}-$no}{
    ...
    \exprevalt{\configt}{x\ is\ \poisoned}{\configt}{1}{\emptyevent}
  }{tw-e-x-is-poisoned-yes}
  %
  \typerule{$e-\trg{n\ has\ \nat}$}{
  }{
...
\myfig{
  ...
  \typerule{$e-\text{ctx}-\trg{stuck}$}{
    \exprevalt{\configt}{e}{\configt'}{stuck}{\lightning}
  }{
    \exect[]{\configt}{K[e]}{\configt'}{stuck}{\lightning}
  }{tw-e-ectx}
  %
  \typerule{$e-\text{ctx}-\trg{return}$}{
  }{
    \exprevalt{\library;K,\kontstack;H;\Delta}{K'[return\ \finalexprnoerr]}{\library;\kontstack;H;\Delta}{K[\finalexprnoerr]}{Ret\ !\finalexprnoerr}
  }{tw-e-returning}
  %
  \typerule{$e-\text{ctx}-\trg{call}$}{
    \trg{\Omega}=\trg{\library;\kontstack;H;\Delta}&
    \trg{\library}=\trg{\library_1,}(\trg{let\ foo\ x := e})\trg{,\library_2} \\
    \trg{\Omega'}=\trg{\library;K,\kontstack;H;\Delta}
  }{
    \exprevalt{\Omega}{K[call\ foo\ \finalexprnoerr]}{\Omega'}{return\ e[x/\finalexprnoerr]}{Call\ ?foo\ \finalexprnoerr}
  }{tw-e-calling}
}{tw-expr-ctx-eval}{Contextual Evaluation of $\mmlAt$ expressions.}
...
\myfig{
  \begin{center}
    \judgbox{\wexect{prog\ \library}{\Omega}{\finalexpr}{\trace}}{,,Run $\mmlAt$ program $\trg{prog\ \library}$, giving dynamic\\state $\configt$ and emitting trace $\trg{\trace}$.''}
  \typerule{$te-\text{wprog}$}{
    \exect{\library;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\Omega}{\finalexpr}{\trace}
  }{
    \wexect{prog\ \library}{\Omega}{\finalexpr}{\trace}
  }{wt-e-prog}
  \end{center}
...
\myfig{
  \begin{center}
    ...
    %
      \mmlAmmlAtcomp{\left[\src{v}/\src{x}\right]}&\ = & \trg{\left[\right.}\mmlAmmlAtcomp{v}\trg{/}\mmlAmmlAtcomp{x}\trg{\left.\right]}
    \end{array}
    $$
  \end{center}
...
\myfig{
  \begin{center}
    $$
    \src{\trace}=\src{Call\ ?main\ 0\cdot Call\ ?foo\ 42\cdot Alloc\ \ell_{0}\ 42\cdot Get\ \ell_{0}\ 1337\cdot\ Dealloc\ \ell_{0}\cdot\ Ret\ !1729\cdot\ Ret\ !1729}
    $$

    $$
    \src{\trace}=\src{Call\ ?main\ 0\cdot }\underbrace{\src{Call\ ?foo\ 42\cdot Alloc\ \ell_{0}\ 42\cdot Get\ \ell_{0}\ 1337\cdot\ Dealloc\ \ell_{0}\cdot\ Ret\ !1729}}_{\src{foo}}\src{\cdot\ Ret\ !1729}
    $$

    Let $\xi$ be the set of component names we care about security-wise.

    \typerule{nowrap}{
      \trg{foo}\in\xi
    }{
      \mmlAmmlAtback[\xi]{Call\ ?foo\ f} = \src{call\ foo}
    }{x}
    \typerule{wrap}{
      \trg{foo}\notin\xi &
      \vdash\trg{\ell}\operatorname{fresh} &
      \mmlAmmlAtback[\xi]{Alloc\ \ell\ 42} = \src{e}
    }{
      \mmlAmmlAtback[\xi]{Call\ ?foo\ f} = \src{e;call\ foo}
    }{y}
  \end{center}
}{x}{x}
