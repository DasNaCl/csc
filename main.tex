...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-top-level}{
      \trg{\loc}\ \text{fresh} &
      \mmlAmmlAtbackdetail{\specificev{Alloc\ \loc\ 42}\cdot\mstrace\cdot\specificev{Dealloc\ \loc}} = \Game', \src{e}
    }{
      \mmlAmmlAtback{\specificev{Ret\ !n_0}\cdot\mstrace\cdot\specificev{Call\ ?n_1}}, \Game', \src{e}
    }{backtrans-top-level}
    %
    \typerule{backtrans-top-level-crash}{
      \mmlAmmlAtbackdetail{\mstrace} = \Game', \src{e}
    }{
      \mmlAmmlAtback{\specificev{Ret\ !n_0}\cdot\mstrace\cdot\specificev{\lightning}} = \Game', \src{e;abort()}
...
\myfig{
  \begin{center}
    \begin{tikzpicture}
      ...
      \pgfmathsetmacro{\hdist}{0.75}
      \pgfmathsetmacro{\vdist}{1.25}
      \pgfmathsetmacro{\halfvdist}{0.625}

      % top row of states
      \node (srcempty) {$\src{\emptyset}$};
      \foreach \s [remember=\s as \cur (initially empty)] in {1,w_1,p,w_2,2} {
        \node[right=\hdist of src\cur] (src\s) {$\src{\Omega_{\s}}$};
      }
      \path[color=\stlccol] (srcempty) edge[draw=none] node {\ $\xrightarrow{\trace[_1]}{}{\kern-3.5pt}_{\text{ctx}}^*$} (src1)
        (src1) edge[draw=none] node {\ $\xrightarrow{\event[_c]}{}{\kern-3.5pt}_{\text{ctx}}$} (srcw\string_1)
        (srcw\string_1) edge[draw=none] node {\ $\xrightarrow{\trace[_p]}{}{\kern-3.5pt}_{\text{ctx}}^*$} (srcp)
        (srcp) edge[draw=none] node {\ $\xrightarrow{\event[_r]}{}{\kern-3.5pt}_{\text{ctx}}$} (srcw\string_2)
        (srcw\string_2) edge[draw=none] node {\ $\xrightarrow{\trace[_2]}{}{\kern-3.5pt}_{\text{ctx}}^*$} (src2)
        ;
      % bottom row of states
      \node[below=\vdist of srcempty] (trgempty) {$\trg{\emptyset}$};
      \foreach \s in {1,w_1,p,w_2,2} {
        \node[below=\vdist of src\s] (trg\s) {$\trg{\Omega_{\s}}$};
      }
      \path[color=\ulccol] (trgempty) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_1]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trg1)
        (trg1) edge[draw=none] node {$\bm{=}$} (trgw\string_1)
        (trgw\string_1) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_p]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trgp)
        (trgp) edge[draw=none] node {$\bm{=}$} (trgw\string_2)
        (trgw\string_2) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_2]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trg2)
        ;

      % state relations
      \path (srcempty) edge[draw=none] node[rotate=-90] {$\approx_\emptyset$} (trgempty)
        (src1) edge[draw=none] node[rotate=-90] {$\approx_{\delta_1}$} (trg1)
        (srcp) edge[draw=none] node[rotate=-90] {$\approx_{\delta_p}$} (trgp)
        (src2) edge[draw=none] node[rotate=-90] {$\approx_{\delta_2}$} (trg2)
        % lollies
        (srcw\string_1) edge[draw=none] node[rotate=90] {$\multimap_{\delta_{w_1}}$} (trgw\string_1)
        (srcw\string_2) edge[draw=none] node[rotate=90] {$\multimap_{\delta_{w_2}}$} (trgw\string_2)
        ;

      % wrapper illustrations
      \drawpolygon src1,srcw\string_1,trgw\string_1;
      \drawpolygon srcp,srcw\string_2,trgp;
      \node[above right=0.2 and -0.2 of srcw\string_1] (wrapper) {\tiny Wrapper};
      \path[->,draw] (wrapper) -- (srcw\string_1);
      \path[->,draw] (wrapper) -- (srcp);
      % legend
      \node[align=left,below=0.3 of trgempty,font=\tiny] (legend) {%
          $\src{\event[_c]}=\src{Call\ ?\finalexprnoerr_i}$\\%
          $\src{\event[_r]}=\src{Ret\ !\finalexprnoerr_o}$
        };
    \end{tikzpicture}
  \end{center}
...
\myfig{
  \begin{center}
    \begin{tikzpicture}
      ...
      \foreach \s in {1,p,2} {
        \ifthenelse{\equal{\s}{1}}{ 
          \node[below=\vdist of srcw\string_\s] (trg\s) {$\trg{\Omega_{\s}}$};
        }{%else
          \node[below=\vdist of src\s] (trg\s) {$\trg{\Omega_{\s}}$};
        }
      }
      \path[color=\ulccol] (trgempty) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_1]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trg1)
        (trg1) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_p]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trgp)
        (trgp) edge[draw=none] node {\ $\xrightarrow{\phantom{\trace[_2]}}{}{\kern-3.5pt}_{\text{ctx}}^*$} (trg2)
        ;

      % state relations
      \path (srcempty) edge[draw=none] node[rotate=-90] {$\approx_\emptyset$} (trgempty)
        (src1) edge[draw=none] node[rotate=-45] {$\approx_{\delta_1}$} (trg1)
        (srcp) edge[draw=none] node[rotate=-90] {$\approx_{\delta_p}$} (trgp)
        (src2) edge[draw=none] node[rotate=-90] {$\approx_{\delta_2}$} (trg2)
        % lollies
        (srcw\string_1) edge[draw=none] node[rotate=90] {$\multimap_{\delta_{w_1}}$} (trg1)
        (srcw\string_2) edge[draw=none] node[rotate=45] {$\multimap_{\delta_{w_2}}$} (trgp)
        ;

      % wrapper illustrations
      \drawpolygon src1,srcw\string_1,trg1;
      \drawpolygon srcp,srcw\string_2,trgp;
      \node[above right=0.2 and -0.2 of srcw\string_1] (wrapper) {\tiny Wrapper};
      \path[->,draw] (wrapper) -- (srcw\string_1);
      \path[->,draw] (wrapper) -- (srcp);
    ...
    \end{tikzpicture}
  \end{center}
}{rtmsp-sketch2}{Sketch of the proof of \thmref{thm:rtmsp}. %
  Arrows in the horizontal direction are the usual step relations, but the term is omitted for aesthetic purposes. %
  Likewise for the emitted target-level traces. %
Note that trace $\src{\trace[_1]}$ starts with $\src{Ret\ !0}$ and $\src{\trace[_2]}$ ends with $\src{Call\ ?\valueexpr}$, where $\src{\valueexpr}$ is the final result of the program, but they are not instrumented by the backtranslation.}
