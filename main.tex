...
\myfig{
  \begin{center}
    \judgbox{\delta(\src{\loc}) = \trg{\loc}}{,,Map from $\mmlAt$ locations to $\mmlAs$ locations.''}
    \judgbox{\delta[\trg{\loc}] = \delta'}{,,Extend $\delta$ with $\trg{\loc}$ and call the extension $\delta'$.''}
    \typerule{backtrans-create-symb}{
      \trg{\loc}\not\in\operatorname{img}\delta &
      \fresh{\delta}{\src{\loc}} &
    }{
      \delta'=[\trg{\loc}]=\delta\cup\left\{\src{\loc}\mapsto\trg{\loc}\right\}
    }{backtrans-create-symb}
    \typerule{backtrans-nocreate-symb}{
      \trg{\loc}\in\operatorname{img}\delta &
    }{
      \delta'[\trg{\loc}]=\delta
