...
\subsubsection{\gls*{tms}-Specific Events}
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\src{a_{tms}} \bnfdef&\ \specificevs{\emptyevent} \mid \specificevs{Alloc\ \loc\ n} \mid \specificevs{Dealloc\ \loc} \mid \specificevs{Get\ \loc\ n} \mid \specificevs{Set\ \loc\ n} \\
                                     & \mid \specificevs{\lightning} \mid \specificevs{Call\ ?n} \mid \specificevs{Ret\ !n}\\
  \end{aligned}
  \end{gather*}
}{w-specific-events}{\gls*{tms}-Specific Events of $\mmlAs$.}
\myfig{
  \begin{center}
    $$
    \begin{array}{rcl}
    ...
    $$
  \end{center}
}{w-events-project-to-specific}{Projection of $\mmlAs$ events to \gls*{tms}-specific events.}
...
\myfig{
  \begin{center}
    ...
    \typerule{TMS-Call-Authentic}{
      ...
      \tmstraceagree{\src{Call\ ?n}}{\varepsilon}
    }{tms-call-auth}
    %
    \typerule{TMS-Ret-Authentic}{
    }{
      \tmstraceagree{\src{Ret\ !n}}{\varepsilon}
    }{tms-ret-auth}
    %
...
\MK{Skip Call and Ret events when folding over trace?}
...
\begin{definition}[\gls{tms} on Traces]\label{def:tempmemsafe}
  ...
  \begin{itemize}
    ...
    \item $\neg(\src{Get\ \loc\ n}\le_{\src{\trace}}\src{Alloc\ \loc\ s})$
    \item $\neg(\src{Set\ \loc\ n\ v}\le_{\src{\trace}}\src{Alloc\ \loc\ s})$
    \item $\neg(\src{Dealloc\ \loc}\le_{\src{\trace}}\src{Get\ \loc\ n})$
...
\begin{definition}[Spatial Memory Safety]\label{def:spatmemsafe}
  ...
  \begin{itemize}
    \item If $\src{Alloc\ \loc\ s}\in\src{\trace}$ and $\src{Get\ \loc\ n}\in\src{\trace}$, then $\src{n}<\src{s}$
...
\myfig{
  ...
  \typerule{$e-\text{wprog}-trg$}{
    ...
    \exect{\Omega_0^\diamond}{e_p[v_0/x]}{\Omega_p}{v_p}{\overline{a_p}} &
...
%    \execs{\hole{\cdot};\hole{\cdot}}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\Omegas}{v}{\overline{a}}
    ...
    \wexect{prog\ e_0\ e_p\ e_1}{\Omega_1}{v_1}{\overline{a_0^\diamond\cdot Call\ ?v_0\cdot a_p\cdot Ret\ !v_p\cdot a_1^\diamond}}
  }{tw-e-prog}
  \end{center}
...
\myfig{
  \begin{center}
    ...
    \typerule{SMS-Call-Authentic}{
      ...
      \smstraceagree{\trg{Call\ ?n}}{\varepsilon}
    }{sms-call-auth}
    %
    \typerule{SMS-Ret-Authentic}{
    }{
      \smstraceagree{\trg{Ret\ !n}}{\varepsilon}
    }{sms-ret-auth}
    %
...
\begin{proof}
  ...
  \begin{passumptions}{F}
    \item\label[ass]{asm:exprcorrect:exec2:trg} $\exec{\mmlAmmlAtcomp{\Omega}}{\mmlAmmlAtcomp{e}}{\mmlAmmlAtcomp{\Omega'}}{\mmlAmmlAtcomp{v}}{\mmlAmmlAtcomp{\bar{a}}_\delta}$
  \end{passumptions}
