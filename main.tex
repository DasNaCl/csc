...
Let $\mathbb{K}$ be the set of unguessable memory location identifiers, i.e. references.

\myfig{
	\vspace{-1em}
  \begin{gather*}
  \begin{aligned}
  \mi{Expressions}~\src{e} &\hspace{0.5cm} \text{side-effect free operation leading a value}\\
  %
  ...
  \mi{References}~\src{\ell}\in\mathbb{K} \\
  %
  \mi{Variables}~\src{V} \bnfdef&\ \src{x} \mid \src{y} \mid \dots \hspace{0.5cm}
...
\myfig{
  ...
  \\
  \typerule{$e-get$}{
    \exprevals{e}{\Delta}{\src{m}} &
    lookup\ \src{H}\ (n_\loc + m) = Some\ \src{v}
  }{
  ...
  \typerule{$e-set$}{
    ...
    \src{H'} = replace\ \src{H}\ (n_\loc + n)\ \src{m}
  }{
