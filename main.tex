...
\subsection{Robustly Preserving $\CRhash$}

\begin{definition}[Indistinguishability of States]
  Two states $\obj{\Omega_{1}},\obj{\Omega_{2}}$ are indistinguishable with respect to a list $\obj{\zeta}$ of public identifiers (written $\zeta\vdash\obj{\Omega_{1}}\approx\obj{\Omega_{2}}$) if for any public identifier $\obj{x}\in\obj{\zeta}$, we have $\obj{\Omega_{1}}[\obj{x}]=\obj{\Omega_{2}}[\obj{x}]$.
\end{definition}
...
\begin{definition}[$\CRhash$ security]
  ...
  Given $\execo{\Omega_{1}}{e_{1}}{\Omega_{2}}{\finalexprnoerr}{c,\trace[_{A}]}$ and $\execo{\Omega_{1}'}{e_{1}}{\Omega_{2}'}{\finalexprnoerr}{c',\trace[_{B}]}$ we say $e_{1}$ is $\CRhash$-secure with respect to $\obj{\zeta}$ iff $(c,branches(\trace_{A}))=(c',branches(\trace_{B}))$
\end{definition}
