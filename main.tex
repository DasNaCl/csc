...
% one program has empty behavior      - how can it have an empty behavior?
...
\end{lemma}
\begin{proof}
  The class with all hyperproperties is the top element $\top=\powerset{\traces}$, while the class with no hyperproperties whatsoever is $\bot=\{\emptyset\}$.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=6mm,every node/.style={align=center}]
    ...
    % edges
    \path (top.south) edge node[fill=white,sloped] {$\supseteq$} (SSC.north);
    \path (SSC.south) edge node[fill=white,sloped] {$\supseteq$} (HSafe.north);
...
Suppose both compilers only satisfy \Cref{def:instrumentation}.
That is, we cannot reasonably combine them, since one instrumentation might break the other.% v1 might be instrumented with load/store ops which breaks v4
Now suppose $\ttcomp{\bullet}_{1}$ satisfies \Cref{def:secure-instrumentation} such that $\cC$ is precisely the class of hyperproperties that ensure no SPECTRE v4 attacks are possible.
