...
\myfig{
  \begin{center}
    ...
    \typerule{trg-filter-context}{
      ...
      \trg{\event_b} \not= \trg{Get\ \loc\ n} &
      \trg{\event_b} \not= \trg{Set\ \loc\ n\ m} \\
      \trg{\event_b} \not= \trg{Call\ \emptyset\ foo\ n} &
      \trg{\event_b} \not= \trg{Ret\ \emptyset\ n} &
    }{
    ...
    \typerule{trg-filter-empty}{
      ...
      \tospecificevt[_{\trg{\bullet}}]{\emptyevent} = \trg{\emptyevent}
    }{trg-filter-empty}
    %
    ...
    \typerule{trg-filter-comp-get}{
      ...
      \tospecificevt[_{\trg{\bullet}}]{Get\ \loc\ n;\comp} = \trg{Use\ \loc\ n}
    }{trg-filter-comp-get}
    %
    \typerule{trg-filter-comp-set}{
    }{
      \tospecificevt[_{\trg{\bullet}}]{Set\ \loc\ n\ \valueexpr;\comp} = \trg{Use\ \loc\ n}
    }{trg-filter-comp-set}
    %
  ...
  \end{center}
}{wt-events-project-to-specific}{Filtering of $\mmlAt$ events, getting rid of unimportant ones.}
...
\end{lemma}
\begin{proof}
  We prove this by contradiction. So, we get the assumption:\MK{minor patches}
...
\begin{lemma}[Component Correctness]\label{lem:component:correctness}
  If
  \begin{assumptions}
    \item $\exec{\trg{\Omega}}{\mmlAmmlAtcomp{e}}{\trg{\Omega'}}{\trg{K[return\ \valueexpr]}}{\trg{\trace}}$
    \item $\xlangstateeq{\Omega}{\Omega}$
  ...
  \begin{goals}
    \item $\delta\subseteq\delta'$
    \item $\execs{\Omega}{e}{\Omega'}{K[return\ \valueexpr]}{\trace}$
    \item $\xlangstateeq{\Omega'}{\Omega'}$
    \item $\xlangtraceeq{\trace}{\trace}$
  \end{goals}
\end{lemma}
...
\begin{lemma}[$\theta_{\trg{\bullet}}$ distributive]\label{lem:trgfilter:distributive}$\;$\\
  \begin{goals}
...
\end{lemma}
\begin{proof}
  Invert \Cref{asm:expr:interact:ret:ctxtocomp:backtranslation:welltyped} giving $\src{e}=\src{let\ z=new\ 42\ in\ delete\ z;\ return\ }\mmlAmmlAtbackv{\interfacevalue}$, where $\fresh{\Game}{\src{z}}$.
...
\begin{lemma}[Backtranslation is Well-Typed]\label{lem:backtranslation:welltyped}
  If
  \begin{assumptions}
  \item\label[ass]{asm:backtranslation:welltyped:backtrans} $\mmlAmmlAtback[\emptyset;\emptyset]{\color{black}\tospecificevt[_{\trg{\bullet}}]{\trace}}=\Game;\delta;\src{\library_{ctx}}$
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtranslation:welltyped:subtop1} $\mmlAmmlAtbackdetail[\emptyset;\emptyset;\trg{\hole{\cdot}}]{\trace[_{0}]}=\Game';\delta';\trg{\overline{\loc}};\src{e_{0}};\src{e_{0}'};\src{let\ x_{0} = new\ n_{0}\ in\ e_{0}''}$
    \item\label[ass]{asm:backtranslation:welltyped:subtop2} $\mmlAmmlAtbackdetail[\Game';\delta';\trg{\overline{\loc}}]{\trace[_{1}]}=\Game'';\delta'';\trg{\hole{\cdot}};\src{e_{1}};\src{e_{1}'};\src{e_{1}''}$.
  \end{passumptions}
...
\begin{lemma}[Backtranslation Correctness of $\trg{Start}$]\label{lem:exprbacktranslation:vbeg:wrapper:correct}
  ...
  \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:exect} $\exect[]{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\commlib;\library;(\hole{\cdot}; main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{main}}{Start;\comp}$
  ...
  Then
  \begin{goals}
  \item\label[goal]{goal:exprbacktranslation:vbeg:wrapper:correct:execs} $\exec{\src{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}}{\src{call\ main\ 0}}{\src{\commlib;\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}}{\src{e_{main}}}{\src{Start;\comp}}$
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:execs0} $\execs[]{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\commlib;\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{let\ \_=42\ in\ e_{main}}{Start;\comp}$
  \end{passumptions}
  On \Cref{asm:exprbacktranslation:vbeg:wrapper:correct:execs0} apply \Cref{tr:w-e-ectx,tr:w-e-let-x}
  \begin{passumptions}{H}
    \setcounter{enumi}{1}
    \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:execs1} $\execs[]{\commlib;\library;(\hole{\cdot},main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{let\ \_{=42\ in\ e_{main}}}{\commlib;\library;(\hole{\cdot},main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{main}}{\emptyevent}$
  \end{passumptions}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:execs} $\execs{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\commlib;\library;(\hole{\cdot},main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{main}}{Start}$
  \end{passumptions}
...
\begin{lemma}[Backtranslation Correctness of $\trg{Ret}$]\label{lem:exprbacktranslation:beg:wrapper:correct}
  ...
  \item $\trg{\library}=\trg{\library_{ctx}\linker}\mmlAmmlAtcomp{\library_{comp}}$
  \item $\trg{\commlib}=\mmlAmmlAtcomp{\commlib}=\operatorname{dom}\ \mmlAmmlAtcomp{\library_{comp}}$
  \item\label[ass]{asm:exprbacktranslation:beg:wrapper:correct:exect} $\exect[]{\Omega}{K_{component}[return\ \interfacevalue]}{\Omega'}{K[\interfacevalue]}{Ret\ \comptoctx\ \valueexpr;\comp}$
  ...
  \item\label[goal]{goal:exprbacktranslation:beg:wrapper:correct:execs} $\exec{\src{\Omega}}{\src{K_{component}[return\ }\mmlAmmlAtbackv[]{\trg{\valueexpr}}\src{]}}{\src{\commlib;\library;\kontstack;\ctx;\memstate'}}{\src{K[}\mmlAmmlAtbackv[]{v}\src{]}}{\src{\trace}}$
...
\begin{proof}
  ...
  Note, \Cref{asm:exprbacktranslation:beg:wrapper:correct:execs0a}, continue with \Cref{tr:w-e-ectx,tr:w-e-let-x}:
  \begin{passumptions}{H}
    \setcounter{enumi}{7}
...
\begin{lemma}[Middle of Backtranslation Correctness]\label{lem:exprbacktranslation:mid:wrapper:correct}
  ...
  \item $\trg{\library}=\trg{\library_{ctx}\linker}\mmlAmmlAtcomp{\library_{comp}}$
  \item $\trg{\commlib}=\mmlAmmlAtcomp{\commlib}=\operatorname{dom}\ \mmlAmmlAtcomp{\library_{comp}}$
  \item\label[ass]{asm:exprbacktranslation:mid:correct:execs} $\exect{\Omega}{e}{\Omega'}{e'}{\trace}$
...
\end{lemma}
\begin{proof}
  Induction on \Cref{asm:exprbacktranslation:mid:correct:nonint}.\MK{carry out induction, however tedious}
...
%  \item\label[goal]{goal:exprbacktranslation:end:wrapper:correct:deltasubset} $\delta\subseteq\delta'$
  ...
  \item\label[goal]{goal:exprbacktranslation:end:wrapper:correct:execs} $\exec{\src{\Omega}}{\src{K[let\ z = new\ 42\ in\ call\ foo\ }\mmlAmmlAtbackv[]{\valueexpr}\src{]}}{\src{\commlib;\library;(K;foo),\kontstack;\comp;\memstate'}}{\src{e_{foo}\gamma}}{\src{\trace}}$
  ...
  \item\label[goal]{goal:exprbacktranslation:end:wrapper:correct:traceeq} $\src{\trace}\thexlangtraceeq[\delta';\src{X'}]\trg{Call\ \ctxtocomp\ foo\ \interfacevalue;\ctx}$
  \end{goals}
\end{lemma}
\begin{proof}
...
%    \item $\delta' = \delta \cup \left\{\src{\loc}\mapsto\trg{z}\right\}$
  ...
  For \Cref{goal:exprbacktranslation:end:wrapper:correct:execs}, first use \Cref{tr:w-e-ectx,tr:w-e-let-new-ctx}, noting \Cref{lem:exprbacktranslation:end:wrapper:correct:freshlocs,lem:exprbacktranslation:end:wrapper:correct:freshz}:
  \begin{passumptions}{H}
    \setcounter{enumi}{10}
    \item\label[ass]{asm:exprbacktranslation:end:wrapper:correct:execs0} $\exec{\src{\Omega}}{\src{K[let\ z = new\ 42\ in\ call\ foo\ \valueexpr]}}{\src{\commlib;\library;\kontstack;\ctx;\memstate'}}{\src{K[call\ foo\ \valueexpr]}}{\src{Alloc\ \loc\ 42;\ctx}}$
  \end{passumptions}
  Now use \Cref{tr:w-e-ectx-ret-notsame}:
  \begin{passumptions}{H}
    \setcounter{enumi}{11}
    \item\label[ass]{asm:exprbacktranslation:end:wrapper:correct:execs1b} $\exec{\src{\commlib;\library;\kontstack;\ctx;\memstate'}}{\src{K[call\ foo\ \valueexpr]}}{\src{\commlib;\library;(K;foo),\kontstack;\ctx;\memstate'}}{\src{e_{foo}[\valueexpr/y]}}{\src{Call\ \ctxtocomp\ foo\ z;\ctx}}$
  \end{passumptions}
  Use \Cref{asm:exprbacktranslation:end:wrapper:correct:execs0,asm:exprbacktranslation:end:wrapper:correct:execs1b} on \Cref{tr:ws-e-trans-important} to get exactly what \Cref{goal:exprbacktranslation:end:wrapper:correct:execs} wants us to prove.
...
%  \item\label[goal]{goal:exprbacktranslation:end:wrapper:correct:deltasubset} $\delta\subseteq\delta'$
  ...
  \item\label[goal]{goal:exprbacktranslation:vend:wrapper:correct:execs} $\exec{\src{\Omega}}{\src{K[let\ z = new\ 42\ in\ delete\ z;return\ }\mmlAmmlAtbackv[]{\valueexpr}\src{]}}{\src{\commlib;\library;\hole{\cdot};\comp;\memstate'}}{\mmlAmmlAtbackv[]{\valueexpr}}{\src{\trace}}$
...
%    \item $\delta' = \delta \cup \left\{\src{\loc}\mapsto\trg{z}\right\}$
  ...
  For \Cref{goal:exprbacktranslation:vend:wrapper:correct:execs}, first use \Cref{tr:w-e-ectx,tr:w-e-let-new-ctx}, noting \Cref{lem:exprbacktranslation:vend:wrapper:correct:freshlocs,lem:exprbacktranslation:vend:wrapper:correct:freshz}:
  \begin{passumptions}{H}
    \setcounter{enumi}{9}
    \item\label[ass]{asm:exprbacktranslation:vend:wrapper:correct:execs0} $\exec{\src{\Omega}}{\src{K[let\ z = new\ 42\ in\ delete\ z; return\ \valueexpr]}}{\src{\commlib;\library;\kontstack;\ctx;\ \memstate_{\poisonless}}}{\src{K[delete\ z; return\ \valueexpr]}}{\src{Alloc\ \loc\ 42;\ctx}}$
  \end{passumptions}

  Next, use \Cref{tr:w-e-ectx,tr:w-e-delete,tr:w-e-let-x} on \Cref{asm:exprbacktranslation:vend:wrapper:correct:execs0}:

  \begin{passumptions}{H}
    \setcounter{enumi}{10}
    \item\label[ass]{asm:exprbacktranslation:vend:wrapper:correct:execs1} $\exec{\src{\commlib;\library;\kontstack;\ctx;\memstate_{\poisonless}}}{\src{K[let\ \_=delete\ z\ in\ return\ \valueexpr]}}{\src{\commlib;\library;\kontstack;\ctx;\memstate'}}{\src{K[return\ \valueexpr]}}{\src{Dealloc\ \loc;\ctx}}$
  \end{passumptions}
  ...
  Use \Cref{asm:exprbacktranslation:vend:wrapper:correct:execs0,asm:exprbacktranslation:vend:wrapper:correct:execs1,asm:exprbacktranslation:vend:wrapper:correct:execs2} on \Cref{tr:ws-e-trans-important} to get exactly what \Cref{goal:exprbacktranslation:vend:wrapper:correct:execs} wants us to prove.
...
\begin{lemma}[Backtranslation Correctness]\label{lem:backtranslation:correct}
  If
  \begin{assumptions}
  \item\label[ass]{asm:backtrans:correct:exect0} $\exect{\Omega}{call\ main\ 0}{\Omega'}{\valueexpr}{\trace}$
  \item\label[ass]{asm:backtrans:correct:backtrans} $\mmlAmmlAtback[\emptyset;\emptyset]{\color{black}\tospecificevt[_{\trg{\bullet}}]{\trace}}=\Game;\delta;\src{\library_{ctx}}$
  ...
  \begin{goals}
  \item\label[goal]{goal:backtrans:correct:execs} $\exec{\configs}{\src{call\ main\ 0}}{\src{\Omega_{e}'}}{\mmlAmmlAtbackv[]{\valueexpr}}{\src{\trace_{e}}}$
  \item\label[goal]{goal:backtrans:correct:stateeq} $\xlangstateeq[\delta_{e};\src{L_{e}}]{\Omega_{e}'}{\Omega'}$
  \item\label[goal]{goal:backtrans:correct:traceeq} $\src{\trace_{e}}\thexlangtraceeq[\delta_{e};\src{X_{e}}]\tospecificevt[_{\trg{\bullet}}]{\trace}$
  \end{goals}
\end{lemma}
\begin{proof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtranscorr:backtrans:end} $\mmlAmmlAtbackctxtocomp[\Game;\delta;\trg{\overline{\loc_{1}}}]{End\ \interfacevalue_{2}} = \Game';\delta';\trg{\hole{\cdot}}; \src{let\ x_{1}=new\ n_{1}\ in\ e_{1}''}$
  \end{passumptions}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtranscorr:exect0} $\exect[]{\Omega}{call\ main\ 0}{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{0}}{Start;\comp}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{35}
    \item\label[ass]{asm:backtrans:execs0} $\execs{foo,\hole{\cdot};\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\commlib;\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{01}[0/x]}{Start;\comp}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{38}
    \item\label[ass]{asm:backtranscorr:exect1} $\exect{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{0}[0/x]}{foo,\hole{\cdot};\library;\kontstack,(\hole{\cdot};main),\hole{\cdot};\ctx;H^{\ctx}_{0};\hole{\cdot};\Delta_{0}}{e_{1}}{\trace_{0}}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{41}
    \item\label[ass]{asm:backtrans:execs1} $\execs{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{01}[0/x]}{\commlib;\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{K\hole{42}}{\trace[_{0}]}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{54}
    \item\label[ass]{asm:backtranscorr:exect3} $\exect[]{foo,\hole{\cdot};\library;(K;foo),\kontstack,(\hole{\cdot};main),\hole{\cdot};\comp;H^{\ctx}_{0};\hole{\cdot};\Delta_{0}}{e_{foo}[\valueexpr_{0}/y]}{foo,\hole{\cdot};\library;(K;foo),\kontstack,(\hole{\cdot};main),\hole{\cdot};\comp;H^{\ctx}_{0};H^{\comp}_{0};\Delta_{1}}{K_{c}[return\ \valueexpr_{1}]}{\trace[_{comp}]}$
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtrans:execs3} $\execs{foo,\hole{\cdot};\library;(K;foo),(\hole{\cdot};main),\hole{\cdot};\comp;\memstate}{e_{foo}[\valueexpr_{0}/y]}{foo,\hole{\cdot};\library;(K;foo),(\hole{\cdot};main),\hole{\cdot};\comp;\memstate'}{K_{c}[return\ \valueexpr_{1}]}{\trace[_{comp}]}$
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtrans:execs4} $\execs{foo,\hole{\cdot};\library;(K;foo),(\hole{\cdot};main),\hole{\cdot};\comp;\memstate'}{K_{c}[return\ \valueexpr_{1}]}{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate'}{K[\valueexpr_{1}]}{\trace[_{2}]}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{72}
    \item\label[ass]{asm:backtranscorr:exect5} $\exect[]{foo,\hole{\cdot};\library;\kontstack,(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate'}{K[\valueexpr_{1}]}{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate''}{K_{e}[\valueexpr_{2}]}{\trace[_{1}]}$
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{75}
    \item\label[ass]{asm:backtrans:execs5} $\execs{foo,\hole{\cdot};\library;\kontstack,(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate'}{K[\valueexpr_{1}]}{\commlib;\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate'}{K_{e}\hole{42}}{\trace[_{3}]}$
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtrans:execs6} $\execs{foo,\hole{\cdot};\library;(\hole{\cdot};main),\hole{\cdot};\ctx;\memstate''}{K_{e}[42]}{\commlib;\library;\hole{\cdot};\ctx;\memstate''}{\valueexpr_{2}}{\trace_{4}}$
  ...
  We solve \Cref{goal:backtrans:correct:execs} by transitivity, making use of \Cref{asm:backtrans:execs0,asm:backtrans:execs1,asm:backtrans:execs2,asm:backtrans:execs3,asm:backtrans:execs4,asm:backtrans:execs5,asm:backtrans:execs6}.
...
\begin{scontents}[store-env=buffer]
  If
  \begin{assumptions}
  \item\label[ass]{asm:rtmsp:srctms} $\forall \delta_{MS}\ \src{\library_{ctx}}\ \src{\library_{comp}}\ \src{\trace}\ \src{\Omega}\ \src{v}, \wexecs{prog\ \library_{ctx}\ \library_{comp}}{\configs}{\finalexpr}{\trace}\rightarrow\tospecificevs[_{\delta_{MS}}]{\trace}\in\operatorname{tmssafe}$
  \item\label[ass]{asm:rtmsp:trgexec} $\wexec{\trg{prog\ \library_{ctx}\ }\mmlAmmlAtcomp{\library_{comp}}}{\trg{\configt}}{\trg{\finalexpr}}{\trg{\trace}}$
  \end{assumptions}
  then $\exists \delta_{MS}',$
  \begin{goals}
    \item\label[goal]{rtmsp:tms} $\tospecificevs[_{\delta_{MS}'}]{\trace}\in\operatorname{tmssafe}$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{4}
    \item\label[ass]{asm:rtmsp:backtrans} $\mmlAmmlAtbackdetail[\emptyset;\emptyset]{\trg{\trace}}=\Game;\delta;\src{let\ main\ z\ := e},\src{\hole{\cdot}}$.
  \end{passumptions}
...
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAtmmlAtcomp{e, M}=\trg{e'}}{,,Compile $\mmlAt$ expression $\trg{e}$ to $\mmlAt$ expression $\trg{e'}$.\\$\trg{M}$ maps variable names to sizes on heap.''}\\
    \judgbox{\mmlAtmmlAtcomp{\left[\valueexpr / x\right]}=\trg{\left[\valueexpr' / x'\right]}}{,,Compile $\mmlAt$ substitution to $\mmlAt$ substitution.''} \\
    \judgbox{\mmlAtmmlAtcomp{\commlib}=\trg{\commlib'}}{,,Compile $\mmlAt$ component library to $\mmlAt$ component library.''} \\
    \end{gather*}
    $$
    \begin{array}{rll}
    \mmlAtmmlAtcomp{\finalexpr} &\ = & \trg{\finalexpr} \\
    \mmlAtmmlAtcomp{call\ foo\ e} &\ = & \trg{call\ }\mmlAtmmlAtcomp{foo}\ \mmlAtmmlAtcomp{e}\\
    \mmlAtmmlAtcomp{return\ e} &\ = & \trg{return\ }\mmlAtmmlAtcomp{e}\\
    \mmlAtmmlAtcomp{e_1\oplus e_2} &\ = & \mmlAtmmlAtcomp{e_1} \trg{\oplus} \mmlAtmmlAtcomp{e_2} \\
      \mmlAtmmlAtcomp{x[e]} &\ = & \trg{let\ x_{ACCESS}=}\mmlAtmmlAtcomp{e}\trg{\ in\ }\\
                                &&\trg{ifz\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\&&\ \ \trg{x[x_{ACCESS}]}\\
                                && \trg{else\ abort()}\\
    \mmlAtmmlAtcomp{let\ x= e_1\ in\ e_2} &\ = & \trg{let\ x=} \mmlAtmmlAtcomp{e_1}\trg{\ in\ }\mmlAtmmlAtcomp{e_2} \\
      \mmlAtmmlAtcomp{x[e_1]\leftarrow e_2} &\ = & \trg{let\ x_{ACCESS}=}\mmlAtmmlAtcomp{e_1}\trg{\ in\ }\\
      &&\trg{ifz\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\&&\ \ \trg{x[x_{ACCESS}]\leftarrow }\mmlAtmmlAtcomp{e_2} \\
      &&\trg{else\ abort()} \\
    \mmlAtmmlAtcomp{let\ x=new\ e_1\ in\ e_2} &\ = & \trg{let\ x_{\texttt{SIZE}}=}\mmlAtmmlAtcomp{e_1}\trg{\ in\ }\\&&\trg{let\ x}\trg{=new\ x_{\texttt{SIZE}}}\trg{\ in\ }\mmlAtmmlAtcomp{e_2} \\
    \mmlAtmmlAtcomp{delete\ x} &\ = & \trg{delete\ x} \\
    \mmlAtmmlAtcomp{x\ is\ \poisoned} &\ = & \trg{x\ is\ \poisoned} \\
    \mmlAtmmlAtcomp{\lparen e_1;e_2\rparen} &\ = & \trg{\lparen}\mmlAtmmlAtcomp{e_1}\trg{;}\mmlAtmmlAtcomp{e_2}\trg{\rparen}\\
    \mmlAtmmlAtcomp{\pi_1\ e} &\ = & \trg{\pi_1\ }\mmlAtmmlAtcomp{e}\\
    \mmlAtmmlAtcomp{\pi_2\ e} &\ = & \trg{\pi_2\ }\mmlAtmmlAtcomp{e}\\
    \mmlAtmmlAtcomp{e\ has\ \type} &\ = & \mmlAtmmlAtcomp{e}\ \trg{has\ \type}\\
    \mmlAtmmlAtcomp{ifz\ e_1\ then\ e_2\ else\ e_3} &\ = & \trg{ifz\ }\mmlAtmmlAtcomp{e_1}\\
                                                        &&\trg{then\ }\mmlAtmmlAtcomp{e_2} \\
                                                        &&\trg{else\ }\mmlAtmmlAtcomp{e_3} \\[0.5cm]
    %
    \mmlAtmmlAtcomp{\left[\trg{v}/\trg{x}\right]}&\ = & \trg{\left[\right.}\mmlAtmmlAtcomp{v}\trg{/}\mmlAtmmlAtcomp{x}\trg{\left.\right]} \\[0.5cm]
    %
    \mmlAtmmlAtcomp{\hole{\cdot}}&\ = & \trg{\hole{\cdot}} \\
    \mmlAtmmlAtcomp{foo,\commlib}&\ = & \trg{foo,}\mmlAtmmlAtcomp{\commlib} \\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler}{Compiler from $\mmlAt$ to $\mmlAt$.}
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAtmmlAtcomp{F}=\trg{F}}{,,Compile $\mmlAt$ procedures to $\mmlAt$ procedures.''}
    \end{gather*}
    $$
    \begin{array}{rll}
    %
      \mmlAtmmlAtcomp{let\ foo\ x := e}&\ = & \trg{let\ foo\ x := }\mmlAtmmlAtcomp{e}\\
    \end{array}
    $$
    \begin{gather*}
    \judgbox{\mmlAtmmlAtcomp{\library}=\trg{\library}}{,,Compile $\mmlAt$ libraries to $\mmlAt$ libraries.''}
    \end{gather*}
    $$
    \begin{array}{rll}
      \mmlAtmmlAtcomp{\hole{\cdot}}&\ = & \trg{\hole{\cdot}} \\
      \mmlAtmmlAtcomp{F,\library}&\ = & \mmlAtmmlAtcomp{F}\trg{,}\mmlAtmmlAtcomp{\library} \\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler-comp}{Compiler from $\mmlAt$ components to $\mmlAt$ components.}
\myfig{
  \begin{center}
    \begin{gather*}
      \judgbox{\mmlAtmmlAtcomp{K}=\trg{K}}{,,Compile $\mmlAt$ evaluation contexts to $\mmlAt$ evaluation contexts.''}
    \end{gather*}
    $$
    \begin{array}{rll}
      \mmlAtmmlAtcomp{\hole{\cdot}} &\ = & \trg{\hole{\cdot}} \\
      \mmlAtmmlAtcomp{K \oplus e} &\ = & \mmlAtmmlAtcomp{K}\trg{\oplus}\mmlAtmmlAtcomp{e} \\
      \mmlAtmmlAtcomp{\valueexpr \oplus K} &\ = & \mmlAtmmlAtcomp{\valueexpr}\trg{\oplus}\mmlAtmmlAtcomp{K} \\
      \mmlAtmmlAtcomp{x[K]} &\ = & \trg{let\ x_{ACCESS}=}\mmlAtmmlAtcomp{K}\trg{\ in} \\
                                &&\trg{if\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\
                                &&\ \ \trg{x[x_{ACCESS}]}\\
                                &&\trg{else\ abort()}\\
      \mmlAtmmlAtcomp{let\ x\ = K\ in\ e} &\ = & \trg{let\ x\ = }\mmlAtmmlAtcomp{K}\trg{\ in\ }\mmlAtmmlAtcomp{e} \\
      \mmlAtmmlAtcomp{x[K] \leftarrow e} &\ = & \trg{let\ x_{ACCESS} = }\mmlAtmmlAtcomp{K}\trg{\ in}\\
                                &&\trg{if\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\
      &&\ \ \trg{x[x_{ACCESS}]\leftarrow}\mmlAtmmlAtcomp{e} \\
      &&\trg{else\ abort()} \\
      \mmlAtmmlAtcomp{x[n] \leftarrow K} &\ = & \trg{if\ 0\le n<x_{SIZE}\ then}\\
      &&\ \ \trg{x[n]\leftarrow }\mmlAtmmlAtcomp{K} \\
      &&\trg{else\ abort()} \\
      \mmlAtmmlAtcomp{let\ x\ = new\ K\ in\ e} &\ = & \trg{let\ x_{SIZE}=}\mmlAtmmlAtcomp{K}\trg{\ in\ }\\
                                &&\trg{let\ x = new\ x_{SIZE}\ in\ }\mmlAtmmlAtcomp{e} \\
      \mmlAtmmlAtcomp{ifz\ K\ then\ e_{1}\ else\ e_{2}} &\ = & \trg{ifz\ }\mmlAtmmlAtcomp{K}\trg{\ then\ }\\
                               &&\ \ \mmlAtmmlAtcomp{e_{1}}\\
                               &&\trg{else\ }\\
                               &&\ \ \mmlAtmmlAtcomp{e_{2}} \\
      \mmlAtmmlAtcomp{call\ foo\ K} &\ = & \trg{call\ foo\ }\mmlAtmmlAtcomp{K}\\
      \mmlAtmmlAtcomp{return\ K} &\ = & \trg{return\ }\mmlAtmmlAtcomp{K}\\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler-ctx}{Compiling $\mmlAt$ evaluation contexts to $\mmlAt$ evaluation contexts.}

\begin{lemma}[Injective Compiler]\label{lem:smscomp:inject}
  If
  \begin{assumptions}
    \item $\mmlAtmmlAtcomp{e}=\trg{e_0}$
    \item $\mmlAtmmlAtcomp{e}=\trg{e_1}$
  \end{assumptions}
  Then
  \begin{goals}
    \item $\trg{e_0}=\trg{e_1}$
...
\begin{lemma}[Base SMS via Monitor]
  If
  \begin{assumptions}
    \item\label[ass]{asm:basesmsviamonitor:step} $\exprevalt{\Omega}{e_{0}}{\Omega'}{e_{0}'}{\event}$
    \item $\trg{e}=\mmlAtmmlAtcomp{e_{0}}$
    \item $\trg{\Omega}=\trg{\cfstate;\comp;\memstate}$
    \item\label[ass]{asm:basesmsviamonitor:comp:step} $\exect{\Omega}{e}{\Omega'}{e'}{\event}$
    \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
  \end{assumptions}
  Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
  \begin{goals}
    \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
    \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{\event}}{\absevent}$
    \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
    \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{\Omega'}.\trg{\Delta}}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
  First note
  \begin{itemize}
    ...
    \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta}$
  \end{itemize}
  Induction on $\exprevalt{\Omega}{e_{0}}{\Omega'}{e_{0}'}{\event}$ (\Cref{asm:basesmsviamonitor:step}).
  \begin{description}
    \item $\trg{e}=\trg{n_{1}\oplus n_{2}}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{n_{3}}=\trg{n_{1}}\oplus\trg{n_{2}}$
        \item $\exprevalt{\Omega}{n_{1}\oplus n_{2}}{\Omega'}{n_{3}}{\emptyevent}$
        \item $\mmlAtmmlAtcomp{n_{1}\oplus n_{2}}=\trg{n_{1}\oplus n_{2}}$
        \item $\exprevalt{\Omega}{n_{1}\oplus n_{2}}{\Omega}{e'}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
        \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
        \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{\emptyevent}}{\absevent}$
        \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{\Omega}.\trg{\Delta}}$
      \end{goals}
      Choose $\absevent=\bm{\emptyevent}$, $\delta_{\text{MS}}'=\delta_{\text{MS}}$, and $\smsmonitor[']=\smsmonitor$.

      All goals are completely straightforward.

    \item $\trg{e}=\trg{let\ x=\finalexprnoerr\ in\ e}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\exprevalt{\Omega}{let\ x=\finalexprnoerr\ in\ e}{\Omega'}{e[\finalexprnoerr/x]}{\emptyevent}$
        \item $\mmlAtmmlAtcomp{let\ x=\finalexprnoerr\ in\ e}=\trg{let\ x=\finalexprnoerr\ in\ }\mmlAtmmlAtcomp{e}$
        \item $\exprevalt{\Omega}{let\ x=\finalexprnoerr\ in\ {\color{black}\mmlAtmmlAtcomp{e}}}{\Omega}{e'}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{if\ 0\ then\ e_{1}\ else\ e_{2}}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\exprevalt{\Omega}{if\ 0\ then\ e_{1}\ else\ e_{2}}{\Omega'}{e_{1}}{\emptyevent}$
        \item $\mmlAtmmlAtcomp{if\ 0\ then\ e_{1}\ else\ e_{2}}=\trg{if\ 0\ then\ }\mmlAtmmlAtcomp{e_{1}}\trg{\ else\ }\mmlAtmmlAtcomp{e_{2}}$
        \item $\exprevalt{\Omega}{if\ 0\ then\ {\color{black}\mmlAtmmlAtcomp{e_{1}}}\ else\ {\color{black}\mmlAtmmlAtcomp{e_{2}}}}{\Omega}{e'}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{if\ S(n)\ then\ e_{1}\ else\ e_{2}}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\exprevalt{\Omega}{if\ S(n)\ then\ e_{1}\ else\ e_{2}}{\Omega'}{e_{2}}{\emptyevent}$
        \item $\mmlAtmmlAtcomp{if\ S(n)\ then\ e_{1}\ else\ e_{2}}=\trg{if\ S(n)\ then\ }\mmlAtmmlAtcomp{e_{1}}\trg{\ else\ }\mmlAtmmlAtcomp{e_{2}}$
        \item $\exprevalt{\Omega}{if\ S(n)\ then\ {\color{black}\mmlAtmmlAtcomp{e_{1}}}\ else\ {\color{black}\mmlAtmmlAtcomp{e_{2}}}}{\Omega}{e'}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
      ...
      {\huge\color{green}TODO: continue here}

    \item $\trg{e}=\trg{\pi_{2}\langle n_{1} ; n_{2} \rangle}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{\pi_{2}\langle n_{1} ; n_{2} \rangle}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{\pi_{1}\langle n_{1} ; n_{2} \rangle}{\Omega}{n_{2}}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{\pi_{1}\langle n_{1} ; n_{2} \rangle}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{\pi_{1}\langle n_{1} ; n_{2} \rangle}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{\pi_{1}\langle n_{1} ; n_{2} \rangle}{\Omega}{n_{1}}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{n\ has\ \nat}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{n\ has\ \nat}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{n\ has\ \nat}{\Omega}{0}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{x\ has\ \nat}$\textbf{:}
      ...
      \begin{passumptions}{H}
        \item $\trg{x\ has\ \nat}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{x\ has\ \nat}{\Omega}{1}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{\langle n_{1} ; n_{2} \rangle\ has\ \nat}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{\langle n_{1} ; n_{2} \rangle\ has\ \nat}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{\langle n_{1} ; n_{2} \rangle\ has\ \nat}{\Omega}{1}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{abort()}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item\label[ass]{asm:abortnotcompiling} $\trg{abort()}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\exprevalt{\Omega}{abort()}{\Omega}{stuck}{\lightning}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
        \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
        \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{\lightning}}{\absevent}$
        \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{\Omega}.\trg{\Delta}}$
      \end{goals}

      This case is spurious, since $\trg{abort()}$ is never the result of compiling an $\mmlAt$ expression: \Cref{asm:abortnotcompiling}.


    \item $\trg{e}=\trg{x\ is\ \poisoned}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{x\ is\ \poisoned}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\trg{\Omega}=\trg{\cfstate;\comp;\memstate}$
        \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta}$
        \item $\trg{\Delta}=\trg{\Delta_{1},x\mapsto(\loc;\sandboxtag;\poisoned;n),\Delta_{2}}$
        \item $\exprevalt{\Omega}{x\ is\ \poisoned}{\Omega}{0}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{x\ is\ \poisoned}$\textbf{:}
      ...
      \begin{passumptions}{H}
        ...
        \item $\trg{\Omega}=\trg{\cfstate;\comp;\memstate}$
        \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta}$
        \item $\trg{\Delta}=\trg{\Delta_{1},x\mapsto(\loc;\sandboxtag;\poisonless;n),\Delta_{2}}$
        \item $\exprevalt{\Omega}{x\ is\ \poisoned}{\Omega}{1}{\emptyevent}$
        \item $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Omega}.\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
    ...
    \item $\trg{e}=\trg{delete\ x}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{delete\ x}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\trg{\Omega}=\trg{\cfstate;\comp;\memstate}$
        \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta_{1},x\mapsto(\loc;\comp;\poisonless;n),\Delta_{2}}$
        \item $\trg{\Omega'}=\trg{\cfstate;\comp;\memstate'}$
        \item $\trg{\memstate'}=\trg{H^{\ctx};H^{\comp};\Delta_{1},x\mapsto(\loc;\comp;\poisoned;n),\Delta_{2}}$
        \item $\exprevalt{\Omega}{delete\ x}{\Omega'}{0}{Dealloc\ \loc;\comp}$
        \item\label[ass]{asm:smssafepstep:storeagree:0} $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Delta_{1},x\mapsto(\loc;\comp;\poisonless;n),\Delta_{2}}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
        \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
        \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{Dealloc\ \loc}}{\absevent}$
        \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{\Delta_{1},x\mapsto(\loc;\comp;\poisoned;n),\Delta_{2}}}$
      \end{goals}

      Note that by \Cref{asm:smssafepstep:storeagree:0}, $\trg{\loc}\in \operatorname{dom}\delta_{MS}$, so let $\delta_{MS}(\trg{\loc})=\specificev{\loc}$ and also $(\specificev{\loc},m)\in\smsmonitor$.

      Choose ${\absevent}=\textbf{Dealloc}\ \specificev{\loc}$, $\delta_{\text{MS}}'=\delta_{\text{MS}}$, and $\smsmonitor[']=\smsmonitor\setminus\{(\specificev{\loc}, m)\}$.

      First goal is by reflexivity.

      Second goal by \Cref{tr:sms-dealloc-auth}.

      Third goal by \Cref{tr:sms-dealloc}.

      Fourth goal is a consequence of \Cref{asm:smssafepstep:storeagree:0} using \Cref{tr:store-empty-agree,tr:store-cons-agree}.

    \item $\trg{e}=\trg{let\ x=new\ n\ in\ e}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{let\ x=new\ n\ in\ e}=\mmlAtmmlAtcomp{e_{0}}$
        ...
        \item $\exprevalt{\Omega}{let\ x=new\ n\ in\ e}{\Omega'}{e[z/x]}{Alloc\ \loc\ n;\comp}$
        \item\label[ass]{asm:smssafepstep:storeagree:1} $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
        \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
        \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{Alloc\ \loc\ n}}{\absevent}$
        \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{z\mapsto(\loc;\comp;\poisonless;n),\Delta}}$
      \end{goals}

      {\large\color{red} this does not work, let\ x=new\ n\ in\ e is never a result of the compiler}

      Choose ${\absevent}=\textbf{Alloc}\ \specificev{\loc}\ n$, $\delta_{\text{MS}}'=\delta_{\text{MS}}\cup\{\trg{\loc}\mapsto\specificev{\loc}\}$, and $\smsmonitor[']=\smsmonitor\cup\{(\specificev{\loc}, n)\}$.

      First goal is by transitivity, reflexivity, and the fact that the empty set subsets any other set.

      Second goal by \Cref{tr:sms-alloc-auth}.

      Third goal by \Cref{tr:sms-alloc}.

      Fourth goal is a consequence of \Cref{asm:smssafepstep:storeagree:1} using \Cref{tr:store-empty-agree,tr:store-cons-agree}.

    \item $\trg{e}=\trg{x[n]}$\textbf{:}
      If
      \begin{passumptions}{H}
        \item $\trg{x[n]}=\mmlAtmmlAtcomp{e_{0}}$
        \item $\trg{\Omega}=\trg{\cfstate;\comp;\memstate}$
        \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta_{1},x\mapsto(\loc,\sandboxtag,\poison,n),\Delta_{2}}$
        \item $\exprevalt{\Omega}{x[n]}{\Omega}{v}{Read\ \loc\ n;\comp}$
        \item\label[ass]{asm:smssafepstep:storeagree:2} $\storeagreesms[\delta_{\text{MS}}]{\smsmonitor}{\trg{\Delta}}$
      \end{passumptions}
      Then $\exists\ \absevent\ \delta_{\text{MS}}'\ \smsmonitor[']$
      \begin{goals}
        \item $\delta_{\text{MS}}\subseteq\delta_{\text{MS}}'$
        \item $\smstraceagree{\tospecificevt[_{\delta_{\text{MS}}'}]{Read\ \loc\ n}}{\absevent}$
        \item $\monitorcheck[]{\smsmonitor}{\smsmonitor[']}{\absevent}$
        \item $\storeagreetms[\delta_{\text{MS}}']{\smsmonitor[']}{\trg{\Delta_{1},x\mapsto(\loc,\sandboxtag,\poison,n),\Delta_{2}}}$
      \end{goals}



      Choose $\absevent=\textbf{Read}\ \specificev{\loc}\ n$, $\delta'_{\text{MS}}=\delta_{\text{MS}}$, and $\smsmonitor[']=\smsmonitor$.

      The first goal is easy by reflexivity.


      The second goal

      The third goal by \Cref{tr:sms-use}.
