...
\begin{proof}
  ...
  First, note that $\src{\Gamma_{0}}=\src{\library}\downarrow=\src{foo\mapsto\nat\to\nat,main\mapsto\nat\to\nat}$ by \Cref{tr:w-lib-proj-empty,tr:w-lib-proj-cons}.
  Consequently, $\src{main}\in\operatorname{dom}\src{\Gamma_{0}}=\left\{\src{foo},\src{main}\right\}$ and $\src{\Gamma_{0}}\vdash\src{\commlib}\ \operatorname{ok}$ by \Cref{tr:w-t-relevant-empty,tr:w-t-relevant-cons,tr:t-var} noting that $\src{\nat\to\nat}\not=\src{\ptr}$.
