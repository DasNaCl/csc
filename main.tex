...
\myfig{
  \begin{center}
    ...
    \begin{array}{rcl}
      ...
      \tospecificevs{\trace} &\ = & \operatorname{map}\ \thetospecificfilter\ (\operatorname{filter}\ \src{\varepsilon}\ \src{\trace})\\[0.5cm]
      %
...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-alloc}{
      ...
      \mmlAmmlAtbackdetail{\specificevt{Alloc\ \loc\ n}\cdot\mstrace} = \Game'', \src{let\ x = new\ }\mmlAmmlAtbackv{n}\src{\ in\ e}
    }{backtrans-alloc}
    %
...
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtback{\mstrace} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and an $\mmlAs$\\ expression from memory-specific $\mmlAt$ ,,top-level'' trace $\trg{\mstrace}$.''}$\;$\\
    %
    \typerule{backtrans-top-level}{
      \mmlAmmlAtbackdetail{\mstrace} = \Game', \src{e}
    }{
      \mmlAmmlAtback{\specificevt{Ret\ !n_0}\cdot\mstrace\cdot\specificevt{Call\ ?n_1}}, \Game', \src{e}
    }{backtrans-top-level}
    %
    \typerule{backtrans-top-level-crash}{
      \mmlAmmlAtbackdetail{\mstrace} = \Game', \src{e}
    }{
      \mmlAmmlAtback{\specificevt{Ret\ !n_0}\cdot\mstrace\cdot\specificevt{\lightning}} = \Game', \src{e;abort()}
...
\begin{incompleteproof}
  ...
  \item $\mmlAmmlAtback{\specificevt{Ret\ !n_0}\cdot\msevent\cdot\mstrace\cdot\specificevt{Call\ ?n_1}}=\Game',\src{e}$
  ...
  \item $\trg{\mstrace}=\thetospecificfilter\left(\trg{\trace}\right)$
  \item $\mmlAmmlAtback[\Game_{\IH}]{\specificevt{Ret\ !n_0}\cdot\mstrace\cdot\specificevt{Call\ ?n_1}}=\Game_{\IH}',\src{e_{\IH}}$
  ...
  \begin{description}
    \item \textbf{Case $\trg{\msevent}=\specificevt{Alloc\ \loc\ n}$:}
      Note that:
      \begin{passumptions}{H}
    \item $\mmlAmmlAtback{\specificevt{Ret\ !n_0}\cdot\specificevt{Alloc\ \loc\ n}\cdot\mstrace\cdot\specificevt{Call\ ?n_1}}=\Game',\src{let\ x=new\ }\mmlAmmlAtbackv{n}\src{\ in\ e_a}$
    \item $\Game_x=\Game[\trg{\loc}]$
    \item $\src{x}=\Game_x(\trg{\loc})$
    \item $\mmlAmmlAtback[\Game_x]{\mstrace}=\Game',\src{e_a}$
      \end{passumptions}

      Inversion on \Cref{asm:backtranscorr:indu:spectrace} to get $\specificevt{Alloc\ \loc\ n}=\trg{Alloc\ \loc\ n}$, subsequently inverting \Cref{asm:backtranscorr:indu:step} yields:
      \begin{passumptions}{H}
      \setcounter{enumi}{4}
    ...
    \item\label[ass]{asm:backtranscorr:indu:steps} $\exect{H';z\mapsto\loc_{\poisonless},\Delta}{e'}{\Omega''}{v}{\trace}$
      ...
      \item\label[ass]{asm:backtranscorr:indu:backtrans} $\mmlAmmlAtback[\Game_x]{\specificevt{Ret\ !n_0}\cdot\mstrace\cdot\specificevt{Call\ ?n_1}}=\Game',\src{e'}$
      \end{passumptions}
    ...
    \item \textbf{Case $\trg{\msevent}=\specificevt{Dealloc\ \loc}$:}
      todo.

    \item \textbf{Case $\trg{\msevent}=\specificevt{Use\ \loc\ n}$:}
      todo.
...
\begin{incompleteproof}
  ...
  \item\label[ass]{rtmsp:runt:e2} $\exect{\Omega_p}{e_2}{\Omega_2}{\valueexpr_2}{\trace[_2]}$
  ...
  \item $\xlangbackstateeq[\delta_1]{\Omega_1}{\Omega_1}$
  ...
  Apply \thmref{lem:expression:correctness} on \Cref{rtmsp:runt:ep:compv} using $\xlangstateeq[\delta_1]{\Omega_1}{\Omega_1}$\MK{where do we get this?} and $\typechecks{}{\Omega\triangleright e_p}{\nat}$:
  \begin{passumptions}{F}
  \item $\delta_1\subseteq\delta_p$
  \item $\exec{\src{\Omega_1}}{\src{e_p}}{\src{\Omega_p}}{\src{\valueexpr_p}}{\src{\trace[_p]}}$
  \item $\xlangstateeq[\delta_p]{\Omega_p}{\Omega_p}$
  ...
  Analogousy, $\xlangbackstateeq[\delta_p]{\Omega_p}{\Omega_p}$\MK{why}.
  Using these facts, we apply $\thmref{lem:backtranslation:correct}$ on \Cref{rtmsp:runt:e2}:
  \begin{passumptions}{H^{\tiny 2}}
  \item $\delta_p\subseteq\delta_2$, where $\delta_2:\src{L}\to\trg{L}$
  \item $\exec{\src{\Omega_p}}{\src{e_2}}{\src{\Omega_2}}{\mmlAmmlAtbackv{v_2}}{\src{\trace[_2]}}$
  \item $\xlangbackstateeq[\delta_2]{\Omega_2}{\Omega_2}$
  \item ${\src{Ret\ !}\mmlAmmlAtbackv{0}\src{\cdot\trace[_2]\cdot Call\ ?}\mmlAmmlAtbackv{v_1}}\thexlangtraceeq[\delta_1]{\trg{Ret\ !0\cdot\trace[_2]\cdot Call\ ?v_1}}$
  \end{passumptions}
