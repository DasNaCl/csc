...
\myfig{
  \begin{center}
    \judgbox{\execs{\configs}{e}{\configs'}{e'}{As}}{,,Expression $\src{e}$ evaluates under configuration $\configs$ to $\src{e'}$ and new configuration $\src{\configs'}$,\\emitting list of events $\src{\bar{a}}$.''}
    %
    \typerule{$e-refl$}{
    }{
...
\begin{lemma}[$\mmlA$ is not spatially memory safe]
  There is an $\mmlA$ component which does not robustly satisfy \Cref{def:spatmemsafe}:
  $\rsat{\src{p}}{\src{smsafe}}$
\end{lemma}
\begin{proof} %TODO: counterexample must be a component, not a program
