...
\myfig{
  ...
  \begin{gather*}
  \begin{aligned}
  \mi{Expressions}~\src{e} \bnfdef& \text{side-effect free operation leading a value}\\
  %
  ...
  \mi{Locations}~\src{\ell}\in\mathbb{K} \\
  %
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\src{e} \bnfdef& \src{Internal} \mid \src{Alloc\ x\ n} \mid \src{Dealloc\ x} \mid \src{Read\ x\ n} \mid \src{Write\ x\ n\ m}
  \end{aligned}
  \end{gather*}
...
\myfig{
  ...
  \typerule{$e-new$}{
    ...
    \evals{H}{A}{\Delta}{let\ x:= new\ e}{Alloc\ x\ n}{skip}{H'}{A,x\mapsto|H|}{\Delta}
  }{w-e-new}
  %
  \typerule{$e-delete$}{
  }{
    \evals{H}{A,x\mapsto \loc}{\Delta}{delete\ x}{Dealloc\ x}{skip}{H}{A}{\Delta}
  }{w-e-delete}
  %
  \\
  \typerule{$e-get$}{
    \exprevals{e}{\Delta}{\src{n}} &
    lookup\ \src{H}\ \loc + n = Some\ v
  }{
    \evals{H}{A,x\mapsto\loc}{\Delta}{let\ y := x[e]}{Read\ x\ n}{skip}{H}{A,x\mapsto\loc}{\Delta,y\mapsto v}
  }{w-e-get}
  %
  \\
  \typerule{$e-set$}{
    ...
    \src{H'} = replace\ \src{H}\ \loc + n\ \src{m}
  }{
    \evals{H}{A,x\mapsto\loc}{\Delta}{x[e_1] := e_2}{Write\ x\ n\  m}{skip}{H'}{A,x\mapsto\loc}{\Delta}
  }{w-e-set}
