...
\myfig{
  ...
  \begin{gather*}
  \begin{aligned}
  \mi{Expressions}~\src{e} \bnfdef&\ \src{x} \mid \src{v} \mid \src{e \oplus e} \hspace{3cm} \text{where } \src{\oplus}\in\{\src{+},\src{-},\src{*}\}\\
  %
  ...
  \mi{References}~\src{\ell}\in\refs \\
  %
  \mi{Variables}~&\ \src{x} \mid \src{y} \mid \dots \hspace{0.5cm}
  \mi{Contexts}~\src{C} \bnfdef\ \src{c_{1};\hole{\cdot};c_{2}} \hspace{0.5cm}
  \mi{Programs}~\src{p} \bnfdef\ \src{prog\ c} \\
  %
  \mi{Typing. Env.}~\src{\Gamma} \bnfdef&\ \src{\hole{\cdot}} \mid \Gammas,\src{x:\tau} \hspace{0.5cm}
  \mi{Eval. Env.}~\src{\Delta} \bnfdef\ \src{\hole{\cdot}} \mid \Deltas,\src{x\mapsto v} \mid \Deltas,\src{\loc\mapsto n}\\
  %
  \mi{Heaps}~\src{H} \bnfdef&\ \src{\hole{\cdot}} \mid \src{H}::\src{n} \hspace{0.5cm}
  \mi{Location\ Mappings}~\src{A}\bnfdef\ \src{\hole{\cdot}} \mid \src{A,x\mapsto \loc} \\
  %
  \mi{Configs}~\configs \bnfdef&\ \config{H}{A}{\Delta}{c} \\
  %
...
\myfig{
  ...
  \typechecks{\hole{\cdot}}{prog\ c}{\Gamma}
	}{w-t-prog}
  \end{center}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\src{b} \bnfdef& \src{\emptyevent} \mid \src{Alloc\ x\ n} \mid \src{Dealloc\ x} \mid \src{Read\ x\ n} \mid \src{Write\ x\ n\ m}
  \end{aligned}
  \end{gather*}
...
\myfig{
  \judgbox{\exprevals{e}{\Delta}{v}}{,,Expression $\src{e}$ evaluates under environment $\src{\Delta}$ to value $\src{v}$.''}
  %
  \typerule{$e-var$}{
  ...
  %
  \typerule{$e-binop$}{
    \exprevals{e_1}{\Delta}{v_1} &
    \exprevals{e_2}{\Delta}{v_2} &
    v = v_1 \oplus v_2
  }{
    \exprevals{e_1\oplus e_2}{\Delta}{v}
  }{w-e-binop}
}{w-expr-eval}{Evaluation of $\whileA$ expressions.}

\myfig{
  \judgbox{\evals{\configs}{b}{\configs'}}{,,Make a step from configuration $\configs$ emitting event $\src{b}$ to configuration $\src{\configs'}$.''}
  %
%  \typerule{$e-ctx$}{
%    \evals{\config{H}{A}{\Delta}{c}}{b}{\config{H'}{A'}{\Delta'}{c'}}
%  }{
%    \ctxevals{\config{H}{A}{\Delta}{K[c]}}{b}{\config{H'}{A'}{\Delta'}{K[c']}}
%  }{w-t-ctx}
  %
  \typerule{$e-skip$}{
  }{
    \evals{\config{H}{A}{\Delta}{skip}}{\emptyevent}{\config{H}{A}{\Delta}{skip}}
  }{w-e-skip}
  %
  \typerule{$e-lseq$}{
    \evals{\config{H}{A}{\Delta}{p_0}}{b}{\config{H'}{A'}{\Delta'}{p_0'}}
  }{
    \evals{\config{H}{A}{\Delta}{p_0;p_1}}{b}{\config{H'}{A'}{\Delta'}{p_0';p_1}}
  }{w-e-lseq}
  %
  \typerule{$e-rseq$}{
  }{
    \evals{\config{H}{A}{\Delta}{skip;p_1}}{\emptyevent}{\config{H}{A}{\Delta}{p_1}}
  }{w-e-rseq}
  %
  \typerule{$e-assign$}{
    ...
    \evals{\config{H}{A}{\Delta}{let\ x := e}}{\emptyevent}{\config{H}{A}{\Delta,x\mapsto v}{skip}}
  }{w-e-assign}
  %
  \typerule{$e-loopcont$}{
    \exprevals{e}{\Delta}{v} &
    ...
    \evals{\config{H}{A}{\Delta}{while\ e\ \{c\}}}{\emptyevent}{\config{H}{A}{\Delta}{c;while\ e\ \{c\}}}
  }{w-e-loopcont}
  %
  \typerule{$e-loopstop$}{
    \exprevals{e}{\Delta}{v} &
    ...
    \evals{\config{H}{A}{\Delta}{while\ e\ \{c\}}}{\emptyevent}{\config{H}{A}{\Delta}{skip}}
  }{w-e-loopstop}
  %
  \\
  \typerule{$e-new$}{
    \text{fresh }\src{\loc}\in\refs&
    \exprevals{e}{\Delta}{n} &
    \src{H'} = \src{H}\triangle n
  }{
    \evals{\config{H}{A}{\Delta}{let\ x:= new\ e}}{Alloc\ x\ n}{\config{H'}{A,x\mapsto\loc}{\Delta,\loc\mapsto|H|}{skip}}
  }{w-e-new}
  %
  \typerule{$e-delete$}{
  }{
    \evals{\config{H}{A,x\mapsto \loc}{\Delta,\loc\mapsto n_\loc}{delete\ x}}{Dealloc\ x}{\config{H}{A,x\mapsto\loc}{\Delta}{skip}}
  }{w-e-delete}
  %
  \\
  \typerule{$e-get$}{
    \exprevals{e}{\Delta}{m} &
    \src{H}\ (n_\loc + m) = Some\ \src{v}
  }{
    \evals{\config{H}{A,x\mapsto\loc}{\Delta,\loc\mapsto n_\loc}{let\ y := x[e]}}{Read\ x\ m}{\config{H}{A,x\mapsto\loc}{\Delta,\loc\mapsto n_\loc,y\mapsto v}{skip}}
  }{w-e-get}
  %
  \\
  \typerule{$e-set$}{
    \exprevals{e_1}{\Delta}{n} &
    \exprevals{e_2}{\Delta}{m} &
    \src{H'} = \src{H}\ (n_\loc + n\mapsto \src{m})
  }{
    \evals{\config{H}{A,x\mapsto\loc}{\Delta,\loc\mapsto n_\loc}{x[e_1] := e_2}}{Write\ x\ n\  m}{\config{H'}{A,x\mapsto\loc}{\Delta}{skip}}
  }{w-e-set}
...
\myfig{
  $$
  \begin{array}{lllcl}
    H & \triangle & n & = & match\ n\ with \\
         &   &   & | & 0 \Rightarrow H \\
         &   &   & | & S m \Rightarrow (H\triangle m) :: 0 \\
  \end{array}
  $$
  \vspace{0.5cm}
...
\begin{lemma}[$\whileA$ progress]
  Given $\typechecks{\Gamma}{p}{\Gamma'}$ there exist $\src{p'}, \src{b}, \src{H}, \src{H'}, \src{A}, \src{A'}, \src{\Delta}, \src{\Delta'}$ such that $\evals{\config{H}{A}{\Delta}{p}}{b}{\config{p'}{H'}{A'}{\Delta'}}$.
\end{lemma}
\begin{proof}
  %Straightforward induction on $\typechecks{\Gamma}{p}{\Gamma'}$.

 {\color{red}\Cref{tr:w-e-get} might fail!}
\end{proof}

\begin{lemma}[$\whileA$ preservation]
  Given $\typechecks{\Gamma}{p}{\Gamma'}$ and %
  $\evals{\config{H}{A}{\Delta}{p}}{b}{\config{p'}{H'}{A'}{\Delta'}}$ we have %
...
\myfig{
  \typerule{$ce-more$}{
    \evals{\config{H}{A}{\Delta}{p}}{b}{\config{p'}{H'}{A'}{\Delta'}} &
...
\begin{array}{rll}
  mk\_trace\ \src{p}\ (H : \Downarrow_{\infty}\src{p}) &=& match\ H\ with\\
                                                       &|& (\evals{\configs}{b}{\configs'},\  \Downarrow_{\infty}\src{p'}) \Rightarrow \seqcons{\src{b}}{(mk\_trace\ \src{p}\ (\Downarrow_{\infty}\src{p')})}\\
                                                      &end&
\end{array}
$$
\vspace{0.5cm}
...
\begin{definition}[Unicity]
  Given an event $\src{e}$ and a trace $\trace$, the event $\src{e}$ occurs exactly once written $\src{e}!_{\trace}$ iff

  $\exists n, \trace[n]=\src{e} \wedge \nexists m, n\not= m \wedge \trace[m] = \src{e}$
\end{definition}

\begin{definition}[Sequentiality]
  Given two events $\src{e_{0},e_{1}}$ and a trace $\trace$, event $\src{e_{0}}$ occurs before $\src{e_{1}}$ written
...
\begin{definition}[Temporal Memory Safety]\label{def:tempmemsafe}
  We define the property $\src{tsafe}$ as follows:
  Given any trace $\trace$, we say it is temporal memory safe iff

  $\forall \src{v\ s}, (\src{Alloc\ v\ s})!_{\trace}\ \implies (\src{Dealloc\ v})!_{\trace}\ \wedge (\src{Alloc\ v\ s})\le_{\trace} (\src{Dealloc\ v})$
\end{definition}

\begin{lemma}[$\whileA$ is temporal memory safe]
  For any $\whileA$ program $\src{p}$, it robustly satisfies \Cref{def:tempmemsafe}: $\rsat{\src{p}}{\src{tsafe}}$
\end{lemma}
\begin{proof}
   TODO
\end{proof}
