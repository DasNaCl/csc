...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=6mm,every node/.style={align=center}]
    ...
    \path (Determ.south) edge node[fill=white,rotate=-4] {$\supseteq$} (Bot.north);
    \path (NI.south east) edge node[fill=white,rotate=-17] {$\supseteq$} (Bot.north);
    \path (NonTerm.south west) edge node[fill=white,rotate=11] {$\subseteq$} (Bot.north);
    \path (Mutex.south west) edge node[fill=white,rotate=33] {$\subseteq$} (Bot.north);
  \end{tikzpicture}
