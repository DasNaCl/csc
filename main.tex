...
\myfig{
  \begin{center}
    ...
    \judgbox{\mmlAmmlAtback{\trace} = \src{context\ e_0\ e_1}}{,,Given $\mmlAt$ trace, construct an $\mmlAs$ expression that\\does what the trace describes.''}$\;$\\
...
\begin{lemma}[State Agreement Insertion]\label{lem:backtranslation:stateagreement:insertion}
  ...
  \item\label[ass]{ass:stateagreeinsertion:deltasagree} Mimicking $\delta,\atled$ \thmref{def:location:mimicking}
  \end{assumptions}
  Then
  \begin{goals}
...
\begin{lemma}[Backtranslation Correctness]\label{lem:backtranslation:correct}
  ...
  \item ${\tospecificevctxs{Ret\ !n_0\cdot\trace\cdot Call\ ?n_1}}\thexlangtraceeq{\tospecificevctxt{Ret\ !n_0\cdot\trace\cdot Call\ ?n_1}}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
  % Cannot use expr correctness, because e is target
\end{incompleteproof}
...
\begin{lemma}[Agreement Propagation]\label{lem:trace:agreement:propagation}
  ...
  \item $\xlangtraceeq{\mstrace}{\mstrace}$
  \item $\tmstraceagree[\delta^{\src{\bullet}}]{\src{\mstrace}}{\trace}$
  \end{assumptions}
  Then $\exists \delta^{\trg{\bullet}}_{\text{tms}}$,
  \begin{goals}
  \item $\tmstraceagree[\delta^{\trg{\bullet}}]{\trg{\mstrace}}{\trace}$
  \end{goals}
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
  \item $\exect{\emptyset;\emptyset}{e_1}{\Omega_1}{\valueexpr_1}{\trace[_1]}$
  \item $\exec{\trg{\Omega_1}}{\mmlAmmlAtcomp{e_p}}{\trg{\Omega_p}}{\trg{\valueexpr_p}}{\trg{\trace[_p]}}$
