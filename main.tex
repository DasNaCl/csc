  ...
  \begin{gather*}
    \begin{aligned}
      \src{E}\bnfdef &\ \src{\hole{\cdot}}\mid \src{E\oplus e}\mid \src{v\oplus E}\mid \src{x[e]}\mid
                        \src{let\ x:=E\ in\ e}\mid \src{x\leftarrow E}\mid \src{new\ E}\mid \src{if\ E\ then\ e_{1}\ else\ e_{2}}
    \end{aligned}
  \end{gather*}
}{w-expr-eval-ectx}{Evaluation Contexts of $\mmlA$.}

\myfig{
  \judgbox{\exprevals{\configs}{E[e]}{\configs'}{E[e']}}{,,Given an evaluation context $\src{E}$ and an expression $\src{e}$, it evaluates under\\configuration $\configs$ to $\src{e'}$ and new configuration $\src{\configs'}$ in context $\src{E}$.''}
  \judgbox{\exprevals{\configs}{e}{\configs'}{e'}}{,,Expression $\src{e}$ evaluates under configuration $\configs$ to $\src{e'}$ and new configuration $\src{\configs'}$.''}
  %
  \typerule{$e-ctx$}{
    \exprevals{\configs}{e}{\configs'}{e'}
  }{
    \exprevals{\configs}{E[e]}{\configs'}{E[e']}
  }{w-e-ectx}
  %
  \typerule{$e-var$}{
    \src{x\mapsto v}\in\Deltas
  }{
    \exprevals{\configs}{x}{\configs}{v}
  }{w-e-var}
  %
  \typerule{$e-\src{\oplus}$}{
    \src{n_1}+\src{n_2}=\src{n_3}
  }{
    \exprevals{\configs}{n_1+n_2}{\configs}{n_3}
  }{w-e-oplus}
  %
  \typerule{$e-\src{get}$}{
    ...
    \exprevals{\config{H}{A}{\Delta}}{x[n]}{\config{H}{A}{\Delta}}{v}
  }{w-e-get}
  %
  \typerule{$e-\src{let}$}{
    \src{\Delta'} = \Deltas\uplus \{\src{x\mapsto v}\}
  }{
    \exprevals{\config{H}{A}{\Delta}}{let\ x:=v\ in\ e}{\config{H}{A}{\Delta'}}{e}
  }{w-e-let}
  %
  \typerule{$e-\src{delete}$}{
    \src{\Delta'} = \Deltas \setminus \{\src{x\mapsto\loc}\}
  }{
    \exprevals{\config{H}{A}{\Delta}}{delete\ x}{\config{H}{A}{\Delta'}}{0}
  }{w-e-let-new}
  %
  \typerule{$e-\src{new}$}{
    \src{H'} = \grows{H}{n} &
    ...
    \exprevals{\config{H}{A}{\Delta}}{new\ n}{\config{H'}{A'}{\Delta}}{\loc}
  }{w-e-let-new}
  %
  \typerule{$e-\src{if}-\bot$}{
  }{
    \exprevals{\configs}{if\ 0\ then\ e_1\ else\ e_2}{\configs}{e_2}
  }{w-e-if-false}
  %
  \typerule{$e-\src{if}-\top$}{
    \src{v}\neq\src{0}
  }{
    \exprevals{\configs}{if\ v\ then\ e_1\ else\ e_2}{\configs}{e_1}
  }{w-e-if-true}
  %
}{w-expr-eval}{Evaluation of $\mmlA$ expressions.}

% use A(x) instead of A,x->L
