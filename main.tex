...
\myfig{
  \begin{center}
  \judgbox{\plugt{e_0}{e_1}=\trg{e_2}}{,,Replace any $\trg{hole:x}$ in $\trg{e_1}$ by $\trg{e_0}$, resulting in $\trg{e_2}$.''}
    \typerule{t-plug-hole}{
...
%   }{
%     \mmlAmmlAtback[\Game]{\specificev{Ret\ !\finalexprnoerr_0}\cdot\mstrace\cdot\specificev{Call\ ?\finalexprnoerr_1}} = \Game', \trg{\loc}, \src{let\ x=\finalexprnoerr_1\ in\ e}
%   }{backtrans-top-level-0}
%   %
%   \typerule{backtrans-top-level-1}{
%     \mmlAmmlAtbackdetail{\specificev{Dealloc\ \loc}\cdot\mstrace} = \Game', \src{e}
%   }{
%     \mmlAmmlAtback[\Game]{\specificev{Ret\ !n_0}\cdot\mstrace\cdot\specificev{Call\ ?n_1}} = \Game', (), \src{e}
%   }{backtrans-top-level-1}
    \typerule{backtrans-top-level}{
      ...
      \fresh{\Game}{\trg{\loc}}\\
      \mmlAmmlAtbackdetail[\Game,\src{\type_0\to\type_p}]{\mstrace[_0]\cdot\specificev{Alloc\ \loc\ 42}} = \Game', \src{let\ x=}\mmlAmmlAtbackv{\finalexprnoerr_0}\src{\ in\ e_0} \\
      \mmlAmmlAtbackdetail[\Game',()]{\mstrace[_1]}=\Game'',\src{e_1}
    }{
      \mmlAmmlAtback[\Game]{\specificev{Ret\ !0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_0}\cdot\mstrace[_p]\cdot\specificev{Ret\ !\finalexprnoerr_p}\cdot\mstrace[_1]\cdot\specificev{Call\ ?\finalexprnoerr_1}} = \Game'', \src{context\ e_0\ e_1}
    }{backtrans-top-level}
    %
...
Crash\MK{the crash rule is not precise. we need to backtranslate call/ret events}
We use the syntactic sugar $\src{e_1;e_2}$ for $\src{let\ \_ = e_1\ in\ e_2}$.
Note that \Cref{tr:backtrans-top-level} introduce a {\em wrapper} that performs a memory effect, in case the trace is not stuck.
