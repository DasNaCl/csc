...
\myfig{
  \begin{center}
    ...
    \typerule{cons-ignore-memstate-eq}{
      \src{\loc}\in\src{L}&
      \src{H'}=\overbrace{\src{0,\dots,0}}^{42-times}\src{,H} &
      \xlangstateeq{\Delta;H}{\Delta;H}
    }{
      \xlangstateeq{x\mapsto\loc_{\poison},\Delta;H'}{\Delta;H}
    }{cons-ignore-memstate-eq}
    %
    \judgbox{n\vdash\xlangstateeq{H}{H}}{,,The $\mmlAs$ heap $\src{H}$ agrees with $\mmlAt$ one $\trg{H}$ over $n$ entries from the\\beginning. $\src{L}$ contains locations introduced by the backtranslation\\wrapper.''}\\
    %
...
\begin{lemma}[Very Beginning of Backtranslation Correctness]\label{lem:exprbacktranslation:vbeg:wrapper:correct}
  ...
  \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:exect} $\exect{\Omega}{e}{\Omega'}{e'}{Call\ \comptoctx\ main\ 0}$
  ...
  \begin{goals}
  \item\label[goal]{goal:exprbacktranslation:vbeg:wrapper:correct:execs} $\exec{\src{\Omega}}{\src{call\ main\ 0}}{\src{\commlib;\library;\hole{\cdot}^{main}_{\ctxtocomp},\hole{\cdot};\memstate}}{\src{e_{main}}}{\src{\trace}}$\MK{use step index (technicality)}
  \item\label[goal]{goal:exprbacktranslation:vbeg:wrapper:correct:backstateeq} $\xlangbackstateeq[\delta;\src{L}]{\Omega'}{\src{\commlib;\library;\hole{\cdot}^{main}_{\ctxtocomp};\memstate}}$
  \item\label[goal]{goal:exprbacktranslation:vbeg:wrapper:correct:eventeq} $\src{\trace}\thexlangtraceeq[\delta;\src{X}]\trg{Call\ \comptoctx\ main\ 0}$
  \end{goals}
\end{lemma}
\begin{proof}
  Choose $\src{\event}=\src{Call\ \comptoctx\ main\ 0}$. Obviously, $\src{main}\in\operatorname{dom}\left\{\src{main}\mapsto\src{42;e_{main}}\right\}$.
  For \Cref{goal:exprbacktranslation:vbeg:wrapper:correct:execs}, use \Cref{tr:w-e-ectx-call-value,tr:w-e-ectx,tr:w-e-let-x}.
  Invert \Cref{asm:exprbacktranslation:vbeg:wrapper:correct:exect}, \Cref{goal:exprbacktranslation:vbeg:wrapper:correct:backstateeq,goal:exprbacktranslation:vbeg:wrapper:correct:eventeq} follow by reflexivity.
\end{proof}
