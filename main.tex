...
\myfig{
  ...
  & \mid \src{let\ x=new\ e_{1}\ in\ e_{2}} \mid \src{delete\ x} \mid \src{return\ e} \mid \src{call\ x\ e} \\
  & \mid \src{ifz\ e_1\ then\ e_2\ else\ e_3} \mid \src{abort()} \hspace{0.5cm} \text{where } \src{\oplus}\in\{\src{+},\src{-},\src{\times},\src{<}\}\\
  %
  ...
  \mi{Eval. Ctx.}~\src{K}\bnfdef &\ \src{\hole{\cdot}}\mid \src{K\oplus e}\mid \src{v\oplus K}\mid \src{x[K]}\mid \src{let\ x=K\ in\ e}\\
                     ...
                     &\ \mid\src{ifz\ K\ then\ e_1\ else\ e_2} \mid \src{call\ x\ K}\mid \src{return\ K} \\
  %
  ...
  %
  \mi{Symbol}~\src{\asymbol} \bnfdef\ \src{K}\times\types_{\lambda}  \\
  \mi{Cont.\ Stack}~\src{\kontstack} \bnfdef&\ \src{\hole{\cdot}} \mid \src{K},\src{\kontstack} \hspace{0.5cm}
  \mi{Library}~\src{\library} \bnfdef\ \src{\hole{\cdot}} \mid \src{x}\mapsto\src{\asymbol},\src{\library}\\
  \mi{State}~\configs\ \bnfdef&\ \src{\library;\kontstack;H;\Delta} \hspace{0.5cm}
  %
  \mi{Programs}\ \src{prog\ \library} \\
  %
...
\myfig{
  ...
  \end{center}
}{w-ectx-ty}{Checking of $\mmlAs$ functions/evaluation contexts.}
...
\myfig{
  ...
  \begin{center}
    ...
    %
    \typerule{$\src{\library}$-cons}{
      \src{\library}\downarrow = \src{\Gamma}
    }{
      \src{foo\mapsto} (\src{K},\src{\type}_\lambda)\src{,\library}\downarrow = \src{foo\mapsto\type}_\lambda,\src{\Gamma}
    }{w-lib-proj-cons}
  \end{center}
}{w-gamma-of-symbols}{Extracting type annotations.}
...
\myfig{
  \begin{center}
    ...
    \typerule{t-$\src{\library}$-cons}{
      \src{\type^{(1)}}\ \operatorname{int} &
      \src{\type^{(2)}}\ \operatorname{int} &
      \src{\Gamma}\vdash\src{\library}\ \operatorname{ok} &
      \typechecks{\Gamma}{K}{\type^{(1)}\to\type^{(2)}}
    }{
      \src{\Gamma}\vdash\src{foo\mapsto (K,\type^{(1)}\to\type^{(2)}),\library}\ \operatorname{ok}
    }{w-t-lib-cons}

    \judgbox{\vdash\src{prog\ \library}\operatorname{ok}}{,,$\mmlAs$ program $\src{prog\ \library}$ typechecks.''}$\;$\\
  %
  \typerule{$t-\text{prog}$}{
    \src{main}\in\operatorname{dom} \src{\library} &
    \src{\library}\downarrow = \src{\Gamma_0} &
    \src{\Gamma_0}\vdash\src{\library}\ \operatorname{ok}
	}{
    \vdash\src{prog\ \library}\operatorname{ok}
	}{w-t-wprog}
  ...
  \typechecks{}{\library;\kontstack;H;\Delta\triangleright e}{\type}
	}{w-t-prog}
  \end{center}
...
\myfig{
  \begin{center}
    ...
    \typerule{lib-merge-cons}{
      ...
      \src{foo\mapsto \asymbol,\library}\equiv\src{\library_1}\cup\src{foo\mapsto \asymbol,\library_2}
    }{src-lib-merge-cons}
...
\myfig{
  ...
  \typerule{$e-\src{get}$}{
    ...
    \exprevals{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{x[n]}{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{v}{Get\ \loc\ n}
  }{w-e-get}
  %
  \typerule{$e-\src{set}$}{
    \src{H'} = \src{H}(\src{\loc+n}\mapsto \src{v})
  }{
    \exprevals{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{x[n]\leftarrow v}{\library;\kontstack;H';\Delta_1,x\mapsto\loc_\poison,\Delta_2}{v}{Set\ \loc\ n\ v}
  }{w-e-set}
  %
  ...
  \typerule{$e-\src{delete}$}{
    ...
    \exprevals{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poison,\Delta_2}{delete\ x}{\library;\kontstack;H;\Delta_1,x\mapsto\loc_\poisoned,\Delta_2}{0}{Dealloc\ \loc}
  }{w-e-delete}
  %
  \typerule{$e-\src{new}$}{
  ...
  \exprevals{\library;\kontstack;H;\Delta}{let\ x=new\ n\ in\ e}{\library;\kontstack;H';z\mapsto \loc_\poisonless,\Delta}{e[z/x]}{Alloc\ \loc\ n}
  }{w-e-let-new}
  %
...
\myfig{
  \begin{center}
    ...
    \typerule{$e-\text{ctx}-\src{stuck}$}{
      \exprevals{\Omega}{e}{\Omega'}{stuck}{\event}
    }{
      \execs[]{\Omega}{K[e]}{\Omega'}{stuck}{\event}
    }{w-e-ectx-stuck}
    %
    \typerule{$e-\text{ctx}-\src{call}$}{
      \src{\Omega}=\src{\library_1,foo\mapsto K_{foo},\library_2;\kontstack;H;\Delta}
    }{
      \execs[]{\Omega}{K[call\ foo\ \finalexprnoerr]}{\Omega}{(return\ K_{foo})[f]}{Call\ ?foo\ f}
    }{w-e-ectx-call}
    %
    \typerule{$e-\text{ctx}-\src{return}$}{
    }{
      \execs[]{\library;K,\kontstack;H;\Delta}{K'[return\ \finalexprnoerr]}{\library;\kontstack;H;\Delta}{K[f]}{Ret\ !\finalexprnoerr}
    }{w-e-ectx-ret}
  \end{center}
...
\myfig{
  ...
  \typerule{$e-\text{wprog}$}{
    \vdash\src{prog\ \library}\operatorname{ok} &
    \execs{\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\Omega}{\finalexpr}{\trace}
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\configs_1}{\finalexpr}{\trace}
  }{ws-e-prog}
  \end{center}
...
%    \judgbox{\tospecificevctxs{\trace} = \src{\mstrace}}{,,Project an $\mmlAs$ trace $\src{\trace}$ to $\src{\mstrace}$, ignoring component.''}
    $$
    \begin{array}{rcll}
      \tospecificevs[_{\delta_{MS}}]{Alloc\ \loc\ n} &\ = & \specificev{Alloc\ \loc\ n},&\text{where }\specificev{\loc}=\delta_{MS}(\src{\loc})\text{ and }\specificev{n}=\src{n} \\
      \tospecificevs[_{\delta_{MS}}]{Dealloc\ \loc} &\ = & \specificev{Dealloc\ \loc} &\text{where }\specificev{\loc}=\delta_{MS}(\src{\loc}) \\
      \tospecificevs[_{\delta_{MS}}]{Get\ \loc\ n} &\ = & \specificev{Use\ \loc\ n} &\text{where }\specificev{\loc}=\delta_{MS}(\src{\loc})\text{ and }\specificev{n}=\src{n} \\
      \tospecificevs[_{\delta_{MS}}]{Set\ \loc\ n} &\ = & \specificev{Use\ \loc\ n} &\text{where }\specificev{\loc}=\delta_{MS}(\src{\loc})\text{ and }\specificev{n}=\src{n} \\
      \tospecificevs[_{\delta_{MS}}]{Call\ ?n} &\ = & \specificev{\varepsilon} &\\
      \tospecificevs[_{\delta_{MS}}]{Ret\ !n} &\ = & \specificev{\varepsilon} &\\
      \tospecificevs[_{\delta_{MS}}]{\lightning} &\ = & \specificev{\lightning} &\\
    \end{array}
    $$
    ...
    \typerule{$\src{\mstrace}$-cons-$\theta$}{
      \tospecificevs[_{\delta_{MS}}]{\msevent}=\msevent[_0]&
      \tospecificevs[_{\delta_{MS}}]{\mstrace}=\mstrace[_0]&
    }{
      \tospecificevs[_{\delta_{MS}}]{\msevent\cdot\mstrace}=\msevent[_0]\cdot\mstrace[_0]
    }{src-cons-empty-map}
    \typerule{$\src{\mstrace}$-cons-$\theta$-ignore}{
      \tospecificevs[_{\delta_{MS}}]{\msevent}=\specificev{\varepsilon}&
      \tospecificevs[_{\delta_{MS}}]{\mstrace}=\mstrace[_0]&
    }{
      \tospecificevs[_{\delta_{MS}}]{\msevent\cdot\mstrace}=\mstrace[_0]
    }{src-cons-empty-map}
  \end{center}
...
As before, when doing structural induction over primitive steps (\Cref{fig:w-expr-eval}) we may encounter the $\src{\emptyevent}$, for which $\tospecificevs{\emptyevent}=\specificev{\emptyevent}$, which needs a ,,partner'' in the abstract events as defined in \Cref{fig:tms-monitor}: $\bm{\varepsilon}$.
...
\myfig{
  \begin{center}
    \judgbox{\delta_{\text{tms}}(\specificev{\loc})=\bm{\loc}}{,,A map from MS-specific locations $\specificev{\loc}$ to abstract memory locations $\bm{\loc}$.''}
...
We now define temporal and spatial memory safety in terms of traces on events $\msevent$.
...
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtbackdetail[\Game,\src{(x:\type_1)\to\type_2}]{\mstrace} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$ and\\an $\mmlAs$ expression from memory-specific $\mmlAt$ trace $\trg{\mstrace}$.''}$\;$\\
    \judgbox{\mmlAmmlAtbackdetail[\Game,\src{(x:\type_1)\to\type_2}]{\specificev{Call\ ?\finalexprnoerr}} = \Game',\trg{\ell},\src{e}}{,,Given a \varmapbacktransds $\Game$ and $\src{(x:\type_{1})\to\type_{2}}$,\\construct a \varmapbacktransds $\Game'$, $\trg{\ell}$,\\and an $\mmlAs$ expression from memory-specific $\mmlAt$\\event $\trg{\specificev{Call\ !\finalexprnoerr}}$.''}$\;$\\
    \judgbox{\mmlAmmlAtbackdetail[\Game,\trg{\ell},\src{e_0}]{\specificev{Ret\ !\finalexprnoerr}} = \Game',\src{e}}{,,Given a \varmapbacktransds $\Game$, $\trg{\ell}$, and $\src{e_{0}}$,\\construct a \varmapbacktransds $\Game'$ and an $\mmlAs$ expression from\\memory-specific $\mmlAt$ event $\trg{\specificev{Ret\ !\finalexprnoerr}}$.''}$\;$\\
    %
...
\myfig{
  \begin{center}
    \judgbox{\mmlAmmlAtback[\Game,\src{(x:\type_1)\to\type_2},\src{e_{0}},\trg{\ell}]{\mstrace} = \Game',\src{e_{wrap}},\src{e},\trg{\ell'}}{,,Given a \varmapbacktransds $\Game$, construct a \varmapbacktransds $\Game'$\\and two $\mmlAs$ expressions from\\memory-specific $\mmlAt$ ,,interfacing''-trace $\trg{\mstrace}$.''}$\;$\\
    ...
    \typerule{backtrans-subtop-level}{\vspace{0.33cm}
      ...
      \mmlAmmlAtbackdetail[\Game,\trg{\ell_0},\src{e_{INIT}}]{\specificev{Ret\ !\finalexprnoerr_{0}}} = \Game',\src{e_{wrapend}} &
      \mmlAmmlAtbackdetail[\Game',\src{(\_:\nat)\to\nat}]{\mstrace[_0]} = \Game'', \src{e_0^{(1)}} \\
      \mmlAmmlAtbackdetail[\Game'',\src{(x:\type_1)\to\type_2}]{\specificev{Call\ ?\finalexprnoerr_1}} = \Game''',\trg{\loc},\src{e_0^{(2)}} &
      \src{e_0^{(1p)}} = \plugs{0}{e_0^{(1)}} &
      \src{e} = \src{e_0^{(1p)};e_0^{(2)}}
    }{
      \mmlAmmlAtback[\Game,\src{(x:\type_1)\to\type_2},\src{e_{INIT}},\trg{\ell_0}]{\mstrace} = \Game''', \src{e_{wrapend}}, \src{e}, \trg{\ell}
    ...
    \typerule{backtrans-top-level}{\vspace{0.33cm}
      ...
      \mmlAmmlAtbackdetail[\Game,\src{(\_:\nat)\to\nat}]{\specificev{Call\ ?0}}=\Game_{INIT},\trg{\ell_{INIT}},\src{e_{INIT}}\\\vspace{0.33cm}
      \mmlAmmlAtbackdetail[\Game_{INIT},\src{(x:\type_{1})\to\type_{2}},\src{e_{INIT}},\trg{\ell_{INIT}}]{\specificev{Ret\ !0}\cdot\mstrace[_0]\cdot\specificev{Call\ ?\finalexprnoerr_0}} = \Game',\src{e_{wrapend}^{(0)}},\src{e_{0}'},\trg{\ell}\\\vspace{0.33cm}
      \mmlAmmlAtbackdetail[\Game',\src{(\_:\nat)\to\nat},\src{e_{0}'},\trg{\ell}]{\specificev{Ret\ !\finalexprnoerr_{p}}\cdot\mstrace[_1]\cdot\specificev{Call\ ?\finalexprnoerr_1}} = \Game'',\src{e_{wrapend}^{(1)}},\src{e_{1}'},\trg{\ell_{END}}\\
      \mmlAmmlAtbackdetail[\Game'',\trg{\ell_{END}},\src{e_1'}]{\specificev{Ret\ !0}} = \Game''', \src{e_1^{(H)}}\\
      \src{e_0^{(0)}} = \plugs{0}{e_{wrapend}^{(0)}} &
      \src{e_0} = \src{e_0^{(0)};e_{wrapend}^{(1)}} \\
      \src{e_1} = \plugs{0}{e_1'} \\
    }{
