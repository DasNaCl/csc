...
\begin{incompleteproof}
  ...
  \begin{description}
    ...
    \item\label[ass]{asm:backtranscorr:call:origexect} $\exect{\Omega}{e}{\Omega'}{\finalexprnoerr}{Call\ \ctxtocomp\ foo\ \interfacevalue\cdot\trace}$
      ...
      Since the call event is $\ctxtocomp$, we can conclude\MK{technical lemma} that $\trg{foo}\in\mmlAmmlAtcomp{\commlib}$ an, thus, $\trg{foo}\in\mmlAmmlAtcomp{\library_{comp}}$.
      Consequently, $\mmlAmmlAtback[\Game_{z}]{foo}\in\src{\library_{comp}}$\MK{technical lemma}.

          {\color{red}use fact that the call is in the component. somehow go from approx to lolli. do expr. correctness}
...
%     Using that, by \Cref{tr:w-e-let-new}, we know $\execs[]{\Omega}{e}{\commlib;\library;\kontstack;H';z\mapsto\loc_{\poisonless},\Delta}{e_{b}}{Alloc\ \loc\ 42}$, where $\src{e_{b}}=\src{call\ }\mmlAmmlAtback[\Game_{z}]{foo}\ \mmlAmmlAtbackv[\Game_{z}]{\interfacevalue}\ \src{; delete\ z;e_{a}}$.
