...
\myfig{
  \begin{center}
    \begin{tikzpicture}[state/.style={minimum height=0.6cm}]
      ...
      \path[->,very thick] (mmlAtmemsafe) edge[sloped,above] node {\footnotesize CCT+MS} (asm);
      \path[->,dashed,very thick] (mmlAs) edge[sloped,below,bend right=33] node {CCT+MS} (asm);
...
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\msevent \bnfdef&\ \specificev{\emptyevent}\mid \specificev{Alloc\ \loc\ n} \mid \specificev{Dealloc\ \loc} \mid \specificev{Use\ \loc\ n} \mid \specificev{\lightning} |\ \specificev{Branch\ n} \mid \specificev{Binop\ n} \\
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    ...
    \typerule{TMS-Branch-Authentic}{
      ...
      \tmstraceagree{\specificev{Branch\ n}}{\bm{\varepsilon}}
    }{tms-branch-auth}
    %
    \typerule{TMS-Binop-Authentic}{
    }{
      \tmstraceagree{\specificev{Binop\ n}}{\bm{\varepsilon}}
    }{tms-binop-auth}
    %
...
\myfig{
  \begin{center}
    ...
    \typerule{SMS-Branch-Authentic}{
      ...
      \smstraceagree{\specificev{Branch\ n}}{\bm{\varepsilon}}
    }{sms-branch-auth}
    %
    \typerule{SMS-Binop-Authentic}{
    }{
      \smstraceagree{\specificev{Binop\ n}}{\bm{\varepsilon}}
    }{sms-binop-auth}
    %
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      \mi{Abstract\ Store}~\smsmonitor =&\ \specificev{L}\times\nat \\
      \mi{Abstract\ Events}~\absevent \bnfdef&\ \bm{\varepsilon}\mid \textbf{Alloc} \mid \textbf{Dealloc} \mid \textbf{Use} \mid \abstermination \mid \textbf{Branch}\\
      &\mid \textbf{Binop}\\
    \end{aligned}
  \end{gather*}
}{cct-monitor}{\gls{cct} Monitor.}
...
\myfig{
  \begin{center}
    \judgbox{\monitorcheck[]{\cctmonitor}{\cctmonitor[']}{\absevent}}{,,Given abstract state $\cctmonitor$, step to abstract state $\cctmonitor'$\\ emitting an abstract event $\absevent$.''}
    \judgbox{\monitorcheck{\cctmonitor}{\cctmonitor[']}{\abstrace}}{,,Reflexive-Transitive closure of above judgement.\\$\abstrace$ is a list of abstract events.''}
    %
    \typerule{cct-None}{
    }{
      \monitorcheck[]{\cctmonitor}{\cctmonitor}{\bm{\varepsilon}}
    }{cct-none}
    %
    \typerule{cct-Abort}{
    }{
      \monitorcheck[]{\cctmonitor}{\cctmonitor}{\bm{\lightning}}
    }{cct-abort}
    %
    \typerule{cct-Refl}{
    }{
      \monitorcheck{\cctmonitor}{\cctmonitor}{\hole{\cdot}}
    }{cct-refl}
    %
    \typerule{cct-Trans}{
      \monitorcheck[]{\cctmonitor}{\cctmonitor[']}{\absevent}&
      \monitorcheck{\cctmonitor[']}{\cctmonitor['']}{\abstrace}
    }{
      \monitorcheck{\cctmonitor}{\cctmonitor['']}{\absevent\cdot\abstrace}
    }{cct-trans}
  \end{center}
}{cct-step}{Steps of \gls{cct} Monitor.}

\myfig{
  \begin{center}
    \judgbox{\ccttraceagree{\msevent}{\absevent}}{,,Memory-Safety specific event $\msevent$ describes same action as $\absevent$.''}
    \judgbox{\ccttraceagree[^*]{\mstrace}{\abstrace}}{,,Memory-Safety specific trace $\mstrace$ describes same trace as $\abstrace$.''}
    %
    \typerule{CCT-Alloc-Authentic}{
    }{
      \ccttraceagree{\specificev{Alloc\ \loc\ n}}{\textbf{Alloc}}
    }{cct-alloc-auth}
    %
    \typerule{CCT-Dealloc-Authentic}{
    }{
      \ccttraceagree{\specificev{Dealloc\ \loc}}{\textbf{Dealloc}}
    }{cct-dealloc-auth}
    %
    \typerule{CCT-Use-Authentic}{
    }{
      \ccttraceagree{\specificev{Use\ \loc\ n}}{\textbf{Use}}
    }{cct-use-auth}
    %
    \typerule{CCT-Abort-Authentic}{
    }{
      \ccttraceagree{\specificev{\lightning}}{\bm{\lightning}}
    }{cct-abort-auth}
    %
    \typerule{CCT-None-Authentic}{
    }{
      \ccttraceagree{\specificev{\varepsilon}}{\bm{\varepsilon}}
    }{cct-none-auth}
    %
    \typerule{CCT-Branch-Authentic}{
    }{
      \ccttraceagree{\specificev{Branch\ n}}{\textbf{Branch}}
    }{cct-branch-auth}
    %
    \typerule{CCT-Binop-Authentic}{
    }{
      \ccttraceagree{\specificev{Binop\ n}}{\textbf{Binop}}
    }{cct-binop-auth}
    %
    \typerule{CCT-Refl-Authentic}{
    }{
      \ccttraceagree[^*]{\hole{\cdot}}{\hole{\cdot}}
    }{cct-refl-auth}
    %
    \typerule{CCT-Trans-Authentic}{
      \ccttraceagree{\msevent}{\absevent} &
      \ccttraceagree[^*]{\mstrace}{\abstrace}
    }{
      \ccttraceagree[^*]{\msevent\cdot\mstrace}{\absevent\cdot\abstrace}
    }{cct-trans-auth}
  \end{center}
}{cct-trace-agreement}{Trace Agreement.}
...
\myfig{
  \begin{center}
    \judgbox{\delta_{MS}(\trg{\loc})=\specificev{\loc}}{,,A map from $\mmlAt$ memory locations $\trg{\loc}$ to MS-specific locations $\specificev{\loc}$.''}
    \judgbox{\tospecificevt[_{\delta_{MS}}]{\event} = \msevent}{,,Project an $\mmlAt$ event to MS-specific events.''}
    %
    ...
    \typerule{t-filter-comp-call}{
      ...
      \tospecificevt[_{\delta_{MS}}]{Call\ \comm\ foo\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{t-filter-comp-call}
    %
    \typerule{t-filter-comp-ret}{
    }{
      \tospecificevt[_{\delta_{MS}}]{Ret\ \comm\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{t-filter-comp-ret}
    %
    ...
    %
    \judgbox{\tospecificevt[^*_{\delta_{MS}}]{\trace} = \mstrace}{,,Project an $\mmlAt$ trace to MS-specific traces.''}
    %
  ...
  \end{center}
}{wt-events-project-to-specific}{Projection of $\mmlAt$ events to \gls*{ms}-specific events.}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:exprbacktranslation:vbeg:wrapper:correct:execs} $\execsn[^{2}]{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0[0/x]}{\commlib;\library;(\hole{\cdot},main),\hole{\cdot};\ctx;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{e_{main}[0/x]}{Start}$
  \end{passumptions}
...
\subsection{Robustly Preserving $\CCT$}
...
The underlying assumption the programmer does in any other language we've seen before, that is, $\mmlAs$, $\mmlAt$, or $\mmlAtms$, is that they are writing $\CCT$ code.
The upcoming compilation pass makes sure this assumption is actually true.

\myfig{
	\vspace{-1em}
  \begin{gather*}
  \begin{aligned}
  \mi{Final\ Result}\ \obj{\finalexprnoerr} \bnfdef&\ \obj{\valueexpr} \mid \obj{x} \hspace{0.5cm}
  \mi{May\ be\ a\ Result}\ \obj{\finalexpr} \bnfdef\ \obj{\finalexprnoerr} \mid \obj{stuck} \\
  %
  \mi{Expressions}~\obj{e} \bnfdef&\ \obj{\finalexpr} \mid \obj{e_{1} \oplus e_{2}} \mid \obj{e_{1} \widehat{\oplus} e_{2}} \mid \obj{load\ x\ e} \mid \obj{\widehat{load}\ x\ e} \\
  & \mid \obj{let\ x = e_{1}\ in\ e_{2}} \mid \obj{write\ x\ e_{1}\ e_{2}} \mid \obj{\widehat{write}\ x\ e_{1}\ e_{2}} \\
  & \mid \obj{let\ x = new\ e_{1}\ in\ e_{2}} \mid \obj{delete\ x} \\
  & \mid \obj{return\ e} \mid \obj{call\ foo\ e} \mid \obj{ifz\ e_{1}\ then\ e_{2}\ else\ e_{3}} \\
  & \mid \obj{\widehat{ifz}\ e_1\ then\ e_2\ else\ e_3} \mid \obj{abort()} \mid \obj{x\ is\ \poisoned} \\
  & \mid \obj{\langle e_{1},e_{2}\rangle} \mid \obj{\pi_{1} e} \mid \obj{\pi_{2} e} \mid \obj{e\ has\ \type} \\
   &\text{where } \obj{\oplus}\in\{\obj{+},\obj{-},\obj{\times},\obj{\widehat{\times}},\obj{<},\obj{\widehat{<}},\obj{/},\obj{\widehat{/}}\}\\
  %
  ...
  \mi{Eval. Ctx.}~\obj{K}\bnfdef &\ \obj{\hole{\cdot}}\mid \obj{K\oplus e}\mid \obj{v\oplus K}\mid \obj{load\ x\ K}\mid \obj{let\ x=K\ in\ e}\\
                     &\ \mid\obj{write\ x\ K\ e}\mid \obj{write\ x\ v\ K}\mid \obj{let\ x=new\ K\ in\ e} \\
                     ...
                     &\ \mid\obj{ifz\ K\ then\ e_{1}\ else\ e_{2}} \mid \obj{\widehat{ifz}\ K\ then\ e_{1}\ else\ e_{2}}\\
                     &\ \mid\obj{K\widehat{\oplus}e} \mid \obj{\valueexpr \widehat{\oplus} K} \mid \obj{\widehat{load}\ x\ K} \mid \obj{\widehat{write}\ x\ K\ e} \mid \obj{\widehat{write}\ x\ \valueexpr\ K} \\
  %
  ...
  \mi{State}~\obj{\Omega}\ \bnfdef\ \obj{\cfstate;\sandboxtag;\memstate}\\
...
\myfig{
  ...
  \mi{Events}~\obj{\event} \bnfdef&\ \obj{\emptyevent} \mid \obj{\event_{b};\sandboxtag}
  \end{aligned}
  \end{gather*}
}{to-events}{Events of $\mmlAo$.}

\myfig{
  \judgbox{\exprevalo{\Omega}{e}{\Omega'}{e'}{\event}}{,,Expression $\obj{e}$ evaluates under Omegauration $\Omega$ to $\obj{e'}$ and\\new Omegauration $\obj{\Omega'}$, emitting event $\obj{\event}$.''}
  %
  \typerule{$e-\obj{\oplus}-$noleak}{
    \obj{n_1}\oplus\obj{n_2}=\obj{n_3} &
    \obj{\oplus}\notin\{\obj{*},\obj{/},\obj{<}\}
  }{
    \exprevalo{\Omega}{n_1\oplus n_2}{\Omega}{n_3}{\emptyevent}
  }{to-e-oplus-noleak}
  %
  \typerule{$e-\obj{\oplus}-$leak}{
    ...
    \exprevalo{\Omega}{n_1\oplus n_2}{\Omega}{n_3}{Binop\ n_1\ n_2}
  }{to-e-oplus-leak}
  %
  \typerule{$e-\obj{get}-\in-$leak}{
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{load\ x\ n}{\cfstate;\sandboxtag';\memstate}{H^{\sandboxtag}(\loc+n)}{Get\ \loc\ n;\sandboxtag}
  }{to-e-get-in-leak}
  %
  \typerule{$e-\obj{get}-\notin-$leak}{
    \obj{\memstate}=\obj{H^{\ctx};H^{\comp};\Delta_1,x\mapsto(\loc;\sandboxtag;\poison;m),\Delta_2} &
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{load\ x\ n}{\cfstate;\sandboxtag';\memstate}{1729}{Get\ \loc\ n;\sandboxtag}
  }{to-e-get-notin-leak}
  %
  \typerule{$e-\obj{get}-\in-$noleak}{
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{\widehat{load}\ x\ n}{\cfstate;\sandboxtag';\memstate}{H^{\sandboxtag}(\loc+n)}{\widehat{Get}\ \loc\ n;\sandboxtag}
  }{to-e-get-in-noleak}
  %
  \typerule{$e-\obj{get}-\notin-$noleak}{
    \obj{\memstate}=\obj{H^{\ctx};H^{\comp};\Delta_1,x\mapsto(\loc;\sandboxtag;\poison;m),\Delta_2} &
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{\widehat{load}\ x\ n}{\cfstate;\sandboxtag';\memstate}{1729}{\widehat{Get}\ \loc\ n;\sandboxtag}
  ...
  \typerule{$e-\obj{set}-\ctx-$leak}{
    ...
    \exprevalo{\cfstate;\ctx;\memstate}{write\ x\ n\ v}{\cfstate;\ctx;\memstate'}{v}{Set\ \loc\ n\ v;\ctx}
  }{to-e-set-ctx-leak}
  %
  \typerule{$e-\obj{set}-\comp-$leak}{
    ...
    \exprevalo{\cfstate;\comp;\memstate}{write\ x\ n\ v}{\cfstate;\comp;\memstate'}{v}{Set\ \loc\ n\ v;\comp}
  }{to-e-set-comp-leak}
  %
  \typerule{$e-\obj{set}-\ctx-$noleak}{
    ...
    \exprevalo{\cfstate;\ctx;\memstate}{\widehat{write}\ x\ n\ v}{\cfstate;\ctx;\memstate'}{v}{\widehat{Set}\ \loc\ n\ v;\ctx}
  }{to-e-set-ctx-noleak}
  %
  \typerule{$e-\obj{set}-\comp-$noleak}{
    ...
    \exprevalo{\cfstate;\comp;\memstate}{\widehat{write}\ x\ n\ v}{\cfstate;\comp;\memstate'}{v}{\widehat{Set}\ \loc\ n\ v;\comp}
  }{to-e-set-comp-noleak}
  %
  \typerule{$e-\obj{let}-\obj{\finalexprnoerr}$}{
  }{
    \exprevalo{\Omega}{let\ x=\finalexprnoerr\ in\ e}{\Omega}{e[\finalexprnoerr/x]}{\emptyevent}
  }{to-e-let-f}
  %
  \typerule{$e-\obj{delete}$}{
    ...
    \exprevalo{\cfstate;\sandboxtag;\memstate}{delete\ x}{\cfstate;\sandboxtag;\memstate'}{0}{Dealloc\ \loc;\sandboxtag}
  }{to-e-delete}
  %
}{to-expr-eval}{Evaluation of $\mmlAo$ expressions.}
...
\myfig{
  ...
  \typerule{$e-\obj{new}-ctx$}{
    ...
    \exprevalo{\cfstate;\ctx;H^{\ctx};H^{\comp};\Delta}{let\ x=new\ n\ in\ e}{\cfstate;\ctx;\memstate}{e[z/x]}{Alloc\ \loc\ n;\ctx}
  }{to-e-let-new-ctx}
  %
  \typerule{$e-\obj{new}-comp$}{
    ...
    \exprevalo{\cfstate;\comp;H^{\ctx};H^{\comp};\Delta}{let\ x=new\ n\ in\ e}{\cfstate;\comp;\memstate}{e[z/x]}{Alloc\ \loc\ n;\comp}
  }{to-e-let-new-comp}
  %
  \typerule{$e-\obj{ifz}-$true-noleak}{
  }{
    \exprevalo{\cfstate;\sandboxtag;\memstate}{ifz\ 0\ then\ e_1\ else\ e_2}{\cfstate;\sandboxtag;\memstate}{e_1}{Branch\ 0;\sandboxtag}
  }{to-e-if-true-noleak}
  %
  \typerule{$e-\obj{ifz}-$false-noleak}{
  }{
    \exprevalo{\cfstate;\sandboxtag;\memstate}{ifz\ S(n)\ then\ e_1\ else\ e_2}{\cfstate;\sandboxtag;\memstate}{e_2}{Branch\ S(n);\sandboxtag}
  }{to-e-if-false-noleak}
  %
  \typerule{$e-\obj{ifz}-$true-leak}{
  }{
    \exprevalo{\cfstate;\sandboxtag;\memstate}{\widehat{ifz}\ 0\ then\ e_1\ else\ e_2}{\cfstate;\sandboxtag;\memstate}{e_1}{\emptyevent;\sandboxtag}
  }{to-e-if-true-leak}
  %
  \typerule{$e-\obj{ifz}-$false-leak}{
  }{
    \exprevalo{\cfstate;\sandboxtag;\memstate}{\widehat{ifz}\ S(n)\ then\ e_1\ else\ e_2}{\cfstate;\sandboxtag;\memstate}{e_2}{\emptyevent;\sandboxtag}
  }{to-e-if-false-leak}
  %
  \typerule{$e-\obj{abort}$}{
  }{
    \exprevalo{\Omega}{abort()}{\Omega}{stuck}{\lightning}
  }{to-e-abort}
  \typerule{$e-\obj{\pi_1}$}{
  }{
    \exprevalo{\Omega}{\pi_1\langle n_1;n_2\rangle}{\Omega}{n_1}{\emptyevent}
  }{to-e-proj1}
  %
  \typerule{$e-\obj{\pi_2}$}{
  }{
    \exprevalo{\Omega}{\pi_2\langle n_1;n_2\rangle}{\Omega}{n_2}{\emptyevent}
  }{to-e-proj2}
  %
  \typerule{$e-\obj{x\ is\ \poisoned}-$yes}{
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{x\ is\ \poisoned}{\cfstate;\sandboxtag';\memstate}{0}{\emptyevent}
  ...
  \typerule{$e-\obj{x\ is\ \poisoned}-$no}{
    ...
    \exprevalo{\cfstate;\sandboxtag';\memstate}{x\ is\ \poisoned}{\cfstate;\sandboxtag';\memstate}{1}{\emptyevent}
  }{to-e-x-is-poisoned-no}
  %
  \typerule{$e-\obj{n\ has\ \nat}$}{
  }{
    \exprevalo{\Omega}{n\ has\ \nat}{\Omega}{0}{\emptyevent}
  }{to-e-n-hasnat}
  %
  \typerule{$e-pair-\obj{has\ \nat}$}{
  }{
    \exprevalo{\Omega}{\langle n_1,n_2\rangle\ has\ \nat}{\Omega}{1}{\emptyevent}
  }{to-e-pair-hasnat}
  %
  \typerule{$e-\obj{x\ has\ \nat}$}{
  }{
    \exprevalo{\Omega}{x\ has\ \nat}{\Omega}{1}{\emptyevent}
  }{to-e-x-hasnat}
  %
  \typerule{$e-\obj{n\ has\ \nat\times\nat}$}{
  }{
...
}{to-expr-eval-continued}{Evaluation of $\mmlAo$ expressions, continued.}

% \begin{definition}[$\CRhash$ security]
%   Let $\obj{\zeta}$ be a list of public identifiers and states $\obj{\Omega_{1}},\obj{\Omega_{1}'}$ such that $\obj{\zeta}\vdash\obj{\Omega_{1}}\approx\obj{\Omega_{2}}$.
%   Given $\execo{\Omega_{1}}{e_{1}}{\Omega_{2}}{\finalexprnoerr}{c,\trace[_{A}]}$ and $\execo{\Omega_{1}'}{e_{1}}{\Omega_{2}'}{\finalexprnoerr}{c',\trace[_{B}]}$ we say $e_{1}$ is $\CRhash$-secure with respect to $\obj{\zeta}$ iff $(\obj{c},\operatorname{mems-and-branches}(\obj{\trace_{A}}))=(\obj{c'},\operatorname{mems-and-branches}(\obj{\trace_{B}}))$
% \end{definition}
%
\myfig{
  \begin{center}
    \judgbox{\delta_{MS}(\obj{\loc})=\specificev{\loc}}{,,A map from $\mmlAo$ memory locations $\obj{\loc}$ to MS-specific locations $\specificev{\loc}$.''}
    \judgbox{\tospecificevo[_{\delta_{MS}}]{\event} = \msevent}{,,Project an $\mmlAo$ event to MS-specific events.''}
    %
    \typerule{o-filter-context}{
      \obj{\event_b} \not= \obj{\lightning}
    }{
      \tospecificevo[_{\delta_{MS}}]{\event_b;\ctx} = \specificev{\emptyevent}
    }{o-filter-context}
    %
    \typerule{o-filter-comp-start}{
    }{
      \tospecificevo[_{\delta_{MS}}]{Start;\comp} = \specificev{\emptyevent}
    }{o-filter-comp-start}
    %
    \typerule{o-filter-comp-alloc}{
      ...
      \tospecificevo[_{\delta_{MS}}]{Alloc\ \loc\ n;\comp} = \specificev{Alloc\ \loc\ n}
    }{o-filter-comp-alloc}
    %
    \typerule{o-filter-comp-dealloc}{
      \delta_{MS}(\obj{\loc})=\specificev{\loc}
    }{
      \tospecificevo[_{\delta_{MS}}]{Dealloc\ \loc;\comp} = \specificev{Dealloc\ \loc}
    ...
    \typerule{o-filter-comp-get-noleak}{
      ...
      \tospecificevo[_{\delta_{MS}}]{\widehat{Get}\ \loc\ n;\comp} = \specificev{Use\ \loc\ n}
    ...
    \typerule{o-filter-comp-set-noleak}{
      ...
      \tospecificevo[_{\delta_{MS}}]{\widehat{Set}\ \loc\ n\ \valueexpr;\comp} = \specificev{Use\ \loc\ n}
    ...
    \typerule{o-filter-comp-get-leak}{
      ...
      \tospecificevo[_{\delta_{MS}}]{Get\ \loc\ n;\comp} = \specificev{Use\ \loc\ n}
    ...
    \typerule{o-filter-comp-set-leak}{
      ...
      \tospecificevo[_{\delta_{MS}}]{Set\ \loc\ n\ \valueexpr;\comp} = \specificev{Use\ \loc\ n}
    }{o-filter-comp-set-leak}
    %
    \typerule{o-filter-comp-call}{
    }{
      \tospecificevo[_{\delta_{MS}}]{Call\ \comm\ foo\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{o-filter-comp-call}
    %
    \typerule{o-filter-comp-ret}{
    }{
      \tospecificevo[_{\delta_{MS}}]{Ret\ \comm\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{o-filter-comp-ret}
    %
    \typerule{o-filter-comp-binop}{
    }{
      \tospecificevo[_{\delta_{MS}}]{Binop\ n_1\ n_2;\comp} = \specificev{\emptyevent}
    }{o-filter-comp-binop}
    %
    \typerule{o-filter-comp-branch}{
    }{
      \tospecificevo[_{\delta_{MS}}]{Branch\ n;\comp} = \specificev{\emptyevent}
    }{o-filter-comp-branch}
    %
    \typerule{o-filter-abort}{
    }{
      \tospecificevo[_{\delta_{MS}}]{\lightning;\sandboxtag} = \specificev{\lightning}
    }{o-filter-abort}
    %
    \judgbox{\tospecificevo[^*_{\delta_{MS}}]{\trace} = \mstrace}{,,Project an $\mmlAo$ trace to MS-specific traces.''}
    %
  ...
  \end{center}
}{wo-ms-events-project-to-specific}{Projection of $\mmlAo$ events to \gls*{ms}-specific events.}

\myfig{
  \begin{center}
    \judgbox{\delta_{CCT}(\obj{\loc})=\specificev{\loc}}{,,A map from $\mmlAo$ memory locations $\obj{\loc}$ to CCT-specific locations $\specificev{\loc}$.''}
    \judgbox{\tospecificevo[_{\delta_{CCT}}]{\event} = \cctevent}{,,Project an $\mmlAo$ event to CCT-specific events.''}
    %
    \typerule{o-cct-filter-context}{
      \obj{\event_b} \not= \obj{\lightning}
    }{
      \tospecificevo[_{\delta_{CCT}}]{\event_b;\ctx} = \specificev{\emptyevent}
    }{o-cct-filter-context}
    %
    \typerule{o-cct-filter-comp-start}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{Start;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-start}
    %
    \typerule{o-cct-filter-comp-alloc}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc} &
      \specificev{n}=\obj{n}
    }{
      \tospecificevo[_{\delta_{CCT}}]{Alloc\ \loc\ n;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-alloc}
    %
    \typerule{o-cct-filter-comp-dealloc}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc}
    }{
      \tospecificevo[_{\delta_{CCT}}]{Dealloc\ \loc;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-dealloc}
    %
    \typerule{o-cct-filter-comp-get-noleak}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc} &
      \specificev{n}=\obj{n}
    }{
      \tospecificevo[_{\delta_{CCT}}]{\widehat{Get}\ \loc\ n;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-get-noleak}
    %
    \typerule{o-cct-filter-comp-set-noleak}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc} &
      \specificev{n}=\obj{n}
    }{
      \tospecificevo[_{\delta_{CCT}}]{\widehat{Set}\ \loc\ n\ \valueexpr;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-set-noleak}
    %
    \typerule{o-cct-filter-comp-get-leak}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc} &
      \specificev{n}=\obj{n}
    }{
      \tospecificevo[_{\delta_{CCT}}]{Get\ \loc\ n;\comp} = \specificev{Use\ \loc\ n}
    }{o-cct-filter-comp-get-leak}
    %
    \typerule{o-cct-filter-comp-set-leak}{
      \delta_{CCT}(\obj{\loc})=\specificev{\loc} &
      \specificev{n}=\obj{n}
    }{
      \tospecificevo[_{\delta_{CCT}}]{Set\ \loc\ n\ \valueexpr;\comp} = \specificev{Use\ \loc\ n}
    }{o-cct-filter-comp-set-leak}
    %
    \typerule{o-cct-filter-comp-call}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{Call\ \comm\ foo\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-call}
    %
    \typerule{o-cct-filter-comp-ret}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{Ret\ \comm\ \interfacevalue;\comp} = \specificev{\emptyevent}
    }{o-cct-filter-comp-ret}
    %
    \typerule{o-cct-filter-comp-binop}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{Binop\ n_1\ n_2;\comp} = \specificev{Binop\ n_1\ n_2}
    }{o-cct-filter-comp-binop}
    %
    \typerule{o-cct-filter-comp-branch}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{Branch\ n;\comp} = \specificev{Branch\ n}
    }{o-cct-filter-comp-branch}
    %
    \typerule{o-cct-filter-abort}{
    }{
      \tospecificevo[_{\delta_{CCT}}]{\lightning;\sandboxtag} = \specificev{\lightning}
    }{o-cct-filter-abort}
    %
    \judgbox{\tospecificevo[^*_{\delta_{CCT}}]{\trace} = \ccttrace}{,,Project an $\mmlAo$ trace to CCT-specific traces.''}
    %
    \typerule{o-cct-filter-empty}{
    }{
      \tospecificevo[^*_{\delta_{CCT}}]{\hole{\cdot}} = \specificev{\hole{\cdot}}
    }{o-cct-filter-empty}
    %
    \typerule{o-cct-filter-cons-relevant}{
      \tospecificevo[_{\delta_{CCT}}]{\event} = \specificev{\event} &
      \tospecificevo[^*_{\delta_{CCT}}]{\trace} = \ccttrace &
      \specificev{\event} \not= \specificev{\emptyevent}
    }{
      \tospecificevo[^*_{\delta_{CCT}}]{\event\cdot\trace} = \specificev{\event}\cdot\ccttrace
    }{o-cct-filter-cons-relevant}
    %
    \typerule{o-cct-filter-cons-relevant}{
      \tospecificevo[_{\delta_{CCT}}]{\event} = \specificev{\emptyevent} &
      \tospecificevo[^*_{\delta_{CCT}}]{\trace} = \ccttrace
    }{
      \tospecificevo[^*_{\delta_{CCT}}]{\event\cdot\trace} = \ccttrace
    }{o-cct-filter-cons}
  \end{center}
}{wo-cct-events-project-to-specific}{Projection of $\mmlAo$ events to \gls*{cct}-specific events.}

\myfig{
  \begin{center}
    \judgbox{\delta(\irl{\loc})=\obj{\loc}}{,,Map from $\mmlAtms$ locations $\irl{\loc}$ to $\mmlAtcct$ locations $\obj{\loc}$.''} \\
    \judgbox{\xlangtraceeqc{\trace}{\trace}}{,,The $\mmlAtms$ trace $\irl{\trace}$ describes the same actions as $\mmlAtcct$ trace $\obj{\trace}$.''} \\
    \typerule{cct-empty-trace-eq}{
    }{
      \xlangtraceeqc{\hole{\cdot}}{\hole{\cdot}}
    }{cct-empty-trace-eq}
    %
    \typerule{cct-cons-trace-eq}{
      \irl{\sandboxtag}=\obj{\sandboxtag} &
      \xlangeventeqc{\event_{b}}{\event_{b}} &
      \xlangtraceeqc{\trace}{\trace}
    }{
      \xlangtraceeqc{\event_{b};\sandboxtag\cdot\trace}{\event_{b};\sandboxtag\cdot\trace}
    }{cct-cons-trace-eq}
    %
    ...
    %
    \typerule{cct-start-event-eq}{
    }{
      \xlangeventeqc{Start}{Start}
    }{cct-start-event-eq}
    %
    \typerule{cct-end-event-eq}{
      \mmlAtmsmmlAtcctcomp{\valueexpr} = \obj{\valueexpr}
    }{
      \xlangeventeqc{End\ \valueexpr}{End\ \valueexpr}
    }{cct-end-event-eq}
    %
    \typerule{cct-alloc-event-eq}{
      \mmlAtmsmmlAtcctcomp{n}=\obj{n} &
      \delta(\irl{\loc})=\obj{\loc}
    }{
      \xlangeventeqc{Alloc\ \loc\ n}{Alloc\ \loc\ n}
    }{cct-alloc-event-eq}
    %
    \typerule{cct-dealloc-event-eq}{
      \delta(\irl{\loc})=\obj{\loc}
    }{
      \xlangeventeqc{Dealloc\ \loc}{Dealloc\ \loc}
    }{cct-dealloc-event-eq}
    %
    \typerule{cct-get-event-eq-noleak}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \delta(\irl{\loc}) = \obj{\loc}
    }{
      \xlangeventeqc{Set\ \loc\ n\ \valueexpr}{\widehat{Get}\ \loc\ n}
    }{cct-get-event-eq-noleak}
    %
    \typerule{cct-get-event-eq-leak}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \delta(\irl{\loc}) = \obj{\loc}
    }{
      \xlangeventeqc{Get\ \loc\ n}{Get\ \loc\ n}
    }{cct-get-event-eq-leak}
    %
    \typerule{cct-set-event-eq-leak}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \mmlAtmsmmlAtcctcomp{\valueexpr} = \obj{\valueexpr} &
      \delta(\irl{\loc}) = \obj{\loc}
    }{
      \xlangeventeqc{Set\ \loc\ n\ \valueexpr}{Set\ \loc\ n\ \valueexpr}
    }{cct-set-event-eq-leak}
    %
    \typerule{cct-set-event-eq-noleak}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \mmlAtmsmmlAtcctcomp{\valueexpr} = \obj{\valueexpr} &
      \delta(\irl{\loc}) = \obj{\loc}
    }{
      \xlangeventeqc{Set\ \loc\ n\ \valueexpr}{\widehat{Set}\ \loc\ n\ \valueexpr}
    }{cct-set-event-eq-noleak}
    %
    \typerule{cct-call-event-eq}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \mmlAtmsmmlAtcctcomp{foo} = \obj{foo} &
      \irl{\comm} = \obj{\comm}
    }{
      \xlangeventeqc{Call\ \comm\ foo\ n}{Call\ \comm\ foo\ n}
    }{cct-call-event-eq}
    %
    \typerule{cct-ret-event-eq}{
      \mmlAtmsmmlAtcctcomp{n} = \obj{n} &
      \irl{\comm} = \obj{\comm}
    }{
    ...
    %
    \typerule{cct-$\irl{\emptyevent}$-event-eq}{
    }{
      \xlangeventeqc{\emptyevent}{\emptyevent}
    }{cct-emptyevent-event-eq}
    %
    \typerule{cct-$\irl{\lightning}$-event-eq}{
    }{
      \xlangeventeqc{\lightning}{\lightning}
    }{cct-abort-event-eq}
    %
    \typerule{cct-binop-event-eq-leak}{
    }{
      \xlangeventeqc{\emptyevent}{Binop\ n_1\ n_2}
    }{cct-binop-event-eq-leak}
    %
    \typerule{cct-binop-event-eq-leak}{
    }{
      \xlangeventeqc{\emptyevent}{Branch\ n}
    }{cct-branch-event-eq-leak}
    $\;$\\
    %
    ...
    }{cct-sctxtocomp-eq-tctxtocomp}
    %
    \typerule{$\irl{\comptoctx}=\obj{\comptoctx}$}{
    }{
      \irl{\comptoctx}=\obj{\comptoctx}
    }{cct-scomptoctx-eq-tcomptoctx}
    %
    \typerule{$\irl{\nocomm}=\obj{\nocomm}$}{
    }{
      \irl{\nocomm}=\obj{\nocomm}
    }{cct-snocomm-eq-tnocomm}
  \end{center}
}{cct-pres-trace-relation}{Trace Relation from $\mmlAtms$ to $\mmlAtcct$.}

\myfig{
  \begin{center}
    \judgbox{\delta(\irl{\loc})=\obj{\loc}}{,,The $\mmlAtms$ memory location $\irl{\loc}$ corresponds to\\the $\mmlAtcct$ memory location $\obj{\loc}$.''} \\
    \judgbox{\xlangstateeqc{\Omega}{\Omega}}{,,The $\mmlAtms$ state $\irl{\Omega}$ agrees with $\mmlAtms$ state $\obj{\Omega}$.''} \\
    %
    \typerule{cct-state-eq}{
      \irl{\Omega}=\irl{\cfstate;\sandboxtag;\memstate} &
      \obj{\Omega}=\obj{\cfstate;\sandboxtag;\memstate} &
      \irl{\sandboxtag} = \obj{\sandboxtag} \\
    ...
    %
    \typerule{cct-abort-state-eq}{
    }{
      \xlangstateeqc{\lightning}{\lightning}
    }{cct-abort-eq}
    %
    \judgbox{\xlangstateeqc{\memstate}{\memstate}}{,,The $\mmlAtms$ memory-state $\irl{\memstate}$ agrees with $\mmlAtcct$ one $\obj{\memstate}$. $\irl{L}$ contains locations\\introduced by the backtranslation wrapper.''}\\
    %
    \typerule{cct-empty-memstate-eq}{
    }{
      \xlangstateeqc{\hole{\cdot};\hole{\cdot};\hole{\cdot}}{\hole{\cdot};\hole{\cdot};\hole{\cdot}}
    }{cct-empty-memstate-eq}
    %
    \typerule{cct-comp-cons-memstate-eq}{
      \irl{\loc}\not\in\irl{L} &
      \irl{n}=\obj{n} &
    ...
    }{cct-comp-cons-memstate-eq}
    %
    \typerule{cct-ctx-cons-memstate-eq}{
      \irl{\loc}\not\in\irl{L} &
    ...
    }{cct-ctx-cons-memstate-eq}
    %
    \typerule{cct-whatever-cons-memstate-eq}{
      \irl{\loc}\in\irl{L} &
      \xlangstateeqc{H^{\ctx};H^{\comp};\Delta}{H^{\ctx};H^{\comp};\Delta}
    }{
      \xlangstateeqc{H^{\ctx};H^{\comp};x\mapsto(\loc;\sandboxtag;\poison;n),\Delta}{H^{\ctx};H^{\comp};\Delta}
    }{cct-whatever-cons-memstate-eq}
    %
    %
    ...
    %
    \judgbox{\xlangstateeqc[]{\cfstate}{\cfstate}}{,,The $\mmlAtms$ control-flow-state $\irl{\cfstate}$ agrees with $\mmlAtcct$ one $\obj{\cfstate}$.''} \\
    %
    \typerule{cct-cfstate-eq}{
      ...
      \obj{\commlib}=\mmlAtmsmmlAtcctcomp{\commlib}
    }{
      \xlangstateeqc[]{\cfstate}{\cfstate}
    }{cct-cfstate-eq}$\;$\\
    %
    %
    \judgbox{\irl{\poison}=\obj{\poison}}{,,$\mmlAtms$ poison equals $\mmlAtcct$ one $\obj{\poison}$.''} \\
    %
    \typerule{$\poisoned$-equal}{
    }{
    ...
    }{cct-poisonless-equal}
  \end{center}
}{cct-pres-state-relations-first}{State Relation from $\mmlAtms$ to $\mmlAtcct$. This is meant to relate the states whenever we are ,,inside'' a component.}

\myfig{
  \begin{center}
    %
    \judgbox{\xlangstateeqc[]{\library}{\library}}{,,The procedures of $\mmlAtms$ agree with $\mmlAtcct$ ones.''} \\
    %
    \typerule{cct-empty-commlib-lib-eq}{
    }{
      \xlangstateeqc[]{\hole{\cdot}}{\library}
    }{cct-empty-commlib-eq}
    %
    \typerule{cct-cons-commlib-eq}{
      \mmlAtmsmmlAtcctcomp{let\ foo\ x:\type_{\color{black}\lambda} := e} = \obj{let\ foo\ x := e} &
    ...
    %
    %
    \judgbox{\xlangstateeqc[\irl{\commlib}]{\kontstack}{\kontstack}}{,,The stack of $\mmlAtms$ continuations $\irl{\kontstack}$ agrees with $\mmlAtcct$ one $\obj{\kontstack}$.\\Only checks $\irl{foo}\in\irl{\commlib}$''} \\
    %
    \typerule{cct-empty-kontstack-eq}{
    }{
      \xlangstateeqc[\irl{\commlib}]{\hole{\cdot}}{\hole{\cdot}}
    }{cct-empty-kontstack-eq}
    %
    \typerule{cct-ign-cons-kontstack-eq}{
      \irl{foo}\notin\irl{\commlib} &
      \obj{foo}=\mmlAtmsmmlAtcctcomp{foo} &
      \xlangstateeqc[\irl{\commlib}]{\kontstack}{\kontstack}
    }{
      \xlangstateeqc[\irl{\commlib}]{\kontstack}{(K;foo),\kontstack}
    }{cct-ign-cons-kontstack-eq}
    %
    \typerule{cons-kontstack-eq}{
    ...
    }{cct-cons-kontstack-eq}
  \end{center}
}{cct-pres-memstate-relations}{Memory-State Relations from $\mmlAtms$ to $\mmlAtcct$.}
...
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAtmsmmlAtcctcomp{e}=\obj{e'}}{,,Compile $\mmlAtms$ expression $\irl{e}$ to $\mmlAtcct$ expression $\obj{e'}$.''}\\
    \judgbox{\mmlAtmsmmlAtcctcomp{\left[\valueexpr / x\right]}=\obj{\left[\valueexpr / x\right]}}{,,Compile $\mmlAtms$ substitution to $\mmlAtcct$ substitution.''} \\
    \judgbox{\mmlAtmsmmlAtcctcomp{\commlib}=\obj{\commlib'}}{,,Compile $\mmlAtms$ component library to $\mmlAtcct$ component library.''} \\
    \end{gather*}
    $$
    \begin{array}{rll}
    \mmlAtmsmmlAtcctcomp{\finalexpr} &\ = & \obj{\finalexpr} \\
    \mmlAtmsmmlAtcctcomp{call\ foo\ e} &\ = & \obj{call\ }\mmlAtmsmmlAtcctcomp{foo}\ \mmlAtmsmmlAtcctcomp{e}\\
    \mmlAtmsmmlAtcctcomp{return\ e} &\ = & \obj{return\ }\mmlAtmsmmlAtcctcomp{e}\\
    \mmlAtmsmmlAtcctcomp{e_1\oplus e_2} &\ = & \mmlAtmsmmlAtcctcomp{e_1} \obj{\widehat{\oplus}} \mmlAtmsmmlAtcctcomp{e_2} \\
      \mmlAtmsmmlAtcctcomp{x[e]} &\ = & \obj{\widehat{load}\ x\ }\mmlAtmsmmlAtcctcomp{e}\\
    \mmlAtmsmmlAtcctcomp{let\ x= e_1\ in\ e_2} &\ = & \obj{let\ x=} \mmlAtmsmmlAtcctcomp{e_1}\obj{\ in\ }\mmlAtmsmmlAtcctcomp{e_2} \\
    \mmlAtmsmmlAtcctcomp{x[e_1]\leftarrow e_2} &\ = & \obj{\widehat{write}\ x\ }\mmlAtmsmmlAtcctcomp{e_1}\ \mmlAtmsmmlAtcctcomp{e_2} \\
    \mmlAtmsmmlAtcctcomp{let\ x=new\ e_1\ in\ e_2} &\ = & \obj{let\ x=new\ }\mmlAtmsmmlAtcctcomp{e_1}\obj{\ in\ }\mmlAtmsmmlAtcctcomp{e_2} \\
    \mmlAtmsmmlAtcctcomp{delete\ x} &\ = & \obj{delete\ x} \\
    \mmlAtmsmmlAtcctcomp{x\ is\ \poisoned} &\ = & \obj{x\ is\ \poisoned} \\
    \mmlAtmsmmlAtcctcomp{\lparen e_1;e_2\rparen} &\ = & \obj{\lparen}\mmlAtmsmmlAtcctcomp{e_1}\obj{;}\mmlAtmsmmlAtcctcomp{e_2}\obj{\rparen}\\
    \mmlAtmsmmlAtcctcomp{\pi_1\ e} &\ = & \obj{\pi_1\ }\mmlAtmsmmlAtcctcomp{e}\\
    \mmlAtmsmmlAtcctcomp{\pi_2\ e} &\ = & \obj{\pi_2\ }\mmlAtmsmmlAtcctcomp{e}\\
    \mmlAtmsmmlAtcctcomp{e\ has\ \type} &\ = & \mmlAtmsmmlAtcctcomp{e}\ \obj{has\ \type}\\
    \mmlAtmsmmlAtcctcomp{ifz\ e_1\ then\ e_2\ else\ e_3} &\ = & \obj{\widehat{ifz}\ }\mmlAtmsmmlAtcctcomp{e_1}\\
                                                        &&\obj{then\ }\mmlAtmsmmlAtcctcomp{e_2} \\
                                                        &&\obj{else\ }\mmlAtmsmmlAtcctcomp{e_3} \\[0.5cm]
    %
    \mmlAtmsmmlAtcctcomp{\left[\valueexpr/x\right]}&\ = & \obj{\left[\right.}\mmlAtmsmmlAtcctcomp{v}\obj{/}\mmlAtmsmmlAtcctcomp{x}\obj{\left.\right]} \\[0.5cm]
    %
    \mmlAtmsmmlAtcctcomp{\hole{\cdot}}&\ = & \obj{\hole{\cdot}} \\
    \mmlAtmsmmlAtcctcomp{foo,\commlib}&\ = & \obj{foo,}\mmlAtmsmmlAtcctcomp{\commlib} \\
    \end{array}
    $$
  \end{center}
}{cct-pres-compiler}{Compiler from $\mmlAtms$ to $\mmlAtcct$.}
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAtmsmmlAtcctcomp{F}=\obj{F}}{,,Compile $\mmlAtms$ procedures to $\mmlAtcct$ procedures.''}
    \end{gather*}
    $$
    \begin{array}{rll}
    %
      \mmlAtmsmmlAtcctcomp{let\ foo\ x := e}&\ = & \obj{let\ foo\ x := }\mmlAtmsmmlAtcctcomp{e}\\
    \end{array}
    $$
    \begin{gather*}
    \judgbox{\mmlAtmsmmlAtcctcomp{\library}=\obj{\library}}{,,Compile $\mmlAtms$ libraries to $\mmlAtcct$ libraries.''}
    \end{gather*}
    $$
    \begin{array}{rll}
      \mmlAtmsmmlAtcctcomp{\hole{\cdot}}&\ = & \obj{\hole{\cdot}} \\
      \mmlAtmsmmlAtcctcomp{F,\library}&\ = & \mmlAtmsmmlAtcctcomp{F}\irl{,}\mmlAtmsmmlAtcctcomp{\library} \\
    \end{array}
    $$
  \end{center}
}{cct-pres-compiler-comp}{Compiler from $\mmlAtms$ components to $\mmlAtcct$ components.}

\cleardoublepage

\begin{lemma}[Primitive Forward Simulation]\label{lem:prim:forwardsim:cct}
  If
  \begin{assumptions}
    \item\label[ass]{asm:prim:forwardsim:cct:execi} $\exprevali{\Omega}{e\gamma}{\Omega'}{e'\gamma'}{\event}$
    \item $\monitorcheck[]{\cctmonitor}{\cctmonitor'}{\absevent}$
    \item $\theta_{\delta}(\irl{\event})=\absevent$
    \item $\xlangstateeqc{\Omega}{\Omega}$
    \item $\storeagreecct{\cctmonitor}{\obj{\Omega}}$
  \end{assumptions}
  Then $\exists \delta_{cct}'\ \obj{\event}\ \obj{\Omega'}$,
  \begin{goals}
    \item $\exprevalo{\Omega}{\color{black}\mmlAtmsmmlAtcctcomp{e}\mmlAtmsmmlAtcctcomp{\gamma}}{\Omega'}{\color{black}\mmlAtmsmmlAtcctcomp{e'}\mmlAtmsmmlAtcctcomp{\gamma'}}{\event}$
    \item $\xlangeventeqc[\delta_{cct}']{\event}{\event}$
    \item $\xlangstateeqc[\delta_{cct}']{\Omega'}{\Omega'}$
    \item $\theta_{\delta_{cct}'}(\obj{\event})=\absevent$
  \end{goals}
\end{lemma}
\begin{proof}
  Case analysis on \Cref{asm:prim:forwardsim:cct:execi}.
\end{proof}

\begin{lemma}[Contextual Forward Simulation]\label{lem:ctx:forwardsim:cct}
  If
  \begin{assumptions}
    \item\label[ass]{asm:ctx:forwardsim:cct:execi} $\exectms[]{\Omega}{e\gamma}{\Omega'}{e'\gamma'}{\event}$
    \item $\monitorcheck[]{\cctmonitor}{\cctmonitor'}{\absevent}$
    \item $\theta_{\delta}(\irl{\event})=\absevent$
    \item $\xlangstateeqc{\Omega}{\Omega}$
    \item $\storeagreecct{\cctmonitor}{\obj{\Omega}}$
  ...
  Then $\exists \delta_{cct}'\ \obj{\event}\ \obj{\Omega'}$,
  \begin{goals}
    \item $\execo[]{\Omega}{\color{black}\mmlAtmsmmlAtcctcomp{e}\mmlAtmsmmlAtcctcomp{\gamma}}{\Omega'}{\color{black}\mmlAtmsmmlAtcctcomp{e'}\mmlAtmsmmlAtcctcomp{\gamma'}}{\event}$
    \item $\xlangeventeqc[\delta_{cct}']{\event}{\event}$
    \item $\xlangstateeqc[\delta_{cct}']{\Omega'}{\Omega'}$
    \item $\theta_{\delta_{cct}'}(\obj{\event})=\absevent$
  \end{goals}
\end{lemma}
\begin{proof}
  Case analysis on \Cref{asm:ctx:forwardsim:cct:execi} and using \Thmref{lem:prim:forwardsim:cct}.
\end{proof}

\begin{lemma}[Forward Simulation]\label{lem:steps:forwardsim:cct}
  If
  \begin{assumptions}
    \item\label[ass]{asm:steps:forwardsim:cct:execi} $\exectms{\Omega}{e\gamma}{\Omega'}{e'\gamma'}{\event}$
    \item $\monitorcheck[]{\cctmonitor}{\cctmonitor'}{\absevent}$
    \item $\theta_{\delta}(\irl{\event})=\absevent$
    \item $\xlangstateeqc{\Omega}{\Omega}$
    \item $\storeagreecct{\cctmonitor}{\obj{\Omega}}$
...
\end{lemma}
\begin{proof}
  Case analysis on \Cref{asm:steps:forwardsim:cct:execi} and using \Thmref{lem:ctx:forwardsim:cct}.
\end{proof}
...
\begin{scontents}[store-env=buffer]
  $\;$\\
  \begin{goals}
    \item $\vdash\mmlAtmsmmlAtcctcomp{\bullet}:\lceil\operatorname{scct}\rceil$
  \end{goals}
\end{scontents}
\realthm{thm:rcctp}{7}{Robust Strict Cryptographic Constant Time Preservation}
