...
\myfig{
  ...
  \typerule{$e-\text{wprog}-$failp}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\trace{_0}} \\
    \execs{\qconfig_0}{e_p[v_0/x]}{\lightning}{stuck}{\trace{_p}} &
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\trace{_0}\cdot Call\ ?v_0\cdot \trace{_p}}
  }{ws-e-prog}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\src{\mstrace} \bnfdef&\ \specificevs{Alloc\ \loc\ n} \mid \specificevs{Dealloc\ \loc} \mid \specificevs{Get\ \loc\ n} \mid \specificevs{Set\ \loc\ n} \\
...
\myfig{
  \begin{center}
    ...
    \judgbox{\tospecificevctxs{\trace} = \src{\mstrace}}{,,Project an $\mmlAs$ trace $\src{\trace}$ to $\src{\mstrace}$, cutting off the component part.''}
    \judgbox{\tospecificevcomps{\trace} = \src{\mstrace}}{,,Project an $\mmlAs$ trace $\src{\trace}$ to $\src{\mstrace}$, cutting off the context part.''}
    $$
    \begin{array}{rcl}
...
\myfig{
  \begin{center}
    \judgbox{\xlangtraceeq{\trace}{\trace}}{,,The $\mmlAs$ trace $\src{\trace}$ is morally equal to the $\mmlAt$ trace $\trg{\trace}$.''} \\
...
\myfig{
  \begin{center}
    \judgbox{\delta(\src{\loc})=\trg{\loc}}{,,The $\mmlAs$ memory location $\src{\loc}$ corresponds to\\the $\mmlAt$ memory location $\trg{\loc}$.''} \\
    \judgbox{\xlangstateeq{\Omega}{\Omega}}{,,The $\mmlAs$ state $\configs$ is morally equal to the $\mmlAt$ state $\configt$.''} \\
    %
    \typerule{state-eq}{
    ...
    \judgbox{\xlangbackstateeq{\Omega}{\Omega}}{,,The $\mmlAt$ state $\configt$ is morally equal to the $\mmlAs$ state $\configs$.''} \\
    %
...
\myfig{
  \begin{center}
    ...
    \judgbox{\mmlAmmlAtback{\trace} = \src{context\ e_0\ e_1}}{,,Given $\mmlAt$ trace, construct an $\mmlAs$ expression that\\morally does what the trace describes.''}$\;$\\
