...
\myfig{
  \judgbox{\exprevals{\configs}{e\gamma}{\configs'}{e'\gamma'}{\event}}{,,Expression $\src{e}$ applied to substitutions $\src{\gamma}$ evaluates under\\configuration $\configs$ to $\src{e'}$ applied to substitutions $\src{\gamma'}$ and new\\configuration $\src{\configs'}$, emitting event $\src{\event}$.''}
  %
  \typerule{$e-\src{\oplus}$}{
  ...
  %
  \typerule{$e-$subst}{
    \src{e'}=\text{substitute }\src{\valueexpr}\text{ for }\src{x}\text{ in }\src{e}
  }{
    \exprevals{\configs}{e[\valueexpr/x],\gamma}{\configs'}{e'\gamma}{\emptyevent}
  }{w-e-subst}
  %
...
For sake of readablity, we will omit $\trg{\gamma}=\trg{\hole{\cdot}}$ when writing down evaluation steps.
That is, whenever we have $\exprevalt{\configt}{e\hole{\cdot}}{\configt'}{e'\hole{\cdot}}{\event}$, it's written $\exprevalt{\configt}{e}{\configt'}{e'}{\event}$.
Moreover, $\trg{\gamma}$ as postfix binds anything before it up to $\triangleright$, so $\trg{let\ x=e_{1}\ in\ e_{2}\gamma}$ is $\trg{(let\ x=e_{1}\ in\ e_{2})\gamma}$.


\myfig{
  \begin{center}
    \judgbox{\execs[]{\configs}{K[e]\gamma}{\configs'}{K'[e']\gamma'}{\event}}{,,Given an evaluation context $\src{K}$ and an expression $\src{e}$\\and a substitution $\src{\gamma}$, it evaluates under\\configuration $\configs$ to $\src{e'}$ and substitution $\src{\gamma'}$ and new\\configuration $\src{\configs'}$ in context $\src{K}$, emitting event $\src{\event}$.''}
    %
    \typerule{$e-\text{ctx}$}{
      \exprevals{\Omega}{e}{\Omega'}{e'\gamma'}{\event}
    }{
      \execs[]{\Omega}{K[e]}{\Omega'}{K[e']\gamma'}{\event}
    }{w-e-ectx}
    %
    \typerule{$e-\text{ctx}-\text{subst}$}{
      \src{K'}=\text{substitute }\src{x}\text{ for }\src{\valueexpr}\text{ in }\src{K} &
      \src{e'}=\text{substitute }\src{x}\text{ for }\src{\valueexpr}\text{ in }\src{e}
    }{
      \execs[]{\Omega}{K[e][\valueexpr/x],\gamma}{\Omega'}{K'[e']\gamma}{\emptyevent}
    }{w-e-ectx-subst}
    %
...
\myfig{
  \begin{center}
    \judgbox{\execs{\configs}{e\gamma}{\configs'}{e'\gamma'}{\trace}}{,,Expression $\src{e}$ applied to substitutions $\src{\gamma}$ evaluates under\\configuration $\configs$ to $\src{e'}$ with substitutions $\src{\gamma'}$ and\\new configuration $\src{\configs'}$, emitting list of events $\src{\trace}$.''}
    %
    \typerule{$es-\text{refl}$}{
    }{
    ...
    %
    \judgbox{\execsn{\configs}{e\gamma}{\configs'}{e'\gamma'}{\trace}}{,,Expression $\src{e}$ applied to substitutions $\src{\gamma}$ evaluates in $n$\\steps under configuration $\configs$ to $\src{e'}$ with substitutions $\src{\gamma'}$ and\\new configuration $\src{\configs'}$, emitting list of events $\src{\trace}$.''}
    %
    \typerule{$en-\text{refl}$}{
    }{
      \execsn[^0]{\configs}{e\gamma}{\configs}{e\gamma}{\emptyevent}
    }{wsn-e-refl}
    %
    \typerule{$en-\text{trans}-$important}{
      \execs[]{\configs}{e\gamma}{\configs'}{e'\gamma'}{\event} &
      \execsn[^n]{\configs'}{e'\gamma'}{\configs''}{e''\gamma''}{\trace} &
      \src{\event}\not=\src{\emptyevent}
    }{
      \execsn[^{(n+1)}]{\configs}{e\gamma}{\configs''}{e''\gamma''}{\event\cdot\trace}
    }{wsn-e-trans-important}
    %
    \typerule{$en-\text{trans}-$unimportant}{
      \execs[]{\configs}{e\gamma}{\configs'}{e'\gamma'}{\emptyevent} &
      \execsn[^n]{\configs'}{e'\gamma'}{\configs''}{e''\gamma''}{\trace} &
    }{
      \execsn[^{(n+1)}]{\configs}{e\gamma}{\configs''}{e''\gamma''}{\trace}
    }{wsn-e-trans-important}
  \end{center}
...
\begin{lemma}[Progress]\label{lem:progress}
  If
  \begin{assumptions}
    \item\label[ass]{asm:progress:typs} $\typechecks{}{\src{\Omega}\triangleright e}{\src{\type}}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\src{e}=\src{abort()}$ or $\exists \src{\Omega'}\ \src{\valueexpr}\ \src{\trace}, \execs{\configs}{e}{\configs'}{\valueexpr}{\trace}$
  \end{goals}.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \begin{description}
    ...
    \case{tr:w-e-subst}
      If
      \begin{passumptions}{H}
        \item $\src{\gamma}=\src{[\finalexprnoerr/x],\gamma'}$
        \item $\typechecks{}{\src{\Omega}\triangleright\src{e}\gamma}{\type}$
        \item $\storeagreetms{\tmsmonitor}{\src{\Omega}}$
      \end{passumptions}
      Then $\exists \absevent\ \delta_{\text{MS}}'\ \tmsmonitor['],$
      \begin{goals}
...
\myfig{
  \judgbox{\exprevalt{\configt}{e\gamma}{\configt'}{e'\gamma'}{\event}}{,,Expression $\trg{e}$ with substitutions $\trg{\gamma}$ evaluates under\\configuration $\configt$ to $\trg{e'}$ with substitutions $\trg{\gamma'}$ and\\new configuration $\trg{\configt'}$, emitting event $\trg{\event}$.''}
  %
  \typerule{$e-\trg{\oplus}$}{
  ...
  %
  \typerule{$e-$subst}{
    \text{substitute }\trg{x}\text{ for }\trg{\valueexpr}\text{ in }\trg{e}
  }{
    \exprevalt{\configt}{e[\valueexpr/x],\gamma}{\configt}{e'\gamma}{\emptyevent}
  }{tw-e-subst}
  %
  \typerule{$e-\trg{get}-\in$}{
...
\myfig{
  \begin{center}
    \judgbox{\exect[]{\configt}{K[e]\gamma}{\configt'}{K[e']\gamma'}{\event}}{,,Given an evaluation context $\trg{K}$ and an expression\\ $\trg{e}$ and a substitution $\trg{\gamma}$, it evaluates under\\configuration $\configt$ to $\trg{e'}$ and substitution $\trg{\gamma'}$ and new\\configuration $\trg{\configt'}$ in context $\trg{K}$, emitting event $\trg{\event}$.''}
    %
    \typerule{$e-\text{ctx}$}{
...
\begin{lemma}[Determinism of Step]\label{lem:determ:step}
  If
  \begin{assumptions}
  \item\label[ass]{asm:determ:step:a} $\exprevalt{\Omega}{e\gamma}{\Omega'}{e_1}{\event_{1}}$
  \item $\exprevalt{\Omega}{e\gamma}{\Omega''}{e_2}{\event_{2}}$
  \end{assumptions}
  Then
  \begin{goals}
...
\begin{lemma}[Determinism of Ctx-Step]\label{lem:determ:ctxstep}
  If
  \begin{assumptions}
  \item\label[ass]{asm:determ:ctxstep:a} $\exect[]{\Omega}{e\gamma}{\Omega'}{e_1}{\event_{2}}$
  \item $\exect[]{\Omega}{e\gamma}{\Omega''}{e_2}{\event_{1}}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{e_{1}}=\trg{e_{2}}$
...
\begin{lemma}[Determinism of $n$-Steps]\label{lem:determ:nsteps}
  If
  \begin{assumptions}
  \item\label[ass]{asm:determ:steps:a} $\exectN{\Omega}{e\gamma}{\Omega'}{e_1}{\trace[_{1}]}$
  \item $\exectN{\Omega}{e\gamma}{\Omega''}{e_2}{\trace[_{2}]}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{e_{1}}=\trg{e_{2}}$
...
\begin{lemma}[Determinism of Steps]\label{lem:determ:steps}
  If
  \begin{assumptions}
  \item\label[ass]{asm:determ:steps:a} $\exect{\Omega}{e\gamma}{\Omega'}{v_1}{\trace[_{1}]}$
  \item $\exect{\Omega}{e\gamma}{\Omega''}{v_2}{\trace[_{2}]}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{v_{1}}=\trg{v_{2}}$
  \item $\trg{\Omega'}=\trg{\Omega''}$
  \item $\trg{\trace[_{1}]}=\trg{\trace[_{2}]}$
  \end{goals}
\end{lemma}
...
\begin{lemma}[Substitution Commutes with $\mmlAmmlAtcomp{\bullet}$]\label{lem:subst:commute:comp}
  \begin{assumptions}
    \item $\mmlAmmlAtcomp{e} = \trg{e}$
    \item $\src{e'} = \text{substitute }\src{x}\text{ with }\src{\valueexpr}\text{ in }\src{e}$
    \item $\trg{e'} = \text{substitute }\mmlAmmlAtcomp{x}\text{ with }\mmlAmmlAtcomp{\valueexpr}\text{ in }\trg{e}$
  \end{assumptions}
  Then
  \begin{goals}
    \item $\trg{e'}=\mmlAmmlAtcomp{e'}$
  \end{goals}
\end{lemma}
\begin{proof}
  Induction on $\src{e}$.
...
\begin{proof}
  ...
  \begin{description}
    ...
    \case{tr:w-e-subst}
      ...
      If
      \begin{passumptions}{H}
      \item $\src{\gamma}=\src{[\valueexpr/x],\gamma'}$
      \item\label[ass]{asm:step:forwardsim:case10:stateeq} $\xlangstateeq{\Omega}{\Omega}$
      \item $\typechecks{}{\Omega\triangleright e\gamma}{\nat}$
      \end{passumptions}
      Then $\exists\delta'\ \trg{\Omega'}\ \trg{\event}$,
      \begin{goals}
        \item $\delta\subseteq\delta'$
      ...
      \begin{goals}
        \item\label[goal]{goal:step:forwardsim:case10:deltasubset} $\delta\subseteq\delta$
        \item\label[goal]{goal:step:forwardsim:case10:eval} $\expreval{\trg{\Omega}}{\mmlAmmlAtcomp{e}\trg{[\valueexpr/x],}\mmlAmmlAtcomp{\gamma'}}{\trg{\Omega}}{\mmlAmmlAtcomp{e'}\mmlAmmlAtcomp{\gamma'}}{\trg{\varepsilon;\sandboxtag}}$
        \item\label[goal]{goal:step:forwardsim:case10:eventeq} $\xlangeventeq{\varepsilon}{\varepsilon}$
        \item\label[goal]{goal:step:forwardsim:case10:stateeq} $\xlangstateeq{\Omega}{\Omega}$
      \end{goals}

      \Cref{goal:step:forwardsim:case10:deltasubset} follows by reflexivity of the subset relation.

      \Cref{goal:step:forwardsim:case10:eval} follows by \Thmref{lem:subst:commute:comp} and \Cref{tr:tw-e-subst}.

      \Cref{goal:step:forwardsim:case10:eventeq} follows by definition of \Cref{tr:emptyevent-event-eq}.

      \Cref{goal:step:forwardsim:case10:stateeq} follows by \Cref{asm:step:forwardsim:case10:stateeq}.
...
\begin{proof}
  ...
  \begin{description}
    \item \textbf{Case $\src{e}=\src{\valueexpr}$:}
      ...
      \begin{description}
        \item \textbf{Case $\delta\subset\delta$:} Trivially false.
...
%    \item $\delta' = \delta \cup \left\{\src{\loc}\mapsto\trg{z}\right\}$
  ...
  Next, use \Cref{tr:w-e-ectx,tr:w-e-delete,tr:w-e-let-f} on \Cref{asm:exprbacktranslation:vend:wrapper:correct:execs0}:
...
\begin{scontents}[store-env=buffer]
  ...
  then $\exists \delta_{MS}',$
  \begin{goals}
    \item\label[goal]{rtmsp:tms} $\tospecificevs[_{\delta_{MS}'}]{\trace}\in\operatorname{tmsafe}$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{4}
    \item\label[ass]{asm:rtmsp:backtrans} $\mmlAmmlAtback[\emptyset;\emptyset]{{\color{black}\theta_{{\trg{\bullet}}}(}{\trg{\trace}}{\color{black})}}=\Game;\delta;\src{let\ main\ z\ := e},\src{\hole{\cdot}}$.
  \end{passumptions}
...
\myfig{
 \begin{center}
    \typerule{$\irl{\gamma}$-$\irl{x_{SIZE}}$-preserve}{
      ...
      \irl{\Delta_1,x\mapsto(\loc;\comp;\poison;m),\Delta_2} \Vdash \irl{[x_{SIZE}/\valueexpr],\gamma}
    }{gamma-xsize-preserve}
    %
    \typerule{$\irl{\gamma}$-$\irl{x_{SIZE}}$-skip}{
      \irl{\Delta} \Vdash \irl{\gamma}
    }{
      \irl{\Delta} \Vdash \irl{[x/\valueexpr],\gamma}
    }{gamma-xsize-skip}
    %
...
\begin{lemma}[Injective Compiler (for values)]\label{lem:smscomp:inject}
  If
  \begin{assumptions}
    \item $\mmlAtmmlAtmscomp{\finalexprnoerr}=\irl{e_0}$
    \item $\mmlAtmmlAtmscomp{\finalexprnoerr}=\irl{e_1}$
  \end{assumptions}
  Then
  \begin{goals}
...
\begin{proof}
  ...
  \begin{description}
    \item \textbf{Case} \ref{tr:tw-e-oplus}\textbf{:}
          If
          \begin{passumptions}{H}
          ...
          Induce on $\trg{\gamma}\prec\irl{\gamma}$ and apply \Cref{tr:tw-e-subst} in inductive case.
          Now, all goals follow immediately from assumptions.

    \item \textbf{Case} \ref{tr:tw-e-get-in}\textbf{:}
          If
          \begin{passumptions}{H}
    ...
    \item \textbf{Case} \ref{tr:tw-e-get-notin}\textbf{:}
          If
          \begin{passumptions}{H}
            \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta_{1},x\mapsto(\loc;\comp;\poison;m),\Delta_{2}}$
            \item $\trg{\gamma}=\trg{\gamma'}=\trg{\hole{\cdot}}$
    ...
    \item \textbf{Case} \ref{tr:tw-e-set-comp}\textbf{:}
          If
          \begin{passumptions}{H}
            \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta_{1},x\mapsto(\loc;\comp;\poison;m),\Delta_{2}}$
    ...
    \item \textbf{Case} \ref{tr:tw-e-let-new-comp}\textbf{:}
          If
          \begin{passumptions}{H}
            \item $\trg{\memstate}=\trg{H^{\ctx};H^{\comp};\Delta}$
    ...
    \item \textbf{Cases} \ref{tr:tw-e-let-new-ctx} \textbf{and} \ref{tr:tw-e-set-ctx}\textbf{:}
          Does not apply, since the sandboxtag is $\trg{\comp}$, but the rule requires $\trg{\ctx}$.

    \item \textbf{Case} \ref{tr:tw-e-delete}\textbf{:}
          If
          \begin{passumptions}{H}
    ...
    \item \textbf{Case} \ref{tr:tw-e-let-f}\textbf{:}
          If
          \begin{passumptions}{H}
            \item $\trg{\Omega}=\irl{\Omega}=\trg{\cfstate;\comp;\memstate}$
            \item $\trg{\gamma}=\trg{\hole{\cdot}}$
    ...
    \item \textbf{Case} \ref{tr:tw-e-subst}\textbf{:}
          If
          \begin{passumptions}{H}
            \item $\trg{\Omega}=\irl{\Omega}=\trg{\cfstate;\comp;\memstate}$
            \item $\trg{\gamma}=\trg{[\valueexpr/x],\gamma'}$
            \item $\exprevalt{\Omega}{e[\valueexpr/x],\gamma'}{\Omega}{e\gamma'}{\emptyevent;\comp}$
            \item $\monitorcheck[]{\smsmonitor}{\smsmonitor'}{\absevent}$
            \item\label[ass]{asm:sms:primforwardsim:storeagree:subst} $\storeagreesms[\delta]{\smsmonitor}{\trg{\Omega}}$
            \item\label[ass]{asm:sms:primforwardsim:trace:subst} $\traceagree[]{\theta_{\delta}\left(\trg{\emptyevent;\comp}\right)}{\absevent}$
            \item $\irl{\event}=\trg{\emptyevent;\comp}$
            \item\label[ass]{asm:sms:primforwardsim:subst:subst} $\irl{\Omega.\Delta}\Vdash\irl{\gamma}$
            \item\label[ass]{asm:sms:primforwardsim:subst:prec} $\trg{\gamma}\prec\irl{\gamma}$
          \end{passumptions}
          Then $\exists \irl{\gamma'}$,
          \begin{goals}
            \item $\exec{\irl{\Omega}}{\mmlAtmmlAtmscomp{e}\irl{\gamma}}{\irl{\Omega}}{\mmlAtmmlAtmscomp{e}\irl{\gamma'}}{\irl{\event}}$
          ...
          From \Cref{asm:sms:primforwardsim:subst:prec}, we can deduce that
          \begin{passumptions}{H}
            \item $\irl{\gamma}=\irl{\gamma_{0},[\valueexpr/x],\gamma_{1}}$
            \item $\irl{\gamma_{0}}=\irl{\hole{\cdot}}$ or for any $\irl{s}\in\irl{\gamma_{0}},\exists \irl{\valueexpr'\ y_{SIZE}}, \irl{s}=\irl{[\valueexpr'/y_{SIZE}]}$
          \end{passumptions}
          Induce on $\irl{y_{0}}$. In the base case, the goals easily go through.
          For the inductive case, we can just apply \Cref{tr:tw-e-subst}.


    \item \textbf{Cases} \ref{tr:tw-e-x-is-poisoned-yes}, \ref{tr:tw-e-x-is-poisoned-no}, \ref{tr:tw-e-proj1}, \ref{tr:tw-e-proj2}, \ref{tr:tw-e-x-hasnat}, \ref{tr:tw-e-n-hasnat}, \ref{tr:tw-e-pair-hasnat}, \ref{tr:tw-e-x-haspair}, \ref{tr:tw-e-n-haspair}, \ref{tr:tw-e-pair-haspair}, \ref{tr:tw-e-if-true} and \ref{tr:tw-e-if-false}\textbf{:}
...
\begin{proof}
  Use \Thmref{lem:backwardsim:toplevel} on \Cref{asm:rsmsp:irlexec}: (remember that $\irl{\trace}=\trg{\trace}$, since $\mmlAtms=\mmlAt$)
  \begin{passumptions}{H}
    \item\label[ass]{asm:rtsmsp:twprog} $\wexec{\trg{prog\ \library_{ctx}\ \library_{comp}}}{\trg{\Omega}}{\trg{\finalexpr}}{\trg{\trace}}$
  \end{passumptions}
  \Cref{asm:rsmsp:srcsms} solves our goal.
\end{proof}

\begin{scontents}[store-env=buffer]
  If
  \begin{assumptions}
  \item\label[ass]{asm:rsmsp:srcsms} $\forall \delta_{MS}\ \src{\library_{ctx}}\ \src{\library_{comp}}\ \src{\trace}\ \src{\Omega}\ \src{v}, \wexect{prog\ \library_{ctx}\ \library_{comp}}{\configt}{\finalexpr}{\trace}\rightarrow\tospecificevs[_{\delta_{MS}}]{\trace}\in\operatorname{msafe}$
...
\begin{proof}
  Immediate consequence of \Thmref{lem:seqcompo} using \Thmref{thm:rtmsp} and \Thmref{thm:rsmsp}.
\end{proof}
