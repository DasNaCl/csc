    ...
    \case{tr:scct-cons-trace-eq}
      ...
      \begin{description}
        ...
        \case{tr:scct-get-event-eq-leak}
          ...
          \begin{passumptions}{F}
%            \item $\delta_{scct}(\delta^{-1}(\obj{\loc}))=\specificev{\loc}$
%            \item $\obj{n} = \specificev{n}$
            \item $\specificev{\sigma}=\bm{\sigma}$
            \item $\absevent=\bm{Any};\bm{\sigma}$.
          \end{passumptions}
          Case analysis on $\bm{\sigma}$.
          \begin{description}
            \item \textbf{Case }$\bm{\sigma}=\unlock$\textbf{:}
                {\color{green}\large what do we do here? make specificev of Use with the unlock go to epsilon? -> would be ok, we don't care about public stuff anyways!}
