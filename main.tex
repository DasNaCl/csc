...
\myfig{
  \begin{center}
    \typerule{TMS-Call-Authentic}{
      ...
      \tmstraceagree{\specificevs{Call\ ?n}}{\varepsilon}
    }{tms-call-auth}
    %
    \typerule{TMS-Ret-Authentic}{
    }{
      \tmstraceagree{\specificevs{Ret\ !n}}{\varepsilon}
    }{tms-ret-auth}
    %
...
\begin{importanttheorem}[$\mmlAs$ is \gls{tms} via Monitor]
  ...
  Then
  \begin{goals}
  \item $\texttt{TMS}(\tospecificevs{\trace})$
  \end{goals}.
...
\begin{incompleteproof}
  ...
  \begin{passumptions}{H}
    ...
    \item $\src{\trace}=\src{\trace{_0}}\cdot\src{Call\ ?v_0}\cdot\src{\trace{_p}}\cdot\src{Ret\ !v_p}\cdot\src{\trace{_1}}$
  \end{passumptions}
  ...
  \begin{passumptions}{F}
    ...
    \item\label[ass]{asm:mmlatmsviamonitor:traceagreeasm} $\tmstraceagree[\delta^{(p)}]{\tospecificevs{\trace{_p}}}{\trace{_p}}$
  ...
  %%% BIGGGGTODO  AAAAAAAA
  Note that $\theta\left(\right)=\specificevs{Call\ ?v_0}\cdot\mstrace{_p}\cdot\specificevs{Ret\ !v_p}$
  Now instantiate the existential in our goal with $\delta_{\text{TMS}}^{(p)}$, $\trace{_p}$, and $\tmsmonitor$, so the remaining goals are:

  \begin{goals}
\item\label[goal]{asm:mmlatmsviamonitor:traceagree} $\tmstraceagree[\delta^{(p)}]{\trace{_p}}{\src{\trace{_0}\cdot \trace{_p}\cdot \trace{_1}}}$
    \item\label[goal]{asm:mmlatmsviamonitor:monitorcheck} and $\monitorcheck{\emptyset}{\tmsmonitor}{\trace{_p}}$
  \end{goals}

  For \Cref{asm:mmlatmsviamonitor:traceagree}, note that by \Cref{tr:tms-trans-auth-ignore}, we can reduce it to $\tmstraceagree[\delta^{(p)}]{\trace{_p}}{\src{\trace{_p}}}$\MK{needs a technical lemma}, which is fulfilled by \Cref{asm:mmlatmsviamonitor:traceagreeasm}.
...
\begin{proof}
  ...
  \begin{goals}
    \item $\tmstraceagree[{\delta^{\color{\ulccol}\bullet}}]{\tospecificevt{\overline{a}}}{\overline{a}}$
    \item $\monitorcheck{\emptyset}{\tmsmonitor}{\overline{a}}$
  \end{goals}
