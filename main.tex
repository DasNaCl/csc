...
\myfig{
  ...
  \mi{Store}~\src{\Delta} \bnfdef\ \src{\hole{\cdot}} \mid \src{x\mapsto\loc^\poison},\Deltas\\
  %
...
%  \judgbox{\typechecks{\Gamma}{C}{\tau_C}{\Gamma'}}{,,Under environment $\Gammas$, the context $\src{C}$ has type $\src{\tau_{C}}$ and yields environment $\src{\Gamma'}$.''}
  ...
  \end{center}
}{w-program-ty}{Checking of $\mmlA$ contexts, components, programs, and whole programs.}
...
\myfig{
  \begin{center}
    ...
    \typechecks{}{x\mapsto\loc^\poisonless,\Delta}{x:\ptr,\Gamma}
    }{t-ptr-store}
    %
    \typerule{$T\text{ref}\nat\text{poison}$}{
    \typechecks{}{\Delta}{\Gamma}
    }{
    \typechecks{}{x\mapsto\loc^\poisoned,\Delta}{\Gamma}
    }{t-ptr-store}
  \end{center}
}{store-ty}{$\mmlA$ store typing.}

\myfig{
  \begin{center}
    $$\src{context}\ (\src{e_0})\ (\src{e_1})\src{\linker}{\src{component\ e_p}} = \src{prog\ (let\ y:=let\ x:=e_0\ in\ e_p\ in\ e_1)}$$
  \end{center}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\src{a} \bnfdef& \src{\emptyevent} \mid \src{Alloc\ \loc\ n} \mid \src{Dealloc\ \loc} \mid \src{Read\ \loc\ n} \mid \src{Write\ \loc\ n}
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \src{let\ x:=K\ in\ e}\mid \src{x[K]\leftarrow e}\mid \src{x[v]\leftarrow K}\mid \src{let\ x:=new\ K\ in\ e}
    \end{aligned}
  \end{gather*}
...
\myfig{
  ...
  \typerule{$e-\src{get}$}{
    ...
    \exprevals{\sconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{x[n]}{\sconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{v}{Read\ \loc\ n}
  }{w-e-get}
  %
  \typerule{$e-\src{set}$}{
    \src{H'} = \src{H}(\src{\loc+n}\mapsto \src{v})
  }{
    \exprevals{\sconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{x[n]\leftarrow v}{\sconfig{H'}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{v}{Set\ \loc\ n\ v}
  }{w-e-set}
  %
  ...
  \typerule{$e-\src{delete}$}{
    ...
    \exprevals{\sconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{delete\ x}{\sconfig{H}{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}{0}{Dealloc\ \loc}
  }{w-e-let-new}
  %
  \typerule{$e-\src{new}$}{
    \text{fresh}\ \src{\loc}&
  ...
  \exprevals{\sconfig{H}{\Delta}}{let\ x=new\ n\ in\ e}{\sconfig{H'}{z\mapsto \loc^\poisonless,\Delta}}{e[z/x]}{Alloc\ \loc\ n}
  }{w-e-let-new}
  %
...
\myfig{
  \begin{center}
    \judgbox{\execs{\configs}{e}{\configs'}{e'}{\bar{a}}}{,,Expression $\src{e}$ evaluates under configuration $\configs$ to $\src{e'}$ and new configuration $\src{\configs'}$,\\emitting list of events $\src{\bar{a}}$.''}
    %
    \typerule{$e-\text{refl}$}{
    }{
...
\myfig{
  \begin{center}
  \typerule{$e-\text{wprog}$}{
    \execs{\hole{\cdot};\hole{\cdot}}{e}{\configs}{v}{\bar{a}}
  }{
  \wexecs{prog\ e}{\configs}{v}{\bar{a}}
  }{w-e-prog}
  \end{center}
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      \mi{Abstract\ References}~L :=& \nat \hspace{0.5cm}
      \mi{Abstract\ Store}~T :=\{A : L, F : L\} \\
      \mi{Abstract\ Events}~a \bnfdef& \emptyevent \mid \text{Alloc}\ \loc \mid \text{Dealloc}\ \loc \mid \text{Use}\ \loc \\
      \mi{References\ Agreement}~\delta :& \src{L}\to L \\[0.6cm]
      T\subseteq_F T' &\text{ iff } T.F \subseteq T'.F\\
      a\not\in T' &\text{ iff } a \not\in {T'.A} \wedge a \not\in {T'.F}\\
      \{\loc\}\cup T &:= \{A : \{\loc\}\cup T.A, F : T.F\}\\
    \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    ...
    \monitorcheck[]{\emptyset}{\emptyset}{\varepsilon}
    }{tms-empty}
    %
    \typerule{TMS-Use}{
      \loc\in T.A &
    ...
    \monitorcheck[]{T}{T}{\text{Use}\ \loc}
    }{tms-use}
    %
    \typerule{TMS-Alloc}{
      T'=\left\{A:T.A\cup\left\{\loc\right\}, F:T.F\right\}&
      \loc\not\in T&
    }{
    \monitorcheck[]{T}{T'}{\text{Alloc}\ \loc}
    }{tms-alloc}
    %
    \typerule{TMS-Dealloc}{
      \loc\in T.A &
      \loc\not\in T.F &
      T'=\left\{A:T.A\setminus\left\{\loc\right\},F: T.F\right\}
    }{
    \monitorcheck[]{T}{T'}{\text{Dealloc}\ \loc}
    }{tms-dealloc}
    %
    \typerule{TMS-Refl}{
    }{
...
\myfig{
  \begin{center}
    \typerule{Alloc-Authentic}{
      \delta(\src{\loc})=\loc
    }{
      \traceagree{\src{Alloc\ \loc\ n}}{\text{Alloc}\ \loc}
    }{tms-alloc-auth}
    %
    \typerule{Dealloc-Authentic}{
      \delta(\src{\loc})=\loc
    }{
    \traceagree{\src{Dealloc\ \loc\ n}}{\text{Dealloc}\ \loc}
    }{tms-dealloc-auth}
    %
    \typerule{Get-Authentic}{
      \delta(\src{\loc})=\loc
    }{
    \traceagree{\src{Get\ \loc\ n}}{\text{Use}\ \loc}
    }{tms-get-auth}
    %
    \typerule{Set-Authentic}{
      \delta(\src{\loc})=\loc
    }{
      \traceagree{\src{Set\ \loc\ n\ v}}{\text{Use}\ \loc}
    }{tms-set-auth}
    %
    \typerule{Refl-Authentic}{
    }{
    \traceagree{\src{\hole{\cdot}}}{\hole{\cdot}}
    }{tms-refl-auth}
    %
    \typerule{Trans-Authentic}{
      \traceagree{\src{a}}{a} &
      \traceagree{\src{\bar{a}}}{\bar{a}}
    }{
      \traceagree{\src{a}\cdot\src{\bar{a}}}{a\cdot\bar{a}}
    }{tms-trans-auth}
  \end{center}
...
\myfig{
  \begin{center}
    \typerule{Empty-Agree}{
      ...
      \storeagree{\emptyset}{\src{\hole{\cdot}}}
    }{store-empty-agree}
    %
    \typerule{Cons-Agree}{
      \delta\left(\src{\loc}\right)=\loc&
      \storeagree{T}{\src{\Delta}}
    }{
      \storeagree{\left\{\loc\right\}\cup T}{\src{x\mapsto\loc^\poisonless,\Delta}}
    }{store-cons-agree}
    %
    \typerule{Ignore-Agree}{
      ...
      \storeagree{T}{\src{x\mapsto\loc^\poisoned,\Delta}}
    }{store-ignore-agree}
  \end{center}
...
% C like memory model, explain why! (ptr + 5) and later (ptr + 5) objects might not be the same

\begin{lemma}[$\mmlA$ is not spatially memory safe]
  There is a well-typed $\mmlA$ component which does not robustly satisfy \Cref{def:spatmemsafe}:
...
\end{lemma}
\begin{proof}
  Easy induction on $\monitorcheck{T}{T'}{a}$ using \Cref{lem:monitor-step-subset}.
\end{proof}
...
\item[$\src{e}=\src{free\ x}$:]
  If
  \begin{assumptions}
    \item $\typechecks{}{\src{H;\Delta_1,x\mapsto\loc^\poison,\Delta_2}\triangleright \src{free\ x}}{\nat}$
    \item\label[ass]{asm:basetmsmonitor:storeagree} $\storeagree{T}{\src{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}$
  \end{assumptions}
  Then $\exists a\ \delta'\ T',$
  \begin{goals}
    \item\label[goal]{goal:basetmsmonitor:subseteq} $\delta\subseteq\delta'$
    \item\label[goal]{goal:basetmsmonitor:traceagree} $\traceagree[\delta']{a}{\src{Dealloc\ \loc}}$
    \item $\monitorcheck[]{T}{T'}{a}$
    \item $\storeagree[\delta']{T'}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.

  Choose $a=\text{Dealloc\ }\loc$ and $\delta'=\delta$.

  \Cref{goal:basetmsmonitor:subseteq}, $\delta\subseteq\delta$, follows trivially.

  For $\traceagree{\text{Dealloc}\ \loc}{\src{Dealloc\ \loc}}$ (\Cref{goal:basetmsmonitor:traceagree}), apply \Cref{tr:tms-dealloc-auth}, where $\delta(\src{\loc})=\loc$ follows by inverting \Cref{asm:basetmsmonitor:storeagree}.
...
\item[\Cref{tr:store-cons-agree}:]
  If
  \begin{assumptions}
  \item $\typechecks{}{\src{H;\Delta_1},\src{x\mapsto\loc^\poisonless},\src{\Delta_2}\triangleright \src{free\ x}}{\nat}$
  \item $\storeagree{T_1}{\src{\Delta_1}}$
  \item $\delta(\src{\loc})=\loc$
\item $\storeagree{T_2}{\src{\Delta_2}}$
  \item $T=T_1\cup\left\{\loc\right\}\cup T_2$
  \end{assumptions}
  then $\exists T'$,
  \begin{goals}
  \item\label[goal]{goal:basetmsmonitor:monitorcheck1} $\monitorcheck[]{T}{T'}{\text{Dealloc}\ \loc}$
  \item\label[goal]{goal:basetmsmonitor:storeagree1} $\storeagree[\delta']{T'}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.

  Choose $T'=T_1\cup T_2$.

  \Cref{goal:basetmsmonitor:monitorcheck1}, $\monitorcheck[]{T}{T'}{\text{Dealloc}\ \loc}$, follows immediately by \Cref{tr:tms-dealloc}.

  \Cref{goal:basetmsmonitor:storeagree1}, $\storeagree[\delta']{T'}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$, follows immediately by \Cref{tr:store-ignore-agree}.

\item[\Cref{tr:store-ignore-agree}:]
  If
  \begin{assumptions}
  \item\label[ass]{asm:basetmsmonitor:typing} $\typechecks{}{\src{H;\Delta_1},\src{x\mapsto\loc^\poisoned},\src{\Delta_2}\triangleright \src{free\ x}}{\nat}$
  \item $\storeagree{T_1}{\src{\Delta_1}}$
  \item $\delta(\src{\loc})=\loc$
  \item $\storeagree{T_2}{\src{\Delta_2}}$
  \item $T=T_1\cup T_2$
  ...
  then $\exists T'$,
  \begin{goals}
  \item\label[goal]{goal:basetmsmonitor:monitorcheck2} $\monitorcheck[]{T}{T'}{\text{Dealloc}\ \loc}$
  \item\label[goal]{goal:basetmsmonitor:storeagree2} $\storeagree[\delta']{T'}{\src{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}$
  \end{goals}.

  Invert \Cref{asm:basetmsmonitor:typing} to get
  \begin{assumptions}
  \item\label[ass]{asm:basetmsmonitor:storechecks} $\typechecks{}{\src{H;\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}{\Gammas}$
...
\begin{lemma}[$\mmlA$ is \gls{tms} via Monitor]
  If
  \begin{assumptions}
  \item $\typechecks{}{\src{component\ e_p}}{\nat}$
  \item $\typechecks{}{\src{context\ e_1\ e_2}}{\nat\to\nat}$
  \item $\src{prog\ p}=\src{context}\ (\src{e_1})\ (\src{e_2})\src{\linker}{\src{component\ e_p}}$
  \item $\wexecs{prog\ p}{\configs}{v}{\bar{a}}$
  \end{assumptions}
...
\myfig{
  ...
  %
  \mi{Store}~\trg{\Delta} \bnfdef&\ \trg{\hole{\cdot}} \mid \trg{x\mapsto\loc^\poison},\Deltat\\
  %
  \mi{Heaps}~\trg{H} \bnfdef&\ \trg{\hole{\cdot}} \mid \trg{H}::\trg{n} \hspace{0.5cm}
  %
  \mi{State}\ \configt \bnfdef\ \tconfig{H}{\Delta} \\
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \trg{let\ x:=K\ in\ e}\mid \trg{x[K]\leftarrow e}\mid \trg{x[v]\leftarrow K}\mid \trg{let\ x:=new\ K\ in\ e}
    \end{aligned}
  \end{gather*}
}{tw-expr-eval-ectx}{Evaluation Contexts of $\mmlA$.}

\myfig{
  \judgbox{\exect[]{\configt}{K[e]}{\configt'}{K[e']}{a}}{,,Given an evaluation context $\trg{K}$ and an expression $\trg{e}$, it evaluates under\\configuration $\configt$ to $\trg{e'}$ and new configuration $\trg{\configt'}$ in context $\trg{K}$, emitting event $\trg{a}$.''}
  \judgbox{\exprevalt{\configt}{e}{\configt'}{e'}{a}}{,,Expression $\trg{e}$ evaluates under configuration $\configt$ to $\trg{e'}$ and new configuration $\trg{\configt'}$,\\emitting event $\trg{a}$.''}
  %
  \typerule{$e-\text{ctx}$}{
  ...
  \typerule{$e-\trg{get}$}{
    ...
    \exprevalt{\tconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{x[n]}{\tconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{v}{Read\ \loc\ n}
  }{tw-e-get}
  %
  \typerule{$e-\trg{set}$}{
    \trg{H'} = \trg{H}(\trg{\loc+n}\mapsto \trg{v})
  }{
    \exprevalt{\tconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{x[n]\leftarrow v}{\tconfig{H'}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{v}{Set\ \loc\ n\ v}
  }{tw-e-set}
  %
  ...
  \typerule{$e-\trg{delete}$}{
    ...
    \exprevalt{\tconfig{H}{\Delta_1,x\mapsto\loc^\poison,\Delta_2}}{delete\ x}{\tconfig{H}{\Delta_1,x\mapsto\loc^\poisoned,\Delta_2}}{0}{Dealloc\ \loc}
  }{tw-e-let-new}
  %
  \typerule{$e-\trg{new}$}{
    \text{fresh}\ \trg{\loc}&
  ...
  \exprevalt{\tconfig{H}{\Delta}}{let\ x=new\ n\ in\ e}{\tconfig{H'}{z\mapsto \loc^\poisonless,\Delta}}{e[z/x]}{Alloc\ \loc\ n}
  }{tw-e-let-new}
  %
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \mi{Abstract\ Store}~T :=\{A : L, F : L\} \\
      \mi{Abstract\ Events}~a \bnfdef& \emptyevent \mid \text{Alloc}\ \loc \mid \text{Dealloc}\ \loc \mid \text{Use}\ \loc \\
      \mi{References\ Agreement}~\delta :& \trg{L}\to L \\[0.6cm]
    \end{aligned}
  \end{gather*}
