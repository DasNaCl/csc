...
\myfig{
  ...
  \mi{Store}~\src{\Delta} \bnfdef\ \src{\hole{\cdot}} \mid \Deltas,\src{x\mapsto\loc^\poison}\\
  %
...
\myfig{
  ...
  \typerule{ptrRSplit}{
    ...
    \src{x:\ptr},\Gammas = \src{\Gamma_2}\circ\src{x:\ptr},\src{\Gamma_2}
  }{t-ptr-r-split}
  \end{center}
...
\myfig{
  ...
  \typerule{$t-set$}{
    \typechecks{\Gamma_1}{x}{\wptr} &
    \typechecks{\Gamma_2}{e_1}{\nat} &
    \typechecks{\Gamma_3}{e_2}{\nat}
  }{
...
\myfig{
  \judgbox{\exprevals{\configs}{K[e]}{\configs'}{K[e']}{a}}{,,Given an evaluation context $\src{K}$ and an expression $\src{e}$, it evaluates under\\configuration $\configs$ to $\src{e'}$ and new configuration $\src{\configs'}$ in context $\src{K}$, emitting event $\src{a}$.''}
  ...
  \typerule{$e-ctx$}{
    ...
    \exprevals{\configs}{K[e]}{\configs'}{K[e']}{a}
  }{w-e-ectx}
  %
  \typerule{$e-\src{\oplus}$}{
  ...
  \exprevals{\config{H}{A}{\Delta}}{let\ x=new\ n\ in\ e}{\config{H'}{z\mapsto \loc,\Delta}}{e[z/x]}{Alloc\ \loc\ n}
  }{w-e-let-new}
  %
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \mi{Abstract\ Store}~T \subseteq L \\
      \mi{Abstract\ Events}~a \bnfdef& \emptyevent \mid Alloc\ \loc \mid Dealloc\ \loc \mid Use\ \loc \\
      \mi{References\ Agreement}~\delta :& \src{L}\to L
    \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    \typerule{Alloc-Authentic}{
      ...
      \traceagree{\src{Alloc\ \loc\ n}}{Alloc\ \loc}
    }{tms-alloc-auth}
    %
    \typerule{Dealloc-Authentic}{
      \delta(\src{\loc})=\loc
    }{
      \traceagree{\src{Dealloc\ \loc\ n}}{Dealloc\ \loc}
    }{tms-dealloc-auth}
    %
    \typerule{Get-Authentic}{
      \delta(\src{\loc})=\loc
    }{
      \traceagree{\src{Get\ \loc\ n}}{Use\ \loc}
    }{tms-get-auth}
    %
    \typerule{Set-Authentic}{
      \delta(\src{\loc})=\loc
    }{
      \traceagree{\src{Set\ \loc\ n\ v}}{Use\ \loc}
    }{tms-set-auth}
    %
    \typerule{Refl-Authentic}{
    }{
    \traceagree{\src{\varepsilon}}{\varepsilon}
    }{tms-refl-auth}
    %
