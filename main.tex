...
\myfig{
  ...
  \mi{Base\ Events}~\src{\event_{b}} \bnfdef&\ \src{Alloc\ \loc\ n} \mid \src{Dealloc\ \loc} \mid \src{Get\ \loc\ n} \mid \src{Set\ \loc\ n} \mid \src{\lightning} \\
...
\myfig{
  ...
  \typerule{$e-\src{get}-\in$}{
    \src{\memstate}=\src{H^{\ctx};H^{\comp};\Delta_1,x\mapsto\loc^{\sandboxtag}_\poison,\Delta_2} &
    \src{\loc}+\src{n}\in\text{dom }\src{H^{\sandboxtag}}
  }{
    \exprevals{\cfstate;\sandboxtag';\memstate}{x[n]}{\cfstate;\sandboxtag';\memstate}{H^{\sandboxtag}(\loc+n)}{Get\ \loc^{\sandboxtag}\ n}
  }{w-e-get-in}
  %
  \typerule{$e-\src{get}-\notin$}{
