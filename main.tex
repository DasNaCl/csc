...
\begin{lemma}[Filters Equal]\label{lem:scctfiltereq}
  If
  \begin{assumptions}
  ...
  Then
  \begin{goals}
    \item $\tospecificevi[^{*}_{\delta_{scct}}]{\trace}=\tospecificevo[^{*}_{\delta_{scct}\circ\delta^{-1}}]{\trace}$
  \end{goals}
\end{lemma}
\begin{proof}
  ...
  \begin{description}
    \case{tr:scct-empty-trace-eq}
      By \Cref{tr:i-scct-filter-empty} we know $\tospecificevi[_{\delta_{scct}}]{\hole{\cdot}}=\specificev{\hole{\cdot}}$ and similarily \Cref{tr:o-scct-filter-empty} we know $\tospecificevo[_{\delta_{scct}\circ\delta^{-1}}]{\hole{\cdot}}=\specificev{\hole{\cdot}}$, which shows the needed equality.
    ...
    \case{tr:scct-cons-trace-eq}
      ...
      \begin{passumptions}{H}
        ...
        \item\label[ass]{asm:scctfiltereq:indu:eq} $\xlangtraceeqc[\delta]{\trace}{\trace}$
        \item $\irl{\sandboxtag}=\obj{\sandboxtag}$
        \item\label[ass]{asm:scctfiltereq:indu:eventeq} $\xlangeventeqc{\event[_{b}]}{\event[_{b}]}$
      \end{passumptions}
      and
      \begin{passumptions}{\IH}
        \item\label[ass]{asm:scctfiltereq:indu:hyp} $\tospecificevi[^{*}_{\delta_{scct}}]{\trace}=\tospecificevo[^{*}_{\delta_{scct}\circ\delta^{-1}}]{\trace}$
      \end{passumptions}
      Then
      \begin{goals}
        \item $\tospecificevi[^{*}_{\delta_{scct}}]{\event[_{b}];\sandboxtag\cdot\trace}=\tospecificevo[^{*}_{\delta_{scct}\circ\delta^{-1}}]{\event[_{b}];\sandboxtag;\sigma\cdot\trace}$
      \end{goals}

      By distributivity\MK{technically, this needs a lemma that says $\theta(\irl{\event\cdot\trace})=\theta(\irl{\event})\cdot\theta(\irl{\trace})$} and transitivity, we split the goal:

      \begin{goals}
        \item\label[goal]{goal:scct:filtereq:eventeq} $\tospecificevi[^{*}_{\delta_{scct}}]{\event[_{b}];\sandboxtag}=\tospecificevo[^{*}_{\delta_{scct}\circ\delta^{-1}}]{\event[_{b}];\sandboxtag;\sigma}$
        \item\label[goal]{goal:scct:filtereq:traceeq} $\tospecificevi[^{*}_{\delta_{scct}}]{\trace}=\tospecificevo[^{*}_{\delta_{scct}\circ\delta^{-1}}]{\trace}$
      \end{goals}

      Obviously, \Cref{asm:scctfiltereq:indu:hyp} solves \Cref{goal:scct:filtereq:traceeq}.

      \noindent
      For \Cref{goal:scct:filtereq:eventeq}, perform case analysis on \Cref{asm:scctfiltereq:indu:eventeq}.
      We leave cases with $\irl{\sandboxtag}=\irl{\ctx}$ out, they are easy by \Cref{tr:o-scct-filter-context,tr:i-scct-filter-context}.
