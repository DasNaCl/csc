...
\myfig{
  \begin{center}
    ...
    \typerule{syntactic-plugging}{
      \operatorname{dom}\src{\library_1}\downarrow\cap\operatorname{dom}\src{\library_2}\downarrow = \emptyset &
      \src{\library}\equiv\src{\library_1}\cup\src{\library_2}
    }{
...
\myfig{
  ...
  \mi{Events}~\src{\event} \bnfdef& \src{\emptyevent} \mid \src{Alloc\ \loc\ n} \mid \src{Dealloc\ \loc} \mid \src{Get\ \loc\ n} \mid \src{Set\ \loc\ n} \\
                             & \src{\lightning} \mid \src{Call\ ?foo\ n} \mid \src{Ret\ !n} \\
  \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{center}
    ...
    \typerule{$e-\text{ctx}-\src{call}$}{
      \src{\Omega}=\src{\library;\kontstack;H;\Delta} &
      \src{\library}=\src{\library_1,}(\src{let\ foo\ x:\type}_\lambda\src{:=e})\src{,\library_2} \\
      \src{\Omega'}=\src{\library;K,\kontstack;H;\Delta}
    }{
      \execs[]{\Omega}{K[call\ foo\ \finalexprnoerr]}{\Omega'}{return\ e[x/\finalexprnoerr]}{Call\ ?foo\ f}
    }{w-e-ectx-call}
    %
...
\myfig{
  \begin{center}
    \judgbox{\wexecs{prog\ \library}{\configs}{\finalexpr}{\trace}}{,,Run $\mmlAs$ program $\src{prog\ \library}$, giving dynamic state $\configs$\\ and emitting trace $\src{\trace}$.''}
  \typerule{$e-\text{wprog}$}{
    \vdash\src{prog\ \library}\operatorname{ok} &
    \execs{\library;\hole{\cdot};\hole{\cdot};\hole{\cdot}}{call\ main\ 0}{\Omega}{\finalexpr}{\trace}
  }{
    \wexecs{prog\ \library}{\configs_1}{\finalexpr}{\trace}
  }{ws-e-prog}
  \end{center}
...
\myfig{
  \begin{center}
    \judgbox{\delta_{MS}(\src{\ell})=\specificev{\ell}}{,,Map $\mmlAs$ locations $\src{\ell}$ to abstract locations $\specificev{\ell}$.''}
