...
\begin{scontents}[store-env=buffer]
  If
  \begin{assumptions}
  \item\label[ass]{asm:rtmsp:srctms} $\forall \src{\library_1}\ \src{\library_2}\ \src{\library}\ \src{\commlib}\ \src{\trace}\ \src{\Omega}\ \src{v}, \src{\library_1\linker\library_2}=\src{\library} \rightarrow \wexecs{prog\ \commlib\ \library}{\configs}{v}{\trace}\rightarrow\texttt{TMS}(\tospecificevs[_{\emptyset}]{\trace})$
  \item $\trg{\library_1\linker}\mmlAmmlAtcomp{\library_2}=\trg{\library}$
  \item\label[ass]{asm:rtmsp:trgexec} $\wexec{\trg{prog\ }\mmlAmmlAtcomp{\commlib}\ \trg{\library}}{\trg{\configt}}{\trg{v}}{\trg{\trace}}$
  \end{assumptions}
  then
  \begin{goals}
