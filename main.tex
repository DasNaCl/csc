...
\begin{incompleteproof}
  ...
  Using \thmref{lem:nonstuck:traces} on \Cref{asm:backtranscorr:compexecbase} allows us to use \thmref{lem:comptrace:invertible} and \thmref{lem:invertible:opstate}, so $\trg{\overline{a_p}}=\mmlAmmlAtcomp{\overline{a_p}}_\delta$, $\trg{\Omega_0}=\mmlAmmlAtcomp{\Omega_0}_\delta$, and $\trg{\Omega_p}=\mmlAmmlAtcomp{\Omega_p}_\delta$.
  ...
  have lemma where backtranslation always results in well typed context

  combine that with well-typed component $\src{e_p}$

  goal (i) trivial

  goal (ii) no idea. we get the $\tmstraceagree[\delta^{\color{\stlccol}\bullet}]{\src{\overline{a}}}{\overline{a}}$ part by \thmref{thm:mmlAs:tmssafe} but no idea what to do about $\tmstraceagree[\delta^{\color{\ulccol}\bullet}]{\trg{\overline{a}}}{\overline{a}}$.
