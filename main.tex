...
% move the x -> L to Δ and L -> n to A
% assign should not have the universal quantification
% add an assignment operation
...
\myfig{
  ...
  %
  \typerule{$t-\src{let}$}{
    \typechecks{\Gamma}{e_1}{\tau}{\Gamma} &
    \typechecks{\Gamma,x:\tau}{e_2}{\tau}{\Gamma'} &
    \src{x:\tau}\notin\src{\Gamma'}\vee \src{x:\nat}\in\src{\Gamma'}
  }{
...
\myfig{
  \judgbox{\typechecks{}{C}{\tau_C}{}}{,,The context $\src{C}$ has type $\src{\tau_{C}}$.''}
  ...
  \typechecks{}{let\ y=let\ x=\hole{\cdot}\ in\ e_1\ in\ e_2}{\nat\to\nat}{}
	}{w-t-prog}
  \end{center}
...
\myfig{
  ...
  \typerule{$e-var$}{
    ...
    \exprevals{\configs}{x}{\configs}{v}{\emptyevent}
  }{w-e-var}
  %
  \typerule{$e-\src{\oplus}$}{
  ...
  \typerule{$e-\src{new}$}{
    ...
    \exprevals{\config{H}{A}{\Delta}}{new\ n}{\config{H'}{A'}{\Delta}}{\loc}{Alloc\ \loc}
  }{w-e-let-new}
  %
...
% add spatial memsafety
% have some connection between Γ and Δ
...
\begin{lemma}[$\mmlA$ is temporal memory safe]
  For any $\mmlA$ program $\src{p}$, it robustly satisfies \Cref{def:tempmemsafe}: $\rsat{\src{p}}{\src{tmsafe}}$
\end{lemma}
\begin{proof}
   TODO
\end{proof}
