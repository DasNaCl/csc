...
\myfig{
  ...
  \typerule{$t-\src{if}$}{
    \typechecks{\Gamma_1}{e_1}{\nat} &
    \typechecks{\Gamma_2}{e_2}{\nat} &
    \typechecks{\Gamma_3}{e_3}{\tau} &
    \typechecks{\Gamma_3}{e_4}{\tau}
  }{
    \typechecks{\Gamma_1\circ\Gamma_2\circ\Gamma_3}{if\ e_1 < e_2\ then\ e_3\ else\ e_4}{\tau}
  }{w-t-if}
  \end{center}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\src{a} \bnfdef& \src{\emptyevent} \mid \src{Alloc\ \loc\ n} \mid \src{Dealloc\ \loc} \mid \src{Get\ \loc\ n} \mid \src{Set\ \loc\ n} \\
...
\myfig{
  \judgbox{\execs[]{\configs}{K[e]}{\configs'}{K[e']}{a}}{,,Given an evaluation context $\src{K}$ and an expression $\src{e}$,\\it evaluates under configuration $\configs$ to $\src{e'}$ and new\\configuration $\src{\configs'}$ in context $\src{K}$, emitting event $\src{a}$.''}
...
\myfig{
  ...
  \typerule{$e-\text{wprog}$}{
    ...
    \execs{\configs_0}{e_p[v_0/x]}{\configs_p}{v_p}{\overline{a_p}} &
    \execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\overline{a_1}} &
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\configs_1}{v_1}{\overline{a_0\cdot Call\ ?v_0\cdot a_p\cdot Ret\ !v_p\cdot a_1}}
  }{ws-e-prog}
  \typerule{$e-\text{wprog}-$fail0}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\lightning}{stuck}{\overline{a_0}} \\
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\overline{a_0}}
  }{ws-e-prog-fail0}
  \typerule{$e-\text{wprog}-$fail1}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\overline{a_0}} \\
    \execs{\configs_0}{e_p[v_0/x]}{\configs_p}{v_p}{\overline{a_p}} &
    \execs{\configs_p}{e_1[v_p/y]}{\lightning}{stuck}{\overline{a_1}} &
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\overline{a_0\cdot Call\ ?v_0\cdot a_p\cdot Ret\ !v_p\cdot a_1}}
  }{ws-e-prog-fail1}
  \end{center}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\src{a_{ms}} \bnfdef&\ \specificevs{\emptyevent} \mid \specificevs{Alloc\ \loc\ n} \mid \specificevs{Dealloc\ \loc} \mid \specificevs{Get\ \loc\ n} \mid \specificevs{Set\ \loc\ n} \\
...
\myfig{
  \begin{center}
    ...
    \begin{array}{rcl}
      ...
      %
      \tospecificevctxs{\overline{a_0}\cdot Call\ ?n_1\cdot\overline{a_p}\cdot Ret\ !n_2\cdot\overline{a_1}} &\ = & \operatorname{map}\ \thetospecificfilter\ \left(\src{\overline{a_0\cdot Call\ ?n_1\cdot Ret\ !n_2\cdot a_1}}\right)\\
      \tospecificevcomps{\overline{a_0}\cdot Call\ ?n_1\cdot\overline{a_p}\cdot Ret\ !n_2\cdot\overline{a_1}} &\ = & \operatorname{map}\ \thetospecificfilter\ \left(\src{\overline{Call\ ?n_1\cdot a_p \cdot Ret\ !n_2}}\right)\\
    \end{array}
    $$
  \end{center}
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \mi{Abstract\ Events}~a \bnfdef& \emptyevent \mid \text{Alloc}\ \loc \mid \text{Dealloc}\ \loc \mid \text{Use}\ \loc \mid \lightning\\
...
\myfig{
  \begin{center}
    \typerule{TMS-Internal-Authentic}{
      ...
      \tmstraceagree{\specificevs{\varepsilon}}{\varepsilon}
    }{tms-internal-auth}
    %
    ...
    \typerule{TMS-Trans-Authentic}{
      ...
      \tmstraceagree{\src{\overline{a_{ms}}}}{\overline{a}}
    }{
      \tmstraceagree{\src{a}\cdot\src{\overline{a_{ms}}}}{a\cdot\overline{a}}
    }{tms-trans-auth}
  \end{center}
...
\begin{definition}[\gls{tms} on Traces]\label{def:tempmemsafe}
  We define the property $\src{tmsafe}(\src{\overline{a_{ms}}})$ as a conjunction of the following conditions:
  \begin{itemize}
    \item $\specificevs{Alloc\ \loc\ s}\le_{\src{\overline{a_{ms}}}}\specificevs{Dealloc\ \loc}$
    \item $\neg(\specificevs{Use\ \loc\ n}\le_{\src{\overline{a_{ms}}}}\specificevs{Alloc\ \loc\ s})$
    \item $\neg(\specificevs{Dealloc\ \loc}\le_{\src{\overline{a_{ms}}}}\specificevs{Use\ \loc\ n})$
    \item $\specificevs{Dealloc\ \loc}\in!\;\src{\overline{a_{ms}}}$
    \item $\specificevs{Alloc\ \loc\ s}\in!\;\src{\overline{a_{ms}}}$
...
\begin{definition}[Spatial Memory Safety]\label{def:spatmemsafe}
  We define the property $\src{smsafe}\left(\src{\overline{a_{ms}}}\right)$ as follows:
  \begin{itemize}
    \item If $\specificevs{Alloc\ \loc\ s}\in\src{\overline{a_{ms}}}$ and $\specificevs{Use\ \loc\ n}\in\src{\overline{a_{ms}}}$, then $\src{n}<\src{s}$
  \end{itemize}
\end{definition}
...
\begin{importanttheorem}[$\mmlAs$ is \gls{tms} via Monitor]
  ...
  Then
  \begin{goals}
  \item $\texttt{TMS}(\tospecificevs{\bar{a}})$
  \end{goals}.
...
\begin{proof}
  ...
  Running the whole program yields trace prefix\\$\src{\trace}=\src{\emptyevent}\cdot\src{Alloc\ \loc\ 42}\cdot\src{Get\ \loc\ 1337}\cdot\src{\emptyevent}\cdot\src{Dealloc\ \loc}\cdot\src{\emptyevent}\cdot\src{\emptyevent}$.\\[0.3cm]
...
\begin{lemma}[Top-Level Progress]\label{lem:toplevel:progress}
  ...
  Then
  \begin{goals}
  \item $\exists \configs\ \src{v}\ \src{\overline{a}}, \wexecs{prog\ e_1\ e_p\ e_2}{\Omega}{v}{\overline{a}}$
  \end{goals}.
\end{lemma}
\begin{incompleteproof}
\end{incompleteproof}
...
\begin{lemma}[Base \gls{tms} via Monitor]
  ...
  \begin{goals}
    \item $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}'$
    \item $\tmstraceagree[\delta']{\tospecificevs{a}}{a}$
    \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{a}$
...
\item[$\src{e}=\src{free\ x}$:]
  ...
  \begin{goals}
    ...
    \item\label[goal]{goal:basetmsmonitor:tmstraceagree} $\traceagree[\delta_{\text{tms}}']{\specificevs{Dealloc\ \loc}}{a}$
    \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{a}$
  ...
  For $\tmstraceagree{\specificevs{Dealloc\ \loc}}{\text{Dealloc}\ \loc}$ (\Cref{goal:basetmsmonitor:traceagree}), apply \Cref{tr:tms-dealloc-auth}, where $\delta_{\text{tms}}(\src{\loc})=\loc$ follows by inverting \Cref{asm:basetmsmonitor:storeagree}.
...
\begin{theorem}[$\mmlAs$ is \gls{tms} via Monitor]\label{thm:mmlAs:tmssafe}
  ...
  Then
  \begin{goals}
  \item $\texttt{TMS}(\tospecificevs{\overline{a}})$
  \end{goals}.
\end{theorem}
\begin{incompleteproof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:mmlatmsviamonitor:execp} $\execs{\configs_0}{e_p[v_0/x]}{\configs_p}{v_p}{\overline{a_p}}$
    \item\label[ass]{asm:mmlatmsviamonitor:exec1} $\execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\overline{a_1}}$
    \item $\src{\overline{a}}=\src{\overline{a_0}}\cdot\src{Call\ ?v_0}\cdot\src{\overline{a_p}}\cdot\src{Ret\ !v_p}\cdot\src{\overline{a_1}}$
  \end{passumptions}
  ...
  \end{equation}\MK{need importance for store to be able to relate $\src{\Omega_0.\Delta}$ with $\emptyset$}
  ...
  \begin{passumptions}{F}
    ...
    \item\label[ass]{asm:mmlatmsviamonitor:traceagreeasm} $\tmstraceagree[\delta^{(p)}]{\tospecificevs{\overline{a_p}}}{\overline{a_p}}$
  ...
  \begin{goals}
    \item\label[goal]{asm:mmlatmsviamonitor:traceagree} $\tmstraceagree[\delta^{(p)}]{\overline{a_p}}{\src{\overline{a_0\cdot a_p\cdot a_1}}}$
...
\myfig{
  \begin{center}
    ...
    \typerule{SMS-Trans-Authentic}{
      ...
      \smstraceagree{\trg{\overline{a_{ms}}}}{\overline{a}}
    }{
      \smstraceagree{\specificevt{a}\cdot\trg{\overline{a_{ms}}}}{a\cdot\overline{a}}
    }{sms-trans-auth}
  \end{center}
...
\begin{definition}[Trace is temporal memory safe via monitor.]\label{def:trg:trace:tms}
  We say $\texttt{TMS}(\trg{\overline{a_{ms}}})$ iff $\exists \delta\ \overline{a}\ \tmsmonitor$ such that:
  \begin{itemize}
    \item $\tmstraceagree{\trg{\overline{a_{ms}}}}{\overline{a}}$
...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-alloc}{
      \Game'=\Game[\trg{\loc}] &
      \mmlAmmlAtbackdetail[\Game']{\overline{a_{ms}}} = \Game'',\src{e} &
    }{
      \mmlAmmlAtbackdetail{\specificevt{Alloc\ \loc\ n}\cdot\overline{a_{ms}}} = \Game'', \src{let\ }\Game'(\trg{\loc})\src{ = new\ }\mmlAmmlAtbackv{n}\src{\ in\ e}
    }{backtrans-alloc}
    %
    \typerule{backtrans-dealloc}{
      ...
      \mmlAmmlAtbackdetail{\specificevt{Dealloc\ \loc}\cdot\overline{a_{ms}}} = \Game'', \src{delete\ }\Game'(\trg{\loc})\src{; e}
    }{backtrans-dealloc}
    %
    \typerule{backtrans-use}{
      ...
      \mmlAmmlAtbackdetail{\specificevt{Use\ \loc\ n}\cdot\overline{a_{ms}}} = \Game'',\src{x[}\mmlAmmlAtbackv{n}\src{];e}
    }{backtrans-use}
    %
    \typerule{backtrans-abort}{
      \mmlAmmlAtbackdetail{\overline{a_{ms}}} = \Game',\src{e}
    }{
      \mmlAmmlAtbackdetail{\specificevt{\lightning}\cdot\overline{a_{ms}}} = \Game',\src{abort();e}
    }{backtrans-abort}
    %
    \typerule{backtrans-internal}{
      \mmlAmmlAtbackdetail{\overline{a_{ms}}} = \Game',\src{e}
    }{
      \mmlAmmlAtbackdetail{\specificevt{\varepsilon}\cdot\overline{a_{ms}}} = \Game',\src{let\ \_ = 42\ in\ e}
    }{backtrans-internal}
    %
