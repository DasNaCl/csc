...
\myfig{
  ...
  & \mid \src{if\ e_1<e_2\ then\ e_3\ else\ e_4} \mid \src{abort()} 
  ...
  %
  \mi{Final\ Result}\ \src{\finalexpr} \bnfdef\ \src{\valueexpr} \mid \src{stuck} \\
  \end{aligned}
  \end{gather*}
...
%     \typerule{sconsMark}{
%     }{
%     \left(\src{x\mapsto\loc,\Delta}\right)^\src{\diamond} = \src{x\mapsto\loc^\diamond,}
%     }{s-state-cons-mark}
%   \end{center}
% }{w-state-marker}{$\mmlAs$ context unimportance-marking.}
...
\myfig{
  ...
  \typerule{$t-\src{let}$}{
    ...
    \typechecks{\Gamma_1\circ\Gamma_2}{let\ x=e_1\ in\ e_2}{\type}
  }{t-let}
  %
  \typerule{$t-\src{new}$}{
    \typechecks{\Gamma_1}{e_1}{\nat} &
  ...
  \typerule{$t-\src{if}$}{
    \typechecks{\Gamma_1}{e_1}{\nat} &
    \typechecks{\Gamma_2}{e_2}{\nat} &
    \typechecks{\Gamma_3}{e_3}{\type} &
    \typechecks{\Gamma_3}{e_4}{\type}
  }{
    \typechecks{\Gamma_1\circ\Gamma_2\circ\Gamma_3}{if\ e_1 < e_2\ then\ e_3\ else\ e_4}{\type}
  }{w-t-if}
  \end{center}
...
\myfig{
%  \judgbox{\typechecks{\Gamma}{C}{\type_C}{\Gamma'}}{,,Under environment $\Gammas$, the context $\src{C}$ has type $\src{\type_{C}}$ and yields environment $\src{\Gamma'}$.''}
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      \src{let\ x=K\ in\ e}\\
                     ...
                     &\ \mid\src{if\ K<e'\ then\ e_1\ else\ e_2} \mid\src{if\ n<K\ then\ e_1\ else\ e_2}
    \end{aligned}
  \end{gather*}
...
\myfig{
  ...
  %
  \typerule{$e-\src{if}-$less}{
    n_1<n_2
  }{
  \exprevals{\Omega}{if\ n_1<n_2\ then\ e_1\ else\ e_2}{\Omega}{e_1}{\emptyevent}
  }{w-e-if-less}
  %
  \typerule{$e-\src{if}-$noless}{
    n_1\ge n_2
  }{
  \exprevals{\Omega}{if\ n_1<n_2\ then\ e_1\ else\ e_2}{\Omega}{e_2}{\emptyevent}
  }{w-e-if-noless}
  %
  \typerule{$e-\src{abort}$}{
  }{
...
\myfig{
  ...
  \typerule{$e-\text{wprog}$}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\trace{_0}} \\
    \execs{\qconfig_0}{e_p[v_0/x]}{\configs_p}{v_p}{\trace{_p}} &
    \execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\trace{_1}} &
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\configs_1}{v_1}{\trace{_0}\cdot Call\ ?v_0\cdot \trace{_p}\cdot Ret\ !v_p\cdot \trace{_1}}
  }{ws-e-prog}
  \typerule{$e-\text{wprog}-$fail0}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\lightning}{stuck}{\trace{_0}} \\
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\trace{_0}}
  }{ws-e-prog-fail0}
  \typerule{$e-\text{wprog}-$fail1}{
    \typechecks{}{component\ e_p}{\nat\to\nat} &
    \execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\trace{_0}} \\
    \execs{\qconfig_0}{e_p[v_0/x]}{\configs_p}{v_p}{\trace{_p}} &
    \execs{\configs_p}{e_1[v_p/y]}{\lightning}{stuck}{\trace{_1}} &
  }{
    \wexecs{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\trace{_0}\cdot Call\ ?v_0\cdot \trace{_p}\cdot Ret\ !v_p\cdot \trace{_1}}
  }{ws-e-prog-fail1}
  \end{center}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
    \mi{Events}~\src{\mstrace} \bnfdef&\ \specificevs{\emptyevent} \mid \specificevs{Alloc\ \loc\ n} \mid \specificevs{Dealloc\ \loc} \mid \specificevs{Get\ \loc\ n} \mid \specificevs{Set\ \loc\ n} \\
                                    & \mid \specificevs{\lightning} \mid \specificevs{Call\ ?n} \mid \specificevs{Ret\ !n}\\
  \end{aligned}
  \end{gather*}
}{w-specific-events}{\gls*{ms}-Specific Events of $\mmlAs$.}
\myfig{
  \begin{center}
    $$
    \begin{array}{rcl}
      \tospecificevs{Alloc\ \loc\ n} &\ = & \specificevs{Alloc\ \loc\ n} \\
      \tospecificevs{Dealloc\ \loc} &\ = & \specificevs{Dealloc\ \loc} \\
      \tospecificevs{Get\ \loc\ n} &\ = & \specificevs{Use\ \loc\ n} \\
      \tospecificevs{Set\ \loc\ n} &\ = & \specificevs{Use\ \loc\ n} \\
      \tospecificevs{Call\ ?n} &\ = & \specificevs{Call\ ?n} \\
      \tospecificevs{Ret\ !n} &\ = & \specificevs{Ret\ !n} \\
      \tospecificevs{\lightning} &\ = & \specificevs{\lightning} \\[0.3cm]
      %
      \tospecificevctxs{\trace{_0}\cdot Call\ ?n_1\cdot\trace{_p}\cdot Ret\ !n_2\cdot\trace{_1}} &\ = & \operatorname{map}\ \thetospecificfilter\ \left(\src{\trace{_0}\cdot Call\ ?n_1\cdot Ret\ !n_2\cdot \trace{_1}}\right)\\
    \tospecificevcomps{\trace{_0}\cdot Call\ ?n_1\cdot\trace{_p}\cdot Ret\ !n_2\cdot\trace{_1}} &\ = & \operatorname{map}\ \thetospecificfilter\ \left(\src{Call\ ?n_1\cdot \trace{_p} \cdot Ret\ !n_2}\right)\\
    \end{array}
    $$
  \end{center}
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \mi{Abstract\ Events}~\event \bnfdef& \text{Alloc}\ \loc \mid \text{Dealloc}\ \loc \mid \text{Use}\ \loc \mid \lightning\\
      ...
      \event\not\in \tmsmonitor['] &\text{ iff } \event \not\in {\tmsmonitor['.A]} \wedge \event \not\in {\tmsmonitor['.F]}\\
...
\myfig{
  \begin{center}
    ...
    %
    \typerule{TMS-Trans}{
    \monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\event}&
    \monitorcheck{\tmsmonitor[']}{\tmsmonitor['']}{\trace}&
    }{
    \monitorcheck{\tmsmonitor}{\tmsmonitor['']}{\event\cdot\trace}
    }{tms-trans}
  \end{center}
...
\myfig{
  \begin{center}
    ...
    \typerule{TMS-Alloc-Authentic}{
      ...
      \tmstraceagree{\specificevs{Alloc\ \loc\ n}}{\text{Alloc}\ \loc}
    }{tms-alloc-auth}
    %
    \typerule{TMS-Dealloc-Authentic}{
      \delta_{\text{tms}}(\src{\loc})=\loc
    }{
    \tmstraceagree{\specificevs{Dealloc\ \loc}}{\text{Dealloc}\ \loc}
    }{tms-dealloc-auth}
    %
    \typerule{TMS-Use-Authentic}{
      \delta_{\text{tms}}(\src{\loc})=\loc
    }{
      \tmstraceagree{\specificevs{Use\ \loc\ n}}{\text{Use}\ \loc}
    }{tms-use-auth}
    %
    ...
    \typerule{TMS-Trans-Authentic}{
      \tmstraceagree{\src{\msevent}}{\event} &
      \tmstraceagree{\src{\mstrace}}{\trace}
    }{
      \tmstraceagree{\src{\msevent}\cdot\src{\mstrace}}{\event\cdot\trace}
    }{tms-trans-auth}
    %
    \typerule{TMS-Call-Authentic}{
      \tmstraceagree{\src{\mstrace}}{\trace}
    }{
      \tmstraceagree{\specificevs{Call\ ?n}\cdot\src{\mstrace}}{\trace}
    }{tms-call-auth}
    %
    \typerule{TMS-Ret-Authentic}{
      \tmstraceagree{\src{\mstrace}}{\trace}
    }{
      \tmstraceagree{\src{\mstrace}\cdot\specificevs{Ret\ !n}}{\trace}
    }{tms-ret-auth}
  \end{center}
...
\begin{proof}
  ...
  \Cref{def:spatmemsafe} requires $\src{Alloc\ \loc\ 42}\in\src{\trace}$ and $\src{Get\ \loc\ 1337}$, which hold.\\[0.3cm]
  But, $\src{1337}\not<\src{42}$, hence $\nrsat{\src{p}}{\src{smsafe}}$.
\end{proof}
...
\end{lemma}
\begin{proof}
  Easy induction on $\monitorcheck{\tmsmonitor}{\tmsmonitor[']}{\event}$ using \Thmref{lem:monitor-step-subset}.
\end{proof}
...
\begin{lemma}[Progress]
  ...
  Then
  \begin{goals}
  \item $\src{e}=\src{\valueexpr}$\MK{note that we have a $\src{\valueexpr}$ and not $\src{\finalexpr}$. works as long we cannot typecheck $\src{abort}$} or $\exists \src{\Omega'}\ \src{e'}\ \src{\trace}, \execs{\configs}{e}{\configs'}{e'}{\trace}$
  \end{goals}.
\end{lemma}
\begin{incompleteproof}
\end{incompleteproof}
...
\item[$\src{e}=\src{free\ x}$:]
  ...
  \begin{goals}
    ...
    \item\label[goal]{goal:basetmsmonitor:tmstraceagree} $\traceagree[\delta_{\text{tms}}']{\specificevs{Dealloc\ \loc}}{\event}$
    \item $\monitorcheck[]{\tmsmonitor}{\tmsmonitor[']}{\event}$
    \item $\storeagreetms[\delta']{\tmsmonitor[']}{\src{\Delta_1,x\mapsto\loc_\poisoned,\Delta_2}}$
  \end{goals}.

  Choose $\event=\text{Dealloc\ }\loc$ and $\delta_{\text{tms}}'=\delta_{\text{tms}}$.

  \Cref{goal:basetmsmonitor:subseteq}, $\delta_{\text{tms}}\subseteq\delta_{\text{tms}}$, follows trivially.

  For $\tmstraceagree{\specificevs{Dealloc\ \loc}}{\text{Dealloc}\ \loc}$ (\Cref{goal:basetmsmonitor:tmstraceagree}), apply \Cref{tr:tms-dealloc-auth}, where $\delta_{\text{tms}}(\src{\loc})=\loc$ follows by inverting \Cref{asm:basetmsmonitor:storeagree}.
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:mmlatmsviamonitor:exec0} $\execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\trace{_0}}$
    \item\label[ass]{asm:mmlatmsviamonitor:execp} $\execs{\qconfig{_0}}{e_p[v_0/x]}{\configs_p}{v_p}{\trace{_p}}$
    \item\label[ass]{asm:mmlatmsviamonitor:exec1} $\execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\trace{_1}}$
    \item\label[ass]{asm:mmlatmsviamonitor:tracedef} $\src{\trace}=\src{\trace{_0}}\cdot\src{Call\ ?v_0}\cdot\src{\trace{_p}}\cdot\src{Ret\ !v_p}\cdot\src{\trace{_1}}$
  \end{passumptions}
  Note that the other cases don't apply, since it is assumed here (\Cref{asm:mmlatmsviamonitor:run}) that the program terminates in a value instead of $\src{stuck}$.
  ...
  \begin{passumptions}{H^t}
    ...
    \item\label[ass]{asm:mmlatmsviamonitor:typep} $\typechecks{x:\nat}{e_p}{\nat}$
    \item\label[ass]{asm:mmlatmsviamonitor:type1} $\typechecks{y:\nat}{e_1}{\nat}$
  \end{passumptions}
  ...
  Now obtain witnesses $\trace{_p}$, $\delta_{\text{TMS}}^{(p)}$, and $\tmsmonitor$ from supplying \Thmref{lem:generaltms:monitor} with \Cref{asm:mmlatmsviamonitor:typesubstx}, \Cref{asm:mmlatmsviamonitor:execp}, and \Cref{asm:mmlatmsviamonitor:agree}, such that:

  \begin{passumptions}{F}
    \item $\emptyset\subseteq\delta_{\text{TMS}}^{(p)}$
    \item\label[ass]{asm:mmlatmsviamonitor:traceagreeasm} $\tmstraceagree[\delta^{(p)}]{\trace{_p}}{\tospecificevs{\trace{_p}}}$
    \item\label[ass]{asm:mmlatmsviamonitor:monitorcheckasm} $\monitorcheck{\emptyset}{\tmsmonitor}{\trace{_p}}$
    \item $\storeagreetms[\delta^{(p)}]{\tmsmonitor}{\src{\Omega_p}.\src{\Delta}}$
  \end{passumptions}

  %%% BIGGGGTODO  AAAAAAAA
  \noindent
  Now instantiate the existential in our goal with $\delta_{\text{TMS}}^{(p)}$, $\trace{_p}$, and $\tmsmonitor$.
  Note that $\tospecificevctxs{\trace}=\specificevs{Call\ ?v_0}\cdot\tospecificevs{\trace{_p}}\cdot\specificevs{Ret\ !v_p}$ by \Cref{asm:mmlatmsviamonitor:tracedef}, so the remaining goals are:

  \begin{goals}
    \item\label[goal]{asm:mmlatmsviamonitor:traceagree} $\tmstraceagree[\delta^{(p)}]{\trace{_p}}{\specificevs{Call\ ?v_0}\cdot\tospecificevs{\trace{_p}}\cdot\specificevs{Ret\ !v_p}}$
    \item\label[goal]{asm:mmlatmsviamonitor:monitorcheck} and $\monitorcheck{\emptyset}{\tmsmonitor}{\trace{_p}}$
  \end{goals}

  For \Cref{asm:mmlatmsviamonitor:traceagree}, note that by \Cref{tr:tms-call-auth} and \Cref{tr:tms-ret-auth}, we can reduce it to $\tmstraceagree[\delta^{(p)}]{\trace{_p}}{\tospecificevs{\trace{_p}}}$, which is fulfilled by \Cref{asm:mmlatmsviamonitor:traceagreeasm}.
...
\myfig{
  ...
  \mi{Expressions}~\trg{e} \bnfdef&\ \trg{x} \mid \trg{v} \mid \trg{e_{1} \oplus e_{2}} \mid \trg{x[e]} \mid \trg{let\ x = e_{1}\ in\ e_{2}}\mid \trg{x[e_{1}]\leftarrow e_{2}} \\
    ...
    & \mid \trg{if\ e_1 < e_2\ then\ e_3\ else\ e_4} \mid \trg{abort()} \\
  ...
  \begin{gather*}
    \begin{aligned}
      ...
      \trg{let\ x=K\ in\ e}\\
                     ...
                     &\mid \trg{if\ K < e_1\ then\ e_2\ else\ e_3} \mid \trg{if\ n < K\ then\ e\ else\ e_4}
    \end{aligned}
  \end{gather*}
...
\myfig{
  \begin{gather*}
  \begin{aligned}
  \mi{Events}~\trg{a} \bnfdef& \trg{\emptyevent} \mid \trg{Alloc\ \loc\ n} \mid \trg{Dealloc\ \loc} \mid \trg{Get\ \loc\ n} \mid \trg{Set\ \loc\ n}\\
...
\myfig{
  ...
  %
  \typerule{$e-\trg{if}-$less}{
    \trg{n_1}<\trg{n_2}
  }{
    \exprevalt{\configt}{if\ n_1<n_2\ then\ e_1\ else\ e_2}{\configt}{e_1}{\emptyevent}
  }{tw-e-if-less}
  %
  \typerule{$e-\trg{if}-$noless}{
    \trg{n_1}\ge\trg{n_2}
  }{
    \exprevalt{\configt}{if\ n_1<n_2\ then\ e_1\ else\ e_2}{\configt}{e_2}{\emptyevent}
  }{tw-e-if-less}
  %
  \typerule{$e-\trg{abort}$}{
  }{
...
\myfig{
  \begin{center}
    \judgbox{\exect{\configt}{e}{\configt'}{e'}{\overline{a}}}{,,Expression $\trg{e}$ evaluates under configuration $\configt$ to $\trg{e'}$ and\\new configuration $\trg{\configt'}$, emitting list of events $\trg{\overline{a}}$.''}
    %
    \typerule{$et-\text{refl}$}{
    }{
    ...
    \typerule{$et-\text{trans}-$important}{
      \exect[]{\configt}{e}{\configt'}{e'}{a}&
      \exect{\configt'}{e'}{\configt''}{e''}{\overline{a}}&
      \trg{a}\not=\trg{\emptyevent}
    }{
      \exect{\configt}{e}{\configt''}{e''}{a \cdot \bar{a}}
...
\myfig{
  ...
  \typerule{$e-\text{wprog}-\trg{trg}$}{
    \exect{\hole{\cdot};\hole{\cdot}}{e_0}{\Omega_0}{v_0}{\overline{a_0}} \\
    \exect{\Omega_0}{e_p[v_0/x]}{\Omega_p}{v_p}{\overline{a_p}} &
    \exect{\Omega_p}{e_1[v_p/y]}{\Omega_1}{v_1}{\overline{a_1}} &
%    \execs{\hole{\cdot};\hole{\cdot}}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\Omegas}{v}{\overline{a}}
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\Omega_1}{v_1}{\overline{a_0\cdot Call\ ?v_0\cdot a_p\cdot Ret\ !v_p\cdot a_1}}
  }{wt-e-prog}
  \typerule{$e-\text{wprog}-\trg{trg}-$fail0}{
    \exect{\hole{\cdot};\hole{\cdot}}{e_0}{\lightning}{stuck}{\overline{a_0}} \\
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\overline{a_0}}
  }{wt-e-prog-fail0}
  \typerule{$e-\text{wprog}-\trg{trg}-$failp}{
    \exect{\hole{\cdot};\hole{\cdot}}{e_0}{\Omega_0}{v_0}{\overline{a_0}} &
    \exect{\Omega_0}{e_p[v_0/x]}{\lightning}{stuck}{\overline{a_p}} &
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\overline{a_0\cdot Call\ ?v_0\cdot a_p}}
  }{wt-e-prog-failp}
  \typerule{$e-\text{wprog}-\trg{trg}$-fail1}{
    \exect{\hole{\cdot};\hole{\cdot}}{e_0}{\Omega_0}{v_0}{\overline{a_0}} \\
    \exect{\Omega_0}{e_p[v_0/x]}{\Omega_p}{v_p}{\overline{a_p}} &
    \exect{\Omega_p}{e_1[v_p/y]}{\lightning}{stuck}{\overline{a_1}} &
  }{
    \wexect{prog\ e_0\ e_p\ e_1}{\lightning}{stuck}{\overline{a_0\cdot Call\ ?v_0\cdot a_p\cdot Ret\ !v_p\cdot a_1}}
  }{wt-e-prog}
  \end{center}
...
\begin{lemma}[Determinism of Step]\label{lem:determ:step}
  If
  \begin{assumptions}
  \item $\exprevalt{\Omega}{e}{\Omega'}{v_1}{a}$
  \item $\exprevalt{\Omega}{e}{\Omega''}{v_2}{a}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{v'}=\trg{v''}$
...
\begin{lemma}[Determinism of Ctx-Step]\label{lem:determ:ctxstep}
  If
  \begin{assumptions}
  \item $\exect[]{\Omega}{e}{\Omega'}{v_1}{a}$
  \item $\exect[]{\Omega}{e}{\Omega''}{v_2}{a}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{v'}=\trg{v''}$
...
\begin{lemma}[Determinism of Steps]\label{lem:determ:steps}
  If
  \begin{assumptions}
  \item $\exect{\Omega}{e}{\Omega'}{v_1}{\bar{a}}$
  \item $\exect{\Omega}{e}{\Omega''}{v_2}{\bar{a}}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{v'}=\trg{v''}$
...
\begin{definition}[Stuck Trace]\label{def:stuck:trgtrace}
  A trace $\trg{\overline{a}}$ is stuck if it ends with $\trg{\lightning}$.
\end{definition}
...
\begin{lemma}[Non-stuck Trace Generation]\label{lem:nonstuck:traces}
  If
  \begin{assumptions}
  \item $\exect{\Omega}{e}{\Omega'}{v}{\overline{a}}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\trg{\overline{a}}$ is not stuck.
...
\myfig{
  \begin{gather*}
    \begin{aligned}
      ...
      \mi{Abstract\ Events}~a \bnfdef&\ \emptyevent \mid \text{Alloc}\ \loc\ n \mid \text{Dealloc}\ \loc \mid \text{Use}\ \loc\ n \mid \lightning\\
...
\myfig{
  \begin{center}
    \typerule{SMS-Internal-Authentic}{
      ...
      \smstraceagree{\specificevt{\varepsilon}}{\varepsilon}
    }{sms-internal-auth}
    %
    \typerule{SMS-Call-Authentic}{
    }{
      \smstraceagree{\specificevt{Call\ ?n}}{\varepsilon}
    }{sms-call-auth}
    %
    \typerule{SMS-Ret-Authentic}{
    }{
      \smstraceagree{\specificevt{Ret\ !n}}{\varepsilon}
    }{sms-ret-auth}
    %
    \typerule{SMS-Alloc-Authentic}{
      \delta_{\text{sms}}(\trg{\loc})=\loc
    }{
      \smstraceagree{\specificevt{Alloc\ \loc\ n}}{\text{Alloc}\ \loc\ n}
    }{sms-alloc-auth}
    %
    \typerule{SMS-Dealloc-Authentic}{
      \delta_{\text{sms}}(\trg{\loc})=\loc
    }{
    \smstraceagree{\specificevt{Dealloc\ \loc}}{\text{Dealloc}\ \loc}
    }{sms-dealloc-auth}
    %
    \typerule{SMS-Get-Authentic}{
      \delta_{\text{sms}}(\trg{\loc})=\loc
    }{
    \smstraceagree{\specificevt{Get\ \loc\ n}}{\text{Use}\ \loc\ n}
    }{sms-get-auth}
    %
    \typerule{SMS-Set-Authentic}{
      \delta_{\text{sms}}(\trg{\loc})=\loc
    }{
      \smstraceagree{\specificevt{Set\ \loc\ n\ v}}{\text{Use}\ \loc\ n}
    }{sms-set-auth}
    %
    ...
    \typerule{SMS-Trans-Authentic}{
      \smstraceagree{\specificevt{a}}{a} &
      \smstraceagree{\trg{\overline{\mstrace}}}{\overline{a}}
    }{
      \smstraceagree{\specificevt{a}\cdot\trg{\overline{\mstrace}}}{a\cdot\overline{a}}
    }{sms-trans-auth}
  \end{center}
...
\begin{definition}[Trace is temporal memory safe via monitor.]\label{def:trg:trace:tms}
  We say $\texttt{TMS}(\trg{\overline{\mstrace}})$ iff $\exists \delta\ \overline{a}\ \tmsmonitor$ such that:
  \begin{itemize}
    \item $\tmstraceagree{\trg{\overline{\mstrace}}}{\overline{a}}$
    \item and $\monitorcheck{\emptyset}{\tmsmonitor}{\overline{a}}$
  \end{itemize}
\end{definition}
...
\myfig{
  \begin{center}
    ...
    \mmlAmmlAtcomp{if\ e_1<e_2\ then\ e_3\ else\ e_4} &\ = & \trg{if\ }\mmlAmmlAtcomp{e_1}\trg{<}\mmlAmmlAtcomp{e_2}\\
                                                      &&\trg{then\ }\mmlAmmlAtcomp{e_3} \\
                                                      &&\trg{else\ }\mmlAmmlAtcomp{e_4} \\[0.5cm]
    %
    ...
    \typerule{cons-trace-eq}{
      \xlangeventeq{a}{a} &
      \xlangtraceeq{\overline{a}}{\overline{a}}
    }{
      \xlangtraceeq{a\cdot\overline{a}}{a\cdot\overline{a}}
    }{cons-trace-eq}
    %
    ...
    \typerule{cons-locmap-eq}{
      ...
      \xlangstateeq{x\mapsto\loc,\Delta}{x\mapsto\loc,\Delta}
    }{cons-locmap-eq}
  \end{center}
...
\begin{lemma}[Forward Simulation]\label{lem:forwardsim}
  If
  \begin{assumptions}
    \item $\execs{\Omega}{e}{\Omega'}{v}{\bar{a}}$
  \end{assumptions}
  Then $\exists\delta$,
  \begin{goals}
    \item $\exec{\mmlAmmlAtcomp{\Omega}}{\mmlAmmlAtcomp{e}}{\mmlAmmlAtcomp{\Omega'}}{\mmlAmmlAtcomp{v}}{\mmlAmmlAtcomp{\bar{a}}_\delta}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
  induction on $\src{e}$
...
\begin{lemma}[Expression Correctness]\label{lem:expression:correctness}
  If
  \begin{assumptions}
    \item\label[ass]{asm:exprcorrect:exec1:trg} $\exec{\mmlAmmlAtcomp{\Omega}}{\mmlAmmlAtcomp{e}}{\mmlAmmlAtcomp{\Omega'}}{\mmlAmmlAtcomp{v}}{\mmlAmmlAtcomp{\bar{a}}_\delta}$
  \end{assumptions}
  Then
  \begin{goals}
...
\end{lemma}
\begin{proof}
  We prove this by contradiction, so say:
  \begin{passumptions}{H}
    \item\label[ass]{asm:exprcorrect:exec2} $\execs{\Omega}{e}{\Omega''}{v'}{\bar{a}}$
    \item\label[ass]{asm:exprcorrect:unequal1} $\src{v}\not=\src{v'}$
    \item\label[ass]{asm:exprcorrect:unequal2} $\src{\Omega'}\not=\src{\Omega''}$
      \MK{Should also have an $\overline{a'}\not=\overline{a}$}
  \end{passumptions}

  Apply \thmref{lem:forwardsim} on \Cref{asm:exprcorrect:exec2} to get:
  \begin{passumptions}{F}
    \item\label[ass]{asm:exprcorrect:exec2:trg} $\exec{\mmlAmmlAtcomp{\Omega}}{\mmlAmmlAtcomp{e}}{\mmlAmmlAtcomp{\Omega'}}{\mmlAmmlAtcomp{v}}{\mmlAmmlAtcomp{\overline{a}}_\delta}$
  \end{passumptions}

  The compiler is deterministic, so using \Cref{asm:exprcorrect:unequal1} on \thmref{lem:determcomp} gives:
  \begin{passumptions}{G}
    \item\label[ass]{asm:exprcorrect:compiled:unequal} $\mmlAmmlAtcomp{v}\not=\mmlAmmlAtcomp{v'}$
  \end{passumptions}

  Using \Cref{asm:exprcorrect:exec1:trg} and \Cref{asm:exprcorrect:exec2:trg} on \thmref{lem:determ:steps} we have:
  \begin{passumptions}{G}
    \setcounter{enumi}{1}
    \item\label[ass]{asm:exprcorrect:compiled:equal} $\mmlAmmlAtcomp{v}=\mmlAmmlAtcomp{v'}$
  \end{passumptions}
  
  \Cref{asm:exprcorrect:compiled:equal} contradicts \Cref{asm:exprcorrect:compiled:unequal}.
\end{proof}
...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-alloc}{
      \Game'=\Game[\trg{\loc}] &
      \mmlAmmlAtbackdetail[\Game']{\overline{\mstrace}} = \Game'',\src{e} &
    }{
      \mmlAmmlAtbackdetail{\specificevt{Alloc\ \loc\ n}\cdot\overline{\mstrace}} = \Game'', \src{let\ }\Game'(\trg{\loc})\src{ = new\ }\mmlAmmlAtbackv{n}\src{\ in\ e}
    }{backtrans-alloc}
    %
    \typerule{backtrans-dealloc}{
      ...
      \mmlAmmlAtbackdetail{\specificevt{Dealloc\ \loc}\cdot\overline{\mstrace}} = \Game'', \src{delete\ }\Game'(\trg{\loc})\src{; e}
    }{backtrans-dealloc}
    %
    \typerule{backtrans-use}{
      ...
      \mmlAmmlAtbackdetail{\specificevt{Use\ \loc\ n}\cdot\overline{\mstrace}} = \Game'',\src{x[}\mmlAmmlAtbackv{n}\src{];e}
    }{backtrans-use}
    %
    \typerule{backtrans-abort}{
      \mmlAmmlAtbackdetail{\overline{\mstrace}} = \Game',\src{e}
    }{
      \mmlAmmlAtbackdetail{\specificevt{\lightning}\cdot\overline{\mstrace}} = \Game',\src{abort();e}
    }{backtrans-abort}
    %
    \typerule{backtrans-internal}{
      \mmlAmmlAtbackdetail{\overline{\mstrace}} = \Game',\src{e}
    }{
      \mmlAmmlAtbackdetail{\specificevt{\varepsilon}\cdot\overline{\mstrace}} = \Game',\src{let\ \_ = 42\ in\ e}
    }{backtrans-internal}
    %
%    \typerule{backtrans-call}{
%    }{
%      \mmlAmmlAtbackdetail{\specificevt{Call\ ?n}} = \mmlAmmlAtbackv{n}
    % TODO: intuitively, we want `let x = n in call foo x`
%    }{backtrans-call}
    %
%    \typerule{backtrans-ret}{
%    }{
%    \mmlAmmlAtbackdetail{\specificevt{Ret\ !n}} = \mmlAmmlAtbackv{n}
%    }{backtrans-ret}
    %
    \typerule{backtrans-toplevel}{
      \tospecificevctxt{\overline{a}} = \trg{\overline{a_0}\cdot Call\ ?n\cdot Ret\ !n\cdot \overline{a_1}} \\
      \mmlAmmlAtbackdetail[\emptyset]{\overline{a_0}} = \Game,\src{e_0} &
      \mmlAmmlAtbackdetail[\Game]{\overline{a_1}} = \Game',\src{e_1} &
    }{
      \mmlAmmlAtback{\overline{a}} = \src{context\ e_0\ e_1}
    }{backtrans-top}
  \end{center}
}{tms-pres-backtrans}{Partial Trace-Based Backtranslation from $\mmlAt$ events to $\mmlAs$ terms.}
...
\begin{lemma}[Compiling Traces Injectivity]\label{lem:comptrace:injective}\ \\
  Given some injective $\delta$, $\mmlAmmlAtcomp{\overline{a}}_\delta$ is injective.
\end{lemma}
\begin{proof}
  Induction on $\src{\overline{a}}$, trivial.
\end{proof}

\begin{lemma}[Non-Stuck Target Traces have Source Traces]\label{lem:comptrace:invertible}\ \\
  Given injective $\delta$ and $\trg{\overline{a}}$ non-stuck, $\exists\src{\overline{a}}, \trg{\overline{a}}=\mmlAmmlAtcomp{\overline{a}}_\delta$.
\end{lemma}
\begin{proof}
  Trivial induction on $\trg{\overline{a}}$ using \thmref{lem:comptrace:injective}.
\end{proof}

\begin{lemma}[Value Insertion]\label{lem:compvals:insertion}\ \\
  Given $\src{v}=\mmlAmmlAtbackv{v}$, then $\mmlAmmlAtcomp{v}=\trg{v}$.
\end{lemma}
\begin{proof}
  Immediate.
\end{proof}
...
\begin{lemma}[Invertible Operational State]\label{lem:invertible:opstate}\ \\
  Given injective $\delta$ and $\trg{\Omega}$ non-stuck, $\exists\src{\Omega}, \trg{\Omega}=\mmlAmmlAtcomp{\Omega}_\delta$.
\end{lemma}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{lemma}[Backtranslation is Well-Typed]\label{lem:backtranslation:welltyped}
  If
  \begin{assumptions}
  \item $\trg{\overline{a}}$ not stuck
  \end{assumptions}
  Then
  \begin{goals}
  \item $\typecheck{}{\mmlAmmlAtback{\overline{a}}}{\src{\nat\to\nat}}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
...
\begin{lemma}[Backtranslation Correctness]\label{lem:backtranslation:correct}
  If
  \begin{assumptions}
  \item\label[ass]{asm:backtranscorr:progexec} $\wexec{\trg{prog\ e_1}\ \mmlAmmlAtcomp{e_p}\ \trg{\ e_2}}{\trg{\configt}}{\trg{v}}{\trg{\overline{a}}}$
  \item\label[ass]{asm:backtranscorr:component:typechecks} $\typechecks{}{component\ e_p}{\nat}$
  \end{assumptions}
  then $\exists \configs\ \src{v}\ \src{\overline{a}}\ \delta_{tms}$,
  \begin{goals}
  \item $\wexecs{prog\ e_1\ e_p\ e_2}{\configs}{v}{\overline{a}}$
  \item $\tmstraceagree{\tospecificevs{\overline{a}}}{\tospecificevt{\overline{a}}}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
  Inverting \Cref{asm:backtranscorr:progexec} gives:
  \begin{passumptions}{H}
  \item $\exect{\hole{\cdot};\hole{\cdot}}{e_0}{\Omega_0}{v_0}{\overline{a_0}}$
  \item\label[ass]{asm:backtranscorr:compexecbase} $\exec{\trg{\Omega_0}}{\mmlAmmlAtcomp{e_p}\trg{[v_0/x]}}{\trg{\Omega_p}}{\trg{v_p}}{\trg{\overline{a_p}}}$
  \item $\exect{\Omega_p}{e_1[v_p/y]}{\Omega_1}{v_1}{\overline{a_1}}$
  \end{passumptions}

  By \thmref{lem:compvals:invertible} we have $\mmlAmmlAtcomp{v_0}=\trg{v_0}$ and $\mmlAmmlAtcomp{v_p}=\trg{v_p}$.

  Also note that $\mmlAmmlAtcomp{x}=\trg{x}$ by definition of the compiler.

  We build a $\delta_\times:\src{L}\to\trg{L}$ by going over $\trg{\overline{a_0\cdot a_p\cdot a_1}}$ and associating each $\trg{\loc}$ for any event with a corresponding $\src{\loc}$.\MK{fishy. intuitively, this is what we should get out of backtranslation correctness, but we need it to change our assumption suitably so that we can compile $\src{\Omega}$ and $\src{\overline{a}}$}

  Using \thmref{lem:nonstuck:traces} on \Cref{asm:backtranscorr:compexecbase} allows us to use \thmref{lem:comptrace:invertible} and \thmref{lem:invertible:opstate} provided $\delta_\times$, so $\trg{\overline{a_p}}=\mmlAmmlAtcomp{\overline{a_p}}_{\delta_\times}$, $\trg{\Omega_0}=\mmlAmmlAtcomp{\Omega_0}_{\delta_\times}$, and $\trg{\Omega_p}=\mmlAmmlAtcomp{\Omega_p}_{\delta_\times}$.

  Therefore, we can rewrite \Cref{asm:backtranscorr:compexecbase} as follows:
  \begin{passumptions}{H'}
    \setcounter{enumi}{1}
    \item\label[ass]{asm:backtranscorr:compdexecbase} $\exec{\mmlAmmlAtcomp{\Omega_0}}{\mmlAmmlAtcomp{e_p}\trg{[}\mmlAmmlAtcomp{v_0}\trg{/}\mmlAmmlAtcomp{x}\trg{]}}{\mmlAmmlAtcomp{\Omega_p}}{\mmlAmmlAtcomp{v_p}}{\mmlAmmlAtcomp{\overline{a_p}}}$
  \end{passumptions}

  Plug \Cref{asm:backtranscorr:compdexecbase} into \thmref{lem:expression:correctness}:
  \begin{passumptions}{G}
    \item $\execs{\Omega_0}{e_p[v_0/x]}{\Omega_p}{v_p}{\src{\overline{a_p}}}$
  \end{passumptions}

  By \thmref{lem:backtranslation:welltyped} $\src{context\ e_1\ e_2}=\mmlAmmlAtback{\overline{a}}$ well-typed.

  Using the previous result and \Cref{asm:backtranscorr:component:typechecks} we use \thmref{lem:plugging:welltyped}:
  \begin{passumptions}{G}
    \setcounter{enumi}{1}
    \item\label[ass]{asm:progtypechecks} $\typechecks{}{prog\ e_1\ e_p\ e_2}{\nat}$
  \end{passumptions}
  
  Shove \Cref{asm:progtypechecks} into \thmref{lem:toplevel:progress} to give us what we need to instantiate the existentials in our goal upto $\delta_{tms}$:
  \begin{passumptions}{G}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:progruns} $\wexecs{prog\ e_1\ e_p\ e_2}{\Omega}{v}{\overline{a}}$
  \end{passumptions}
  
  \Cref{asm:progruns} has exactly the same shape as one part of the remaining goal, so what is left to show is

  \begin{goals}
  \item $\exists \delta_{tms}, \tmstraceagree{\src{\overline{a}}}{\trg{\overline{a}}}$\MK{instead of relating them over some abstract $\overline{a}$ we may just need to embrace a cross-language relation}
  \end{goals}

  we get the $\tmstraceagree[\delta^{\color{\stlccol}\bullet}]{\src{\overline{a}}}{\overline{a}}$ part by \thmref{thm:mmlAs:tmssafe} but no idea what to do about $\tmstraceagree[\delta^{\color{\ulccol}\bullet}]{\trg{\overline{a}}}{\overline{a}}$.
...
\begin{theorem}[Robust \gls{tms} Preservation]
  If
  \begin{assumptions}
  \item\label[ass]{asm:rtmsp:srctms} $\forall \src{e_1}\ \src{e_2}\ \src{\overline{a}}\ \src{\Omega}\ \src{v},\wexecs{prog\ e_1\ e_p\ e_2}{\configs}{v}{\overline{a}}\rightarrow\texttt{TMS}(\tospecificevs{\overline{a}})$
  \item\label[ass]{asm:rtmsp:trgexec} $\wexec{\trg{prog\ e_1}\ \mmlAmmlAtcomp{e_p}\ \trg{e_2}}{\trg{\configt}}{\trg{v}}{\trg{\overline{a}}}$
  \end{assumptions}
  then
  \begin{goals}
  \item $\texttt{TMS}(\tospecificevt{\overline{a}})$
  \end{goals}
\end{theorem}
\begin{proof}
  We get $\typechecks{}{component\ e_p}{\nat}$ from inverting a sutible number of times \Cref{asm:rtmsp:srctms}.
  Using that and backtranslating the trace $\mmlAmmlAtback{\overline{a}} = \src{context\ e_1\ e_2}$, immediately using both together with \Cref{asm:rtmsp:trgexec} to apply \thmref{lem:backtranslation:correct}:
  \begin{passumptions}{H}
  ...
  By \Cref{def:trg:trace:tms}, our goal can be instantiated with ${\delta^{\color{\ulccol}\bullet}}_{tms}$, $\overline{a}$, and $\tmsmonitor$, so what is left to show is:
  \begin{goals}
