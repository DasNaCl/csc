...
%    \execs{\hole{\cdot};\hole{\cdot}}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\configs}{v}{\overline{a}}
    ...
    \wexecs{prog\ e_0\ e_p\ e_1}{\configs}{v}{\overline{a_0^\diamond\cdot a_p\cdot a_1^\diamond}}
  }{w-e-prog}
  \end{center}
...
\begin{proof}
  ...
  \begin{assumptions}
    ...
    %\item[$H_1$:]\label[ass]{asm:mmlatmsviamonitor:run2} $\execs{\hole{\cdot};\hole{\cdot}}{let\ y=let\ x=e_0^\diamond\ in\ e_p\ in\ e_1^\diamond}{\configs}{v}{\overline{a}}$
    \item[$H_1$:]\label[ass]{asm:mmlatmsviamonitor:exec0} $\execs{\hole{\cdot};\hole{\cdot}}{e_0}{\configs_0}{v_0}{\overline{a_0}}$
    \item[$H_2$:]\label[ass]{asm:mmlatmsviamonitor:execp} $\execs{\configs_0^\diamond}{e_p[v_0/x]}{\configs_p}{v_p}{\overline{a_p}}$
    \item[$H_3$:]\label[ass]{asm:mmlatmsviamonitor:exec1} $\execs{\configs_p}{e_1[v_p/y]}{\configs_1}{v_1}{\overline{a_1}}$
    \item[$H_4$:] $\src{\overline{a}}=\src{\overline{a_0^\diamond}}\cdot\src{\overline{a_p}}\cdot\src{\overline{a_1^\diamond}}$
  \end{assumptions}
  ...
  \begin{assumptions}
    \item[$H^t_0$:]\label[ass]{asm:mmlatmsviamonitor:type0} $\typechecks{\hole{\cdot}}{e_0}{\nat}$
    \item[$H^t_1$:]\label[ass]{asm:mmlatmsviamonitor:typep} $\typechecks{x:nat}{e_p}{\nat}$
    \item[$H^t_2$:]\label[ass]{asm:mmlatmsviamonitor:type1} $\typechecks{y:nat}{e_1}{\nat}$
  \end{assumptions}
...
%  The assumptions of $\Cref{lem:generaltms:monitor}$ are satisfied by \Cref{asm:mmlatmsviamonitor:storetypenil}, $H_1$ (\Cref{asm:mmlatmsviamonitor:exec0}), and \Cref{asm:mmlatmsviamonitor:agreeempty}.

 % Therefore, we get witnesses $\overline{a_0}$, $\delta_{\text{tms}}^{(0)}$, and $\tmsmonitor[^{(0)}]$ such that:
 % \begin{assumptions}
 %   \item $\emptyset\subseteq\delta_{\text{tms}}^{(0)}$
 %   \item $\tmstraceagree[\delta^{(0)}]{\overline{a_0}}{\src{\overline{a_0}}}$
 %   \item $\monitorcheck{\emptyset}{\tmsmonitor[^{(0)}]}{\overline{a_0}}$
 %   \item $\storeagreetms[\delta^{(0)}]{\tmsmonitor[^{(0)}]}{\src{\Omega_0}.\src{\Delta}}$
 % \end{assumptions}

  Plugging $H^t_0$ (\Cref{asm:mmlatmsviamonitor:type0}) and $H_1$ (\Cref{asm:mmlatmsviamonitor:exec0}) into \Cref{lem:steps-preservation} and inverting the result, we obtain
  \begin{assumptions}
    \item $\typechecks{}{\Omega_0.\Delta}{\Gamma_0}$
    \item $\typechecks{\Gamma_0}{v_0}{\nat}$
  \end{assumptions}

  Note that $\src{\Gamma_0} = \src{\hole{\cdot}} \circ \src{\Gamma_0}$.\MK{technical lemma needed}

  Using $H^t_1$ (\Cref{asm:mmlatmsviamonitor:type0}) we apply \Cref{lem:substitution} to obtain:
  \begin{equation}
  ...
  By induction on $\src{\Omega_0^\diamond}$ using \Cref{tr:store-ignore-agree}, we have:
  \begin{equation}
    \storeagreetms[\emptyset]{\emptyset}{\src{\Omega_0.\Delta^\diamond}}\label{asm:mmlatmsviamonitor:agree}
  \end{equation}

  Now obtain witnesses $\overline{a_p}$, $\delta_{\text{tms}}^{(p)}$, and $\tmsmonitor$ from supplying \Cref{lem:generaltms:monitor} with \Cref{asm:mmlatmsviamonitor:typesubstx}, $H_2$ (\Cref{asm:mmlatmsviamonitor:execp}), and \Cref{asm:mmlatmsviamonitor:agree}, such that:

  \begin{assumptions}
    \item $\emptyset\subseteq\delta_{\text{tms}}^{(p)}$
    \item $\tmstraceagree[\delta_{\text{tms}}^{(p)}]{\overline{a_p}}{\src{\overline{a_p}}}$
    \item $\monitorcheck{\emptyset}{\tmsmonitor}{\overline{a_p}}$
    \item $\storeagreetms[\delta_{\text{tms}}^{(p)}]{\tmsmonitor}{\src{\Omega_p}.\src{\Delta}}$
  \end{assumptions}
