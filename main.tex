...
\myfig{
  \begin{center}
    ...
    \mmlAmmlAtcomp{\varepsilon^i,\bar{a}}_\delta&\ = & \trg{\varepsilon,}\mmlAmmlAtcomp{\bar{a}}_\delta\\
    \mmlAmmlAtcomp{(Alloc\ \loc\ n)^i,\bar{a}}_\delta&\ = & \trg{Alloc\ }\delta(\src{\loc})\trg{\ n,}\mmlAmmlAtcomp{\bar{a}}_\delta\\
    \mmlAmmlAtcomp{(Dealloc\ \loc)^i,\bar{a}}_\delta&\ = & \trg{Dealloc\ }\delta(\src{\loc})\trg{,}\mmlAmmlAtcomp{\bar{a}}_\delta\\
    \mmlAmmlAtcomp{(Get\ \loc\ n)^i,\bar{a}}_\delta&\ = & \trg{Get\ }\delta(\src{\loc})\trg{\ n,}\mmlAmmlAtcomp{\bar{a}}_\delta\\
    \mmlAmmlAtcomp{(Set\ \loc\ n)^i,\bar{a}}_\delta&\ = & \trg{Set\ }\delta(\src{\loc})\trg{\ n,}\mmlAmmlAtcomp{\bar{a}}_\delta\\
    \end{array}
    $$
...
\myfig{
  \begin{center}
    ...
    \mmlAmmlAtbackdetail{\specificevt{Ret\ !n}} = \src{if\ y = }\mmlAmmlAtbackv{n}\src{\ then\ 1\ else\ 2}
      %TODO: actually add if-else to src
    }{backtrans-ret}
    $$
    \begin{array}{rll} 
      %
      % TODO: fix top-level backtranslation
