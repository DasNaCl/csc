...
\end{lemma}
\begin{incompleteproof}
  \MK{more detail}Suppose control is never handled over to $\mmlAtmmlAtmscomp{\library_{comp}}$; that is, there is no $\irl{Call\ ?\ foo\ n}$ in $\irl{\trace}$.
...
\begin{definition}[$\CCT$ security]
  ...
  Given $\execo{\Omega_{1}}{e_{1}}{\Omega_{2}}{\finalexprnoerr}{\trace[_{A}]}$ and $\execo{\Omega_{1}'}{e_{1}}{\Omega_{2}'}{\finalexprnoerr}{\trace[_{B}]}$ we say $e_{1}$ is $\CRhash$-secure with respect to $\obj{\zeta}$ iff $(\operatorname{mems-and-branches}(\obj{\trace_{A}}))=(\operatorname{mems-and-branches}(\obj{\trace_{B}}))$
\end{definition}
