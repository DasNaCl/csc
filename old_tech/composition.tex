...
\end{lemma}
\begin{proof}
  For the first conjunct $\rtp{\ttcompN{\ttcomp{\bullet}_{(2)}}_{(1)}}{\cC_{2}\cap\cC_{1}}$: we view the composed compiler as the sequential composition of $\ttcomp{\bullet}_{(1)}$ followed by $\ttcomp{\bullet}_{(2)}$.
  Applying \Thmref{lem:seqcompo} with $\rtp{\ttcomp{\bullet}_{(1)}}{\cC_{1}}$ and $\rtp{\ttcomp{\bullet}_{(2)}}{\cC_{2}}$ yields the result.
  The second conjunct follows symmetrically by applying \Thmref{lem:seqcompo} with the compilers in reversed order.
\end{proof}
...
\end{lemma}
\begin{proof}
  For the first conjunct $\rtpsigma[\sim_1\circ\sim_2]{\ttcompN{\ttcomp{\bullet}_{(2)}}_{(1)}}{\cC_{2}\cap\cC_{1}}$: apply \Thmref{lem:seqcompo:sigma} with the well-formedness $\wfcsig{\sim_2}{\trg{\cC_1}}$, the preservation $\rtpsigma[\sim_1]{\ttcomp{\bullet}_{(1)}}{\sigma_{\sim_2}(\trg{\cC_1})}$, and $\rtpsigma[\sim_2]{\ttcomp{\bullet}_{(2)}}{\trg{\cC_{2}}}$.
  For the second conjunct: apply \Thmref{lem:seqcompo:sigma} symmetrically using $\wfcsig{\sim_1}{\trg{\cC_2}}$, $\rtpsigma[\sim_2]{\ttcomp{\bullet}_{(2)}}{\sigma_{\sim_1}(\trg{\cC_{2}})}$, and $\rtpsigma[\sim_1]{\ttcomp{\bullet}_{(1)}}{\trg{\cC_{1}}}$.
\end{proof}
...
\end{lemma}
\begin{proof}
  Dual to \Cref{lem:swappable:sigma}, using \Thmref{lem:seqcompo:tau} instead.
\end{proof}
