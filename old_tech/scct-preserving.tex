...
\myfig{
  \begin{center}
    ...
    \begin{array}{rll}
      \mmlAtmsmmlAtscctcomp{\hole{\cdot}}&\ = & \obj{\hole{\cdot}} \\
      \mmlAtmsmmlAtscctcomp{F,\library}&\ = & \mmlAtmsmmlAtscctcomp{F}\obj{,}\mmlAtmsmmlAtscctcomp{\library} \\
    \end{array}
    $$
  \end{center}
...
\begin{proof}
  ...
  \begin{description}
    ...
    \case{tr:scct-cons-trace-eq}
      ...
      For cases with $\irl{\sandboxtag}=\irl{\ctx}$: by \Cref{tr:i-scct-filter-context}, we have $\tospecificevi[^{*}_{\delta_{scct}}]{\event[_{b}];\ctx}=\specificev{\varepsilon}$.
      Furthermore, inverting \Cref{asm:scctfiltereq:objtoabsev} with a context sandboxtag yields $\absevent=\bm{\varepsilon}$.
      Thus, \Cref{goal:scctfiltereq:irltoabsev} reduces to showing $\traceagree{\specificev{\varepsilon}}{\bm{\varepsilon}}$, which follows by \Cref{tr:scct-none-auth}.
...
%      \mmlAtmsmmlAtscctsubtopN[\emptyset;\emptyset;\obj{\hole{\cdot}}]{\tospecificevo[^*_\obj{\bullet}]{\trace[_0]}} = \Game; \delta; \obj{\overline{\loc}}; \irl{e_0};\irl{e_0'};\irl{let\ x_0 = new\ n_0\ in\ e_0''} \\
      \mmlAtmsmmlAtscctsubtopN[]{\tospecificevo[^*_\obj{\bullet}]{\trace[_0]}} = \irl{e_0};\irl{e_0'};\irl{e_0''} \\
      \mmlAtmsmmlAtscctsubtopN[]{\tospecificevo[^*_\obj{\bullet}]{\trace[_1]}} = \irl{e_1};\irl{e_1'};\irl{e_1''} \\
...
\begin{lemma}[Backtranslation Correctness of $\obj{Start}$]\label{lem:scct:exprbacktranslation:vbeg:wrapper:correct}
  ...
  \item $\mmlAtmsmmlAtscctcomptoctxN[]{\tospecificevo[_{\obj{\bullet}}]{Start;\comp;\sigma''}}=\irl{42}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{Ret}$]\label{lem:scct:exprbacktranslation:beg:wrapper:correct}
  ...
  \item $\mmlAtmsmmlAtscctcomptoctxN[]{\tospecificevo[_{\obj{\bullet}}]{Ret\ \comptoctx\ \valueexpr;\comp;\sigma}}=\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{lemma}[Middle of Backtranslation Correctness]\label{lem:scct:exprbacktranslation:mid:wrapper:correct}
  ...
  \item\label[ass]{asm:scct:exprbacktranslation:mid:correct:backtrans} $\mmlAtmsmmlAtscctdetailN[]{\tospecificevo[^{*}_{\obj{\bullet}}]{\trace}}=\irl{e}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{Call\ \ctxtocomp}$]\label{lem:scct:exprbacktranslation:end:wrapper:correct}
  ...
  \item $\mmlAtmsmmlAtscctctxtocompN[]{\tospecificevo[_{\obj{\bullet}}]{Call\ \ctxtocomp\ foo\ \valueexpr}}=\irl{call\ foo\ }\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{End}$]\label{lem:scct:exprbacktranslation:vend:wrapper:correct}
  ...
  \item\label[ass]{asm:scct:exprbacktranslation:vend:wrapper:correct:backtrans} $\mmlAtmsmmlAtscctctxtocompN[]{\tospecificevo[_{\obj{\bullet}}]{End\ \valueexpr;\ctx;\sigma}}=\irl{return\ }\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{incompleteproof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:scct:backtrans:subtop1} $\mmlAtmsmmlAtscctsubtopN[]{\tospecificevo[^{*}_{\obj{\bullet}}]{\trace[_{0}]}}=\irl{e_{0};e_{0}';e_{0}''}$
    \item\label[ass]{asm:scct:backtrans:subtop2} $\mmlAtmsmmlAtscctsubtopN[]{\tospecificevo[^{*}_{\obj{\bullet}}]{\trace[_{1}]}}=\irl{e_{1};e_{1}';e_{1}''}$
  \end{passumptions}
  ...
  \end{passumptions}
  This reduces further in three steps: the first two steps evaluate the leading $\irl{42;42}$ pairs using the let-binding and projection rules, each producing an empty event since values in sequence position are discarded.
  The third step evaluates $\irl{call\ foo\ \valueexpr}$ using \Cref{tr:tw-e-ectx-call-notsame}, producing the $\irl{Call\ \ctxtocomp\ foo\ \valueexpr}$ event since $foo\neq main$.
  Thus:
  \begin{passumptions}{H}
    \setcounter{enumi}{21}
...
\begin{theorem}[TMS Relation Correctness for $\mmlAtmsmmlAtscctcomp{\bullet}$]\label{thm:tms-rel-corr:lmslscct}
  \begin{align*}
    \text{ if }&
    \tospecificevN[^*_{\delta_{MS}}]{\sigma_{\tospecificevo[^*_{\delta_{MS}}]{\bullet}}(\tmssafe)} = \tmssafe
    \text{ and }
    \xlangtraceeqn{\sigma_{\xlangtraceeq{}{}}(\sigma_{\tospecificevi[^*_{\delta_{MS}}]{\bullet}}(\tmssafe))}{\sigma_{\tospecificevo[^*_{\delta_{MS}}]{\bullet}}(\tmssafe)}
    \\
    \text{ then }&
    \tospecificevN[^*_{\delta_{MS}}]{\sigma_{\xlangtraceeq{}{}}(\sigma_{\tospecificevo[^*_{\delta_{MS}}]{\bullet}}(\tmssafe))} = \tmssafe
  \end{align*}
\end{theorem}
\begin{proof}
  By unfolding \Cref{def:sigma} and applying the first assumption to rewrite the inner $\sigma$.
  The cross-language relation $\xlangtraceeqc{}{}$ preserves allocation and deallocation structure, maintaining temporal memory safety.
...
\begin{scontents}[store-env=buffer]
  $\;$\\
  \begin{goals}
    \item $\rtpsigma[\sim]{\mmlAtmsmmlAtscctcomp{\bullet}}{\operatorname{\sCCT}}$
  \end{goals}
\end{scontents}
\realthm{thm:rscctp}{7}{Robust Strict Cryptographic Constant Time Preservation}
\begin{incompleteproof}
  Analogous to \Cref{thm:rtmsp}.
  Unfolding $\rtpsigma[\sim]{\mmlAtmsmmlAtscctcomp{\bullet}}{\operatorname{\sCCT}}$, the source assumption is $\rsat{\irl{\library_{\comp}}}{\sigma_{\sim}(\operatorname{\sCCT})}$.
  The cross-language trace relation $\sim$ is defined by $\xlangtraceeqc{\trace}{\trace}$.
  By backtranslation correctness, we get a source trace related by $\sim$.
  By $\sigma_{\sim}$ definition, we conclude the target trace satisfies $\operatorname{\sCCT}$.
\end{incompleteproof}

\begin{scontents}[store-env=buffer]
  $\;$\\
  \begin{goals}
    \item $\rtpsigma[\sim_1\circ\sim_2]{\mmlAtmsmmlAtscctcompN{\mmlAtmmlAtmscompN{\mmlAmmlAtcomp{\bullet}}}}{\operatorname{msafe}\cap\operatorname{\sCCT}}$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \item\label[ass]{ass:crpms} $\rtpsigma[\sim_1]{\mmlAtmmlAtmscompN{\mmlAmmlAtcomp{\bullet}}}{\operatorname{msafe}}$
    \item\label[ass]{ass:crpscct} $\rtpsigma[\sim_2]{\mmlAtmsmmlAtscctcomp{\bullet}}{\operatorname{\sCCT}}$
  \end{passumptions}
  By \Thmref{lem:seqcompo:sigma} using \Cref{ass:crpms,ass:crpscct}, composing the cross-language relations $\sim_1$ and $\sim_2$, we have the result.
\end{proof}
