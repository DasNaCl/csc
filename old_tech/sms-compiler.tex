...
\begin{proof}
  ...
  \begin{description}
    ...
    \case{tr:tw-e-get-in}
          ...
          \end{passumptions}
          From $\trg{n}=\bm{n}$, $\bm{n}<\bm{m}$, and $\bm{m}=\trg{m}$, we conclude $\trg{n}<\trg{m}$ by transitivity of $<$ and equality.
          Since $\trg{m}=\irl{m}$ by the state relation, we have $\trg{n}<\irl{m}$.
    ...
    \case{tr:tw-e-let-new-comp}
          ...
          The first goal follows by composing the above execution steps using \Cref{tr:tw-e-let-f,tr:tw-e-let-new-comp}: the first step produces an empty event, and the second produces $\irl{Alloc\ \loc\ n;\comp}$, so the combined trace is $\irl{Alloc\ \loc\ n;\comp}$ as required.
          The second goal (store agreement) follows since the monitor is updated to include $(\specificev{\loc},\bm{n})$ which corresponds to the newly allocated location in $\irl{\Omega'}$.
          The third goal ($\irl{\gamma'}\Vdash\irl{\Omega.\Delta}$) follows by \Cref{tr:sms-subst-subset-cons,tr:sms-subst-subset-xsize}, since $\irl{\gamma'}=\irl{[n/x_{SIZE}],[z/x],\gamma}$ extends $\irl{\gamma}$ with substitutions for $x_{SIZE}$ and $x$ that are consistent with the extended $\irl{\Delta}$.
          The fourth goal ($\trg{\gamma'}\prec\irl{\gamma'}$) follows since the substitution relation is preserved under extension with consistent bindings.
...
\end{theorem}
\begin{proof}
  By unfolding \Cref{def:sigma} and applying the first assumption to rewrite the inner $\sigma$.
  Since $\mmlAtms = \mmlAt$, the trace relation is identity, so $\sigma_{\xlangtraceeq{}{}}$ preserves the property.
  The second assumption ensures well-formedness of the composition.
\end{proof}
...
\end{theorem}
\begin{proof}
  By unfolding \Cref{def:sigma,def:sms} and applying the first assumption.
  The property is preserved because memory accesses must be in bounds (\Cref{def:spatmemsafe}), and the filter preserves allocation sizes and access offsets.
\end{proof}
...
\end{theorem}
\begin{proof}
  By unfolding \Cref{def:sigma,def:scct} and applying the first assumption.
  The property is preserved because the filter and trace relation preserve the structure of branching and memory access events relevant to sCCT.
\end{proof}
...
\end{theorem}
\begin{proof}
  By unfolding \Cref{def:sigma,def:spec} and applying the first assumption.
  The property is preserved because the filter and trace relation preserve speculative execution barriers and transient events.
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:sms} and applying the first assumption.
  The cross-language relation preserves allocation sizes and access offsets, ensuring spatial memory safety (\Cref{def:spatmemsafe}) is maintained.
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:scct} and applying the first assumption.
  The cross-language relation ensures no leaks occur: branching and memory access patterns are preserved, maintaining strict cryptographic constant time (\Cref{def:scct}).
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:spec} and applying the first assumption.
  The cross-language relation preserves speculative execution barriers and transient events, maintaining speculation safety (\Cref{def:spec}).
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma} and applying the first assumption to rewrite the inner $\sigma$.
  The cross-language relation $\xlangtraceeq{}{}$ preserves allocation and deallocation structure, maintaining temporal memory safety.
  By \Thmref{lem:msfiltereq}, related traces have equal filtered forms, so the property is preserved.
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:sms} and applying the first assumption.
  By \Thmref{lem:xlangtraceeq:smsafe:preserve}, the cross-language relation preserves allocation sizes and access offsets, maintaining spatial memory safety (\Cref{def:spatmemsafe}).
  By \Thmref{lem:msfiltereq}, related traces have equal filtered forms.
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:scct} and applying the first assumption.
  The cross-language relation preserves branching and memory access patterns, maintaining strict cryptographic constant time.
  By \Thmref{lem:msfiltereq}, related traces have equal filtered forms.
\end{proof}
...
\begin{proof}
  By unfolding \Cref{def:sigma,def:spec} and applying the first assumption.
  The cross-language relation preserves speculative execution barriers and transient events, maintaining speculation safety.
  By \Thmref{lem:msfiltereq}, related traces have equal filtered forms.
\end{proof}
