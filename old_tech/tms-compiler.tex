...
\begin{proof}
  ...
  Assume:
  \begin{passumptions}{I}
    \item\label[ass]{asm:compcorr:toplevel} $\forall\delta'\ \src{\Omega'}\ \src{\trace},\neg\left(\delta\subseteq\delta'\right)\vee\neg\left(\exectN{\Omega}{e}{\Omega'}{K[return\ \valueexpr]}{\trace}\right)\vee\xlangstateneq[\delta']{\Omega'}{\Omega'}\vee\xlangtraceneq[\delta']{\trace}{\trace}$
  \end{passumptions}
  and need to prove $\bot$.

  Apply \Thmref{lem:progress} on the assumption that the expression is well-typed.
  Since the component expression $\src{e}$ is well-typed and the configuration $\src{\Omega}$ is well-formed, there exist $\src{\Omega'}$, $\src{\finalexpr}$, and $\src{\trace}$ such that:
  \begin{passumptions}{H}
    \item\label[ass]{asm:compcorr:srcexec} $\execs{\Omega}{e}{\Omega'}{\finalexpr}{\trace}$
  \end{passumptions}

  Apply \Thmref{lem:forwardsim} on \Cref{asm:compcorr:srcexec} using the state relation and typing assumptions:
  \begin{passumptions}{F}
    \item\label[ass]{asm:compcorr:deltasubset} $\delta\subseteq\delta_{v}$
    \item\label[ass]{asm:compcorr:trgexec} $\exec{\trg{\Omega}}{\mmlAmmlAtcomp{e}}{\trg{\Omega_{v}'}}{\mmlAmmlAtcomp{\finalexpr}}{\trg{\trace[_{v}]}}$
    \item\label[ass]{asm:compcorr:stateeqv} $\xlangstateeq[\delta_{v}]{\Omega'}{\Omega_{v}'}$
    \item\label[ass]{asm:compcorr:traceeqv} $\xlangtraceeq[\delta_{v}]{\trace}{\trace[_{v}]}$
  \end{passumptions}

  Apply \Thmref{lem:determ:steps} on the assumed target execution and \Cref{asm:compcorr:trgexec}:
  \begin{passumptions}{D}
    \item\label[ass]{asm:compcorr:finaleq} $\mmlAmmlAtcomp{\finalexpr}=\trg{K[return\ \valueexpr]}$
    \item\label[ass]{asm:compcorr:stateeq} $\trg{\Omega_{v}'}=\trg{\Omega'}$
    \item\label[ass]{asm:compcorr:traceeq} $\trg{\trace[_{v}]}=\trg{\trace}$
  \end{passumptions}

  By \Cref{asm:compcorr:finaleq} and injectivity of compilation (\Thmref{lem:injective:comp}):
  \begin{passumptions}{H}
    \setcounter{enumi}{6}
    \item\label[ass]{asm:compcorr:srcfinaleq} $\src{\finalexpr}=\src{K[return\ \valueexpr]}$
  \end{passumptions}

  Rewrite using \Cref{asm:compcorr:stateeq} in \Cref{asm:compcorr:stateeqv} and \Cref{asm:compcorr:traceeq} in \Cref{asm:compcorr:traceeqv}:
  \begin{passumptions}{F'}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:compcorr:stateeqfinal} $\xlangstateeq[\delta_{v}]{\Omega'}{\Omega'}$
    \item\label[ass]{asm:compcorr:traceeqfinal} $\xlangtraceeq[\delta_{v}]{\trace}{\trace}$
  \end{passumptions}

  Rewrite using \Cref{asm:compcorr:srcfinaleq} in \Cref{asm:compcorr:srcexec}:
  \begin{passumptions}{H}
    \setcounter{enumi}{7}
    \item\label[ass]{asm:compcorr:srcexecfinal} $\execs{\Omega}{e}{\Omega'}{K[return\ \valueexpr]}{\trace}$
  \end{passumptions}

  Specialize \Cref{asm:compcorr:toplevel} with $\delta'=\delta_{v}$, $\src{\Omega'}=\src{\Omega'}$, and $\src{\trace}=\src{\trace}$.
  We proceed by case analysis:
  \begin{description}
    \item \textbf{Case }$\neg\left(\delta\subseteq\delta_{v}\right)$\textbf{:}
      Immediate contradiction with \Cref{asm:compcorr:deltasubset}.
    \item \textbf{Case }$\neg\left(\execs{\Omega}{e}{\Omega'}{K[return\ \valueexpr]}{\trace}\right)$\textbf{:}
      This contradicts \Cref{asm:compcorr:srcexecfinal}.
    \item \textbf{Case }$\xlangstateneq[\delta_{v}]{\Omega'}{\Omega'}$\textbf{:}
      Immediate contradiction with \Cref{asm:compcorr:stateeqfinal}.
    \item \textbf{Case }$\xlangtraceneq[\delta_{v}]{\trace}{\trace}$\textbf{:}
      Immediate contradiction with \Cref{asm:compcorr:traceeqfinal}.
  \end{description}
\end{proof}
...
\end{lemma}
\begin{proof}
  By definition of $\xlangstateeq{\cdot}{\cdot}$ on control-flow states, we need to show that each component is related.
  The component library $\commlib$ is trivially equal on both sides.
  The library relation $\xlangstateeq{\library}{\library}$ holds by assumption.
  The empty continuation stack $\hole{\cdot}$ is trivially related to itself.
  Combining these, the goal follows by the constructor for control-flow state equality.
\end{proof}
...
\end{lemma}
\begin{proof}
  The set $\src{L}$ contains locations introduced by the backtranslation wrapper, which are ignored by the state relation $\xlangbackstateeq{\cdot}{\cdot}$.
  Since $\src{\loc}\in\src{L}$, the binding $\src{x\mapsto(\loc;\sandboxtag;\poison;m)}$ is not compared against any target binding.
  Therefore, changing $\src{\poison}$ to $\src{\poison'}$ does not affect the relation.
  All other bindings in $\src{\Delta_{1}}$ and $\src{\Delta_{2}}$, as well as the heap $\src{H}$, remain unchanged, so the goal follows from the assumption $\xlangbackstateeq{\memstate}{\memstate}$.
\end{proof}
...
\end{lemma}
\begin{proof}
  Since $\src{\loc}\in\operatorname{dom}\delta$, by definition of $\delta$ there exists a corresponding target location $\trg{\loc}$ such that $\delta(\src{\loc})=\trg{\loc}$.
  The state relation $\xlangstateeq{H^{\ctx};H^{\comp};\Delta}{H^{\ctx};H^{\comp};\Delta}$ requires that for each $\src{\loc}\in\operatorname{dom}\delta$, there is a binding in $\src{\Delta}$ containing that location.
  Therefore, we can decompose $\src{\Delta}$ into $\src{\Delta_{1},x\mapsto(\loc;\sandboxtag;\poison;m),\Delta_{2}}$ where the binding for $\src{x}$ contains $\src{\loc}$.
\end{proof}
...
\begin{lemma}[Cross-Language Relation Preserves SMS]\label{lem:xlangtraceeq:smsafe:preserve}
  The cross-language trace relation $\sim$ defined by $\xlangtraceeq{\trace}{\trace}$
  is well-formed with respect to $\operatorname{smsafe}$, i.e., $\wf{\sim}{\operatorname{smsafe}}$.

  Equivalently: if $\xlangtraceeq{\trace}{\trace}$ and the filtered source trace
  $\tospecificevs[^*_{\delta_{MS}}]{\src{\trace}} \in \operatorname{smsafe}$, then
  $\tospecificevt[^*_{\delta_{MS}'}]{\trg{\trace}} \in \operatorname{smsafe}$.
\end{lemma}
\begin{proof}
  By induction on $\xlangtraceeq{\trace}{\trace}$.

  The key cases are the memory events:

  \textbf{Case \Cref{tr:alloc-event-eq}:}
  From $\xlangeventeq{Alloc\ \loc\ n}{Alloc\ \loc\ n}$, we have $\mmlAmmlAtcomp{n} = \trg{n}$.
  Since compilation preserves numeric values, the allocation size is identical.
  The SMS monitor records $(\delta_{MS}(\src{\loc}), \src{n})$ for source and
  $(\delta_{MS}'(\trg{\loc}), \trg{n})$ for target.
  By the location correspondence $\delta$, these represent the same abstract allocation.

  \textbf{Case \Cref{tr:get-event-eq}:}
  From $\xlangeventeq{Get\ \loc\ n}{Get\ \loc\ n}$, we have $\mmlAmmlAtcomp{n} = \trg{n}$.
  The access offset is identical in source and target.
  If the source access is in-bounds (satisfies smsafe), then the target access
  at the same offset into an allocation of the same size is also in-bounds.

  \textbf{Case \Cref{tr:set-event-eq}:}
  Analogous to Get case.

  \textbf{Case \Cref{tr:dealloc-event-eq}:}
  Deallocation events are related by location correspondence.
  The SMS monitor removes the same abstract allocation from both monitors.

  By induction, the entire trace preserves smsafe.
\end{proof}
...
\begin{proof}
  ...
  \end{passumptions}
  \Cref{goal:exprbacktranslation:beg:wrapper:correct:Xsubset} follows immediately by reflexivity, since we instantiated $\src{X'}$ to be exactly $\src{X}\cup\left\{\src{Dealloc\ \loc}\right\}$.
...
%    \item $\delta' = \delta \cup \left\{\src{\loc}\mapsto\trg{z}\right\}$
  ...
  \end{goals}
  \Cref{goal:exprbacktranslation:vend:wrapper:correct:eventeq} follows by \Cref{tr:cons-trace-eq,tr:end-event-eq,tr:empty-trace-eq}.
\end{proof}
...
%  then
  \begin{goals}
    \item $\rtpsigma[\sim]{\mmlAmmlAtcomp{\bullet}}{\operatorname{msafe}}$
  \end{goals}
\end{scontents}

\realthm{thm:rtmsp}{4}{Robust Memory Safety Preservation}
\begin{proof}
  Unfolding the definition of $\rtpsigma[\sim]{\mmlAmmlAtcomp{\bullet}}{\operatorname{msafe}}$:
  If
  \begin{assumptions}
    \item\label[ass]{asm:rtmsp:srcsat} $\rsat{\src{\library_{\comp}}}{\sigma_{\sim}(\operatorname{msafe})}$
  \end{assumptions}
  then
  \begin{goals}
    \item\label[goal]{asm:rtmsp:rsatt} $\rsat{\mmlAmmlAtcomp{\library_{\comp}}}{\operatorname{msafe}}$
  \end{goals}

  By \Cref{def:memsafe}, $\operatorname{msafe} = \operatorname{tmssafe} \cap \operatorname{smsafe}$.

  By distributivity of $\sigma$ over intersection:
  \begin{passumptions}{H}
    \item $\sigma_{\sim}(\operatorname{msafe}) = \sigma_{\sim}(\operatorname{tmssafe}) \cap \sigma_{\sim}(\operatorname{smsafe})$
  \end{passumptions}

  So \Cref{asm:rtmsp:srcsat} gives us:
  \begin{passumptions}{H}
    \setcounter{enumi}{1}
    \item\label[ass]{asm:rtmsp:srctms} $\rsat{\src{\library_{\comp}}}{\sigma_{\sim}(\operatorname{tmssafe})}$
    \item\label[ass]{asm:rtmsp:srcsms} $\rsat{\src{\library_{\comp}}}{\sigma_{\sim}(\operatorname{smsafe})}$
  \end{passumptions}

  Note: \Cref{asm:rtmsp:srctms} is satisfiable because well-typed source programs satisfy $\operatorname{tmssafe}$ by type safety, and $\operatorname{tmssafe}\subseteq\sigma_{\sim}(\operatorname{tmssafe})$ follows from \Thmref{lem:msfiltereq} (related traces have equal filtered forms, so if the source trace satisfies $\operatorname{tmssafe}$, all related target traces do as well).
  ...
  \begin{goals}
  \item $\tospecificevt[_{\delta_{\text{MS}}}]{\trace}\in\operatorname{msafe}$
  \end{goals}

  We must show $\trg{\trace} \in \operatorname{tmssafe} \cap \operatorname{smsafe}$, i.e., two sub-goals.
  ...
  \textbf{Sub-goal 1: $\trg{\trace} \in \operatorname{tmssafe}$}

  By \Cref{asm:rtmsp:srctms}, since $\src{\trace}$ comes from linking $\src{\library_{\comp}}$ with the backtranslated context, we have $\src{\trace}\in\sigma_{\sim}(\operatorname{tmssafe})$.

  By \Cref{def:sigma}, since $\xlangtraceeq{\trace}{\trace}$, we conclude $\trg{\trace}\in\operatorname{tmssafe}$.

  \textbf{Sub-goal 2: $\trg{\trace} \in \operatorname{smsafe}$}

  By \Cref{asm:rtmsp:srcsms}, $\src{\trace} \in \sigma_{\sim}(\operatorname{smsafe})$.

  By \Thmref{lem:xlangtraceeq:smsafe:preserve}, the cross-language trace relation $\sim$ is well-formed with respect to $\operatorname{smsafe}$.

  By \Cref{def:sigma}, since $\xlangtraceeq{\trace}{\trace}$, we conclude $\trg{\trace}\in\operatorname{smsafe}$.

  \textbf{Conclusion:} Combining sub-goals 1 and 2, $\trg{\trace} \in \operatorname{tmssafe} \cap \operatorname{smsafe} = \operatorname{msafe}$.
