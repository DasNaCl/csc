...
\subsubsection{Proof Sketch of Robust Safety Property Preservation}

The proof of \Cref{thm:cca:rtp:tms} uses standard techniques in secure compilation literature that are also used for similar theorems in this paper.
...
By \Cref{thm:wt:tms}, the fact that this program satisfies \gls{tms} is obtained and by typesafety, it is known that there must be some $\Ltms$ execution.
The proof now needs to relate the two execution with eachother.
\Cref{fig:proofdiag:rtp} visualizes this situation.
...
Finally, since both execution traces are similar in the sense that their projections $\msfilterLtms{\src{\trace}}$ and $\msfilterL{\trg{\trace}}$ yield specification traces that satisfy \Cref{def:monsat}, the claim follows \Cref{lem:mon:tmsafe}.
...
\MPin{
  ...
  MK: but, we don't actually rewrite it to RSC when doing the proof! The compilers are not robustly preserving with respect to an arbitrary safety property, but, e.g., just for \gls{tms}!
}
