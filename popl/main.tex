...
With this, a secure compiler needs to ensure that no malicious target-level context can supply unitialised memory and thereby sidestep the guarantees of the source language.
For example, if the target language is CHERI~\MK{cit} or MS-WASM~\cite{michael2023mswasm}\MK{check this} the compiler can ensure this by leavaraging the object capabilities. % , e.g., by inserting appropriate checks prior to entering the body of \texttt{strncpy} or using capabilities~\cite{michael2023mswasm}.
...
To consider a stronger model similarly to what existing work does~\cite{abate2019jour,abate2021extacc,patrignani2021rsc}\MP{many more things on RS here}, the concept of satisfaction can be extended with {\em robustness}.
...
To define when a compiler is secure, this paper uses the robust compilation framework~\cite{abate2019jour}, which the following definition summarizes.
...
To remedy this, this section defines the property \gls{scct}, a stricter variant of \gls{cct} that enforces the policy that no secret should appear on a trace.\MP{cit? did we take some inspiration}
