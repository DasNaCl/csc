...
Programs ($\runtimetermvar$) without free variables are considered {\em whole}.
...
Moreover, the paper assumes for such executions that $\runtimetermvar[']$ is {\em final}, decided by the judgement $\isval{\runtimetermvar}$.\MK{define final?}
...
%  A whole program $\runtimetermvar$ satisfies $\pi$ ($\sat{\runtimetermvar}{\pi}$) iff $\behav{\runtimetermvar}\in\lift{\pi}$.
%\end{definition}
\begin{definition}[Class Satisfaction]\label{def:classsat}
  A whole program $\runtimetermvar$ satisfies $\class$ ($\sat{\runtimetermvar}{\class}$) iff $\behav{\runtimetermvar}\in\class$.
\end{definition}
...
\section{Composing Properties\pages{3}}\label{sec:compprop}
