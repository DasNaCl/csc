...
\MPpost{
	citations here should include mswasm, swasey's stuff and michel's and move
}
...
\begin{example}[Double Free in Bluetooth Subsystem]
  ...
  Nevertheless, the component-level code of \texttt{hci\_dev\_do\_open} is linked with \texttt{hci\_cmd\_work} and does not atomically check whether the pointer\MP{what pointer?} has been freed already:
...
\begin{example}[Data-independent timing mode]
  ...
  \MPin{
  can't we call strncpy but with src == low and dest == high?
  }
