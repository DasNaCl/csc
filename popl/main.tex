...
\end{definition}
\MPin{
	why is this defined differently than MS? they should be defined in the same way
}
...
Both are secure with respect to \gls{ms}, the proof of which is relatively simple, because \gls{dce} and \gls{cf} do not change the way memory accesses happen.
...
This section discusses work on robust preservation\MP{
	robust compilation.
	NOT robust preservation
} (\Cref{subsec:relw:seccomprtp}) and on other secure compilation criteria (\Cref{subsec:relw:seccompcrit}).
...
\MP{perhaps say taht we believe this scales to different trace models too and differnet kinds of props, but the ocmposition of monitors becomes tricky}
The work relating robust preservation with universal composability~\cite{patrignani2022universal} is closest to what this paper presents.\MP{why}
...
Different mechanisms for \MP{enforcing?}memory safety exist that also consider the secure compilation domain, i.e., have an active attacker model.
...
It\MP{what} also works in a more advanced setting with structured fields accesses and also introduces a table-lookup for pointers that are stored in memory.
This paper only considers pointers to arrays of simple objects\MP{NOT OBJECTS}, i.e., there are no pointers to pointers or structures.
...
The approach to \MP{preserving?enforcing?} cryptographic constant time in this paper is high-level, where a programming language exposes a way to switch the semantics to a data (operand) independent timing mode.
