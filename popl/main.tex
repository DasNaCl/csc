...
int strncpy(char n, char *x, char *y) {
...
To this end, this paper chooses the robust preservation framework, which is a model of secure compilation that is well--established~\cite{abate2019jour,abate2021extacc,patrignani2021rsc}. % and has a stronger attacker model than a passive attacker whose only means of interacting with a program are timing analyses and so on.
Unfortunately, this framework of secure compilers does not support modularity: One has to make a secure compiler for all possible properties {\em or} for a combination of \gls{ms} and \gls{cct}.

\MKin{
blablabla continue blablabla
}
