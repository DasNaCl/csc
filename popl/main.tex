...
%  Robustly Safe Compilers ensure that programs that do not go wrong when linked with an arbitrary source-level attacker also do not go wrong in their compiled counterpart when linked with untrusted target-level code.
  Robustly safe programs do not go wrong when linked with arbitrary, untrusted code.
  Compilers preserving such robust safety do this on a theoretical level.
  The criterion requires challenging proofs which, so far, do not exist in real-world compiler implementations.
  ...
  It is an open question whether one can prove these individual passes as robust safety preserving or not.
  ...
  The source code will be made publicly available upon this paper's acceptance.
\end{abstract}
