...
\begin{example}[A program not satisfying \gls{tms}]
  Consider the following \texttt{C++11} program that calls \texttt{strncpy} (\Cref{ex:strncpy}) and prints the result to the standard output stream.
...
int main() {  // allocates 12 chars containing a greeting message
  ...
  Remembering the definition of \texttt{strncpy} (\Cref{ex:strncpy}), it is easy to see that it does not deallocate its arguments.
  ...
  It is easy to see that $\montmssafe{\abstrace[']}$ and from \Thmref{lem:mon:tmssafe}, it follows that the program satisfies \Thmref{def:trace:tmsdef}, even though the program still violates \gls{sms}.
\end{example}
