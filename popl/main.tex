...
\gls{ms} is a security property that ensures that there are no out-of-bounds accesses as well as no reads or writes to uninitialized memory.
...
Even though mitigations for such attacks exist~\cite{bond2017vale,almeida2017jasmin}, some compilers may optimize these mitigations away~\cite{barthe2018sec}.
...
Consider a program context that calls \texttt{strncpy} with an argument for \texttt{src} that points to uninitialized memory.
...
With this, a secure compiler needs to ensure that no malicious target-level context can supply unitialised memory and thereby sidestep the guarantees of the source language, e.g., by inserting appropriate checks prior to entering the body of \texttt{strncpy}.
\MPin{
	do we have examples of such wrapper-chesk inserting secure compilers?
	mswasm compiler ... not really.
	or we could say that another solution is  using capabilities, and then the mswasm compiler can be used
}
\end{example}
...
The paper then defines several secure compiler passes, where each is either preserving a different security property (e.g., temporal or spatial memory safety) or performing a security-preserving optimisation, (e.g., applying constant-folding or dead-code elimination).
\MP{
	acro? non acro?
}
...
This program behaviour is insecure with respect to a canonical notion of (temporal) memory safety dictating no use-after-frees of pointers~\cite{nagarakatte2010cets}\MP{should cite catalin's MS paper too}, because it reads from a memory location that was freed already.
...
In the following, we indicate the execution of a whole program $\wholeprogvar$ that terminates in state $\runtimetermvar$ according to the language semantics and produces trace $\trace$ as: $\progstepto{\wholeprogvar}{\runtimetermvar}{\trace}$.
With this, we define property satisfaction as follows:
...
To consider a stronger model similarly to what existing work does~\cite{abate2019jour,abate2021extacc,patrignani2021rsc}\MP{many more things on RS here}, we extend the concept of satisfaction with {\em robustness}.
...
As typically done in works that consider the execution of partial programs~\cite{abate2019jour,devriese2018parametricity}, \MP{Add papers from Amal. Papers from just Dominique on FAC. RS works.}
...
\begin{example}[Double Free in Bluetooth Subsystem]
  ...
  The bug\MP{not bug, vulnerability. uniform below (perhaps?)} arises due to a race condition where the context-level function \texttt{hci\_cmd\_work} was not expected to behave maliciously, since it resides in the same source-code repository where the vulnerability occurs.
...
Unfortunately, correct compilers may be insecure compilers~\cite{patrignani2019survey}\MP{cite kennedy and teh general SC dagstuhl and ACM papers too}, and programs translated by insecure compilers can violate security properties that the programmer assumes to hold.
...
\MPin{
	somewhere we should say that a class of props can also include a single prop
}
...
\begin{example}[Types]
  ...
  Thus, even if $\src{foo}$ may have been robust with respect to the no-reads-out-of-bounds property\MP{sms?}, its compiled counterpart is not and therefore the compiler fails to attain \Thmref{def:rtp}.
\end{example}
...
\gls{sms} disregards\MP{prohibits?} out-of-bounds accesses:
...
\end{definition}
\MPin{
	cite catalin's paper and the mswasm paper for the def of ms
}


\subsubsection{Strict Cryptographic Constant Time}

\gls{cct} is a hyperproperty\MP{hypersafety?}\MP{what is a hp?} and, thus, not as easy to check with just monitors.\MP{chy? cit?} %Beutner et al
Hence, we define \gls{scct}, a stricter variant that enforces the policy that no secret should appear on a trace.\MP{cit? did we take some inspiration}
...
\end{definition}
\MPin{
	comment on the composition at this levle?
}
...
To lessen the burden when proving\MP{proving what}, each monitor makes use\MP{uses} of a custom trace model that contains only the relevant information related to the property the monitor checks.
...
This relation projects the events\MP{whaddaya mean?}, while the stutter-equivalence $\traceagree{\pi}{\trace}{\abstrace}$ remains the same for all properties $\pi$, yet is indexed by $\pi$ to emphasize the kind of event agreement.\MP{unclear sentence}
...
In the reflexive case of $\monitorcheck{\tmsmonitor}{\tmsmonitor}{\absevent}$, the monitor $\tmsmonitor$ must not contain allocated locations, i.e., $\tmsmonitor.allocated=\emptyset$.\MP{wut?}
...
\begin{example}[A program satisfying \gls{tms}]
  ...
  \begin{lstlisting}[language=c,basicstyle=\small\ttfamily,morekeywords={new,delete}]
...
\end{example}
\MPin{
	example is good, but it'd be nice to tie it to the running example?
}
...
\begin{example}[Normal invocation of \texttt{strncpy}]\label{ex:strncpy:sms}
  ...
  Because of this, there is an out of bounds\MP{hyphenate: out-of-bounds} memory access right before exiting the function.
  ...
  It is easy to see that,\MP{erase. we don't know what's easy} after the allocation events, the state of the monitor is $\left\{(\loc_{x};\comp;2),(\loc_{y};\comp;2)\right\}$.
  Furthermore, all uses up to the last are acceptable by the monitor, but the last event does not satisfy the premise $2<2$ in \Cref{tr:sms-use}. Therefore, the whole program (\texttt{strncpy} linked with this kind of context) is not \gls{sms}.
\end{example}
...
\MPin{
	section 3.1 never defined the $\cap$ operator on properties, nor on classes.
	it uses the 'and'.
	rectify
}
...
Consider the compilation passes of LLVM~\cite{lattner2004llvm}.
In LLVM, compiler passes are handled in the \texttt{PassManager} class.
By means of manipulating the internal state of the \texttt{PassManager} object, the schedule of compiler optimisation passes can be hardcoded.
That way, an order of optimisations can be found by hand that yields the best results.
Take \gls{cf}, which rewrites constant expressions to the constant they evaluate to, and \gls{dce}, which removes dead code by rewriting conditional branches.
...
Now, given \bul{$\cc{\src{L}}{\trg{L}}$ robustly preserves $\class[_{1}]$} and \rul{$\cc{\obj{L}}{\trg{L}}$ robustly preserves $\class[_{2}]$}, it follows that \oul{their upper composition $\cc{\src{L}+\obj{L}}{\trg{L}}$ robustly preserves the intersection of classes $\class[_{1}]$ and $\class[_{2}]$}.
...
Now, given \bul{$\cc{\src{L}}{\trg{L}}$ robustly preserves $\class[_{1}]$} and \rul{$\cc{\src{L}}{\obj{L}}$ robustly preserves $\class[_{2}]$}, it follows that \oul{their lower composition $\cc{\src{L}}{\trg{L}+\obj{L}}$ robustly preserves the intersection of classes $\class[_{1}]$ and $\class[_{2}]$}.
...
In this paper, $\Ltms$ is the only statically typed language and exhibits the property that all well-typed programs are \gls{tms} (\Cref{subsec:ltms}).
However, not all $\Ltms$ programs are \gls{sms}.
That is, there are well-typed $\Ltms$ programs that perform an out-of-bounds access.
In contrast, $\Ltrg$ is untyped and does not provide any guarantees with regards to \gls{ms} (\Cref{subsec:lsms}).
...
This paper considers \gls{cct} as an architecture-dependent property that should not be one of the programmer's concerns~\cite{cauligi2019fact}.
...
In spirit of this, the language $\Lscct$ allows violating \gls{cct} by emitting events on, e.g., branching and division, that contain secrets (\Cref{subsec:lscct}), but provides a way to read and write to a {\em model--specific register} that enables a ``\gls{cct}-mode''.
...
% }
% oh... that's good to know! the tech report is also riddled with this... :/

Above is a fraction of the shared syntax of all the programming languages of this paper.
\MPin{why a fraction? what exprs are missing
MK: Things that are missing here are: Evaluation contexts, states $\Omega$ and transitively other definitions affected by this
}
...
\MPin{
	below is semantics info. should go with their symbols and the semantics rules.
	and a better description of the semantics symbols.
  MK: this moved to appendix! should we bring it back? what exactly should be left out?
}
...
Possible events that are emitted during execution are similar to the ones of the specification trace model (\Cref{subsec:propdefs}).
...
The proof of this property relies on a projection $\msfilterLtms{\src{\event}}=\event$ that projects $\src{L_{\tmssafe}}$ events to specfication events $\event$, because the properties defined earlier (\Cref{subsec:propdefs}) are defined in the specification trace model.
...
While the previous languages (\Cref{subsec:ltms,subsec:lsms,subsec:lms}) are not concerned with \gls{cct}, $\Lscct$ extends $\Lms$ (\Cref{subsec:lms}) with a register flag for a data (operand) independent timing mode\footnote{As present in both Arm~\cite[p.~543]{arm-refman} and Intel~\cite[p.~80]{intel-refman} processors.}.
For the other languages seen earlier, this mode is intuitively always-on.
\MP{spell out the formal changes in an align* environmtn} % this necessitates that we present $\Omega$ also in the shared lang defs
...
\Cref{ex:lscct} illustrates these changes.
...
The compiler only inserts a dynamic typecheck to respect the well-typedness of function bodies.
...
Compiling the \texttt{strncpy} function from \Cref{sec:introduction} with $\cca$, the compiler would in this case ensure that the arguments are valid.

\begin{theorem}[Compiler $\cca$ is secure with respect to \gls{tms}]\label{thm:cca:rtp:tms}
  $\rtp{\cca}{\tmssafe}$ % \Coqed
\end{theorem}

\subsubsection{Proof Sketch of Robust Temporal Memory Safety Preservation}
...
\MPin{
	what does this mean?
	what is a backtranslation?
}
\MPin{
	why can we use a BT?
	it's because the def in section 2.2 is eq to anotehr one, because the props are Safety, we use the RSC equivalent def.
	all these intuitions are missing.
	they are technical, so we only keep them here.
}
...
Well-typing\MP{well-typedness} means the $\src{L_{\tmssafe}}$ program runs, so the rest of the proof is about relating the $\trg{L}$-level execution with the $\src{L_{\tmssafe}}$-level execution.

For the $\trg{L}$-execution, the compiler wrapper\MP{undefined terminology. call it like this before} code at state $\trg{\Omega_{1}}$ may either crash the program or continue execution $\trg{\Omega_{w_{1}}}$ right before control is transferred to the component.
In this specific case, the compiler needs not insert additional code after the component is done with its execution, but a symmetric situation can happen here.\MP{vague and imprecise}
Similarly, the backtranslation may also insert wrapper code prior and after switching to and from the component to the context. \MP{wut? why is a wrapper needed?}
\MKin{
  more detail or not? ;
...
\MPin{
	the general idea that later proofs use the same structure is also not here.
}
...
Again, linking this back to the \texttt{strncpy} example (\Cref{ex:strncpy:sms}) corresponds to the insertion of bounds check right in front of the accesses to \texttt{y[i]} and \texttt{x[i]}.
In turn, the event $\ev{Use}\ \loc_{x}\ 2;\comp;\unlock$ would not be emitted during execution.
\MPin{
	example code?
    MK: still needed now that we reference ex. 3.10?
}
...
\Cref{thm:ccab:rtp:ms} follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtp}.
...
\end{center}
\MPin{
	substitution are a runtime semantics thing.
	what you want is a compiler with 2 arguments, where in one you keep track of the static substitions to perform,
	and 2 cases on how to handle variables: if they're in the second arg, then substitute, otherwise don't
  %
  MK: No? I get how they are used as part of the runtime semantics, but a compiler can just as well make use of a substitution function that models the runtime semantics. I see how these two are different. However, I think it's easier in this case to be a bit inprecise, since adding another argument to the compiler breaks our compositionality framework (unless it's hidden with a monad...) so, IMO, we should view substitution here as a meta-level, but still language-dependent, function that the compiler has access to. If we don't do this, we essentially re-encode the definition of subst into the compiler
}

The two optimising compiler passes from $\Lms$ to $\Lms$ perform \gls{dce} and \gls{cf}, respectively.
Both are secure with respect to \gls{ms}, the proof of which is relatively easy, because \gls{dce} and \gls{cf} do not change the way memory accesses happen.
...
\end{theorem}
\MPin{
This you can write the proof of.
Probably even in coq
%
MK: Should this appear here in the paper? It's basically identical to \Cref{thm:ccab:rtp:ms}
}
