...
But, $\src{L_{\tmssafe}}$ is statically typed, where the type system is inspired by $L^{3}$~\cite{morrisett2005L3}.\MP{fabulous paper from amal?relevant?}
\MP{and therefore omitted?}
...
While the previous languages (\Cref{subsec:ltms,subsec:lsms,subsec:lms}) are not concerned with \gls{cct}, $\Lscct$ extends $\Lms$ (\Cref{subsec:lms}) with a register flag for a data (operand) independent timing mode, a feature that is present in both Arm~\cite[p.~543]{arm-refman} and Intel~\cite[p.~80]{intel-refman} processors.
...
\MPin{formalisation additions in algin* env}
...
With this technical setup, the information wether\MP{spelling} a read or write happened on a secret value is not hidden by the semantics, e.g., by emitting $\obj{\emptyevent}$, but when projecting to specification events.
...
\begin{example}[$\Lscct$ with and without data independent timing]\label{ex:lscct}
  ...
  \MPin{shouldn't we have 2 traces, one with the bit set, one without}
...
\MPin{some text should introduce the compiler defs}
...
The proof of \Cref{thm:cca:rtp:tms} uses standard techniques in secure compilation literature that are also used for the following theorems of this section.
\MP{so what?}
...
It is shown for that obtained context $\src{\library_{\ctx}}$ that the whole program $\link{\src{\library_{\ctx}}}{\src{\library_{comp}}}$ is well-typed.\MP{passive}
By \Cref{thm:wt:tms}, the fact that this program satisfies \gls{tms} is obtained and by typesafety, it is known that there must be some $\Ltms$ execution.\MP{passive passive passive, and imprecise}
The proof now needs to relate the two execution with each other.
\Cref{fig:proofdiag:rtp} visualizes this situation\MP{imprec}.
\begin{figure}[!ht]
  \begin{center}
...
\end{figure}
\MPin{
	thicker lines and dots needed, not visible otherwise
}
...
\MPin{
	show the result of the compilation as listing, and then describe the trace a bit more in detail ?
}
...
\Cref{thm:ccab:rtp:ms}\MP{states ... } follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtp}.
...
%
	\MKin{ 
	Should this appear here in the paper? It's basically identical to \Cref{thm:ccab:rtp:ms}
