...
\begin{itemize}
  \item %
        ...
        A formal framework to reason about the composition of security properties (\Cref{sec:compprop}).
        ...
        Using the framework, the ease for formal reasoning with compositions of security properties is demonstrated by instantiating the framework on \gls{tms}, \gls{sms}, and \gls{cct}, which are ultimately composed to \gls{ms} and a combination of \gls{ms} and \gls{cct}.
  ...
  \item %
        ...
        Foundational, theoretical work on different forms of compositions of secure compilers (\Cref{sec:compcomp}).
        %Concretely, it investigates upper and lower compositions, as well as the sequential composition.
        First, this paper proves theorems which state that a compiler with multiple input or output languages is secure as long as the compiler is composed of several secure passes with one input and one output language, respectively.
        Second, a theorem about the sequential composition of secure compilers is proven, which states that sequentially composing secure compilers is, again, secure with respect to the composition of the properties they preserve.
  ...
  \item %
        ...
        A case-study demonstrating the power of the formal frameworks from the previous contributions (\Cref{sec:casestud}).
        ...
        Furthermore, the chain entails two passes for \gls{ms}\MKin{maybe patch \gls{ms} when optimizing passes are done} that are optimizing: One performs a simple \gls{dce} and the other \gls{cf}.
...
For the other parts of the paper, the notions of security properties and secure compilers are introduced (\Cref{sec:background}).
%We conclude in \Cref{sec:concl} and give an outlook for future work.
The work presented in this paper is compared to others from the literature (\Cref{sec:relwork}) and there is a conclusion with an outlook for future work (\Cref{sec:concl}).
...
Since this paper considers composing compilers, we'll adopt notation similar to tombstone diagrams\MK{cite} to easen the formal burden.
...
The power of the frameworks from \Cref{sec:compprop,sec:compcomp} is demonstrated by composing these compilers for a secure, but optimizing, compilation chain that robustly preserves \gls{ms} and \gls{cct}.
Concretely, the section starts starts by defining languages $\src{L_{TMS}}$ and $\trg{L}$ and describes a compiler $\cc{\src{L_{TMS}}\to\trg{L}}{\src{\bullet}}$ for the robust preservation of \gls{tms} between the two languages (\Cref{subsec:cs:tms}).
Then, it defines $\irl{L_{MS}}$ and a compiler $\cc{\trg{L}\to\irl{L_{MS}}}{\trg{\bullet}}$ that robustly preserves \gls{sms} (\Cref{subsec:cs:ms}).
By the framework presented in \Cref{sec:compcomp,sec:compprop}, a compiler $\cc{\src{L_{TMS}}\to\trg{L}\to\irl{L_{MS}}}{\src{\bullet}}$ for the robust preservation of \gls{ms} by composing $\cc{\src{L_{TMS}}\to\trg{L}}{\src{\bullet}}$ and $\cc{\trg{L}\to\irl{L_{MS}}}{\trg{\bullet}}$ is attained.
Two secure compilers $\cc{{\irl{L_{MS}}\xrightarrow{\gls{dce}}\irl{L_{MS}}}}{\irl{\bullet}}$ and $\cc{\irl{L_{MS}}\xrightarrow{\gls{cf}}\irl{L_{MS}}}{\irl{\bullet}}$ are defined that perform \gls{dce} and \gls{cf}, respectively, to model an optimzing pipeline happening somewhere in the compilation chain (\Cref{subsec:cs:opts}).
Finally, the paper defines $\obj{L_{CCT}}$ and a compiler $\cc{\irl{L_{MS}}\to\obj{L_{CCT}}}{\irl{\bullet}}$ that robustly preserves \gls{cct} (\Cref{subsec:cs:scct}).
