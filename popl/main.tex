...
Without memory safety, private data can easily leak or be corrupted~\cite{lemay2021ccc}.
...
This is of course not what programmers expect and necessitates the need for a {\em secure} compiler that ensures such mitigations persist in the code they generate.
...
Without a framework for composing secure compiler passes, it is not possible to enable separation of concerns, e.g., to have a secure compilation pass that ensures \gls{ms} that is developed independently of another secure pass for \gls{cct} as showcased in \Cref{ex:strncpy}.
...
Even under these assumptions, there is a subtle \gls{ms} bug in the program which can be exploited to read sensitive data: The bounds check, i.e., \texttt{i < n}, should happen {\it prior} to the access to memory location \texttt{x[i]}.
...
The fix commonly employed in such a case is to rewrite the program to run a {\it fixed} number of iterations regardless of the length of \texttt{x} or \texttt{y}. \MP{ can we write this is a compiler-inserted fix?}
...
These fixes alone are not enough to make this program secure in a robust setting, i.e., not violating either \gls{ms} or \gls{cct} in a potentially malicious context.
...
Unfortunately, these checks do not work in C without additional features, since there is no way to check whether a pointer points into a valid allocated region or not.
...
%       }
  ...
  \item %
        ...
        Namely, the divide-and-conquer approach to software engineering is a viable strategy even for the developement of secure compilers.
...
\begin{example}[Normal invocation of \texttt{strncpy}]\label{ex:strncpy:sms}
  ...
  It is easy to see that, after the allocation events, the state of the monitor is $\left\{(\loc_{x};\comp;2),(\loc_{y};\comp;2)\right\}$.
...
% MK: I'll go over the doc with the antidote spellchecker as soon as it is reasonably stable.
...
%}  MK: that for sure is less confusing... Good suggestion!
Both examples can be seen in practice~\cite{nagarakatte2009soft,nagarakatte2010cets,akritidis2009baggy,wegman1991ccp,manjikian1997fusion}, it is infeasible to list them all\MP{survey to cite? this is a bit informal}. % MK: hard to find a survey on different compiler passes... I can't even find one just for memsafety
...
%% MK: Things that are missing here are: Evaluation contexts, states Ω and transitively other definitions affected by this
...
% MK: this moved to appendix! should we bring it back?
}
...
The key difference is that, as standard in secure compilation work, the traces have a call and return event that signals context switches.\MP{why? to help with proofs?}
...
\MP{intuition? these are aux functions for the semantics}
...
$\src{L_{\tmssafe}}$ does not extend the base syntax at term level.\MP{avoid negations. ltms programs use the same syntax presentd before without extensions}
What makes $\src{L_{\tmssafe}}$ special is that it has a type system inspired by $L^{3}$~\cite{morrisett2005L3} that exhibits the property that every well-typed $\src{L_{\tmssafe}}$ program satisfies \gls{tms}.\MPin{it's not really special ... Just say  "L3 has a type system ... ""}
The proof of this property relies on a projection $\msfilterLtms{\src{\event}}=\event$ that projects $\src{L_{\tmssafe}}$ events to specfication events $\event$. \MP{why}
Hereby, $\delta$ maps $\src{L_{\tmssafe}}$ locations $\src{\loc}$ to locations $\loc$ of the specification trace model.\MP{why, where is this needed}
The projection $\msfilterLtms{\src{\event}}$ for the most part does the expected\MP{what}.
...
If an object\MP{?} a pointer bound to variable $\trg{x}$ points to is allocated ($\trg{\poisonless}$), the check $\trg{\lispoisoned{x}}$ yields $\trg{1}$.
Likewise, if it\MP{what} has been poisoned ($\trg{\poisoned}$), the check $\trg{\lispoisoned{x}}$ evaluates to $\trg{0}$.
Besides these checks, the language remains exactly the same\MP{to what}.
...
This paper chooses to introduce\MP{uses / defines . use 1 word} $\Lms$, despite it being exactly equal to $\Ltrg$.
The main purpose of this decision is to enhance readability.\MPin{you can shorten these 2 sentences into 1. yes the section will be supershort, it's ok}
...
$\Lscct$ extends the dynamic state\MPin{i really prefer operational state ... or runtime state. i think i'm not used to this terminology.} of $\trg{L}$ (\Cref{subsec:lms}) with a register for a data (operand) independent timing mode.
For the other languages seen earlier, this mode is intuitively always-on.
\MP{spell out the formal changes in an align* environmtn}
...
\MPin{
	an example program and how it uses the flag and what kind of traces are produced is really needed here
}
...
\MP{use modifer [!h]. i HATE figs that float outside their section}
\begin{figure}[!h]
  \begin{center}
    \begin{tikzpicture}
      \node (S) {$\src{L_{\tmssafe}}$};
...
\MPin{
	explain how to read the figure: arrows mean, annotations mean ... ...
}
It\MP{what} demonstrates the power of the framework (\Cref{sec:compprop,sec:compcomp}) by composing these compilers for a secure and optimising compilation chain that robustly preserves \gls{msscct}.
...
Since these properties compose into \MP{}, composing these passes yields a compiler that robustly preserves \gls{ms} (\Cref{thm:ccab:rtp:ms}).
...
\MPin{
	this environment should just be a align*
}
  $$
  \begin{array}{rcl}
...
\MPin{
	this should be a subsection, not a proof sketch.
}
...
Up to getting and setting memory\MP{reading and writing? uniform lingo}, the compiler\MP{notation} is the identity function, i.e., all omitted cases are a recoloring of $\trg{L}$ to $\Lms$.\MP{reword as before}
For memory accesses, however, the compiler inserts a bounds-check that enforces \gls{sms}.
...
It is now possible to use \Cref{thm:rtp} to extract a compiler from $\Ltms$ to $\Lms$ that is \gls{ms} by applying \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms}.
\MP{
...
\MPin{
	the point is different: the proof is easy because the transofrmations do not change the way memory is touched,
	and from a security perspective, the attacker has no extra power since the target is the same as in the source.
	.
	rewrite.
}
...
\MPin{
	reuse the word 'wrapper' here
}
The context can overwrite the flag and exit the mode, but upon invoking a callable object\MP{again with the word object} that is part of the component, the flag would be set again.
Because of this, the compiler is secure with respect to \gls{scct}, similarily\MP{misspelled} proven as in \Cref{subsec:cs:tms}.
...
Let $\ccmsscct$ be the compiler that is the composition of $\cca$, $\ccb$, $\cccf$, $\ccdce$, and $\ccscct$, then the following theorem can be shown\MP{holds}.
