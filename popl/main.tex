...
\begin{figure}[h]
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \caption{Visualisation of the optimizing compilation pipeline that attains a combination of \gls{ms} and \gls{cct}.}
\end{figure}
...
Concretely, the section starts starts by defining languages $\src{L_{TMS}}$ and $\trg{L}$ and describes a compiler $\cc{\src{L_{TMS}}}{\trg{L}}$ for the robust preservation of \gls{tms} between the two languages (\Cref{subsec:cs:tms}).
Then, it defines $\irl{L_{MS}}$ and a compiler $\cc{\trg{L}}{\irl{L_{MS}}}$ that robustly preserves \gls{sms} (\Cref{subsec:cs:ms}).
By the framework presented in \Cref{sec:compcomp,sec:compprop}, a compiler $\rtp{\cc{\src{L_{TMS}}}{\trg{L}}\circ\cc{\trg{L}}{\irl{L_{MS}}}}{\mssafe}$ is attained.
Two secure compilers $\cc[_{\gls{dce}}]{\irl{L_{MS}}}{\irl{L_{MS}}}$ and $\cc[_{\gls{cf}}]{\irl{L_{MS}}}{\irl{L_{MS}}}$ are defined that perform \gls{dce} and \gls{cf}, respectively, to model an optimzing pipeline happening somewhere in the compilation chain (\Cref{subsec:cs:opts}).
Finally, the paper defines $\obj{L_{CCT}}$ and a compiler $\cc{\irl{L_{MS}}}{\obj{L_{CCT}}}$ that robustly preserves \gls{cct} (\Cref{subsec:cs:scct}).
Composing it with $\cc{\src{L_{TMS}}}{\irl{L_{MS}}}$ and the optimizing compilers $\cc[_{\gls{dce}}]{\irl{L_{MS}}}{\irl{L_{MS}}}$ and $\cc[_{\gls{cf}}]{\irl{L_{MS}}}{\irl{L_{MS}}}$ yields $\cc{\src{L_{TMS}}}{\obj{L_{CCT}}}$, a compiler that robustly preserves a combination of \gls{ms} and \gls{cct}.

\MKin{add image and do introduction using that}
