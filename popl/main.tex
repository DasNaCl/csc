...
\section{Background\pages{1}}\label{sec:background}
To reason about security, this section makes the concepts of properties and the satisfaction thereof precise (\Cref{subsec:bg:tprop}).
Furthermore, the section introduces robustness in the context of both satisfaction and secure compilers (\Cref{subsec:bg:rtp}) in similar fashion to existing work~\cite{abate2019jour}.
...
This notation also used for prepending events to traces, i.e., $\event\cdot\trace$.
...
Finally, the satisfaction of a class (or lifted property) is defined as follows:
...
The following definition extends on \Thmref{def:propsat} to cover {\em robustness} and exploits this interaction between the context and the component:
...
The syntactic description of programs written in $\src{L}$ is denoted as $\src{\progvar}$, while for ones written in $\trg{L}$ this paper writes $\trg{\progvar}$.
...
The properties are of practical importance and also of interest in the case study (\Cref{sec:casestud}) this paper presents.
