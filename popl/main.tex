...
\begin{gather*}
  \begin{aligned}
  \mi{(Finals)}~\finalexprnoerr&\bnfdef\ \valueexpr \mid x\hspace{0.5cm}
  \mi{(Results)}~\finalexpr\bnfdef\ \finalexprnoerr \mid stuck\hspace{0.5cm}
  ...
  \mi{(Expressions)}~\expr&\bnfdef\ \finalexpr \mid \lbinop{\expr[_{1}]}{\expr[_{2}]} \mid \lget{x}{\expr} \mid \lnew{x}{\expr[_{1}]}{\expr[_{2}]} \mid \ldelete{x} \mid \lset{x}{\expr[_{1}]}{\expr[_{2}]} \\
    & \mid \llet{x}{\expr[_{1}]}{\expr[_{2}]} \mid \lreturn{\expr} \mid \lcall{g}{\expr} \mid \lifz{\expr}{\expr[_{1}]}{\expr[_{2}]} \\
  \mi{(State)}~\statevar&\bnfdef \tup{\cfstate}{\sandboxtag,\memstate} \hspace{0.5cm}
  \mi{(Runtime~Terms)}~\runtimetermvar\bnfdef\rtt{\statevar}{\expr} \hspace{0.5cm}
...
Expressions ($\expr$) must not contain the result $stuck$ unless they are entailed as runtime--term ($\rtt{\statevar}{\expr}$).
...
All programming languages presented in this section have an environmental semantics and all but $\src{L}$ are dynamically typed.
...
\begin{center}
  \judgbox{\pstepto{\runtimetermvar}{\runtimetermvar[']}{\event}}{,,Primitive step from runtime--term $\runtimetermvar$ to $\runtimetermvar[']$ emitting $\event$.''}
  \typerule{$e-get-\in$}{
    \sandboxtag=\Omega.\sandboxtag &
    \statevar.\store(x)=\tup{\loc}{\sandboxtag,\poison,m} &
    \loc+n\in\statevar.\heap[^{\sandboxtag}]
  }{
    \pstepto{\rtt{\Omega}{\lget{x}{n}}}{\rtt{\Omega}{\heap[^{\sandboxtag}](\loc+n)}}{\tup{\ev{Get}\ \loc\ n}{\sandboxtag}}
  }{e-get-in}
  %
  \typerule{$e-get-\notin$}{
    \sandboxtag=\Omega.\sandboxtag &
    ...
    \pstepto{\rtt{\Omega}{\lget{x}{n}}}{\rtt{\Omega}{1729}}{\tup{\ev{Get}\ \loc\ n}{\sandboxtag}}
  }{e-get-notin}
\end{center}

\Cref{tr:e-get-in,tr:e-get-notin} both evaluate a read by looking up the metadata in the store.
But, note that neither performs any bounds-checking and emits a $\ev{Get}\ \loc\ n$ event regardless of whether the access is out of bounds or not.
