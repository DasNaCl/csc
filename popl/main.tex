...
Because of this hostile environment, secure compilers attain security by relying on protection mechanisms, such as ... and ...\MK{cits}.
...
This is because,\MP{this is a german comma. english does not work this way :) please erase.} the loop can terminate early: As soon as the string--\MP{only 1 dash here (also happened before but i already edited)}terminating character \texttt{'\textbackslash 0'} is encountered, the loop exits.
...
Consider a program context that calls \texttt{strncpy} with a very large \texttt{n}, some uninitialised memory \texttt{x}, and \texttt{y} whatever\MP{? whatever?}.
Such a function invocation leads to a series of reads from uninitialised memory, which is an immediate \gls{ms} issue.
\MPin{
	we have a very very high-level definition of MS, so the issue may not be clear
}
To mitigate these situations, it is important that \texttt{strncpy} ensures that the provided arguments satisfy the assumptions being made by the programmer, since the environment\MP{env? ctx? tetrminology} can behave in a hostile manner.
Unfortunately, these checks do not work in C\MP{why are we talking of C now?} without additional features, since there is no way to check wether\MP{please use a spell checker, this word is written 'whether'} a pointer points into a valid allocated region or not.
...
\MPin{
	this bit should be introduced earlier
}
...
%        }
  ...
  \item %
        ...
        For this, this paper studies the sequential composition of compilers, as typically done when engineering compilation chains, as well as an upper and a lower composition that discuss multi--in- and multi--output compilers.\MP{upper and lower?}
        % 
...
\MPin{
	missing 3.2 ref? added below?
}
Then, this section introduces monitors for each of the properties presented earlier (\Cref{??}).
...
The purpose of the model is to define key security properties of interest, such as \MP{ms?} a stricter variant of cryptographic constant time.
...
The latter is necessary to be able to ignore actions done by a spurious context that, e.g., immediately deallocates a memory location twice, thus violating \gls{tms}. \MP{cits?}
...
These two events alone allow\MP{allow who? allow needs an object, this is improper english} already a partial description of \gls{tms} by requiring that, e.g., there is only one deallocation event that carries a location $\loc$.
...
Typical operations with data--dependent timing are branches and certain binary operations, such as division\footnote{This is highly architecture-dependent\MPin{hyphen needed}, but division is an operation that serves as a classic example for a data-dependent timing instruction.}.
...
The use of monitors is because they have some structure useful for inductive proofs, whereas traces lack this structure, thus making proofs difficult.\MP{not true. safety props are enforced by monitors}
