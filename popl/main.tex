...
\gls{ms} is a security property obtained by composing \gls{sms}, which ensures array accesses are all within bounds, and \gls{tms}, which ensures pointers are only used when they are valid~\cite{azevedo2018meaningsofms}\MP{more cits}.
...
the memory location past the last element will leak.
...
Unfortunately, fixing \gls{sms} is not enough to make \texttt{strncpy} secure, because of timing attacks.
...
Thus, robust compilers can be used to prevent these vulnerabilities, e.g., by targeting capability-based languages such as CHERI~\cite{woodruff2014CHERI}, Arm Morello~\cite{arm-morello}, or MSWasm~\cite{michael2023mswasm}, where pointers are always initialised.
\end{example}
...
% Different kinds of compilation passes exist, e.g., there are usually several compilation passes that perform optimisations~\cite{lattner2004llvm,googlev8,androidstudio,kuepper2023cryptopt,manjikian1997fusion,wegman1991ccp} or source-code instrumentations~\cite{nagarakatte2009soft,nagarakatte2010cets,akritidis2009baggy,dhumbumroong2020boundwarden,jung2021pico,nam2019framer,shankaranarayana2023tailcheck,younan2010paricheck,zhou2023fatptrs,bond2017vale,almeida2017jasmin,kuepper2023cryptopt,cauligi2019fact} that enforce that the compiled program fulfils a property of interest.
Worse, without a framework for composing secure compiler passes, it is not possible to enable separation of concerns, e.g., to have a secure compilation pass that ensures \gls{ms} that is developed independently of another secure pass for \gls{cct}.
...
\begin{itemize}
  \item %
        ...
        First, this paper formalises security properties (\Cref{sec:compprop}) by focussing on those properties that are of interest for real-world compiler writers (as identified by the plethora of work enforcing such properties individually~\cite{akritidis2009baggy,nagarakatte2009soft,nagarakatte2010cets,dhumbumroong2020boundwarden,jung2021pico,nam2019framer,shankaranarayana2023tailcheck,younan2010paricheck,zhou2023fatptrs,bond2017vale,almeida2017jasmin,kuepper2023cryptopt,cauligi2019fact}), namely \gls{tms}, \gls{sms} and \gls{cct}.
        % 
        Starting from ways to formalise and enforce those properties individually, this paper shows how to compose their formalisation as well as their enforcement.
        % 
  ...
  \item %
        ...
        For this, this paper studies sequential compiler composition as well as compilers with multiple inputs or multiple outputs, as typically done in existing compilation chains.
        %
...
\begin{example}[Types]
  ...
  Thus, even if $\src{foo}$ may have been robust with respect to the \gls{sms}, its compiled counterpart is not and therefore the compiler fails to attain \Thmref{def:rtp}.
\end{example}
...
\begin{example}[A program not satisfying \gls{tms}]
...
States $\statevar$ are tuples\footnote{Throughout the paper, the subsitution notation is also used to update entries in states $\statevar$.} containing a control flow state $\cfstate$, control tags $\sandboxtag$, and a memory state $\memstate$.
A control flow state $\cfstate$ entails a library, which provides definitions for functions when calling, as well as a stack of continuations $\ectxstack$.
...
The bounds information is a mere proof artifact that has no semantic significance.
For details see the accompanying tech report~\MK{cit}.
Memory states $\memstate$ have two separate heaps, essentially modelling a sandbox, to prevent contexts from performing pointer arithmetic and reading from or writing to the data owned by a component.
...
To this end, the poison tag $\poison$ indicates whether a pointer has been freed $\poisoned$ or is still allocated $\poisonless$, so that uses of pointers can happen even after their deallocation without the semantics getting stuck.
...
All languages use the same trace model.
In this trace model, events are either the empty event \emptyevent, the program crash event $\lightning$, or a tuple consisting of a control tag $\sandboxtag$ and a security tag $\securitytag$.
The former indicates wether the component \comp or the context \ctx is to blame for emitting this event, the latter indicates the secrecy level of values of the emitted event, i.e., either $\lock$ or $\unlock$.
...
\Cref{tr:e-dealloc,tr:e-get-in,tr:e-set-notin} demonstrate the use of the poison tag $\poison$ as metadata for pointers instead of removing them from the store $\Delta$.
Take deallocation as an example, the premise does not care at all about the actual state of the poison tag $\poison$ and just overwrites it, marking the location as freed $\poisoned$.
Besides that, the poison tag does not have any semantic meaning, at least in general.
...
The \Cref{tr:e-new} allocates enough space on the respective heap, either $H^{\ctx}$ or $H^{\comp}$ depending on the execution context, i.e., the value of $\Omega.\sandboxtag$, and adds the appropriate metadata associated to the pointer in $\Delta$.
Reading from and writing to memory have to cases: Either the heap is large enough or not and, depending on that, either the actual value stored at that location is read from or written to, or some garbage data is returned.
However, note that the execution does not get stuck in such cases, performs a step, and emits an appropriate event.
Also note that wether a pointer is poisoned or not is not reflected on the trace.
...
Finally, since both execution traces are similar in the sense that their projections $\msfilterLtms{\src{\trace}}$ and $\msfilterL{\trg{\trace}}$ yield specification traces that satisfy \Cref{def:monsat}, the claim follows \Cref{lem:mon:tmssafe}.
...
\begin{example}[Instrumented \texttt{strncpy}]
  ...
  void strncpy(size_t n, size_t dst_size, char *dst,
    ...
    \end{lstlisting}
    When calling this in similar fashion to \Cref{ex:strncpy:sms}, the event $\ev{Use}\ \loc_{x}\ 2;\comp;\unlock$ would not be emitted during execution.
\end{example}
...
\Cref{thm:ccab:rtp:ms} strates that the composition of $\cca$ and $\ccb$ is secure with respect to \gls{ms} and follows from \Cref{thm:cca:rtp:tms,thm:ccb:rtp:sms} using \Cref{thm:rtp}.
...
\MPin{
  perhaps say taht we believe this scales to different trace models too and differnet kinds of props, but the ocmposition of monitors becomes tricky
  \MKin{conclusion?}
}
