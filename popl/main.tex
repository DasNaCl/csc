...
Given a safety property, a finite prefix of a longer trace $\trace$ is enough to decide whether a trace is admitted or not by the property. % $\trace\in\pi$ or $\trace\notin\pi$.
...
Given a whole program $\runtimetermvar$, i.e., \MP{ fill in}, the {\em behaviour} of a whole program $\runtimetermvar$ ($\behav{\runtimetermvar}$) is the set containing all traces the program can emit during execution according to its language semantics.
...
\end{definition}
\MPin{
	this definition has 1/2 explained in text, 1/2 in formalism.
}
\MP{
	can't we use p for programs?
}
...
As typically done in a secure compilation context~\cite{abate2019jour}, \MP{others do this too, parametricity results e.g.} this paper also assumes that programs are the result of linking a {\em context} ($\ctx$) and a {\em component} ($\comp$), which are just any syntactic description.
...
The following extends on \Thmref{def:propsat} to cover {\em robustness} and exploits this interaction between the context and the component:
...
A {\em compiler} ($\cc{\src{L}}{\trg{L}}$) translates syntactic descriptions of programs from a {\em source} ($\src{L}$) into a {\em target} ($\trg{L}$) programming language.\MP{good sentence}
...
It is considered good if this translation is semantics-preserving, i.e., correct. \MP{passive}
Correct compilers may still be insecure compilers\MK{citation needed}.
...
\MPin{does not really flow.
% 
start with compilers (as now),
then: for a compiler to be correct we expect ... ... (no def)
then: for a compiler to be secure we use the robust compilation framework of ... that says ... ... def
}
...
\MPin{
	again, hybrid definition.
	keep one version.
	i suggest: text outside (typically before the def, in the text leading to it), formalism inside
}
...
\section{Example Properties and their Monitors\pages{3}}\label{sec:compprop}

This section introduces a trace model (\Cref{subsec:propdefs}) that is used to define properties (\gls{tms}, \gls{sms}, \gls{ms}, and \gls{scct}).
The properties are of practical importance and also of interest in the case study (\Cref{sec:casestud:defs,sec:casestud:rtp}) this paper presents.
...
So, this section also presents monitors for each of the defined properties (\Cref{subsec:monitors}) and introduces monitor-based satisfaction.
...
\section{Composing Secure Compilers\pages{1\sfrac{1}{2}}}\label{sec:compcomp}
...
\section{Case Study: Preliminary Definitions\pages{6}}\label{sec:casestud:defs}
...
\MPin{
	explain with example ? leave text for app
}
