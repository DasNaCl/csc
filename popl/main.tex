...
For sake of simplicity, assume that the length of the array \texttt{dst} points to is at least as large as \texttt{n} as well as \texttt{src} terminated by \texttt{'\textbackslash 0'}.
Even under these assumptions, there is a subtle \gls{ms} bug in the program: The bounds check \texttt{i < n} should happen {\it prior} to the access to memory location \texttt{x[i]}:
...
A compilation pass that enforces spatial memory safety, such as Softbounds~\cite{nagarakatte2009soft} or BaggyBounds~\cite{akritidis2009baggy}, could have prevented this accidental error.
...
Thus, the program execution time is proportional to the length of the array \texttt{src} points to.
Often, programmers rewrite programs like these to have loops that run with a fixed number of iterations.
However, compiler passes are capable of rewriting such programs~\cite{cauligi2019fact} and a modification by hand is not necessary.
...
Consider a program context \texttt{strncpy(1729,dst,src)} that calls \texttt{strncpy} with pointers \texttt{dst} and \texttt{src}, where \texttt{src} is some uninitialized memory.
...
To consider a stronger model similarly to what existing work does~\cite{abate2019jour,abate2021extacc,patrignani2021rsc}, the concept of satisfaction is extended with {\em robustness}.
