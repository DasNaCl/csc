...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[->] (S) to[sloped,above] node (tmsedge) {\text{\scriptsize\gls{tms}}} (T);
      ...
      % Sections
      \node[above=1.5 of tmsedge] (sectms) {{\scriptsize\Cref{subsec:cs:tms}}};
      \node[right=0.0 of sectms] (secsms) {{\scriptsize\Cref{subsec:cs:ms}}};
      \node[right=1.25 of secsms] (secopts) {{\scriptsize\Cref{subsec:cs:opts}}};
      \node[right=1.25 of secopts] (secscct) {{\scriptsize\Cref{subsec:cs:scct}}};

      \draw[dashed] ($(sectms)-(0.7,0)$) -- ($(sectms)-(0.7,1.75)$);
      \draw[dashed] ($(sectms)-(-0.7,0)$) -- ($(sectms)-(-0.7,1.75)$);
      \draw[dashed] ($(secsms)-(-0.7,0)$) -- ($(secsms)-(-0.7,1.75)$);
      \draw[dashed] ($(secscct)-(0.7,0)$) -- ($(secscct)-(0.7,0.75)$);
      \draw[dashed] ($(secscct)-(-0.7,0)$) -- ($(secscct)-(-0.7,0.75)$);
    \end{tikzpicture}
  \end{center}
...
\begin{gather*}
  \begin{aligned}
    ...
    \mi{(Type~Environment)}~\src{\typenv}\bnfdef&\ \src{\hole{\cdot}} \mid \hastype{\src{x}}{\src{\type}},\src{\typenv} \\
...
\begin{center}
  ...
  \judgbox{\filterLtms{\src{\event}}=\event}{,,Project an $\src{L_{\tmssafe}}$ event $\src{\event}$ to a specification event $\event$.''}
  \typerule{$\src{L_{\tmssafe}}$-filter-context}{
    \src{\event[_b]}\not=\src{\lightning}
  }{
    \filterLtms{\src{\tup{\event[_b]}{\ctx}} = \emptyevent}
  }{ltms-filter-context}
  %
  \typerule{$\src{L_{\tmssafe}}$-filter-abort}{
  }{
    \filterLtms{\src{\lightning}} = \lightning
  }{ltms-filter-abort}
  %
%  \typerule{$\src{L_{\tmssafe}}$-filter-start}{
%  }{
%    \filterLtms{\src{\tup{Start}{\comp}}} = \emptyevent
%  }{ltms-filter-start}
  %
  \typerule{$\src{L_{\tmssafe}}$-filter-alloc}{
    ...
    \filterLtms{\src{\tup{Alloc\ \loc\ n}{\comp}}} = \tup{\ev{Alloc}\ \loc\ n}{\comp,\unlock}
  }{ltms-filter-alloc}
\end{center}
...
\MKin{show at least one forward sim lemma or not?}
...
\subsection{$\trg{L}$: An Unsafe Language}\label{subsec:lsms}
...
The $\trg{L}$ language does not provide any guarantees with regards to memory safety.
...
The projection $\filterL{\trace}$ is entirely similar to $\filterLtms{\trace}$.
...
\MKin{x-lang relations $\approx_{\delta;\src{L}}$, $\multimap_{\delta;\src{L}}$, as well as $\cong_{\delta;\src{X}}$}
...
\MKin{compiler $\ccdce$ doing \gls{dce}}
\MKin{compiler $\cccf$ doing \gls{cf}}

\MKin{backward sims}
\MKin{rtp of ms}
\MKin{composition: rtp of ms, order independent}

\subsection{$\obj{L_{\scctsafe}}$: A Language with Access to a Model Specific Register}\label{subsec:lscct}
...
\MKin{compiler $\irl{L_{MS}}\to\obj{L_{\scctsafe}}$}
