...
Together, \gls*{sms}, \gls*{tms} and \gls*{scct}, a variant of \gls*{cct}, yield \gls*{msscct}, which is the gold standard of security properties for secure applications: programs attaining \gls*{msscct} do not leak sensitive data either through erroneous memory accesses, nor through timing side-channels.
...
Thus, robust compilers can be used to prevent, e.g., vulnerabilities resulting from uninitialised memory, e.g., by targeting capability-based languages such as CHERI~\cite{woodruff2014CHERI}, Arm Morello~\cite{arm-morello}, or MSWasm~\cite{michael2023mswasm}, where the compiler relies on capabilities to check that pointers are always initialised.
\end{example}
...
A way to show this, is relating trace $\trg{\trace}$ to some $\Ltms$ trace $\src{\trace}$ (which already satisfies the property as per the assumptions).
...
The trace $\trg{\trace}$ is split into different parts, as commonly done in secure compilation works~\cite{}\MP{catalin's CCS and catalin+akram's paper},where each part contains the events that either the context or the component does, but not both.
...
These two arguments yield the following source execution $\progstepto{\src{\prog{\library_{\ctx}}{\library_{\comp}}}}{\src{\runtimetermvar}}{\src{\trace}}$.
...
Then, by specializing the robust satisfaction assumption $\rsat{\src{\library_{\comp}}}{\pi}$ to use the context $\src{\library_{\ctx}}$ obtained from the backtranslation which yields
 % the context segments of the target trace $\trg{\trace}$, and the 
the following source execution $\progstepto{\src{\prog{\library_{\ctx}}{\library_{\comp}}}}{\src{\runtimetermvar}}{\src{\trace}}$.
This gives $\msfilterLtms{\src{\trace}}\in\pi$, which was the proof goal.
