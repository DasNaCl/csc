...
\begin{abstract}
  ...
  Secrecy cannot be achieved with just memory safety alone, since private data could be leaked by differences in execution time for varying program inputs.
  This leakage is avoidable by ensuring that programs are cryptographic constant time, i.e., differnt inputs do not change execution time.
  ...
  Unfortunately, it is an open question wether secure compilers can be engineered in a modular way.
  Ideally, compiler engineers develop two different compilation passes, one that ensures memory safety and another one that tackles cryptographic constant time, because this style of engineering is well--established for its simplifying divide--and--conquer approach as well as enhanced reusability.
  ...
  Parts of the results are verified in the Coq proof assistant.
...
% It is an open question wether secure compilers compose in a well--behaved manner.
...
Because of this, \gls{ms} is a necessity for programs to be \gls{cct}, which is an important security property for secrecy.
Programs that attain \gls{cct} are secure with respect to timing attacks~\cite{kocher1996timing}.
Timing attacks may expose secrets from a simple performance analysis.
These attacks have become more and more relevant since \citeauthor{kocher1996timing}'s paper, especially given the era of cloud--computing~\cite{aviram2010cloudtime,kumar2019cloudsecsurvey}, where programs share the same hardware but are supposed to run in a sandboxed environment that does not leak secrets.
...
Even though mitigations for such attacks exist~\cite{bond2017vale,almeida2017jasmin}, it has been shown in literature that some compilers may optimize these mitigations away~\cite{barthe2018sec}.
This is of course not what the programmer expects and necessitates the need for a {\em secure} compiler that ensures security properties hold even when the program runs in a hostile environment, i.e., interacts with other programs by any means.
Because of this hostile environment, secure compilers attain security by relying on certain protection mechanisms, such as ... and ...\MK{cits}.
...
Consider the program bolow which copies a \texttt{C}-style string \texttt{x} into \texttt{y} up to a length of \texttt{n}:
...
For sake of simplicity it is assumed that the buffer \texttt{y} is at least as large as \texttt{n} as well as \texttt{x} terminated by \texttt{'\textbackslash 0'}.
Even under these assumptions, there is a subtle \gls{ms} bug in the program which can be exploited to read sensitive data:
The bounds check, i.e., \texttt{i < n}, should happen prior to the access to memory location \texttt{x[i]}.
Because otherwise, if \texttt{n} is larger than the length of the array object \texttt{x} points to, the memory location past this array object is being read, which is an out--of--bounds memory access.
Unfortunately, even though flipping the conditions does fix the \gls{ms} issue, \gls{cct} is violated, so the program is susceptible to timing attacks.
...
Thus, program execution time is proportional to the length of the array object \texttt{x}.
The fix commonly employed in such a case is to rewrite the program to run a fixed number of iterations regardless of the length of \texttt{x} or \texttt{y}.
...
It is easy to see that such a function invocation leads to a series of reads from uninitialised memory, an immediate \gls{ms} issue.
...
After all, the environment cannot be trusted and potentially behaves in a hostile manner.
...
Unfortunately, at the time of writing this paper, it is unclear if and how the theory of robust preservation would support modularity.
...
Ideally, both for proof engineering and reusability factor, a secure compiler for just \gls{ms} and a secure compiler for just \gls{cct} can compose in some way to get a secure compiler for the combination of the two.
...
% keyword: pass
In turn, this paper introduces a framework for reasoning about the composition of secure and optimising compiler passes and it showcases the power of this framework by instantiating it on a multi-pass compilation chain.
...
\begin{itemize}
  \item %
        ...
        First, this paper studies the formalisation and the composition of security properties (\Cref{sec:compprop}) by focussing on those safety properties that are of interest for real-world compiler writers (as identified by the plethora of work enforcing such properties individually~\cite{akritidis2009baggy,nagarakatte2009soft,nagarakatte2010cets,dhumbumroong2020boundwarden,jung2021pico,nam2019framer,shankaranarayana2023tailcheck,younan2010paricheck,zhou2023fatptrs,bond2017vale,almeida2017jasmin,kuepper2023cryptopt,cauligi2019fact}\MK{even more cits?}).
        % 
  ...
  \item %
        ...
        %The sequential composition of these compilers yields a compilation chain that is optimising and robustly preserving a combination of \gls{ms} and \gls{cct}.
        ...
        %As for optimisations, the chain has two passes, one performing \gls{cf} and the other \gls{dce}.
        Furthermore, the chain includes two passes for \gls{ms}\MKin{maybe patch \gls{ms} when optimising passes are done} that are optimising: One performs a simple \gls{dce} and the other \gls{cf}.
...
As typically done in secure compilation~\cite{abate2019jour}, \MP{others do this too, parametricity results e.g.} this paper assumes that whole programs are the result of linking partial programs referred to as {\em context} ($\ctx$) and {\em component} ($\comp$).
...
\section{Example Properties and their Monitors\pages{1.5}}\label{sec:compprop}
...
\subsection{Monitors\pages{1.5}}\label{sec:monitors}
...
This section presents the key meta-theoretic results of this paper and builds on prior work~\cite{kruse2022csc}\MK{mention this at all? may reveal identity even though its double-blind}.
...
\end{lstlisting}};% the following is a hack to get my syntax highlighting to work again \end{lstlisting}
  ...
  The source code in the nodes is a glorified compiler intermediate representation and the code gets more optimised towards the righthandside of the figure.}\label{fig:cfdceex}
\end{figure}

%\section{Case Study: Memory Safe and Cryptographic Constant Time Code}\label{sec:casestud}
\section{Case Study: Preliminary Definitions\pages{2}}\label{sec:casestud:defs}
...
The last step ensures that code stays \gls{scct} (\Cref{thm:ccscct:rtp:scct}) which yields the final result that the whole compilation chain robustly preserves \gls{msscct} (\Cref{thm:ccall:rtp:msscct}).
...
Concretely, it uses a trace--based backtranslation to reconstruct an $\src{L_{\tmssafe}}$ context from the $\trg{L}$ trace, which, however, behaves similar to the $\trg{L}$--level context.
...
The language $\irl{L_{\mssafe}}$ is exactly the same as $\trg{L}$ and are considered equal.
