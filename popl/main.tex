...
\begin{abstract}
  ...
  This article discusses different variants of compositions of secure compilers in a formal manner and instantiates parts of these results in a case--study.
  To this end, an optimizing, secure compilation chain that preserves both memory safety and cryptographic constant time is presented.
...
Given their central role for security, this paper focusses on safety properties.
...
\MPin{
	remove.
	what does this sentence add? removing it does not remove any relevant information
}
For the class of safety properties, it is decidable wether a trace admits such a property with just a finite trace prefix.
\MPin{
	passive. also, ``a trace admits a property'' is wrong, it's the other way around
}
As an example, consider a trace describing an interaction with a memory where the deallocation of a pointer $\loc$ precedes its read: $\ev{Dealloc\ \loc}\cdot\ev{Read\ \loc\ 1729}\cdot\dots$
This program behaviour is insecure wrt a canonical notion of (temporal) memory safety dictating no use-after-frees of pointers\MP{cit}, because it reads from a memory location that was freed already.
\MPin{
	rewordeda a little for precision (e.g., what is $\loc?$), check the diff
}
...
\MPin{
	fill the blank of the next sentence.
}
In the following we write that a whole program \MP{} runs according to the language semantics and produces trace \MP{} as \MP{}. 
With this, we define that
\bul{programs $\progvar$ satisfy a property $\pi$} iff \iul{$\progvar$ yields a trace $\trace$} such that \oul{$\trace$ is admissible with respect to $\pi$} (\Cref{def:propsat}).

\begin{definition}[Property Satisfaction]\label{def:propsat}
  % $\progvar$ satisfies $\pi$, 
  \bul{$\sat{\progvar}{\pi}$} 
  % iff 
  $\isdef$
  \iul{running $\progvar$ gives $\trace$}
  \MP{this does not formalise things, see sentence added above and add formalism}
   where \oul{$\trace\in\pi$}.
\end{definition}

\MPin{
	why are we suddenly going from talking about prop sat to talknig about progs and ctxs?
	there's a gap here that needs to be bridged.
	A plausible solution is something along these lines.
	"property satisfaction only considers passive attackers, to consider a stronger model with active attackers existing work (as well as this paper) uses robust property satisfaction, which we define after introducing the required concept of program context and partial programs"
}
Given a partial program $\progvar$, i.e., a program with free variables, the execution of $\progvar$ would get stuck due to the lack of program parts (e.g., function definitions).
...
\MPin{
	cumbersome notation.
	`link ... ... ' should be used to identify the notation of the operation, but we should have a metavariable for the result, like `link(p1,p2) = w'
	W is a known candidate
}
...
As typically done in secure compilation~\cite{abate2019jour}, \MP{others do this too, parametricity results e.g.} this paper assumes that \MP{whole?}programs are the result of linking a {\em context} ($\ctx$) and a {\em component} ($\comp$).
\MPin{
	these sentences identify the need to have 2 different terms: 1 for partial programs (components?) one for whole programs (just programs) and to use them consistently.
}
...
\MP{
	wut?
	needless sentence, erase.
}
With this, \Thmref{def:propsat} can be extended as follows: for \bul{ partialprograms $\progvar$ to robustly satisfy a property $\pi$}, take an \iul{attacker $\contextvar$ and link it with $\progvar$}, \oul{the result of which must satisfy $\pi$}.
\MPin{
...
\begin{definition}[Robust Satisfaction]\label{def:proprsat}
  % $\progvar$ robustly satisfies $\pi$, 
  \bul{$\rsat{\progvar}{\pi}$}
  % iff 
  $\isdef$ for \iul{any partial program $\contextvar$}\MP{use $\forall$} it holds that \oul{$\sat{\ {\contextvar}{\runtimetermvar}}{\pi}$}. \MP{why r?}
\end{definition}
...
Unfortunately, correct compilers may be insecure compilers~\cite{patrignani2019survey}, and programs translated by insecure compilers can violate security properties that the programmer assumes to hold. %, given the presence of a context--level program.
...
To define robust compilation we only need the notion of a class of properties.
\MPin{define.}
...
\begin{definition}[Robust Preservation]\label{def:rtp}
  ...
  %, iff 
  $\isdef$
  \rul{for any $\pi\in\class$ and $\src{p}$} such that \iul{$\rsat{\src{\progvar}}{\pi}$}\MP{ use $\forall$}, it holds that \oul{$\rsat{\cc{\src{L}}{\trg{L}}\left(\src{p}\right)}{\pi}$}.
\end{definition}
