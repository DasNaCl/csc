...
\myfig{
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw[->] (S) to[sloped,above] node[align=center,font=\scriptsize] (tmsedge) {\gls{tms}} (T);
      \draw[->] (T) to[sloped,above] node[align=center,font=\scriptsize] {\gls{sms}} (M);
      \draw[->] (M) to[sloped,below] node[align=center,font=\scriptsize] {\gls{dce}} (D0);
      \draw[->] (M) to[sloped,above] node[align=center,font=\scriptsize] {\gls{cf}} (C0);
      \draw[->] (D0) to[sloped,below] node[align=center,font=\scriptsize] {\gls{cf}} (E);
      \draw[->] (C0) to[sloped,above] node[align=center,font=\scriptsize] {\gls{dce}} (E);
      \draw[->] (E) to[sloped,above] node[align=center,font=\scriptsize] {\gls{scct}} (O);
...
%     \node[right=1.75 of secopts] (secscct) {{\scriptsize\Cref{subsec:cs:scct}}};
      ...
      \draw[thick,dotted,->] (S) to[bend right=33,sloped,above] node[align=center,font=\scriptsize] {\gls{ms}} (M);
      \draw[thick,dotted,->] (M) to[bend right=0,sloped,above] node[align=center,font=\scriptsize] {\gls{ms}} (E);
      \draw[thick,dotted,->] (S) to[out=-45,in=198,sloped,above] node[align=center,font=\scriptsize] {\gls{ms}+\gls{scct}} (O);
...
%     \draw[dashed] ($(secscct)-(-1.1,0)$) -- ($(secscct)-(-1.1,1.25)$);
    \end{tikzpicture}
  \end{center}
}{outline-sketch}{Compilation Pipeline with
\gls{tms},
\gls{sms},
\gls{ms},
\gls{dce},
...
\begin{definition}[\gls{tms} on Traces]\label{def:tempmemsafe}
  ...
  \begin{itemize}
    \item $\specificev{Alloc\ \loc\ s};\sandboxtag;\specificev{\sigma}\le_{{\mstrace}}\specificev{Dealloc\ \loc};\sandboxtag;\specificev{\sigma'}$
    \item $\specificev{Alloc\ \loc\ s};\sandboxtag;\specificev{\sigma}\le_{{\mstrace}}\specificev{Use\ \loc\ s};\sandboxtag;\specificev{\sigma'}$
    \item $\specificev{Use\ \loc\ n};\sandboxtag;\specificev{\sigma}\le_{{\mstrace}}\specificev{Dealloc\ \loc};\sandboxtag;\specificev{\sigma'}$
    \item $\specificev{Alloc\ \loc\ s};\sandboxtag;\specificev{\sigma}\in_{!}\mstrace$
    \item $\specificev{Dealloc\ \loc};\sandboxtag;\specificev{\sigma}\in_{!}\mstrace$
  \end{itemize}
\end{definition}
...
\begin{definition}[Spatial Memory Safety]\label{def:spatmemsafe}
  ...
  \begin{itemize}
    \item If $\specificev{Alloc\ \loc\ s};\sandboxtag;\specificev{\sigma}\in{\mstrace}$ and $\specificev{Use\ \loc\ n};\sandboxtag;\specificev{\sigma}\in{\mstrace}$, then ${n}<{s}$
  \end{itemize}
\end{definition}
...
\myfig{
  \begin{center}
    ...
    \typerule{scct-Abort}{
      ...
      \monitorcheck[]{\scctmonitor}{\scctmonitor}{\bm{\lightning}}
    }{scct-abort}
    %
    \typerule{scct-Refl}{
    }{
...
\myfig{
  \begin{center}
    ...
    \typerule{backtrans-top-level}{
      ...
      \mmlAmmlAtbacksubtop[\emptyset;\emptyset;\trg{\hole{\cdot}}]\tospecificevtN[^*_\trg{\bullet}]{\trace[_0]} = \Game; \delta; \trg{\overline{\loc}}; \src{e_0};\src{e_0'};\src{let\ x_0 = new\ n_0\ in\ e_0''} \\
      \mmlAmmlAtbacksubtop[\Game;\delta;\trg{\overline{\loc}}]\tospecificevtN[^*_\trg{\bullet}]{\trace[_1]} = \Game'; \delta'; \trg{\hole{\cdot}}; \src{e_1};\src{e_1'};\src{e_1''} \\
...
\begin{lemma}[Backtranslation for Non-Interacting Traces is Well-Typed]\label{lem:expr:backtranslation:welltyped}
  ...
  \item $\mmlAmmlAtbackdetail[]\tospecificevtN[^{*}_{\trg{\bullet}}]{\trace}=\src{e}$
  \end{assumptions}
  Then
  \begin{goals}
  \item $\typechecks{}{e}{\nat}$
...
\begin{lemma}[Backtranslation for $\comptoctx$-Interacting Events is Well-Typed]\label{lem:expr:interact:comptoctx:backtranslation:welltyped}
  If
  \begin{assumptions}
  \item\label[ass]{asm:expr:interact:comptoctx:backtranslation:welltyped} $\mmlAmmlAtbackcomptoctx[\Game;\delta;\trg{\overline{\loc}}]\tospecificevtN[^{*}_{\trg{\bullet}}]{\event}=\src{e}$
  \end{assumptions}
...
\begin{lemma}[Backtranslation for Call-$\ctxtocomp$ is Well-Typed]\label{lem:expr:interact:call:comptoctx:backtranslation:welltyped}
  If
  \begin{assumptions}
  \item\label[ass]{asm:expr:interact:call:ctxtocomp:backtranslation:welltyped} $\mmlAmmlAtbackctxtocomp[\Game;\delta;\trg{\loc,\overline{\loc}}]\tospecificevtN[^{*}_{\trg{\bullet}}]{Call\ \ctxtocomp\ foo\ \valueexpr}=\Game';\delta;\trg{\loc,\overline{\loc'}};\src{e'}$
...
\begin{lemma}[Backtranslation for End-$\ctxtocomp$ is Well-Typed]\label{lem:expr:interact:end:ctxtocomp:backtranslation:welltyped}
  If
  \begin{assumptions}
  \item\label[ass]{asm:expr:interact:ret:ctxtocomp:backtranslation:welltyped} $\mmlAmmlAtbackctxtocomp[\Game;\delta;\trg{\hole{\cdot}}]\tospecificevtN[^{*}_{\trg{\bullet}}]{End\ \valueexpr}=\Game';\delta';\trg{\hole{\cdot}};\src{e}$
  \end{assumptions}
  Then,
  \begin{goals}
  \item $\typechecks{}{e}{\nat\to\bot}$
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:backtranslation:welltyped:subtop1} $\mmlAmmlAtbackdetail[\emptyset;\emptyset;\trg{\hole{\cdot}}]\tospecificevtN[^*_{\color{red}\bullet}]{\trace[_{0}]}=\Game';\delta';\trg{\overline{\loc}};\src{e_{0}};\src{e_{0}'};\src{let\ x_{0} = new\ n_{0}\ in\ e_{0}''}$
    \item\label[ass]{asm:backtranslation:welltyped:subtop2} $\mmlAmmlAtbackdetail[\Game';\delta';\trg{\overline{\loc}}]\tospecificevtN[^*_{\color{red}\bullet}]{\trace[_{1}]}=\Game'';\delta'';\trg{\hole{\cdot}};\src{e_{1}};\src{e_{1}'};\src{e_{1}''}$.
  \end{passumptions}
...
\begin{lemma}[Backtranslation Correctness of $\trg{Start}$]\label{lem:exprbacktranslation:vbeg:wrapper:correct}
  ...
  \item $\mmlAmmlAtbackcomptoctx[\emptyset;\emptyset;\trg{\hole{\cdot}}]\tospecificevtN[_{\trg{\bullet}}]{Start}=\trg{\hole{\cdot}};\src{42}$
...
\begin{lemma}[Backtranslation Correctness of $\trg{Ret}$]\label{lem:exprbacktranslation:beg:wrapper:correct}
  ...
  \item $\mmlAmmlAtbackcomptoctx[\Game;\delta;\trg{\overline{\loc}}]\tospecificevtN[_{\trg{\bullet}}]{Ret\ \comptoctx\ \valueexpr}=\trg{\overline{\loc'}};\src{delete\ z;}\mmlAmmlAtbackv[]{\valueexpr}$
...
\begin{lemma}[Middle of Backtranslation Correctness]\label{lem:exprbacktranslation:mid:wrapper:correct}
  ...
  \item\label[ass]{asm:exprbacktranslation:mid:correct:backtrans} $\mmlAmmlAtbackdetail[]\tospecificevtN[^{*}_{\trg{\bullet}}]{\trace}=\src{e}$
...
\begin{lemma}[Backtranslation Correctness of $\trg{Call\ \ctxtocomp}$]\label{lem:exprbacktranslation:end:wrapper:correct}
  ...
  \item $\mmlAmmlAtbackctxtocomp[\Game;\delta;\trg{\overline{\loc}}]\tospecificevtN[_{\trg{\bullet}}]{Call\ \ctxtocomp\ foo\ \valueexpr}=\Game';\delta';\trg{\overline{\loc'}};\src{let\ z = new\ 42\ in\ call\ foo\ }\mmlAmmlAtbackv[]{\valueexpr}$
...
\begin{lemma}[Backtranslation Correctness of $\trg{End}$]\label{lem:exprbacktranslation:vend:wrapper:correct}
  ...
  \item\label[ass]{lem:exprbacktranslation:vend:wrapper:correct:backtrans} $\mmlAmmlAtbackctxtocomp[\Game;\delta;\trg{\hole{\cdot}}]\tospecificevtN[_{\trg{\bullet}}]{End\ n}=\Game';\delta';\trg{\hole{\cdot}};\src{let\ z=new\ 42\ in\ delete\ z;return\ }\mmlAmmlAtbackv[]{\valueexpr}$
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:rsmsp:wexecirl} $\wexec{\irl{prog\ \library_{\ctx}\ }\mmlAtmmlAtmscompN{\library_{\comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}}$
  \end{passumptions}
  then $\exists \delta_{MS}$
  \begin{goals}
...
%    \item\label[goal]{rsmsp:sms} $\tospecificevi[_{\delta_{MS}'}]{\trace}\in\operatorname{msafe}$
    \item $\vdash\mmlAtmmlAtmscomp{\mmlAmmlAtcompN{\bullet}}:\lceil\operatorname{msafe}\rceil$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \item $\rtp{\mmlAtmmlAtmscomp{\mmlAmmlAtcompN{\bullet}}}{\lceil\operatorname{tmsafe}\rceil\cap\lceil\operatorname{smsafe}\rceil}$
  \end{passumptions}
...
\begin{proof}[Proof (Alternative)]
  ...
  Then
  \begin{goals}
    \item\label[goal]{goal:rmtsp:rsat} $\mmlAtmmlAtmscomp{\mmlAmmlAtcompN{\library_{\ctx}}}\vDash_{R}\pi$
  \end{goals}
  ...
  \begin{passumptions}{H}
    \setcounter{enumi}{5}
    \item\label[ass]{asm:rmtsp:smsrsat} $\mmlAtmmlAtmscomp{\mmlAmmlAtcompN{\library_{\ctx}}}\vDash_{R}\pi$
  \end{passumptions}
  This solves \Cref{goal:rmtsp:rsat}.
\end{proof}

\newpage
\subsection{Robust Optimizing Compilers: Constant Folding}
...
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAmsmmlAmscompdce{e}=\irl{e'}}{,,Compile $\mmlAms$ expression $\irl{e}$ to $\mmlAms$ expression $\irl{e'}$.''}\\
    \judgbox{\mmlAmsmmlAmscompdce{\left[\valueexpr / x\right]}=\irl{\left[\valueexpr / x\right]}}{,,Compile $\mmlAms$ substitution to $\mmlAms$ substitution.''} \\
    \judgbox{\mmlAmsmmlAmscompdce{\commlib}=\irl{\commlib'}}{,,Compile $\mmlAms$ component library to $\mmlAms$ component library.''} \\
    \end{gather*}
    $$
    \begin{array}{rll}
    \mmlAmsmmlAmscompdce{\finalexpr} &\ = & \irl{\finalexpr} \\
    \mmlAmsmmlAmscompdce{call\ foo\ e} &\ = & \irl{call\ }\mmlAmsmmlAmscompdce{foo}\ \mmlAmsmmlAmscompdce{e}\\
    \mmlAmsmmlAmscompdce{return\ e} &\ = & \irl{return\ }\mmlAmsmmlAmscompdce{e}\\
    \mmlAmsmmlAmscompdce{e_1\oplus e_2} &\ = & \mmlAmsmmlAmscompdce{e_1} \irl{\oplus} \mmlAmsmmlAmscompdce{e_2} \\
      \mmlAmsmmlAmscompdce{x[e]} &\ = & \irl{let\ x_{ACCESS}=}\mmlAmsmmlAmscompdce{e}\irl{\ in\ }\\
                                &&\irl{ifz\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\&&\ \ \irl{x[x_{ACCESS}]}\\
                                && \irl{else\ abort()}\\
    \mmlAmsmmlAmscompdce{let\ x= e_1\ in\ e_2} &\ = & \irl{let\ x=} \mmlAmsmmlAmscompdce{e_1}\irl{\ in\ }\mmlAmsmmlAmscompdce{e_2} \\
      \mmlAmsmmlAmscompdce{x[e_1]\leftarrow e_2} &\ = & \irl{let\ x_{ACCESS}=}\mmlAmsmmlAmscompdce{e_1}\irl{\ in\ }\\
      &&\irl{ifz\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\&&\ \ \irl{x[x_{ACCESS}]\leftarrow }\mmlAmsmmlAmscompdce{e_2} \\
      &&\irl{else\ abort()} \\
    \mmlAmsmmlAmscompdce{let\ x=new\ e_1\ in\ e_2} &\ = & \irl{let\ x_{\texttt{SIZE}}=}\mmlAmsmmlAmscompdce{e_1}\irl{\ in\ }\\&&\irl{let\ x}\irl{=new\ x_{\texttt{SIZE}}}\irl{\ in\ }\mmlAmsmmlAmscompdce{e_2} \\
    \mmlAmsmmlAmscompdce{delete\ x} &\ = & \irl{delete\ x} \\
    \mmlAmsmmlAmscompdce{x\ is\ \poisoned} &\ = & \irl{x\ is\ \poisoned} \\
    \mmlAmsmmlAmscompdce{\lparen e_1;e_2\rparen} &\ = & \irl{\lparen}\mmlAmsmmlAmscompdce{e_1}\irl{;}\mmlAmsmmlAmscompdce{e_2}\irl{\rparen}\\
    \mmlAmsmmlAmscompdce{\pi_1\ e} &\ = & \irl{\pi_1\ }\mmlAmsmmlAmscompdce{e}\\
    \mmlAmsmmlAmscompdce{\pi_2\ e} &\ = & \irl{\pi_2\ }\mmlAmsmmlAmscompdce{e}\\
    \mmlAmsmmlAmscompdce{e\ has\ \type} &\ = & \mmlAmsmmlAmscompdce{e}\ \irl{has\ \type}\\
    \mmlAmsmmlAmscompdce{ifz\ true\ then\ e_2\ else\ e_3} &\ = & \irl{e_{2}}\\
    \mmlAmsmmlAmscompdce{ifz\ false\ then\ e_2\ else\ e_3} &\ = & \irl{e_{3}}\\
    \mmlAmsmmlAmscompdce{ifz\ e_1\ then\ e_2\ else\ e_3} &\ = & \irl{ifz\ }\mmlAmsmmlAmscompdce{e_1}\\
                                                        &&\irl{then\ }\mmlAmsmmlAmscompdce{e_2} \\
                                                        &&\irl{else\ }\mmlAmsmmlAmscompdce{e_3} \\[0.5cm]
    %
    \mmlAmsmmlAmscompdce{\left[\valueexpr/x\right]}&\ = & \irl{\left[\right.}\mmlAmsmmlAmscompdce{v}\irl{/}\mmlAmsmmlAmscompdce{x}\irl{\left.\right]} \\[0.5cm]
    %
    \mmlAmsmmlAmscompdce{\hole{\cdot}}&\ = & \irl{\hole{\cdot}} \\
    \mmlAmsmmlAmscompdce{foo,\commlib}&\ = & \irl{foo,}\mmlAmsmmlAmscompdce{\commlib} \\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler}{Compiler from $\mmlAms$ to $\mmlAms$.}
\myfig{
  \begin{center}
    \begin{gather*}
    \judgbox{\mmlAmsmmlAmscompdce{F}=\irl{F}}{,,Compile $\mmlAms$ procedures to $\mmlAms$ procedures.''}
    \end{gather*}
    $$
    \begin{array}{rll}
    %
      \mmlAmsmmlAmscompdce{let\ foo\ x := e}&\ = & \irl{let\ foo\ x := }\mmlAmsmmlAmscompdce{e}\\
    \end{array}
    $$
    \begin{gather*}
    \judgbox{\mmlAmsmmlAmscompdce{\library}=\irl{\library}}{,,Compile $\mmlAms$ libraries to $\mmlAms$ libraries.''}
    \end{gather*}
    $$
    \begin{array}{rll}
      \mmlAmsmmlAmscompdce{\hole{\cdot}}&\ = & \irl{\hole{\cdot}} \\
      \mmlAmsmmlAmscompdce{F,\library}&\ = & \mmlAmsmmlAmscompdce{F}\irl{,}\mmlAmsmmlAmscompdce{\library} \\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler-comp}{Compiler from $\mmlAms$ components to $\mmlAms$ components.}
\myfig{
  \begin{center}
    \begin{gather*}
      \judgbox{\mmlAmsmmlAmscompdce{K}=\irl{K}}{,,Compile $\mmlAms$ evaluation contexts to $\mmlAms$ evaluation contexts.''}
    \end{gather*}
    $$
    \begin{array}{rll}
      \mmlAmsmmlAmscompdce{\hole{\cdot}} &\ = & \irl{\hole{\cdot}} \\
      \mmlAmsmmlAmscompdce{K \oplus e} &\ = & \mmlAmsmmlAmscompdce{K}\irl{\oplus}\mmlAmsmmlAmscompdce{e} \\
      \mmlAmsmmlAmscompdce{\valueexpr \oplus K} &\ = & \mmlAmsmmlAmscompdce{\valueexpr}\irl{\oplus}\mmlAmsmmlAmscompdce{K} \\
      \mmlAmsmmlAmscompdce{x[K]} &\ = & \irl{let\ x_{ACCESS}=}\mmlAmsmmlAmscompdce{K}\irl{\ in} \\
                                &&\irl{if\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\
                                &&\ \ \irl{x[x_{ACCESS}]}\\
                                &&\irl{else\ abort()}\\
      \mmlAmsmmlAmscompdce{let\ x\ = K\ in\ e} &\ = & \irl{let\ x\ = }\mmlAmsmmlAmscompdce{K}\irl{\ in\ }\mmlAmsmmlAmscompdce{e} \\
      \mmlAmsmmlAmscompdce{x[K] \leftarrow e} &\ = & \irl{let\ x_{ACCESS} = }\mmlAmsmmlAmscompdce{K}\irl{\ in}\\
                                &&\irl{if\ 0\le x_{ACCESS}<x_{SIZE}\ then}\\
      &&\ \ \irl{x[x_{ACCESS}]\leftarrow}\mmlAmsmmlAmscompdce{e} \\
      &&\irl{else\ abort()} \\
      \mmlAmsmmlAmscompdce{x[n] \leftarrow K} &\ = & \irl{if\ 0\le n<x_{SIZE}\ then}\\
      &&\ \ \irl{x[n]\leftarrow }\mmlAmsmmlAmscompdce{K} \\
      &&\irl{else\ abort()} \\
      \mmlAmsmmlAmscompdce{let\ x\ = new\ K\ in\ e} &\ = & \irl{let\ x_{SIZE}=}\mmlAmsmmlAmscompdce{K}\irl{\ in\ }\\
                                &&\irl{let\ x = new\ x_{SIZE}\ in\ }\mmlAmsmmlAmscompdce{e} \\
      \mmlAmsmmlAmscompdce{ifz\ K\ then\ e_{1}\ else\ e_{2}} &\ = & \irl{ifz\ }\mmlAmsmmlAmscompdce{K}\irl{\ then\ }\\
                               &&\ \ \mmlAmsmmlAmscompdce{e_{1}}\\
                               &&\irl{else\ }\\
                               &&\ \ \mmlAmsmmlAmscompdce{e_{2}} \\
      \mmlAmsmmlAmscompdce{call\ foo\ K} &\ = & \irl{call\ foo\ }\mmlAmsmmlAmscompdce{K}\\
      \mmlAmsmmlAmscompdce{return\ K} &\ = & \irl{return\ }\mmlAmsmmlAmscompdce{K}\\
    \end{array}
    $$
  \end{center}
}{sms-pres-compiler-ctx}{Compiling $\mmlAms$ evaluation contexts to $\mmlAms$ evaluation contexts.}

\begin{lemma}[Injective Compiler (for final expressions)]\label{lem:smscomp:inject}
  If
  \begin{assumptions}
    \item $\mmlAmsmmlAmscompdce{\finalexprnoerr}=\irl{e_0}$
    \item $\mmlAmsmmlAmscompdce{\finalexprnoerr}=\irl{e_1}$
  \end{assumptions}
  Then
  \begin{goals}
    \item $\irl{e_0}=\irl{e_1}$
...
\begin{lemma}[Subst $\mmlAmsmmlAmscompdce{\bullet}$ Compatibility]\label{thm:mscomp:subst:compat}
  $\;$
  \begin{goals}
    \item $\mmlAmsmmlAmscompdce{e\gamma}=\mmlAmsmmlAmscompdce{e}\mmlAmsmmlAmscompdce{\gamma}$
  \end{goals}
\end{lemma}
\begin{proof}
  Induction on $\irl{e}$.
...
\begin{lemma}[Prim. Forward Simulation]\label{lem:sms:ctxforwardsim}
  If
  \begin{assumptions}
    \item\label[ass]{asm:sms:ctxforwardsim:evalt} $\exprevali{\Omega}{e\gamma}{\Omega'}{e'\gamma'}{\event}$
    \item $\irl{\Omega}=\irl{\memstate;\comp;\cfstate}$
    \item $\monitorcheck[]{\smsmonitor}{\smsmonitor'}{\absevent}$
    \item $\storeagreesms[\delta]{\smsmonitor}{\irl{\Omega}}$
    \item $\traceagree[]{\theta_{\delta}\left(\irl{\event}\right)}{\absevent}$
  \end{assumptions}
  Then $\exists \irl{\gamma'}$,
  \begin{goals}
    \item $\exec{\irl{\Omega}}{\mmlAmsmmlAmscompdce{e}\irl{\gamma}}{\irl{\Omega'}}{\mmlAmsmmlAmscompdce{e'}\irl{\gamma'}}{\irl{\event}}$
    \item $\storeagreesms[\delta]{\smsmonitor'}{\irl{\Omega'}}$
    \item $\irl{\Omega'.\Delta}\Vdash\irl{\gamma'}$
    \item $\irl{\gamma'}\prec\irl{\gamma'}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
  Induction on \Cref{asm:sms:ctxforwardsim:evalt}.
...
\begin{lemma}[Ctx. Forward Simulation]\label{lem:sms:ctxforwardsim}
  If
  \begin{assumptions}
    \item $\execi[]{\Omega}{e}{\Omega'}{e'}{\trace}$
    \item $\irl{\Omega}=\irl{\memstate;\comp;\cfstate}$
    \item $\monitorcheck[]{\smsmonitor}{\smsmonitor'}{\absevent}$
    \item $\storeagreesms[\delta]{\smsmonitor}{\irl{\Omega}}$
    \item $\traceagree[]{\theta_{\delta}\left(\irl{\event}\right)}{\absevent}$
  ...
  Then
  \begin{goals}
    \item $\exec{\irl{\Omega}}{\mmlAmsmmlAmscompdce{e}}{\irl{\Omega'}}{\mmlAmsmmlAmscompdce{e'}}{\irl{\event}}$
    \item $\storeagreesms[\delta]{\smsmonitor'}{\irl{\Omega'}}$
  \end{goals}
\end{lemma}
\begin{incompleteproof}
...
\begin{lemma}[Forward Simulation]\label{lem:sms:forwardsim}
  If
  \begin{assumptions}
    \item $\execi{\Omega}{e}{\Omega'}{\finalexprnoerr}{\trace}$
    \item $\monitorcheck{\smsmonitor}{\smsmonitor'}{\abstrace}$
    \item $\traceagree{\theta_{\delta}^{*}(\irl{\trace})}{\abstrace}$
    \item $\storeagreesms[\delta]{\smsmonitor}{\irl{\Omega}}$
  \end{assumptions}
  Then
  \begin{goals}
    \item $\exec{\irl{\Omega}}{\mmlAmsmmlAmscompdce{e}}{\irl{\Omega'}}{\mmlAmsmmlAmscompdce{\finalexprnoerr}}{\irl{\trace}}$
    \item $\storeagreesms[\delta]{\smsmonitor'}{\irl{\Omega'}}$
  \end{goals}
\end{lemma}
\begin{proof}
...
\begin{lemma}[Backward Simulation]\label{lem:sms:backwardsim}
  ...
  \begin{assumptions}
    \item $\exec{\irl{\Omega}}{\mmlAmsmmlAmscompdce{e}}{\irl{\Omega'}}{\mmlAmsmmlAmscompdce{\finalexprnoerr}}{\irl{\trace}}$
    \item $\irl{\Omega}=\irl{\Omega}$
    \item $\irl{\trace}=\irl{\trace}$
    \item $\irl{\Omega'}=\irl{\Omega'}$
  \end{assumptions}
  Then
  \begin{goals}
    \item $\execi{\Omega}{e}{\Omega'}{\finalexprnoerr}{\trace}$\MK{substitutions}
  \end{goals}
\end{lemma}
\begin{proof}
  Follows from forward sim and determinism.
\end{proof}

\begin{lemma}[Top-Level Backward Simulation]\label{lem:backtransms}
  If
  \begin{assumptions}
    \item $\irl{\library}=\irl{\library_{ctx}\linker}\mmlAmsmmlAmscompdce{\library_{comp}}$
    \item $\irl{\commlib}=\mmlAmsmmlAmscompdce{\commlib}=\operatorname{dom}\mmlAmsmmlAmscompdce{\library_{comp}}$
    \item $\exec{\irl{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}}{\irl{call\ main\ 0}}{\irl{\commlib;\library;\kontstack;\ctx;H^{\ctx};H^{\comp};\Delta}}{\irl{\finalexprnoerr}}{\irl{\trace}}$
  \end{assumptions}
  Then $\exists \irl{\trace}\ \irl{\kontstack}\ \irl{H^{\ctx}}\ \irl{H^{\comp}}\ \irl{\Delta}$,
  \begin{goals}
    \item $\exec{\irl{\commlib;\library;\hole{\cdot};\comp;\hole{\cdot};\hole{\cdot};\hole{\cdot}}}{\irl{call\ main\ 0}}{\irl{\commlib;\library;\kontstack;\ctx;H^{\ctx};H^{\comp};\Delta}}{\irl{\finalexprnoerr}}{\irl{\trace}}$
  \end{goals}
\end{lemma}
\begin{proof}
  Suppose control is never handled over to $\mmlAmsmmlAmscompdce{\library_{comp}}$; that is, there is no $\irl{Call\ ?\ foo\ n}$ in $\irl{\trace}$.
  In this case, the goals follow easily, since the semantics of $\mmlAms$ and $\mmlAms$ are identical and $\irl{\library_{ctx}}=\irl{\library_{ctx}}$.

  If control is handled over, we have, at some point in the reduction by transitivty, $\exec{\irl{\Omega_{0}}}{\mmlAmsmmlAmscompdce{e_{foo}}\irl{[\valueexpr/x]}}{\irl{\commlib;\library;\kontstack;\ctx;H^{\ctx};H^{\comp};\Delta}}{\irl{\finalexprnoerr}}{\irl{\trace}}$.
  Thus, we can apply \Thmref{lem:sms:backwardsim} and are done.
\end{proof}
...
\begin{lemma}[Program Backward Simulation]\label{lem:backwardsim:toplevel}
  If
  \begin{assumptions}
    \item $\wexec{\irl{prog\ \library_{ctx}\ }\mmlAmsmmlAmscompdce{\library_{comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}}$
  \end{assumptions}
  Then
  \begin{goals}
...
\begin{definition}[$\mmlAms$ Robust Satisfaction]
  We write $\irl{\library_{ctx}}\vDash_{R}\pi$ for
  $$
    \forall \irl{\library_{comp}}\ \irl{\trace}\ \irl{\Omega}\ \irl{\finalexpr}, \wexect{\irl{prog\ \library_{\ctx}\ \library_{\comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}} \implies \exists \delta_{MS}, \tospecificevt[_{\delta_{MS}}]{\irl{\trace}}\in\pi
  $$
...
% If
% \begin{assumptions}
% \item\label[ass]{asm:rsmsp:srcsms} $\forall \delta_{MS}\ \irl{\library_{ctx}}\ \irl{\library_{comp}}\ \irl{\trace}\ \irl{\Omega}\ \irl{v}, \wexect{prog\ \library_{ctx}\ \library_{comp}}{\configt}{\finalexpr}{\trace}\rightarrow\tospecificevt[_{\delta_{MS}}]{\trace}\in\operatorname{smsafe}$
% \item\label[ass]{asm:rsmsp:irlexec} $\wexec{\irl{prog\ \library_{ctx}\ }\mmlAmsmmlAmscompdce{\library_{comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}}$
% \end{assumptions}
% then $\exists \delta_{MS}',$
  \begin{goals}
%    \item\label[goal]{rsmsp:sms} $\tospecificevi[_{\delta_{MS}'}]{\trace}\in\operatorname{smsafe}$
    \item\label[ass]{asm:rsmsp:comp} $\vdash\mmlAmsmmlAmscompdce{\bullet}:\lceil\operatorname{smsafe}\rceil$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \item $\pi\in\lceil\operatorname{smsafe}\rceil$
    \item\label[ass]{asm:rsmsp:srcsms} $\irl{\library_{\comp}}\vDash_{R}\pi$
  \end{passumptions}
  then
  \begin{goals}
    \item\label[ass]{asm:rsmsp:rsatt} $\mmlAmsmmlAmscompdce{\library_{\comp}}\vDash_{R}\pi$
  \end{goals}
  Unfold \Cref{asm:rsmsp:rsatt}:
  If
  \begin{passumptions}{H}
    \setcounter{enumi}{2}
    \item\label[ass]{asm:rsmsp:wexecirl} $\wexec{\irl{prog\ \library_{\ctx}\ }\mmlAmsmmlAmscompdceN{\library_{\comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}}$
  \end{passumptions}
  then $\exists \delta_{MS}$
  \begin{goals}
    \item $\tospecificevt[_{\delta_{MS}}]{\irl{\trace}}\in\pi$
  \end{goals}
  Use \Thmref{lem:backwardsim:toplevel} on \Cref{asm:rsmsp:wexecirl}: (remember that $\irl{\trace}=\irl{\trace}$, since $\mmlAms=\mmlAms$)
  \begin{passumptions}{H}
    \setcounter{enumi}{3}
    \item\label[ass]{asm:rsmsp:twprog} $\wexec{\irl{prog\ \library_{ctx}\ \library_{comp}}}{\irl{\Omega}}{\irl{\finalexpr}}{\irl{\trace}}$
  \end{passumptions}
  Apply \Cref{asm:rsmsp:srcsms} to our goal.
...
\subsection{Robust Optimizing Compilers: cf}
...
\myfig{
  \begin{center}
    ...
    \typerule{o-scct-filter-comp-alloc}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{Alloc\ \loc\ n;\comp;\sigma} = \specificev{Alloc\ \loc\ n;\comp;\unlock}
    }{o-scct-filter-comp-alloc}
    %
    \typerule{o-scct-filter-comp-dealloc}{
      \delta_{\sCCT}(\obj{\loc})=\specificev{\loc}
    }{
      \tospecificevo[_{\delta_{\sCCT}}]{Dealloc\ \loc;\comp;\sigma} = \specificev{Dealloc\ \loc;\comp;\unlock}
    ...
    \typerule{o-scct-filter-comp-get-noleak}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{\widehat{Get}\ \loc\ n;\comp;\sigma} = \specificev{Use\ \loc\ n;\comp;\unlock}
    ...
    \typerule{o-scct-filter-comp-set-noleak}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{\widehat{Set}\ \loc\ n\ \valueexpr;\comp;\sigma} = \specificev{Use\ \loc\ n;\comp;\unlock}
    ...
    \typerule{o-scct-filter-comp-get-leak}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{Get\ \loc\ n;\comp;\sigma} = \specificev{Use\ \loc\ n;\comp;\sigma}
    ...
    \typerule{o-scct-filter-comp-set-leak}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{Set\ \loc\ n\ \valueexpr;\comp;\sigma} = \specificev{Use\ \loc\ n;\comp;\sigma}
    ...
    \typerule{o-scct-filter-comp-binop}{
      ...
      \tospecificevo[_{\delta_{\sCCT}}]{Binop\ n_1\ n_2;\comp;\sigma} = \specificev{Binop\ n_1\ n_2;\comp;\sigma}
    }{o-scct-filter-comp-binop}
    %
    \typerule{o-scct-filter-comp-branch}{
    }{
      \tospecificevo[_{\delta_{\sCCT}}]{Branch\ n;\comp;\sigma} = \specificev{Branch\ n;\comp;\sigma}
    }{o-scct-filter-comp-branch}
    %
    \typerule{o-scct-filter-abort}{
    }{
      \tospecificevo[_{\delta_{\sCCT}}]{\lightning} = \specificev{\lightning}
    }{o-scct-filter-abort}
    %
...
\myfig{
  \begin{center}
    ...
    \typerule{scct-cons-commlib-eq}{
      \mmlAtmsmmlAtscctcomp{let\ foo\ x:\typelambda := e} = \obj{let\ foo\ x := e} &
      \xlangstateeqc[]{\library}{\library_1,\library_2}
    }{
      \xlangstateeqc[]{(let\ foo\ x:\typelambda := e),\library}{\library_1,(let\ foo\ x := e),\library_2}
    ...
    \case{tr:scct-cons-trace-eq}
      ...
      \begin{description}
        ...
        \case{tr:scct-alloc-event-eq}
          ...
          Note \Cref{tr:i-scct-filter-comp-alloc}, so \Cref{goal:scctfiltereq:irltoabsev} follows from \Cref{tr:scct-alloc-auth}.

        \case{tr:scct-dealloc-event-eq}
          Invert \Cref{asm:scctfiltereq:objtoabsev} to get $\absevent=\bm{Any;\unlock}$.
          Note \Cref{tr:i-scct-filter-comp-dealloc}, so \Cref{goal:scctfiltereq:irltoabsev} follows from \Cref{tr:scct-dealloc-auth}.
...
\begin{lemma}[Primitive Forward Simulation]\label{lem:prim:forwardsim:scct}
  ...
  \begin{goals}
    \item $\exprevalo{\Omega}\mmlAtmsmmlAtscctcompN{\irl{e}}\mmlAtmsmmlAtscctcompN{\irl{\gamma}}{\Omega'}\mmlAtmsmmlAtscctcompN{\irl{e'}}\mmlAtmsmmlAtscctcompN{\irl{\gamma'}}{\event}$
...
\begin{lemma}[Contextual Forward Simulation]\label{lem:ctx:forwardsim:scct}
  ...
  Then $\exists \delta_{scct}'\ \obj{\event}\ \obj{\Omega'}$,
  \begin{goals}
    \item $\execo[]{\Omega}\mmlAtmsmmlAtscctcompN{\irl{e}}\mmlAtmsmmlAtscctcompN{\irl{\gamma}}{\Omega'}\mmlAtmsmmlAtscctcompN{\irl{e'}}\mmlAtmsmmlAtscctcompN{\irl{\gamma'}}{\event}$
    \item $\xlangeventeqc[\delta_{scct}']{\event}{\event}$
    \item $\xlangstateeqc[\delta_{scct}']{\Omega'}{\Omega'}$
    \item $\sccttraceagree{\theta_{\delta_{scct}'}(\obj{\event})}{\absevent}$
  \end{goals}
...
\begin{lemma}[Backward Simulation]
  If
  \begin{assumptions}
    \item $\execo{\Omega}\mmlAtmsmmlAtscctcompN{\irl{e}}\mmlAtmsmmlAtscctcompN{\irl{\gamma}}{\Omega'}\mmlAtmsmmlAtscctcompN{\irl{e'}}\mmlAtmsmmlAtscctcompN{\irl{\gamma'}}{\trace}$
    \item $\xlangstateeqc[\delta_{scct}]{\Omega}{\Omega}$
    \item $\monitorcheck{\scctmonitor}{\scctmonitor'}{\absevent}$
    \item $\sccttraceagree{\theta_{\delta}(\irl{\event})}{\absevent}$
  \end{assumptions}
...
%      \mmlAtmsmmlAtscctsubtop[\emptyset;\emptyset;\obj{\hole{\cdot}}]\tospecificevoN[^*_\obj{\bullet}]{\trace[_0]} = \Game; \delta; \obj{\overline{\loc}}; \irl{e_0};\irl{e_0'};\irl{let\ x_0 = new\ n_0\ in\ e_0''} \\
      \mmlAtmsmmlAtscctsubtop[]\tospecificevoN[^*_\obj{\bullet}]{\trace[_0]} = \irl{e_0};\irl{e_0'};\irl{e_0''} \\
      \mmlAtmsmmlAtscctsubtop[]\tospecificevoN[^*_\obj{\bullet}]{\trace[_1]} = \irl{e_1};\irl{e_1'};\irl{e_1''} \\
...
\begin{lemma}[Backtranslation Correctness of $\obj{Start}$]\label{lem:scct:exprbacktranslation:vbeg:wrapper:correct}
  ...
  \item $\mmlAtmsmmlAtscctcomptoctx[]\tospecificevoN[_{\obj{\bullet}}]{Start;\comp;\sigma''}=\irl{42}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{Ret}$]\label{lem:scct:exprbacktranslation:beg:wrapper:correct}
  ...
  \item $\mmlAtmsmmlAtscctcomptoctx[]\tospecificevoN[_{\obj{\bullet}}]{Ret\ \comptoctx\ \valueexpr;\comp;\sigma}=\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{lemma}[Middle of Backtranslation Correctness]\label{lem:scct:exprbacktranslation:mid:wrapper:correct}
  ...
  \item\label[ass]{asm:scct:exprbacktranslation:mid:correct:backtrans} $\mmlAtmsmmlAtscctdetail[]\tospecificevoN[^{*}_{\obj{\bullet}}]{\trace}=\irl{e}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{Call\ \ctxtocomp}$]\label{lem:scct:exprbacktranslation:end:wrapper:correct}
  ...
  \item $\obj{\library}=\obj{\library_{ctx}\linker}\mmlAtmsmmlAtscctcomp{\library_{comp}}$
  \item $\obj{\commlib}=\mmlAtmsmmlAtscctcomp{\commlib}=\operatorname{dom}\ \mmlAtmsmmlAtscctcomp{\library_{comp}}$
  \item $\execoN[^{2}]{\Omega}{K\hole{call\ foo^{\sigma}\ \valueexpr^{\sigma'}}}{\commlib;\library;(K;foo),\kontstack;\comp;1;\memstate}{\mmlAtmsmmlAtscctcompN{e_{foo}}[\valueexpr^{\sigma'}/y]}{Call\ \ctxtocomp\ foo\ \valueexpr;\ctx;\sigma'}$
  \item $\mmlAtmsmmlAtscctctxtocomp[]\tospecificevoN[_{\obj{\bullet}}]{Call\ \ctxtocomp\ foo\ \valueexpr}=\irl{call\ foo\ }\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{lemma}[Backtranslation Correctness of $\obj{End}$]\label{lem:scct:exprbacktranslation:vend:wrapper:correct}
  ...
  \item\label[ass]{asm:scct:exprbacktranslation:vend:wrapper:correct:backtrans} $\mmlAtmsmmlAtscctctxtocomp[]\tospecificevoN[_{\obj{\bullet}}]{End\ \valueexpr;\ctx;\sigma}=\irl{return\ }\mmlAtmsmmlAtscctv[]{\valueexpr}$
...
\begin{incompleteproof}
  ...
  \begin{passumptions}{H}
    ...
    \item\label[ass]{asm:scct:backtrans:subtop1} $\mmlAtmsmmlAtscctsubtop[]\tospecificevoN[^{*}_{\obj{\bullet}}]{\trace[_{0}]}=\irl{e_{0};e_{0}';e_{0}''}$
    \item\label[ass]{asm:scct:backtrans:subtop2} $\mmlAtmsmmlAtscctsubtop[]\tospecificevoN[^{*}_{\obj{\bullet}}]{\trace[_{1}]}=\irl{e_{1};e_{1}';e_{1}''}$
  \end{passumptions}
...
\begin{scontents}[store-env=buffer]
  $\;$\\
  \begin{goals}
    \item $\vdash\mmlAtmsmmlAtscctcompN{\mmlAtmmlAtmscompN{\mmlAmmlAtcomp{\bullet}}}:\lceil\operatorname{msafe}\rceil\cap\lceil\operatorname{\sCCT}\rceil$
  \end{goals}
\end{scontents}
...
\begin{proof}
  ...
  \begin{passumptions}{H}
    \item\label[ass]{ass:crpms} $\rtp{\mmlAtmmlAtmscompN{\mmlAmmlAtcomp{\bullet}}}{\lceil\operatorname{msafe}\rceil}$
