...
Within the framework of recent work~\cite{kruse22,kruse23}, this extended abstract aims to give cross-language trace relations that describe the semantic effect of mitigation mechanisms for SPECTRE variants and investigate their compatibility with each other.
%
%
...
The following assumes that the source-level traces $\src{\trace}$ are trivially absent of speculative leaks. 
...
This section also assumes the existence of some kind of allocation ($\trg{Alloc(\loc;n)}$), memory load/store ($\trg{Get(\loc;idx;n)}$ and $\trg{Set(\loc;idx;n;v)}$), branch ($\trg{If(b)}$), and indirect branch events ($\trg{Ibranch(v)}$) as well as a marker event for a barrier ($\trg{\times}$), and a rollback event ($\trg{Rlb}$)~\cite{patrignani21b}.
The following sketches the cross-language trace relations describing the semantic effect of popular SPECTRE mitigation techniques.
