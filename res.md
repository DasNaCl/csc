...
- `Rsat(p,π)` is forall C w, if link C p = Ok w and whole w, then sat(w, π)
- `hsat(w,Π)` is behav(w) ∈ Π
- `Rhsat(p,Π)` is forall C w, if link C p = Ok w and whole w, then hsat(w, Π)
...
- if (∀ π, RTP(γ1, π)) and (∀ π, RTP(γ2, π)), then (∀ π, RTP(γ1 ∘ γ2, π))
    - "trivial" imo
    - This holds for any other characterization above, i.e., TP, HP, CTP, RHP, and RCTP
- if RCTP(γ1, Ξ1) and RCTP(γ2, Ξ2), then RCTP(γ1 ∘ γ2, Ξ1 ∩ Ξ2)   (same spiel with CTP)
    ...
    - Lifting a property π to a class Ξ works by using the powerset on π. 
      The equivalent hyperproperty then composes well under RCTP, but composition with RHP(γ', SomeΠ) is unclear (or RTP for that matter).
    - RCTP itself is not used in practice for anything more specific than "Safety"
- RTP without ~ means the compiler is entirely uninteresting, as it never changes anything on the trace. The only thing such a compiler is allowed to do is forget/filter traces, but not manipulate the trace itself.
  Manipulation of the trace itself is very common in practice; see compcert undef behav or basic static typechecking to dynamic typechecking translation
...
- `~` for just traces may not be enough.
