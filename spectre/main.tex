...
Thi input and output projections are assumed to be disjoint.
Let $\phi$ be a selection abstraction in the style of abstract non-interference.
Let $\llbracket\bullet\rrbracket$ be an observer.

\begin{definition}{\definitionlabel[Low-equivalence]{low-equiv}}
  \[
  \lrbraces{ \varProperty \mid
...
\begin{definition}{\definitionlabel[Observer-specific non-interference]{observer-specific-non-interference}}
  A program $p$ is non-interferent if, given a policy $\policy$ and initial states $\sigma,\sigma'$ such that $\sigma\simeq_{\policy}\sigma'$, it holds that $\llbracket p\rrbracket\lrpars{\sigma}=\llbracket p\rrbracket\lrpars{\sigma'}$.
\end{definition}
