...
\begin{definition}{\definitionlabel[Mapping \glsfirst{ms} to \glsfirst{ss}]{mapping-ms-spec}}
  ...
  \end{center}
  \begin{center}
    \judgbox{\mmap{\msTrace}{\specTrace}}{,,Translate \gls*{ms}-traces to \gls*{spec}-traces.''}
...
In order to check \gls*{tms} on traces mechanically, we introduce a monitor that works on more abstract monitor-actions, without any other events besides those relevant to temporal memory safety.
